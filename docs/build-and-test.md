...
- **`ALLOW_SKIPPED_INTEGRATION=1` ⇒ it skips instead**, printing a `SKIPPED`
  line and appending to `$CATALYRST_TESTGATE_SKIPLOG` when that is set. This is
  the developer-laptop escape hatch; CI must never set it.
- **a dependency you explicitly configured but that does not work is always a
...
marker: `sites/test/e2e/require-dep.ts` and `rig/lib/testgate.sh`. What enforces
