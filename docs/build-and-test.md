...
`scripts/export-catalyrst.sh` will not ship a tree whose packaging does not
build. Before it writes the export it runs, through the pinned devshell
toolchain (same `rust-toolchain.toml` resolution as CI):

- `cargo fmt --all -- --check`
- `cargo clippy --workspace --all-targets -- -D warnings`
- a real `nix build` of `sites#sites`
- a forced eval of every `drvPath` in the main flake's package set

It runs *after* the version bump, so the gate certifies the exact tree that
ships. This exists because v0.15.0 shipped a `sites` bundle that could not
build at all (stale `npmDepsHash`, then un-copied ui3 files) — the release
suite is green on all of that, because none of it nix-builds the packages.

`SKIP_NIX_PREFLIGHT=1 scripts/export-catalyrst.sh` bypasses the gate. It is an
emergency hatch (the forge is unreachable, a release must go out to unbreak
something) — a bypassed export should be followed by a real one.
