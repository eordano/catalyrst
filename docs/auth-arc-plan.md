...
> is out of scope (server sync files are concurrent-hot; see §5).
...
2. `catalyrst-credits` — second. All 13 admin routes are already isolated in one
   ...
   per-crate "every admin route names the extractor" scan (§4) is trivially anchored.
...
4. `catalyrst-economy` — last. Most diffuse: 11 `require_admin` sites scattered across
   ...
   settled, and lean on the route scan (§4) to prove none were skipped.
...
1. Per-crate source-discipline test (primary; zero new deps).
   ...
   - *badges / economy* — admin routes are interleaved with public ones, so the scan keys
     ...
     gate is gone) **and** that the handlers previously listed in §1 name the extractor.
...
migrating them is the same four-step transform (§3) plus the same per-crate route scan (§4).
