...
- `retry_failed` re-attempts `failed_deployments` on a slow interval with its own `peer_servers` pool; `SYNC_RETRY_CONCURRENCY` (default 10) rows/pass; `RETRY_FAILED_ENABLED=false` disables. Each row carries its own ladder (`retry_count`, `next_retry_at`, migration 0006): a row is only dispatched once `next_retry_at` has passed, and a failed attempt spends one count and defers the row by 15 min doubled per *already spent* attempt, clamped at 24 h -- the deadline is stamped when the failure is recorded, not when the attempt was dispatched (the batch deployer is asynchronous, so the two differ by the download). At `MAX_FAILED_DEPLOYMENT_RETRIES` (default 10; `0` gives up on the first look) the row is evicted by a batched `DELETE ... retry_count >= cap`, guarded so a give-up decision cannot take a row that a successful deploy cleared and a fresh failure re-created lower. The ladder reaches the cap after ~4.3 days of cumulative backoff, so `RETRY_FAILED_PRUNE_TTL_DAYS` (default 7) only ever ages out rows the worker never touches -- chiefly rows whose `auth_chain` is empty, which are neither retried nor evicted (upstream parity).
