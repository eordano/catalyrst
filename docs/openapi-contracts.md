...
`ui3/src/generated/catalyst/openapi/` and consumed by sites through
`sites/app/lib/catalyst/typed.ts` (`eventsApiPath` / `placesApiPath` /
...
- `sites/scripts/gen-openapi-ts.mjs` converts each `*.openapi.json` in that dir to a
...
7. Declare the spec in the crate's own `Cargo.toml` — this is what enrols it in
   ...
   openapi = "ui3/src/generated/catalyst/openapi/<service>.openapi.json"
   ```

   The declared set is enforced on the way out too: `gen-openapi-ts.mjs` takes it
   from `generated-artefacts.mjs --list catalyrst-openapi` and exits 3 when the
   specs a run actually produced differ, so a crate cannot start or stop emitting a
   spec without the declaration and the committed artefacts moving with it. Then run
   `npm run gen:types` in sites and commit the regenerated
   `ui3/src/generated/catalyst/openapi/` files and `ui3/src/generated/INDEX.json`.
8. In sites, add a `servicePath<...>` export to `sites/app/lib/catalyst/typed.ts`
...
(components-resolved, via `jsonschema`). A coverage ledger then fails the test if
any spec'd operation was never hit, lacks a success case, or lacks an error case.
