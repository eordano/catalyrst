...
- `/admin*` is never exposed on the public edge
  ...
  loopback port or over the tailnet, and now additionally gated by the wallet
  allowlist.
...
- Flags: `HttpOnly; SameSite=Strict; Path=/; Secure`. `Secure` is **on by default**;
  set `ADMIN_COOKIE_INSECURE=1` only for a plain-HTTP tailnet with no TLS terminator
...
| `ADMIN_COOKIE_INSECURE` | drop the cookie `Secure` flag (plain-HTTP tailnet only) | unset ⇒ `Secure` set |
...
| telemetry `/dash/*` | no token — loopback-trusted (see §3); MUST be firewalled to loopback/tailnet | — |
...
1. **Loopback-trusted (no downstream auth).** The bundle's admin endpoint has no
   auth and is meant for loopback/tailnet (e.g. telemetry `POST /dash/issue/state`,
...
> **Deploy invariant:** sibling admin ports must stay loopback/tailnet-only — the
...
- Audit "actor" is now taken from the console-set `X-Catalyrst-Admin` header
  ...
  is best-effort by design (sibling admin ports stay loopback/tailnet-only).
