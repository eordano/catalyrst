...
To instead answer the stock explorer's gateway contract
(`{host}/{subdomain}/{path}`, no explorer code change), use the gateway edge
(`docs/deploy/nginx-catalyrst-gateway.conf`) — see [gateway.md](./gateway.md).
