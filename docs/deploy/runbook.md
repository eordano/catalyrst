...
> Bundle deployment style: one TLS host, six bundle binaries + the content core, all loopback, path-routed by a reverse proxy ([nginx-catalyrst-bundles.conf](./nginx-catalyrst-bundles.conf)). Alternatives (NixOS module, per-service standalone units): [architecture.md section 5](../architecture.md).
...
Bundles bind `BUNDLE_HTTP_PORT` on loopback; pick any consistent port range. Prereqs: PostgreSQL 18 reachable, the marketplace (squid) DB populated, the content DB synced, a LiveKit SFU reachable.
...
Pin the built binaries at stable paths for the units (symlink from `target/release/`).

## 2. Mint least-privilege DB roles

Read-only roles where a bundle only queries; owner roles where a member runs its own sqlx migrations at `build_state()` (the writer role needs DDL on its DB - ownership map in [architecture.md section 3](../architecture.md)):
...
A bundle's env file is the concatenation of its members' env files (each `crates/catalyrst-<member>/src/config.rs` has the authoritative variable list). Duplicate `HTTP_SERVER_PORT`/`HTTP_SERVER_HOST` keys are harmless - the bundle binds only `BUNDLE_HTTP_PORT` - but check any other overlapping key (e.g. `DAPPS_PG_COMPONENT_PSQL_*`, shared by market/economy) resolves to the value you want. `chmod 600` everything.

Gotchas:

- `catalyrst-live` auto-loads `/etc/catalyrst/content.env` at boot in addition to its unit environment.
- LiveKit: comms and worlds fail fast at boot without `LIVEKIT_API_KEY`/`_SECRET` (set `LIVEKIT_ALLOW_DEV_CREDS=1` to opt into the dev defaults; archipelago still boots on them with a warning, minting JWTs a real SFU rejects). Set key/secret/`LIVEKIT_HOST` (+ `LIVEKIT_WEBHOOK_KEY`) consistently across explore (archipelago, worlds) and social (comms) - they share one SFU ([../operations/livekit.md](../operations/livekit.md)).
- Money endpoints: credits/market/economy refuse a non-loopback bind without their admin token set - intended; keep them loopback behind the proxy.
- Template units: `nixos/systemd/catalyrst-*.service` (fill the `<WORKSPACE>`/`<DATA_DIR>` placeholders, `EnvironmentFile=` per bundle).

## 4. Realm discovery (`/about` public URLs)

Clients discover the realm from `GET /about`; point the content core's public URLs at the TLS host:
...
Non-realm-discovered hosts (places/market/communities/...) are reached via client URL substitution + edge path-routing - no `/about` field carries them ([explorer-pointing.md](./explorer-pointing.md)).
...
Reload the proxy with the bundle server block ([nginx-catalyrst-bundles.conf](./nginx-catalyrst-bundles.conf)).
...
Green = every `/health` ok, edge `/about` says `healthy:true` with the realm name, smoke curls return data.
...
- Standalone `catalyrst-market` is only needed to isolate marketplace from economy/price/credits.
- Content-serving throughput: enable nginx `X-Accel-Redirect` zero-copy via `STORAGE_X_ACCEL_BASE` (DEPLOYMENT.md).
