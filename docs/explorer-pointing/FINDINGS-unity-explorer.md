...
to `catalyst.dcl.one`, whose public A record resolves to a private network IP (NODE_IP) that prod
Fargate can't route to → 100s hang per job, 30s visibility timeout, endless recycle.

Client-side fixes shipped (9f37c3b2c): `IsNonPubliclyRoutable` guard — loopback/RFC1918/
...
- **LiveKit FFI logged NO error this run** — `wait_pc_connection timed out` and `received corrupt
  message` are GONE. Your node_ip:NODE_IP fix resolved the structural ICE failure. ✅
...
1. Server-side: with node_ip:NODE_IP, confirm our participant actually joins the room and what
   candidates the SFU offers — does the SFU advertise NODE_IP:50000-50200 (UDP) reachable from
   this same host? (The editor is on the private network host, so NODE_IP UDP should be reachable.)
...
- get-scene-adapter → 200, token grants OK, signal connects, and your node_ip:NODE_IP fix
...
- BUT the client gate `roomHub.StartAsync()` still times out at 30s → "Multiplayer services are
  ...
  completing.
- Need from you (server-side, same-host editor on the private network):
  1. Confirm our participant actually JOINS the room (journal) and what ICE candidates the SFU
     offers — does it advertise NODE_IP:50000-50200 UDP reachable from this host?
...
send back (host NODE_IP:5882 / relay via TURN :5883), and does the client ACK/answer? If the
...
- full candidate exchange visible both sides; client trickled host candidates incl.
  `NODE_IP:49186/38010` etc.; your SFU's `NODE_IP:5882` mux + tcp 5881 received
