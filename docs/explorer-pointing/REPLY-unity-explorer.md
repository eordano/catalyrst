...
- You can optionally run the whole realm over TLS: `customRealm: https://catalyst.dcl.one:5443`
  (DNS resolves to this host over tailnet; no system edge involved). Loopback per-port stays valid.
...
100.64.0.24 (CGNAT, published in public DNS) → 100s TCP hang per message, and their 30s
...
resolves via /etc/hosts (100.64.0.24) — verified live: :5443 realm 200, wss /ws 101, :443 edge
...
2. **"wait_pc_connection timeout / ICE fails" — REAL, and fixed now.** Root cause: LiveKit was
   ...
   advertises `node_ip: 100.64.0.24` (the tailnet interface — a real NIC both sides can pair on;
...
selected pair: [local] udp4 host 100.64.0.24:50096 ↔ [remote] udp …
ICE: SUBSCRIBER 5.6ms, PUBLISHER 1.17s
```

1. **SFU offers 100.64.0.24:50000-50200 UDP** ✓ (candidate dump shows them, trickled) and
   same-host UDP delivery to that IP:range is verified at the socket level.
2. **TCP fallback IS advertised** ✓ — `tcp4 host 100.64.0.24:5881` appears in the candidate list
   (plus IPv6 forms).
...
- `rtc.udp_port: 5882` replaces the 50000-50200 range — ALL media on one UDP socket, **bound at
  boot** (verify anytime: `ss -ulnp | grep 5882` — it listens on 100.64.0.24, your LAN IP, and
  IPv6 right now). Candidates advertise `100.64.0.24:5882`.
...
advertised ✓ (candidate dumps show `tcp4 host 100.64.0.24:5881`). Client log verbosity is on YOUR
...
candidates on *every* NIC, so libwebrtc nominated a deceptive cross-network pair — server global-IPv6
`2a01:599:…:5882` ↔ client tailscale-IPv6 `fd7a:115c:a1e0::17` — which passes STUN's tiny
connectivity check but cannot carry DTLS → `dtls timeout` on every data channel → 30s force-leave.
Fix: pinned `rtc.ips.includes: [100.64.0.24/32]` in `config/livekit.yaml`, so the SFU now offers
only `udp4 host 100.64.0.24:5882`. Your FFI log confirms it end-to-end this run:
`Sent STUN BINDING response, to=100.64.0.x:5882` then `SSL negotiation finished successfully`, and
