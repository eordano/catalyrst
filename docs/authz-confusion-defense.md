...
| `Signer` newtype on the `catalyrst-crypto` verifier entry points | **Adopt next** | Turns `scene_adapter.rs:211` into E0308; pattern already proven in-tree |
...
- **Middleware that fails open.** `catalyrst-comms/src/lib.rs:220-229`: `voice_auth_layer`
  enforces the bearer token only `if let Some(expected) = state.gatekeeper_auth_token`.
  `COMMS_GATEKEEPER_AUTH_TOKEN` appears in neither the deployment's `catalyrst-comms`
  env file nor its template, so in that configuration the community voice moderation routes —
  promote speaker, demote speaker, kick player — take `community_id` and `user_address`
  straight from the path with no authentication. **This needs a decision independently of
  everything else in this document.** A static rule cannot distinguish a layer that
  authenticates from one that is present; only Stage 1b's runtime probe can, and only once
  comms is spec'd.
...
- `HEAD` `apply.rs:589` reproduces the upstream shape exactly; the working tree adds the
  `AND community_id = $3` predicate and remains silent on a freshly minted id, returning
  200 plus a log append plus gossip.
- `community_requests` rows are created only at `writes/requests.rs:230` and creation is
  never gossiped, so a remote node applying a status update legitimately sees zero rows —
  the loudness contract must differ by caller.
- `rest/handlers/client/requests.rs:162` and `catalyrst-comms/src/handlers/scene_adapter.rs:210`
  are unfixed in the working tree; `tests/server_scene_adapter_auth.rs` is a red test
  against the latter.
- `MetaTxSender` (`catalyrst-economy/src/ports/transaction.rs:205`) is a private-field
  newtype minted only by calldata recovery — the `Signer` pattern, already in-tree.
- Workspace measurements: 49 crates, sqlx 0.9, 0 `query!` macros, 222 `AssertSqlSafe`
  sites, 569 `.execute(` sites against 59 `rows_affected` mentions, 41 `try_extract_signer`
  references, `clippy.toml` holding a single line.
- Five crates carry `tests/contract_gate.rs`; three are in `OPENAPI_CRATES`.
  `catalyrst-comms` and `catalyrst-economy` have no `utoipa` dependency.
- `COMMS_GATEKEEPER_AUTH_TOKEN` is absent from both the deployment's `catalyrst-comms`
  env file and its template, so `voice_auth_layer` currently fails open there.
