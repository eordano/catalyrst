...
export function ConversationPanel({mode,community,identity,opened,parent,onClose,onThread,onReply,onRefresh,memberStatuses={}}: {mode:Panel;community:Community;identity:WalletIdentity;opened:Opened;parent?:Message;onClose:()=>void;onThread:(message:Message)=>void;onReply:(text:string)=>Promise<void>;onRefresh:()=>void;memberStatuses?:Record<string,number>}) {
  ...
  return <aside className="conversation-panel" aria-label={title}><header><div><h2>{title}</h2><small>{community.name}</small></div><button aria-label="Close panel" onClick={onClose}><Icon name="close"/></button></header>
