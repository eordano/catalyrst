import { publicEvents, usePublicDiscovery } from "./public-feeds";
...
function useEvents(search = "", identity: WalletIdentity | null = null, mine = false) {
  const privatePage = usePrivateEvents(search, identity, mine);
  const publicPage = usePublicDiscovery(publicEvents, search, !mine);
  return mine ? { ...privatePage, refreshFailed: false } : {
    events: publicPage.data,
    loading: publicPage.loading,
    error: publicPage.error && !publicPage.refreshFailed ? "Events could not be loaded." : "",
    refreshFailed: publicPage.refreshFailed,
    more: publicPage.more,
    loadMore: publicPage.loadMore,
    retry: publicPage.retry,
  };
}
...
export function EventsPreview(props: Props) {
  const { events, error, loading, refreshFailed, retry } = useEvents(),
...
export function EventsPage(props: Props & { search?: string; mine?: boolean }) {
  ...
  const { events, error, loading, refreshFailed, more, loadMore, retry } = useEvents(
    search,
    props.identity,
    mine,
  );
