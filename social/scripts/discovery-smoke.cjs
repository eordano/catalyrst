...
(async () => {
  ...
  try {
    ...
    let friendGoing = false, createFails = true;
    ...
    console.log(
      "Discovery: RSVP/calendar/My RSVPs, friends attending, notification baseline/dedup/wallet isolation, world sharing passed",
    );
  } finally {
    await browser?.close();
    await server.close();
