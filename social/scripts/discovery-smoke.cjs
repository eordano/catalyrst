...
(async () => {
  ...
  try {
    ...
    for (const name of ["Worlds", "Events"]) assert.equal(await page.getByRole("link", { name, exact: true }).count(), 0, `${name} left the navigation`);
    assert.equal(await page.locator(".social-home").getByText("Party World", { exact: true }).count(), 0, "Home no longer previews worlds");
