{
  ...
  outputs =
    ...
    let
      ...
      # default system — notably aarch64-darwin, so `nix develop ~/one/catalyrst/ui3`
      # (and its `.envrc` `use flake`) works on this Mac.
      linuxSystems = [
        "x86_64-linux"
        "aarch64-linux"
      ];
      ...
      packages = forLinuxSystems (
        ...
        rec {
          storybook = pkgs.buildNpmPackage {
            ...
            src = ./.;
            inherit nodejs;
            npmDepsHash = "sha256-0lWDqKWbQJAujzLoQzG2ZOYkixi0YdXnHHsZPlanTbY=";
