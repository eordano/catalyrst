...
ui3 component family ported from the internal Marketplace UX Improvements Figma spec. Structure and layout kept faithful; restyled onto ui3's dark, tokenized surfaces (`--lm-*`, `--rar-*`, `--accent`, `--brand`) - the same divergence NftCard and the rest of `marketplace/` already use. Plain JSX + CSS, no new deps.
## Components
...
```
Page components pull in their children's CSS transitively via the `.tsx` imports; mounting an individual atom means importing its co-located lowercase `.css`.
