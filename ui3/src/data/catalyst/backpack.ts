...
export const EquippedSchema = z.object({
  ...
  name: nullableStr,
  wearables: z.array(z.string()).nullish().transform((v) => v ?? null),
  emotes: z.array(z.string()).nullish().transform((v) => v ?? null),
  ...
  emoteSlots: z.array(SlotBindingSchema).nullish().transform((v) => v ?? null),
});
...
export const EmoteSchema = z
  .object({
    ...
    loop: z.boolean().nullish().transform((v) => v ?? null),
  })
  .transform((e) => ({
    ...e,
    // A category the UI has no tab for is still a stated category, so it lands
    // in the catch-all bucket. One the metadata never stated stays unstated.
    category:
      e.category == null || EMOTE_CATEGORIES.includes(e.category)
        ? e.category
        : "miscellaneous",
  }));
