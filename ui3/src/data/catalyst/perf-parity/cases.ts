...
import { fetchEventCategories, fetchEvents } from "../events";
...
export const CASES: ParityCase[] = [
  ...
  },
  {
    ...
    probes: ["normalization"],
    note:
      "an emote whose metadata states a category the UI has no tab for — it belongs in the " +
  ...
  },
  {
    ...
    probes: [],
    note:
      "the control: this module has no transforms, so it is already parity — a failure here " +
...

  {
    ...
    note:
      ...
      "modes, so both return the two real parcels — and the perf build never reaches " +
  ...
  },
  {
    ...
    note:
      ...
      "put a card on a parcel nobody deployed, so the guard drops it in both modes — the " +
  ...
  },
  {
    ...
    group: "guarded",
    probes: ["row-guard"],
    note: "an unnamed category — no key, no label, nothing to filter by",
    run: () =>
      fetchCategories({
        fetchImpl: jsonFetch([
          [
  ...
  },
  {
    ...
    group: "guarded",
    probes: ["row-guard"],
    note: "a community node with no id — un-keyable in the grid, un-fetchable afterwards",
    run: () =>
      loadCommunities(
        {},
        {
  ...
  },
  {
    ...
    group: "guarded",
    probes: ["row-guard"],
    note: "an emote candidate carrying neither urn nor id — both modes must answer null",
...

  {
    ...
    probes: ["bad-row"],
    note:
      "a row the GUARD accepts and the SCHEMA does not — `likes` is a string. Perf keeps it " +
  ...
  },
  {
    ...
    note:
      ...
      "keeping it costs one empty card — losing the good ones would not be tolerable",
    run: () =>
      loadCommunities(
        {},
        {
