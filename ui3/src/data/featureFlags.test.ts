...
test.each(FEATURE_FLAGS)("$id uses its configured default, with URL taking priority over the browser override", ({ id, defaultEnabled }) => {
  expect(flagState(id)).toEqual({ enabled: defaultEnabled, override: "default", source: "default" });
  ...
  expect(flagState(id).enabled).toBe(defaultEnabled);
});
