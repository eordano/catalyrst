...
function validate(message: PlaySectionMessage, address: string) {
  if (message?.version !== 1 || message.address !== address || !names.includes(message.section)) {
  ...
  if (message.section === "featured" || message.section === "places" || message.section === "outfits") {
    ...
    if (message.section === "events" && "data" in data && Array.isArray(data.data) && "total" in data && typeof data.total === "number") return;
