...
```css
  /* Leading. One canonical value per text role; see foundry/GUIDELINES.md S1.
...
- Sections are separated by the stack's 32px and by their titles. At `--fs-18`
  bold `--text` (S1.3) a title is a real boundary; no rules, no backgrounds.
...
- The one legitimate `EmptyState` use is the route-level 404/outage screen
  outside `.fd`, and even there the head renders first (S2.1 rule 5).
...
| Deviation | Ruling |
|---|---|
| 15 distinct line-heights; ~60 roles with none | **NORMALIZE** -> the six `--lh-*` tokens, S1.1. Zero bare numbers in foundry CSS. |
...
| `.fd-response__sub` (h3, 13px) larger and brighter than its parent h2 (12px) | **NORMALIZE** -> S1.4 hierarchy. |
...
| Deviation | Ruling |
|---|---|
| Three primary-button vocabularies, two brand reds | **NORMALIZE** -> `Button`, `--brand-cta` (S3.4). |
| Four form systems | **NORMALIZE** -> `.fd-form` (S3.5). |
...
| `EmptyState` in-page | **NORMALIZE** -> `.fd-empty` (S2.5). |
...
| Console rail as a second nav layer | **KEEP** -- it is a real sub-hierarchy -- but see S5.1. |
...
- Every prose provenance block (S3.2).
...
- `/foundry/people`: six columns for one row. **Fix:** cards until the roster
  earns a table.
- `/foundry/copilot`: a 30x5 Skills table is the largest contributor to an
...
delete `.fd-pagehead`'s `margin-bottom`; **(S4)** apply the S1.3 role table and
