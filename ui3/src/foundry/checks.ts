...
export function failedChecksPhrase(counts: {
  ...
  if (checksUnevaluable > 0 && genuine <= 0) {
    return `${checksFailed} of ${checksTotal} checks could not be evaluated \u{2014} counted as failed`;
  }
  if (checksUnevaluable > 0) {
    return `${genuine} of ${checksTotal} checks failed, ${checksUnevaluable} more could not be evaluated \u{2014} counted as failed`;
  }
