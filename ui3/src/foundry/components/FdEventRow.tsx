    ...
    <>
      ...
      {storedChars === null ? null : (
        <p className="fd-ev__trunc">
          Truncated for display — {storedChars.toLocaleString()} characters stored.
        </p>
      )}
    </>
  );
      ...
      <div className="fd-ev__body">
        ...
        {shots.length > 0 ? (
          <div className="fd-ev__shots">
            <span className="fd-ev__shotslabel">
              screenshots written by the runner — file names only, evidence
              directories are not served from here
            </span>
            <ul>
              {shots.map((shot) => (
  ...
  return (
    <div className="fd-ev__body">
      <p className="fd-ev__omitted">
        {type} — payload not rendered ({storedChars.toLocaleString()} characters
        stored). Read it from the database or the run&apos;s evidence directory.
      </p>
    </div>
  );
}
...
export default function FdEventRow({ event }: { event: FdEventVM }) {
  const bracket =
