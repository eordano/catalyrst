...
export function marketCellTitle(mc: FdMarketCellVM): string {
  return `This program's reading, ${mc.classifiedAt} \u{2014} ${mc.rationale} (${mc.confidence})`;
}
    ...
    <article className="fd-gamecard">
      ...
      <header className="fd-gamecard__head">
        ...
        {game.verdict ? (
          <a
            ...
            aria-label={
              game.verdictDetail
                ? `${game.verdictLabel ?? (game.verdict === "fail" ? "failed" : "passed")} \u{2014} ${game.verdictDetail}`
                : game.verdict === "fail"
                  ? "last run failed"
                  : "last run passed"
            }
      ...
      <p className="fd-gamecard__chips">
        ...
        {game.lastMovedAt ? (
          <span
            className="fd-chip"
            title="most recent recorded scene movement after the deploy \u{2014} a redeploy, steward note, or stewardship change"
          >
