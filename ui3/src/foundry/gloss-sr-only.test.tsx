...
describe("FdTrajectoryRow \u{2014} full id + arena chip", () => {
  ...
  it("gives the arena runner chip a value-only sr-only twin \u{2014} never prose", () => {
    const html = renderToStaticMarkup(
      createElement(FdTrajectoryRow, { record, onOpen: noop }),
    );
...
describe("FdTrajectoryReplay \u{2014} arena chip", () => {
...
describe("FdResponse \u{2014} none-read confidence", () => {
  it("gives the honest none-of-six-jobs line its reading provenance as an sr-only twin", () => {
    ...
    expect(html).toContain(
      '<span class="u-sr-only"> \u{2014} This program&#x27;s reading, 2026-08-10 \u{2014} Reads no observable machinery for any of the six. (inferred)</span>',
    );
  });
});

describe("FdEventRow \u{2014} NaN-time raw stored time", () => {
  ...
  it("a parseable time keeps using FdTime's own sr twin instead \u{2014} no double gloss", () => {
    const event: FdEventVM = {
      seq: 0,
      type: "turn/start",
