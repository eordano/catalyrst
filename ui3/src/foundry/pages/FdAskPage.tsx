...
function readingProvenance(reading: FdAskReadingVM): string {
  return `This program's reading, ${reading.readAt} \u{2014} ${reading.rationale} (${reading.confidence})`;
}
...
function FdEditAskForm({
  ...
  const bodyError = overLimit
    ? `Keep the description under ${FD_BODY_MAX} characters \u{2014} it is ${body.length} now. Nothing was saved; your text is kept.`
    : errors.body ?? null;
...
export default function FdAskPage({
  ...
  return (
    <div className="fd-page fd-stack fd-ask">
      ...
      <FdSection id="reading" title="This program\u{2019}s reading">
        {reading === null ? (
