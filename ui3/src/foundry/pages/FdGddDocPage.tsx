...
/** The doc chain's newest build request, straight from its ledger row.
 *  fileNote is the broker's on-disk step note -- display detail only; no file,
 *  no note. */
export type FdGddBuildRequestVM = {
  status: "queued" | "building" | "verifying" | "landed" | "failed";
  sceneSlug: string;
  detail: string;
  createdAt: string;
  updatedAt: string;
...
/** The build state of THIS doc chain: the newest request (any status) or the
 *  honest absence, whether the viewer may ask for one (the doc's maker or a
 *  host -- presentation only, the server re-checks in-tx), and the request
 *  wiring. Absent (null) hides the whole block. */
export type FdGddBuildVM = {
  request: FdGddBuildRequestVM | null;
  canRequest: boolean;
  pending: boolean;
  error?: string | null;
  onRequest: (sceneSlug: string) => void;
};
...
export type FdGddDocPageProps = {
  ...
  /** Absent (null) hides the whole build block -- same rule as approval. */
  build?: FdGddBuildVM | null;
  ...
  return (
    <ol className="fd-gdd__versionlist" aria-label="Versions">
      {chain.map((v, i) => {
        ...
        const delta =
          prev && prev.open !== v.open ? `open ${prev.open}\u{2192}${v.open}` : null;
        const body = (
          <>
...
function GddPlayAffordance({
  ...
  if (play.tier === "play") {
    return (
      <>
        ...
        >
          {play.deployed === false
            ? `See the build \u{2014} ${play.sceneTitle}`
  ...
  if (play.tier === "same-concept") {
    ...
    const reading = `this program's reading, ${dayUTC(play.readAt)}${readAgainst} \u{2014} ${play.rationale} (${play.confidence})`;
    return (
      <a
...
const BUILD_STATE_LABEL: Record<string, string> = {
  queued: "Build queued",
  building: "Build underway",
  verifying: "Build verifying",
};

/** An editable suggestion in the shape the server accepts -- never submitted
 *  on its own. */
...
function GddBuildRequestForm({
  ...
  return (
    <div className="fd-gdd__buildform">
      <label className="fd-form__label" htmlFor="fd-gdd-build-slug">
        Scene id
      </label>
      <input
        id="fd-gdd-build-slug"
...
function GddBuildBlock({ doc, build }: { doc: FdGddDocVM; build: FdGddBuildVM }) {
  const req = build.request;
  let body;
  if (req && req.status === "landed") {
    body = (
      <p className="fd-note">
        Built &#x2014;{" "}
        <a href={`/foundry/play/${req.sceneSlug}`}>
          play it at /foundry/play/{req.sceneSlug} &#x2192;
        </a>
        {req.detail ? ` ${req.detail}` : null}
      </p>
    );
  } else if (req && req.status === "failed") {
    body = (
      <>
        <p className="fd-note">
          The build failed{req.detail ? ` \u{2014} ${req.detail}` : ""}.
        </p>
        {build.canRequest ? (
          <GddBuildRequestForm
            label="Request another build"
            initialSlug={req.sceneSlug}
            pending={build.pending}
            onRequest={build.onRequest}
          />
        ) : null}
      </>
    );
    ...
    body = (
      <p className="fd-note">
        <span className="fd-chip">{BUILD_STATE_LABEL[req.status] ?? req.status}</span>{" "}
        &#x2014; since{" "}
        <FdTime iso={req.createdAt} title={stampUTC(req.createdAt)}>
          {dayUTC(req.createdAt)}
        </FdTime>
        {req.detail ? ` \u{2014} ${req.detail}` : null}
        {req.fileNote ? ` \u{B7} ${req.fileNote}` : null}
      </p>
    );
  } else if (build.canRequest) {
    body = (
      <GddBuildRequestForm
        label="Build this"
        initialSlug={suggestSlug(doc.title)}
        pending={build.pending}
        onRequest={build.onRequest}
      />
    );
    ...
    body = (
      <p className="fd-note">
        A build is requested by this doc&apos;s maker or a host.
      </p>
    );
  }
  return (
    <div className="fd-gdd__build">
      {body}
      {build.error ? (
        <p className="fd-alert" role="alert">
          {build.error}
  ...
  return (
    <div className="fd-page fd-stack fd-gdd">
      ...
      <FdSection title="This design doc">
        ...
        {build ? <GddBuildBlock doc={doc} build={build} /> : null}
      </FdSection>
              ...
              <tbody>
                {doc.hypotheses.map((h) => (
                  <tr key={h.id}>
                    ...
                    <td className="fd-table__prose">{h.ifThen ?? "\u{2014}"}</td>
                    <td className="fd-table__prose">{h.test ?? "\u{2014}"}</td>
