...
function shortEntityId(id: string): string {
  return id.length > 14 ? `${id.slice(0, 14)}\u{2026}` : id;
}
...
export default function FdGameDetail({
  ...
  if (designVersions.length > 0) {
    for (const v of designVersions) {
      nodes.push({
        ...
        label: `Design v${v.version} \u{2014} ${
  ...
  for (const r of runs) {
    ...
    nodes.push({
      ...
      label: `${r.provenance} run \u{B7} ${plural(r.events, "event")} \u{B7} ${
  ...
  for (const b of benchOnlyRuns) {
    nodes.push({
      ...
      label:
        b.checksTotal === null
          ? "bot run \u{B7} verdict not recorded"
          : (b.checksFailed ?? 0) > 0
            ? `bot run \u{B7} ${runVerdictReading({
                verdict: "fail",
                checksFailed: b.checksFailed,
                checksTotal: b.checksTotal,
                checksUnevaluable: b.checksUnevaluable,
              }).label}`
            : `bot run \u{B7} all ${plural(b.checksTotal, "check")} passed`,
    });
  }
  nodes.sort(
    (a, b) =>
      ...
      : conceptReadings.map((r) => ({
          ...
          title:
            ...
            (r.readAgainstVersion != null
              ...
              : "") +
            ` \u{2014} ${r.rationale} (${r.confidence})`,
        }));

  const futures: FdHistoryFutureVM[] = gatherings.map((g) => ({
    key: `${g.seriesId}#${g.occurrenceAt}`,
    at: g.occurrenceAt,
    label: `${g.title} \u{B7} ${plural(g.rsvpCount, "person")} coming`,
            ...
            <p className="fd-gamedetail__headchips">
              {steward ? (
                <a
                  ...
                  href="#memory"
                  title={`since ${dayUTC(steward.since)} \u{2014} \u{201C}${steward.basis}\u{201D}`}
                >
                  Stewarded by{" "}
              ...
              {demandAsks.map((a) =>
                a.pledges > 0 ? (
                  <a
                    ...
                    title={`real pledges on \u{201C}${a.title}\u{201D} \u{2014} the ask this program read as answered by this game`}
                  >
              ...
              {verdictReading ? (
                <a
                  ...
                  aria-label={
                    verdictTitle
                      ? `${verdictReading.label} \u{2014} ${verdictTitle}`
      ...
      <FdSection title="The build">
        <dl className="fd-facts">
          ...
          <Fact label="Parcels" prov={game.parcels === null ? null : prov}>
            {game.parcels === null ? "\u{2014}" : groupDigits(game.parcels)}
          </Fact>
          <Fact label="World" prov={game.worldName ? game.sourceNote : null} mono>
            {game.worldName ?? "\u{2014}"}
          </Fact>
