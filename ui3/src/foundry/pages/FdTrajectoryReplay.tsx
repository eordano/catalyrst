...
function StatePanel({ reading }: { reading: FdStateReading }) {
  const from =
    reading.fromSeq === reading.atSeq
      ...
      : `read from the obs/snapshot lines at #${reading.fromSeq}\u{2013}#${reading.atSeq}`;
  return (
          ...
          <div>
            ...
            <dd className="fd-mono">
              ...
              {header.seedLength === null
                ? ""
                : ` \u{B7} ${plural(header.seedLength, "seeded event")}`}
            </dd>
          </div>
        ) : null}
        {header.evidencePath ? (
            ...
            {frames && frames.length > 0 ? (
              <div className="fd-replay__frames" aria-label="Captured frames">
                <p
                  ...
                  title="written by the runner, in shot order \u{2014} not mapped to the cursor"
                >
                  Captured frames
                </p>
