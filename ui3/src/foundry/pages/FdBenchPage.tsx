...
export function checksLabel(report: FdBenchReportVM): string {
  if (report.checksTotal === null) {
    ...
    return report.verdict === null
      ? "snapshot only \u{2014} verdict not recorded"
      : "verdict from the runner's exit status \u{2014} no per-check breakdown recorded";
  }
  ...
  if (failed > 0 && unevaluable === failed) {
    ...
    const could = `${failed} check${failed === 1 ? "" : "s"} could not be evaluated \u{2014} counted as failed`;
    return failed === report.checksTotal
      ? could
      : `${report.checksTotal - failed} of ${report.checksTotal} checks passed \u{2014} ${could}`;
  }
    ...
    <article className="fd-benchcard">
        ...
        {report.runner === "arena" ? (
          ...
          <span className="fd-benchcard__sandbox">
            ...
            {report.verdict ? (
              <span
                className="fd-mono"
                title="the runner's exit status \u{2014} not a verdict on the game"
              >
                exit: {report.verdict}
              </span>
            ) : null}
      ...
      <p className="fd-benchcard__chips">
        {isLocalRealm(report.realm) ? (
          <span
            className="fd-chip"
            title={`realm ${report.realm} \u{2014} a local copy of the scene, not the deployed World`}
          >
            local copy
      ...
      {shown > 0 ? (
        <div className="fd-statrow">
          <FdStat
            label="Runs"
            value={runsTotal}
            title="one card per harness run \u{2014} sandbox simulations are not counted"
          />
          {simsTotal > 0 ? (
            <FdStat
              label="Sandbox sims"
