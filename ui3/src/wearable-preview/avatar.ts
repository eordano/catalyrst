...
// THREE.Cache is deliberately left OFF. The /foundry crowd mounts several stages
...
// "Texture marked for update but no image data found" every frame (this is what
// left 3 of 5 avatars as untextured silhouettes). The content-addressed GLB/PNG
// files are immutable, so the browser HTTP cache still serves the refetch from
// disk — we lose a re-decode, not a round trip. Entity JSON is deduped separately
// (fetchActiveEntities), which is safe because it is not a per-context GPU object.
...
export function createAvatarScene(
  ...
  const sway = opts.spin !== false && !prefersReducedMotion();
