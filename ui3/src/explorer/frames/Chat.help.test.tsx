...
describe("/help against the engine console", () => {
  test("sends /help, hides the raw reply and renders the merged list after the echo, curated entries first and bare engine names after", async () => {
    ...
    const text = help();
    expect(text).toHaveTextContent("/help [command] \u{2014} list the chat commands, or the usage of one");
    expect(text).toHaveTextContent("/fps <fps> \u{2014} set the target frame rate");
  ...
  test("/help <command> answers from the table when it has the entry and asks the engine otherwise", () => {
    ...
    h.type("/help fps");
    expect(h.send).not.toHaveBeenCalled();
    expect(screen.getByText("/fps <fps> \u{2014} set the target frame rate")).toBeInTheDocument();
