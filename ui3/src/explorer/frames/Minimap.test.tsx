...
describe("Minimap scene options", () => {
  ...
  it("disables the owner actions until the owner is known, and explains why they are off inside a world", async () => {
    await openMenu();
    expect(screen.getByRole("menuitem", { name: /^Send feedback to scene owner/ })).toBeDisabled();
    expect(screen.getByRole("menuitem", { name: /^Send tip to scene owner/ })).toBeDisabled();
    expect(screen.getAllByRole("menuitem", { name: /owner unknown/ })).toHaveLength(2);
    cleanup();
  ...
  it("opens the feedback and tip dialogs for the resolved owner", async () => {
    ...
    const user = await openMenu();
    const feedback = screen.getByRole("menuitem", { name: /^Send feedback to scene owner/ });
    expect(feedback).toBeEnabled();
    await user.click(feedback);
    expect(await screen.findByRole("dialog", { name: "Send feedback to scene owner" })).toBeInTheDocument();
    expect(screen.getByText("0x92de\u{2026}4f9c")).toBeInTheDocument();
    await user.click(screen.getByRole("button", { name: "Close" }));

    await user.click(screen.getByRole("button", { name: "Scene options" }));
    await user.click(screen.getByRole("menuitem", { name: /^Send tip to scene owner/ }));
    expect(await screen.findByRole("dialog", { name: "Send tip to scene owner" })).toBeInTheDocument();
  });
});
