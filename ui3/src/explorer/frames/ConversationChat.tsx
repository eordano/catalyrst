...
function AccountConversations({ kind, active }: { kind: ConversationKind; active: boolean }) {
  ...
  return <div className="cchat">
    {conversation && identity.address ? <>
      ...
      {!list.isPending && !list.isError && !list.data?.length && <div className="cchat__notice cchat__empty"><p>{kind === "direct" ? "Add a friend to start a direct conversation." : "Join a community to talk with its members."}</p>{kind === "community" && <Button data-sb-linkto="Explorer/Pages/Communities">Discover communities</Button>}</div>}
...
function Thread({ kind, conversation, address, name, active, draft, onDraftChange }: {
  ...
  return <>
    {thread.isPending && <ContentStatus pending message="Loading messages&hellip;" />}
    {thread.isError && !thread.data && <ContentStatus message="Couldn't load messages." onRetry={() => { void thread.refetch(); }} />}
    {thread.isError && thread.data && <p className="cchat__notice" role="alert">Could not refresh messages. <button onClick={() => { void thread.refetch(); }}>Retry</button></p>}
    {thread.data && <ChatView open={active} onToggle={() => {}} io={io} docked header={false} title={label(conversation)} commands={false} draftValue={draft} onDraftChange={onDraftChange} profileCard={ProfileCard} onViewProfile={viewProfile ? (user, opener) => viewProfile(user.address, opener) : undefined} />}
  </>;
}
