...
export default function Chat({
  ...
  const chatPushes = useBridgeState((s) => s.chat);
  ...
  const me = identity.address ? { address: identity.address, name: identity.name } : null;
  ...
  const onLocation = (x: number, y: number): void => {
    sendBridge("Teleport", { x: x * PARCEL_SIZE + PARCEL_SIZE / 2, z: y * PARCEL_SIZE + PARCEL_SIZE / 2 });
  };
  const onVisitWorld = (name: string): void => {
    sendBridge("ChangeRealm", { realm: name });
  };
        ...
        <div ref={listRef} className={styles.messages}>
          {rows.length === 0 ? (
            <div className={styles.empty}>
              {live ? "No messages yet — say hello to Nearby." : "Connecting to Nearby chat…"}
            </div>
          ) : (
            rows.map((r) =>
              r.kind === "day" ? (
