...
function isPermissionWithdrawnPush(v: unknown): v is PermissionWithdrawnPush {
  return (
    typeof v === "object" &&
    v !== null &&
    (v as { kind?: unknown }).kind === "permissionWithdrawn" &&
    typeof (v as { id?: unknown }).id === "number"
  );
}
...
export default function PermissionPrompt() {
  ...
  useEffect(() => {
    return subscribeBridge((push) => {
      if (isPermissionWithdrawnPush(push)) {
        setQueue((q) => q.filter((x) => x.id !== push.id));
        return;
      }
