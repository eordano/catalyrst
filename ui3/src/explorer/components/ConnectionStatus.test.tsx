...
describe("ConnectionStatus", () => {
  test("keeps connection status without duplicating realm or room facts", () => {
    ...
    expect(screen.getAllByText("Realm")).toHaveLength(1);
  ...
  test("prioritizes engine fps and labels page timing separately", () => {
    ...
    expect(screen.queryByText(/not GPU render time/)).toBeNull();
  });

  test("hides the engine reading without an engine and still reports the page rate", () => {
  ...
  test("renders the debug block from the realm, scene and comms facts", () => {
    ...
    expect(rowValue("Comms")).toContain("archipelago:");
    expect(rowValue("Server")).toContain("8.0.3");
  ...
  test("reads the build from the client after mount", () => {
    mount({ at: undefined, userAgent: undefined, overlayBuild: undefined });
    expect(rowValue("Overlay build")).toBeTruthy();
  });
