...
export const RetryAvailable: Story = {
  beforeEach: () => {
    ...
    bridge.send = (action, payload) => {
      send(action, payload);
      if (action === "GetLifecycleSnapshot") bridge.push({ kind: "lifecycle", snapshot: lifecycleFixture({ connections: [{
        id: "transport:12", protocol: "websocket", scene: null, control: false,
        phase: "down", attempt: 2, error: "Connection closed. Local exploration is available.", canRetry: true,
      }] }) });
    };
    window.dclBridge = bridge;
    return () => { window.dclBridge = previous; };
  },
  args: { connection: { sceneHealth: "ok", sceneRoom: false, globalRoom: false }, realm: "catalyst.example.com", fps: { page: 60, engine: 60, ms: 16.7 }, about: null, sceneId: null, ...PINNED },
};
