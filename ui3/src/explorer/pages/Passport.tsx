...
export default function Passport({
  ...
  return (
    <div className="ep__backdrop ps__backdrop" onClick={event => { if (event.target === event.currentTarget) onClose?.(); }}>
      <div className="ps" aria-busy={loading} data-loading={loading || undefined}>
        {loading && <div className="ps__loading" role="status"><Spinner size={34} /><span>Loading profile&hellip;</span></div>}
          ...
          <header className="ps__head">
            <div className="ps__id">
              ...
              {(identity?.wallet || identity?.address) ? (
                <WalletAddress address={identity?.address || identity?.wallet || ""} />
              ) : null}
            </div>
