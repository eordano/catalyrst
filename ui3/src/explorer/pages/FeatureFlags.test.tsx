...
test("settings overrides persist, reset together, and expose when a reload is needed", async () => {
  ...
  expect(screen.queryByRole("button", { name: "Reload Explorer" })).toBeNull();
  expect(screen.queryByText("Your feature flags are up to date.")).toBeNull();
  ...
  await user.type(screen.getByRole("searchbox"), "connection");
...
test("storage failure leaves the previous selection and an actionable error", async () => {
  ...
  await userEvent.selectOptions(screen.getByLabelText("Interaction feedback"), "disabled");
  expect(screen.getByRole("alert")).toHaveTextContent("Allow site storage");
  expect(screen.getByLabelText("Interaction feedback")).toHaveValue("default");
});
