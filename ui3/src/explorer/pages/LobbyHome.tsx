...
function LoadState({ pending, error, empty, retry }: { pending: boolean; error: boolean; empty: string; retry: () => unknown }) {
  if (pending) return <p className="lh__note" role="status">Loading&#x2026;</p>;
...
export default function LobbyHome({ onEnterWorld, onSignOut }: { onEnterWorld: () => void; onSignOut?: () => void }) {
  const navigate = useNavigate();
  const identity = useBridgeState((s) => s.identity);
  const scene = useBridgeState(s => s.scene);
  const [linkedPreview] = useState(() => { const params = new URLSearchParams(window.location.search); return params.has("preview") || params.has("systemScene"); });
  ...
  const [hasAvatarFrame, setHasAvatarFrame] = useState(false);
  const [headerMenu, setHeaderMenu] = useState<"account" | "notifications" | null>(null);
  const [portrait, setPortrait] = useState(avatarPreview);
  useEffect(() => { if (avatarPreview) setPortrait(avatarPreview); }, [avatarPreview]);
  const look = {
    name: identity.name || "",
  ...
  const featured = usePlaces({ limit: 24, order_by: "most_active" });
  const results = usePlaces({ search: query, limit: 24 }, !!query);
  const worlds = useWorlds({ search: query, limit: 24 }, !!query);
  const events = useEvents({ list: "live", limit: 12 });
  const upcoming = useEvents({ list: "upcoming", limit: 6 });
  ...
  return <main className="lh" aria-label="Decentraland lobby">
    <header className="lh__header">
      ...
      <div className="lh__tools">
        ...
        <button className="lh__credits" aria-label={credits.data ? `${credits.data} credits \u2014 open marketplace` : "Credits \u2014 open marketplace"} onClick={() => navigate("/marketplace")}><LobbyIcon name="credit" /><span>{credits.data ?? "Credits"}</span></button>
        <button className="lh__notification" aria-label={`Notifications${unread ? `, ${unread} unread` : ""}`} aria-expanded={headerMenu === "notifications"} onClick={() => setHeaderMenu(value => value === "notifications" ? null : "notifications")}><SidebarGlyph name="bell" />{unread > 0 && <span className="lh__unread">{unread > 99 ? "99+" : unread}</span>}</button>
    ...
    {search.trim() ? <section className="lh__results" aria-labelledby="lobby-search-title">
      ...
      <section className="lh__avatar" aria-label="Your avatar">
        <div className="lh__avatar-stage">
          <button type="button" className="lh__avatar-model" aria-label="Edit avatar" onClick={editAvatar}>
            <WearablePreview key={avatarAttempt} base={catalystBase()} outfit={{
            ...
            }} controls={false} yaw={-12} spin={false} platform={false} zoom={1.28} pitch={12} targetY={1.05} onStatus={status => { setAvatarStatus(status); if (status === "ready") setHasAvatarFrame(true); }} />
            {!hasAvatarFrame && portrait && <img className="lh__cached-avatar" src={portrait} alt="" />}
            <span className="lh__avatar-label" aria-hidden="true">Customize avatar</span>
          </button>
        </div>
        {avatarStatus === "loading" && <p className="lh__avatar-status" role="status">Loading your avatar&#x2026;</p>}
        {(avatarStatus === "error" || avatarStatus === "empty") && <p className="lh__avatar-status" role="status">Avatar preview unavailable. <Button variant="ghost" size="sm" onClick={() => { setAvatarStatus("loading"); setHasAvatarFrame(false); setAvatarAttempt(attempt => attempt + 1); }}>Retry preview</Button></p>}
      </section>
      <div className="lh__left">
        <section className="lh__welcome" aria-labelledby="lobby-welcome-title">
          <h1 id="lobby-welcome-title" ref={heading} tabIndex={-1}>Welcome {identity.name || "Explorer"}!</h1>
          {linkedPreview ? <div className="lh__empty-card"><strong>{scene.title || "Scene preview"}</strong><button className="lh__jump" onClick={onEnterWorld}>Enter scene <LobbyIcon name="jump" /></button></div> : hero ? <PlaceCard place={hero} hero onVisit={() => visit(placeDestination(hero), hero)} disabled={!placeDestination(hero) || !!jump.jumping} /> : <div className="lh__empty-card"><LoadState pending={featured.isPending} error={featured.isError} empty="Your next adventure is waiting." retry={() => featured.refetch()} /><button className="lh__jump" onClick={onEnterWorld}>Jump in <LobbyIcon name="jump" /></button></div>}
        </section>
        {friendState.friends.length > 0 && <section className="lh__social" aria-labelledby="lobby-friends-title">
          <div className="lh__section-heading"><h2 id="lobby-friends-title">Friends <small>{online.length} Online</small></h2><button className="lh__section-link" aria-label="All friends" onClick={() => navigate("/friends")}>All friends</button></div>
          ...
          })}</LobbyCarousel> : <div className="lh__empty-card lh__empty-card--friends"><p className="lh__note">{identity.isGuest ? "Sign in to find your friends here." : "No friends online right now."}</p><button className="lh__section-link" onClick={() => navigate("/friends")}>Find your friends</button></div>}
        </section>}
      </div>
      {(liveEvents.length > 0 || upcomingEvents.length > 0) && <aside className="lh__right" aria-labelledby="lobby-events-title">
        <div className="lh__section-heading"><h2 id="lobby-events-title">Events</h2><button className="lh__section-link" onClick={() => navigate("/events")}>All events</button></div>
      ...
      <div className={`lh__bottom${recentPlaces.length ? "" : " lh__bottom--new"}`}>
        ...
        <section aria-labelledby="lobby-featured-title"><div className="lh__section-heading"><h2 id="lobby-featured-title">Recommended places</h2><button className="lh__section-link" aria-label="Explore all places" onClick={() => navigate("/places")}>Explore all</button></div>
