...
export default function LobbyHeaderMenu({ kind, onClose, onSignOut }: {
  ...
  return <div ref={ref} className="lh__header-menu" role="dialog" aria-label={kind === "account" ? "Your account" : "Notifications"}>
    ...
    {kind === "notifications" ? <NotificationsPanel floating /> : <>
      ...
      <button data-panel-preload="passport" onClick={() => go("/passport")}><Icon name="user" />Profile</button>
      <button data-panel-preload="backpack" onClick={() => go("/backpack")}><Icon name="pack" />Edit avatar</button>
      <button data-panel-preload="settings" onClick={() => go("/settings")}><Icon name="settings" />Settings</button>
