 ...
 * not lose a session over one. It does mean the two builds take different paths
 * on bad data, so the `boundary` string is what ties a production report back
 * to the dev failure — keep it stable and specific.
 *
 * On a production rejection the ORIGINAL value is returned, not a default. Every
 * caller here already has a fallback for missing data (the bridge keeps the
 * previous field, a store treats a bad blob as absent), and substituting a
 * plausible-looking default would hide the drift this exists to expose.
 */
/**
 * Validate, report, and say whether it held — for callers that must SKIP a bad
 * payload rather than carry on with it.
 *
 * `check` returning the original value suits a caller whose next step tolerates
 * a missing field. The engine bridge is not one: its reducer reads
 * `push.address.slice(...)`, so a wrong-TYPED field is a TypeError one line
 * after the rejection was logged. `?? prev.x` only ever guarded absent fields.
 */
/**
...
function evaluate<T>(
  ...
  if (isDev()) {
    throw new Error(
      `validation failed at ${boundary} — ${detail}\n` +
  ...
  if (first) {
    console.warn(`[validate] ${boundary} — ${detail} (further reports suppressed)`);
  }
