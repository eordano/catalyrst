...
export function badgeText<T>(d: Datum<T>, now?: number): string {
  ...
  if (d.state === "snapshot") {
    ...
    return day ? `${word} · exported ${day}` : word;
  }
  return word;
}
...
export function noteLines<T>(d: Datum<T>, now?: number): string[] {
  switch (d.state) {
    ...
    case "sampled": {
      ...
      if (isStale(d, now))
        lines.push(
          `The last sample is older than ${STALE_SAMPLE_FACTOR}× the ${minutes}-minute cadence — the sampler may have stopped.`,
        );
      return lines;
    }
    case "snapshot": {
  ...
  if (exportSource !== TRUSTED_EXPORT_SOURCE)
    throw new Error(
      `snapshot(): exportSource must be "${TRUSTED_EXPORT_SOURCE}", got "${exportSource}". ` +
        "A synthetic export is not a reading — build an unavailable() datum instead.",
    );
