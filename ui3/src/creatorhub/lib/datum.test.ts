...
describe("when rendering a datum's value", () => {
  ...
  it("should render the no-value glyph — never a zero — for every other state", () => {
...
describe("when deciding whether a sample is stale", () => {
  ...
  it("should never call a live reading stale — staleness is undefined for a real-time read", () => {
...
describe("when tallying the states on a screen", () => {
  it("should count fresh and stale samples separately", () => {
    ...
    expect(tally).toEqual([
      { state: "live", stale: false, word: "Live", glyph: "●", count: 1 },
      { state: "sampled", stale: false, word: "Sampled", glyph: "◐", count: 1 },
      { state: "sampled", stale: true, word: "Stale", glyph: "◐", count: 1 },
    ]);
  });
