...
export const accessFacts: Datum<readonly FactRow[]> = l(
  [
    { label: "Owner", value: "0x313d…9a1" },
    { label: "Deployment", value: "allow-list · 2 wallets" },
...
export const worldNotBuilt: NotBuiltSpec[] = [
  ...
  },
  {
    id: "errors",
    title: "Did it break?",
    why: "catalyrst-telemetry ingests Sentry-shaped events and groups them into issues, but every read is behind require_telemetry_admin and the data carries no scene or owner dimension. There is no query that means “my world”.",
    todayCli: {
      command:
        "# start your explorer with --mcp yourself first — this does not launch it\ncurl -s http://127.0.0.1:8123/unity-explorer-mcp -d '{\"jsonrpc\":\"2.0\",\"id\":1,\"method\":\"tools/call\",\"params\":{\"name\":\"get_scene_logs\",\"arguments\":{}}}'",
      explain:
        "dcl-scene-bots is an MCP client you run beside your own explorer. It catches TypeError · ReferenceError · Cannot read · is not a function · unhandled promise rejection.",
    },
  },
  {
    id: "notify",
    ...
    today: "the ⟳ button at the top of this page.",
  },
  {
    id: "scene-state",
...
export const permissionsCli = {
  command:
    "dcl-one-sdk world permissions grant petbarn.dcl.eth deployment 0x313d…9a1",
  explain:
...
export const sourceGroups: SourceLedgerGroup[] = [
  {
    ...
    rows: [
      ...
      row(
        ...
        E.places,
        ["Activity · world"],
        "user_visits and user_count from the same response are excluded — see below.",
        l("31 likes", E.places),
      ),
      row(
        "world-about",
        ...
        l("urn:decentraland:entity:…", E.worldAbout),
      ),
      row(
        "world-permissions",
        "ACLs + owner",
        ...
        l("allow-list · 2 wallets", E.worldPermissions),
      ),
      row(
        "realm-about",
        "Realm health",
        E.about,
        ["Activity · world"],
        "healthy · acceptingUsers · synchronizationStatus.",
  ...
  },
  {
    ...
    rows: [
      row(
        "creators-stats",
        "Sessions · retention · device · FPS (15 fields)",
        E.creatorsStats,
        ["nothing"],
      ...
      row(
        "world-storage-kv",
        "Scene key–value storage (3 endpoints)",
        `GET ${CATALYST}/world-storage/usage/{world,players,env}`,
        ["Activity · world (as an explanation)"],
  ...
  },
  {
    ...
    rows: [
      ...
      row(
        ...
        "—",
        ["nothing"],
        "catalyrst-notifications is email preferences with zero parcel, land or scene references. Today: the ⟳ button.",
      ),
      row(
        "custom-metrics",
  ...
  },
  {
    ...
    rows: [
      ...
      row(
        ...
        ["nothing"],
        "Fails open to [] on any upstream error, non-JSON or unreachable host — so [] is unreadable.",
      ),
      row(
        "places-visits",
