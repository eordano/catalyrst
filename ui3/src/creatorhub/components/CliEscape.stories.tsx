...
const meta = {
  ...
  parameters: {
    docs: {
      description: {
        component:
          "The escape hatch from an unbuilt panel: the command that really works, copyable. There is deliberately **no Run button** — the browser cannot reach `dcl-one-sdk` or an explorer's `--mcp` port, so a Run control could only report a success it did not cause.",
      },
    },
  },
  argTypes: {
  ...
  args: {
    command: "dcl-one-sdk world permissions grant petbarn.dcl.eth deployment 0x313d…9a1",
    explain:
