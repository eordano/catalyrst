...
const meta = {
  ...
  parameters: {
    docs: {
      description: {
        component:
          "The only way a number reaches a card. There is no `value` prop and no `unavailable` boolean — a caller cannot pass a figure without also passing where it came from. The tile never moves or resizes when its state changes: it swaps its badge, and the value becomes `—` at the same size and weight.",
      },
    },
  },
  argTypes: {
...
export const RealZero: Story = {
  args: {
    ...
    note: "a real zero — sampled 2m ago, nobody in",
  },
};
