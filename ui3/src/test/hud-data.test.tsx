...
describe("chat pushes", () => {
  test("chat starts honest-empty when live, then shows pushed lines", async () => {
    ...
    expect(
      screen.getByText("No messages yet — say hello to Nearby."),
    ).toBeInTheDocument();
    ...
    expect(
      screen.queryByText("No messages yet — say hello to Nearby."),
    ).toBeNull();
  });
...
describe("login code pushes", () => {
  ...
  test("mic pushes drive the voice panel state", async () => {
    ...
    expect(
      screen.getByRole("button", { name: "Mic on — click to mute" }),
