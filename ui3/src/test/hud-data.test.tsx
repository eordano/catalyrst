...
describe("identity and scene pushes", () => {
  test("the profile widget shows a signed-in identity's name, tag and wallet, and a guest's Sign in entry", async () => {
    ...
    await signedIn.user.click(within(sidebar()).getByRole("button", { name: "Profile" }));
...
describe("connection pushes", () => {
  test("the badge is neutral before the first push, ok when healthy, warn on errors or a lost global room", () => {
    const { bridge } = renderHud();
