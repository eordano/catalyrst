...
describe("fullscreen panels", () => {
  test("sidebar Settings, Backpack and Places open their routes inside ExploreChrome; unknown routes fall back to the world", async () => {
    ...
    expect(within(chrome()).getByRole("button", { name: /Settings/ })).toHaveAttribute("aria-current", "page");
...
const FLOATING: { button: string; id: string; title: string }[] = [
  ...
  { button: "Skybox", id: "skybox", title: "NIGHT/DAY" },
  { button: "Friends", id: "friends", title: "Friends" },
  { button: "Notifications", id: "notifications", title: "NOTIFICATIONS" },
];

describe("floating panels", () => {
  test("each floating panel opens beside its sidebar button on the shared surface with a single Close", async () => {
    ...
    expect(container.querySelector("[data-hud-panel]")).toBeNull();
    ...
    for (const { button, id, title } of FLOATING) {
      ...
      expect(container.querySelectorAll("[data-hud-panel]")).toHaveLength(1);
      expect(within(panel).getAllByRole("button", { name: "Close" })).toHaveLength(1);
