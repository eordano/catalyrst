...
describe("fullscreen panels", () => {
  ...
  test("Escape, the close button and the active chrome tab all return to the world HUD; chrome tabs switch panels", async () => {
    ...
    await user.click(within(chrome()).getByRole("button", { current: "page" }));
    expect(path()).toBe("/");
  });
  ...
  test("the ExploreChrome user chip opens the lobby, whose account menu routes to passport", async () => {
    const { user, path } = renderHud();
    await user.click(within(sidebar()).getByRole("button", { name: "Places" }));

    const chip = within(chrome()).getByRole("button", { name: "Open lobby" });
...
describe("anchorBeside", () => {
  test("aligns below the button when it fits, flips upward on overflow, clamps to the margin, and falls back without a button", () => {
    const fallback = { top: PANEL_MARGIN, maxHeight: 800 - 2 * PANEL_MARGIN };
    expect(anchorBeside({ top: 100, bottom: 138, height: 38 }, 300, 800)).toEqual({ top: 100, maxHeight: 800 - PANEL_MARGIN - 100 });
    expect(anchorBeside({ top: 700, bottom: 738, height: 38 }, 300, 800)).toEqual({ top: 738 - 300, maxHeight: 800 - PANEL_MARGIN - (738 - 300) });
    expect(anchorBeside({ top: 700, bottom: 738, height: 38 }, 900, 800)).toEqual(fallback);
    expect(anchorBeside({ top: 82, bottom: 120, height: 38 }, 868, 900)).toEqual({ top: 82, maxHeight: 900 - PANEL_MARGIN - 82 });
    expect(anchorBeside(null, 300, 800)).toEqual(fallback);
    expect(anchorBeside({ top: 0, bottom: 0, height: 0 }, 300, 800)).toEqual(fallback);
  });
});
