...
import { renderHud, makeFriend } from "./harness";
...
vi.mock("../wearable-preview/WearablePreview", () => ({ default: () => <div data-testid="avatar-preview" /> }));
...
test("both flags coexist and the lobby supports narrow screens without the touch HUD over it", async () => {
...
test("flag off preserves the existing view", () => {
...
test("the redesigned Me menu can exit the world to the lobby", async () => {
  localStorage.setItem(SIDEBAR_DESIGN_STORAGE, "1");
  const { user } = renderHud();
  await screen.findByRole("main", { name: "Decentraland lobby" });
  await user.keyboard("{Escape}");
  expect(document.querySelector(".sd")).toBeInTheDocument();
...
test("clicking the central avatar opens the existing Backpack without sending avatar changes", async () => {
  ...
  expect(path()).toBe("/backpack");
  expect(await screen.findByRole("tab", { name: "Wearables", selected: true })).toBeInTheDocument();
...
test("SDK realm links enter their existing scene without replacing the destination", async () => {
  ...
  const { user, bridge } = renderHud();
  await user.click(await screen.findByRole("button", { name: "Enter scene" }));
  expect(screen.queryByRole("main", { name: "Decentraland lobby" })).toBeNull();
...
test("account actions stay in a lobby popup", async () => {
  ...
  expect(screen.queryByText("No live events right now. See what\u2019s coming up.")).toBeNull();
});
