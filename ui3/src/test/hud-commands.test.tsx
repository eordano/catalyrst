...
describe("chat commands", () => {
  ...
  test("/clear answers locally; /help asks the engine and merges its list; /help <cmd> answers from the table", async () => {
    ...
    expect(help).toHaveTextContent(/\/fps.*set the target frame rate/i);
  ...
  test("clicking a sender's avatar opens their public profile over the open chat, and Escape returns to it", async () => {
    const { user, bridge, router, path } = renderHud();
    await user.click(within(sidebar()).getByRole("button", { name: "Chat" }));
    const line = bridge.pushChat({ senderName: "Ripley", message: "hi there" });
    await user.type(chatInput(), "half a thought");

    const avatar = await screen.findByRole("button", { name: "View Ripley" });
    await user.click(avatar);
    expect(path()).toBe("/passport");
    expect(router.state.location.search).toBe(`?address=${line.senderAddress}`);
    ...
    expect(screen.getByText("hi there")).toBeVisible();
    expect(screen.getByRole("button", { name: "View Ripley" })).toHaveFocus();

    await user.keyboard("{Escape}");
    expect(screen.queryByLabelText("Send a message to Nearby chat")).toBeNull();
  });

  test("opening your own profile from the menu still closes the chat", async () => {
    const { user, navigate } = renderHud();
    await user.click(within(sidebar()).getByRole("button", { name: "Chat" }));
    expect(chatInput()).toBeVisible();
  ...
  test("clicking a sender's name opens their profile card; Mention fills the draft", async () => {
    const { user, bridge } = renderHud();
    await user.click(within(sidebar()).getByRole("button", { name: "Chat" }));
    bridge.pushChat({ senderName: "Ripley", message: "hi there" });

    await user.click(await screen.findByRole("button", { name: "Ripley" }));
...
describe("voice and skybox panels", () => {
  ...
  test("disabling auto and sliding time sends SetTimeOfDay", async () => {
    ...
    expect(bridge.expectSent("SetTimeOfDay")).toMatchObject({ auto: false });
