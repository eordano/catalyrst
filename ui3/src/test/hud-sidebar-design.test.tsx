...
afterEach(() => { localStorage.removeItem("dcl.location.pinned"); localStorage.removeItem(SIDEBAR_DESIGN_STORAGE); setMobileOverride(null); });
const dock = (name: string) => within(screen.getByRole("navigation", { name: "Main menu" })).getByRole("button", { name });

test("Gallery opens from Me and Escape returns to the scene", async () => {
  const { user, path, bridge } = renderHud();
  expect(dock("Camera")).toBeInTheDocument();
  expect(within(screen.getByRole("navigation", { name: "Main menu" })).queryByRole("button", { name: "Gallery" })).toBeNull();
  await user.click(dock("Me"));
  await user.click(screen.getByRole("button", { name: "Gallery" }));
  expect(path()).toBe("/gallery");
  expect(await screen.findByRole("navigation", { name: "Explore sections" })).toBeInTheDocument();
...
test("toggling Location dismisses other drawers and popups on both open and close", async () => {
  const { user, path } = renderHud();
  await user.click(dock("Me"));
  await user.click(dock("Location"));
  expect(screen.queryByRole("region", { name: "ME" })).toBeNull();
  expect(dock("Location")).toHaveAttribute("aria-expanded", "true");
  await user.click(dock("Chat"));
  expect(screen.getByLabelText("Send a message to Nearby chat")).toBeInTheDocument();
  await user.click(dock("Location"));
  expect(screen.queryByLabelText("Send a message to Nearby chat")).toBeNull();
  expect(dock("Location")).toHaveAttribute("aria-expanded", "false");
  await user.click(dock("Voice Chat"));
  await user.click(dock("Location"));
...
test("Location owns the scene info and owner actions; Me owns notifications", async () => {
  ...
  expect(screen.getByRole("menuitem", { name: "Send tip" })).toBeInTheDocument();
  await user.click(dock("Me"));
  await user.click(screen.getByRole("button", { name: "Notifications" }));
  expect(await screen.findByRole("region", { name: "Notifications" })).toBeInTheDocument();
});

test("System keeps flags inside Settings and consolidates support", async () => {
  const { user, router } = renderHud();
  await user.click(dock("System"));
  expect(screen.getByRole("link", { name: "Report a bug / Contact support" })).toHaveAttribute("href", `${window.location.origin}/support`);
