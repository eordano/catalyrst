...
describe("sidebar toggles", () => {
  ...
  test("left panels are exclusive and opening the lobby closes chat", async () => {
    const { user } = renderHud();
    ...
    await user.click(sidebarButton("Open lobby"));
  ...
  test("Sign out from the lobby account menu sends a bridge Logout", async () => {
...
describe("sidebar nav", () => {
  test("Events and Help & Support open in-screen pages; help links stay same-origin", async () => {
    ...
    expect(within(screen.getByRole("dialog", { name: "Explore" })).getByText("Open chat")).toBeInTheDocument();
    expect(screen.getByText("Take a photo")).toBeInTheDocument();
    await user.click(screen.getByRole("button", { name: "Sidebar guide" }));
    expect(screen.getByRole("heading", { name: "What's in the sidebar" })).toBeInTheDocument();
    expect(screen.getByText("Portables")).toBeInTheDocument();
  ...
  test("Marketplace opens inside the explorer shell instead of a new tab", async () => {
    ...
    await waitFor(() => expect(screen.getByRole("dialog", { name: "Explore" })).toContainElement(frame));
    expect(open).not.toHaveBeenCalled();
    open.mockRestore();
  });
