...
function badgeText(s: ServerServiceRow): string {
  ...
  if (s.state === "answering") return `answering · HTTP ${s.httpStatus}`;
  return "down";
}
...
function DiskLine({ disk }: { disk: ServerDisk }) {
  ...
  return (
    <p className={`srvops-note ${low ? "srvops-warn-text" : ""}`}>
      ...
      {low
        ? " — free space now: a full disk takes PostgreSQL and every service down with it"
        : ""}
    </p>
  );
}
...
function envChips(row: ServerEnvRow): { label: string; tone: string }[] {
  ...
  if (row.pendingRestart) chips.push({ label: "saved — restart to apply", tone: "warn" });
...
function EnvRowView({
  ...
  const placeholder = row.secret
    ? row.fileValue !== null || row.liveInSites
      ? "hidden — enter a new value to replace"
      : "enter a value"
    : shown === ""
    ...
    <li className="srvops-envrow" id={`env-${row.name}`}>
      ...
      <FormComponent method="post" className="srvops-envform">
        ...
        <button
          ...
          disabled={busy}
        >
          {envButtonLabel("Save", "Saving…", row, "env-save", pendingEnv)}
        </button>
        {row.fileValue !== null ? (
          <button
            ...
            disabled={busy}
          >
            {envButtonLabel("Delete", "Deleting…", row, "env-delete", pendingEnv)}
          </button>
        ) : null}
      </FormComponent>
    </li>
