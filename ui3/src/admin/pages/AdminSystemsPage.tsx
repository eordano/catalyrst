            ...
            {units.map((u) => (
              <tr key={u.unit}>
                ...
                <td className="adsys-dim">
                  {u.active_state === "active" ? relTime(u.active_since, now) : "—"}
                </td>
              </tr>
            ))}
          </tbody>
    ...
    <div className="adsys-probes">
      {probes.map((p) => (
        <a
          ...
          href={p.url}
          title={`${p.url} → ${p.http_status || "no answer"}`}
        >
          <span className="adsys-dot" aria-hidden="true" />
          {p.name}
          <span className="adsys-probe-code">{p.http_status || "×"}</span>
        </a>
      ))}
    </div>
  );
...
function metricLine(row: ExperimentRow): string {
  ...
  return row.metrics.map((m) => `${m.event}: ${m.count}`).join(" · ");
}
