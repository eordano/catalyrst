...
describe("scene root selection shows scene properties", () => {
  ...
  it("reads name and spawn points from the metadata component and counts placed items excluding the root", () => {
    ...
    const [label] = meta.spawnPoints.map(formatSpawn);
    expect(label).toContain("dock");
    expect(label).toMatch(/default/);
...
const META = { name: "Beach House", description: "Sand and sun", spawnPoints: [], layout: { base: { x: 2, y: -3 }, parcels: [{ x: 2, y: -3 }] } };
...
describe("scene card on the live bus with the root active", () => {
  ...
  it("renames the scene through the metadata component and gives Delete and Duplicate root-specific hints", async () => {
    render(<DeWorkspace title="Beach House" viewportSrc="https://catalyst.example.com/_play/?editorSession=00000000-0000-4000-8000-000000000001&x=1" />);
    ...
    expect((screen.getByLabelText("Delete") as HTMLButtonElement).title).toMatch(/be deleted/);
    expect((screen.getByLabelText("Duplicate") as HTMLButtonElement).title).toMatch(/be duplicated/);
  });
