...
export function sceneMetaWithName(raw: unknown, name: string, info?: DeSceneInfo | null): Record<string, unknown> {
  const metadata = unwrap(raw);
  const parcel = (value: string) => {
    const [x, y] = value.split(",").map(Number);
    return { x, y };
  };
  const base = info?.base || "0,0";
  return { ...metadata, name, layout: metadata.layout ?? {
    base: parcel(base), parcels: (info?.parcels?.length ? info.parcels : [base]).map(parcel),
  } };
}
