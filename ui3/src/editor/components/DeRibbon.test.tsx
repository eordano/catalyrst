...
describe("DeRibbon", () => {
  ...
  it("opens on Home with the tabs ranked by observed frequency, and moves between them with the arrow, Home and End keys", () => {
    render(<DeRibbon />);
    ...
    expect(selectedTab()).toBe("Scene & Publish");
    fireEvent.keyDown(home, { key: "Home" });
    expect(selectedTab()).toBe("Home");
  });

  it("keeps Undo, Redo and Play reachable from every tab, with Play disabled rather than hidden while the engine connects", () => {
    const seen: string[] = [];
    const { rerender } = render(<DeRibbon commands={{ play: () => seen.push("play") }} tab="scene" busLive />);
    ...
    rerender(<DeRibbon commands={{ play: () => seen.push("play") }} />);
    const play = within(screen.getByRole("group", { name: "Always available" })).getByLabelText(
      "Play",
  ...
  it("explains why Make interactive is blocked for the scene root, and hides the code tools until opted into without moving a tab", () => {
    ...
    rerender(<DeRibbon commands={{ "wire.quick": noop, code: noop }} tab="interact" showDeveloper />);
  ...
  it("shows snap state honestly and only the meters that were measured, marking one over its limit", () => {
    ...
    expect(screen.getByText("entities 41/317").className).not.toMatch(/over/);
  ...
  it("keeps the spec honest: pinned chrome never repeats inside a tab, every tab and deferred command states why, and deferred commands never ship", () => {
    ...
    for (const t of RIBBON_TABS) expect(t.why.length).toBeGreaterThan(40);
