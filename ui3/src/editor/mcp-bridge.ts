...
export function autoConnect(opts: Omit<ConnectOptions, "url" | "token"> = {}): (() => void) | null {
...
export function connect(opts: ConnectOptions): () => void {
  ...
  if (!url || !token) {
    console.warn("[mcp-bridge] missing relay url or token — not connecting");
    return () => {};
  }
  activeDispose?.();
  ...
  const screenshot = async (req: Extract<CtlRequest, { op: "screenshot" }>): Promise<CtlScreenshotResult> => {
    ...
    void last;
    throw new Error(
      `E_BLANK_FRAME: ${SCREENSHOT_RETRIES} capture attempts returned fully-transparent frames — is the engine rendering? (WebGPU frames are only readable inside the iframe's rAF)`,
    );
  };
