...
1. `export default` - the panel React component. Rendered inside `ExploreChrome`'s body via `<Outlet/>`, already wrapped in `<Suspense>`. No required props: it calls its own data hook and passes results into the reused ui3 page. An unwired/loading panel must still render a real loading/empty state.
