...
function CommunityMenu({ id, joined, onLeft }: CommunityMenuProps) {
  ...
  if (joined) {
    ...
    items.push({
      kind: "button",
      label: leave.isPending ? "Leaving…" : "Leave community",
      danger: true,
          ...
          <div className="cmb__eventlist">
            {events.length === 0 ? (
              ...
              events.map((e) => (
                <UpcomingEventCard
                  key={e.id}
                  event={{ title: e.name ?? "Untitled event", when: e.timeLabel ?? "—", image: isHttpUrl(e.image) ? (e.image ?? undefined) : undefined, hue: hueFromSeed(e.id) }}
                />
              ))
            )}
          </div>
