...
if (perf.mode !== "perf" || !perf.schemasStubbed || perf.validationEnabled) {
  setup.push(
    `the DCL_PERF=1 capture did not resolve the stubbed build ` +
      ...
      `— check that DCL_PERF reached the vitest config`,
  );
}
...
if (anchorFailures === null) {
  console.error(
    "check-perf-parity: the default capture carries no anchor verdict — the capture is older " +
...
if (anchorFailures.length > 0) {
  console.error("");
  console.error("check-perf-parity: FAILED — default mode produces the WRONG value");
...
if (failures.length === 0) {
  ...
  console.log("check-perf-parity: OK — perf mode changes what is checked and nothing else");
  process.exit(0);
}

console.error("");
console.error("check-perf-parity: FAILED — performance mode changes reader OUTPUT, not just checking");
console.error("");
...
console.error("  it — widen the guard to name the field the mapper dereferences. Captures kept at:");
