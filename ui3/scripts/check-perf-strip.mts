...
// It used to carry a baseline of the modules that still held zod in -- the
// catalyst readers, which mixed schemas with fetches in one file and so could
// not be replaced by a stub. Their schemas now live in pure sibling modules
// under src/data/catalyst/schemas, every one of them aliased, and the count the
// baseline tracked is zero. A recorded list of allowed failures is only worth
// keeping while it is shrinking; kept past zero it is just a way to record the
// next regression as acceptable.
//
...
function build(perf) {
  ...
  if (files === 0) throw new Error(`no .js emitted into ${OUT} — the build produced nothing to check`);
...
if (normal.hits.length === 0) {
  problems.push(
    `default build contains no zod markers (${ZOD_MARKERS.join(", ")}) — either validation is not wired in, or these markers no longer identify zod and this check proves nothing`,
  );
}
  ...
  problems.push(
    `perf build still contains zod (${perf.hits.join(", ")})\n` +
      (importers.length > 0
        ...
        : "  no source module imports zod, so the edge is a dependency's or an alias that stopped matching —\n" +
