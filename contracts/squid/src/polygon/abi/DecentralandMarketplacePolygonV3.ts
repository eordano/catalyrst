...
export const events = {
    ...
    Traded: event("0x71dc7036c75ab7570a8b79d4a452c5a4d3ac4fdf0b2cc58d518d979f0ec557ff", "Traded(address,bytes32,bytes32,(address,bytes,(uint256,uint256,uint256,bytes32,uint256,uint256,bytes32,bytes32[],(address,bytes4,bytes,bool)[]),(uint256,address,uint256,address,bytes)[],(uint256,address,uint256,address,bytes)[]))", {"_caller": indexed(p.address), "_signature": indexed(p.bytes32), "_tradeDigest": indexed(p.bytes32), "_trade": p.struct({"signer": p.address, "signature": p.bytes, "checks": p.struct({"uses": p.uint256, "expiration": p.uint256, "effective": p.uint256, "salt": p.bytes32, "contractSignatureIndex": p.uint256, "signerSignatureIndex": p.uint256, "allowedRoot": p.bytes32, "allowedProof": p.array(p.bytes32), "externalChecks": p.array(p.struct({"contractAddress": p.address, "selector": p.bytes4, "value": p.bytes, "required": p.bool}))}), "sent": p.array(p.struct({"assetType": p.uint256, "contractAddress": p.address, "value": p.uint256, "beneficiary": p.address, "extra": p.bytes})), "received": p.array(p.struct({"assetType": p.uint256, "contractAddress": p.address, "value": p.uint256, "beneficiary": p.address, "extra": p.bytes}))})}),
