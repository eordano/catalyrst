 ...
 * pre-existing status quo, not a regression this introduces). Dropping the key does NOT drop the
 * retry budget: the key-less branch carries the same `retryAttempts: Infinity`, so the rationale
 * below governs both branches instead of degrading to portal-client's finite default. Provision
 * SQD_PORTAL_API_KEY and both processors move onto the shared endpoint with an `x-api-key` header,
 * no code change. The shared host is overridable via SQD_PORTAL_URL because the endpoint has already
 * moved once. Replace this fallback with upstream's fail-closed assertNotNull once the key is a
 * permanent part of the env.
 */
export function portalSource(dataset: string): PortalSource {
  ...
  return {
    ...
    // A Portal 503 is what it answers while a fresh chunk is still replicating to its workers, so a
    // later attempt succeeds; exhausting a finite budget kills the processor instead, and a
    // replaced task is what triggers a full reindex. Needs @subsquid/portal-client >= 0.7.0,
    // below that request() overrides this with a hardcoded 6.
