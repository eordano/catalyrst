...
REPORTING_READER_USER="meltano_dapps_ro"
...
if [ -n "$EXISTING_INDEXER" ]; then
  ...
  psql -v ON_ERROR_STOP=1 --username "$DB_USER" --dbname "$DB_NAME" --host "$DB_HOST" --port "$DB_PORT" <<-EOSQL
    ...
    GRANT USAGE ON SCHEMA $NEW_SCHEMA_NAME TO $API_READER_USER, $SQUID_READER_USER, $MARKETPLACE_TRADES_MV_ROLE, $ATLAS_SERVER_READONLY_USER, $REPORTING_READER_USER;
    ...
    ALTER DEFAULT PRIVILEGES FOR ROLE $NEW_DB_USER IN SCHEMA $NEW_SCHEMA_NAME
      GRANT SELECT ON TABLES TO $SQUID_READER_USER, $MARKETPLACE_TRADES_MV_ROLE, $ATLAS_SERVER_READONLY_USER, $REPORTING_READER_USER;
...
# Keep DB_SCHEMA unset: @subsquid/typeorm-config >= 4.2.0 turns it into a per-connection
# `options=-c search_path="<name>"` pin, and promotion RENAMES this schema to
# squid_marketplace -- a pin from the environment goes stale there and no restart can
# recover it. The role default (ALTER USER above, and the promote) stays authoritative;
# SQUID_SCHEMA carries the deployment's identity instead.
