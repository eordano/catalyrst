    ...
    in
    {
      packages = forAllSystems (pkgs:
        let
          ...
          # byte-identical between the two so crane's cargoArtifacts actually
          # hits.
          explorerApiCraneArgs = {
            pname = "catalyrst-explorer-api";
            version = "0.1.0";
            src = rustSrc;
            strictDeps = true;
            cargoExtraArgs = "-p catalyrst-explorer-api --bin catalyrst-explorer-api";
        ...
        rec {
          ...
          # crane prototype -- third-party dep graph compiled once into its own
          # derivation, reused by the package build. The buildRustPackage
          # derivation above is deliberately left in place and untouched.
          catalyrst-explorer-api-deps = craneLib.buildDepsOnly explorerApiCraneArgs;

          catalyrst-explorer-api-crane = craneLib.buildPackage (
            explorerApiCraneArgs // { cargoArtifacts = catalyrst-explorer-api-deps; }
          );
          ...
          catalyrst-all-deps = craneLib.buildDepsOnly allCraneArgs;

          catalyrst-all-crane = craneLib.buildPackage (allCraneArgs // {
            cargoArtifacts = catalyrst-all-deps;
            postInstall = ''
              mkdir -p "$out/share/catalyrst-server"
              cp -r crates/catalyrst-server/migrations "$out/share/catalyrst-server/migrations"
            '';
