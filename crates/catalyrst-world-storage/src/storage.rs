//! Postgres-backed storage for the three namespaces (world / player / env),
//! ports of the upstream `world-storage`, `player-storage` and `env-storage`
//! adapters plus the `storage-limits` logic.
//!
//! Size accounting matches upstream exactly: value size is the UTF-8 byte
//! length of the serialized value (`JSON.stringify` for JSON values, the raw
//! string for env values), and total-size limits are aggregated **per world**
//! (across all scenes), while reads/writes/lists are scoped per `(world, place)`.
...
/// One `{ key, value }` entry returned by list endpoints.
#[derive(Debug)]
...
/// UTF-8 byte length of a serialized value (upstream `calculateValueSizeInBytes`).
pub fn value_size_bytes(serialized: &str) -> i64 {
    serialized.len() as i64
}

/// LIKE pattern for a prefix filter, or None when no prefix supplied.
fn prefix_pattern(prefix: Option<&str>) -> Option<String> {
    prefix.filter(|p| !p.is_empty()).map(|p| format!("{p}%"))
}

/// Validate an upsert against per-value and projected-total size limits
/// (upstream `createUpsertValidator`).
pub fn check_limits(
    new_value_size: i64,
    info: SizeInfo,
