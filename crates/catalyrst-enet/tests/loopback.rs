//! Loopback handshake + reliable delivery over the real `rusty_enet` wire.
//!
//! Binds two hosts on localhost, connects one to the other, and asserts a
//! reliable packet round-trips end-to-end — exercising the full ENet datagram
//! header, CONNECT/VERIFY_CONNECT handshake and reliable ack path (the wire that
//! must be byte-compatible with the native ENet `decentraland/Pulse` speaks).
...
async fn loopback_connect_and_reliable_delivery() {
    ...
    // Initiate the ENet connect handshake from the client to the server.
    client.connect(server_addr, 2).expect("connect");

    // Pump both hosts cooperatively until the server reports the peer connected,
    // remembering the server-side peer id we'll deliver to.
    ...
    // Server sends a reliable packet to the connected peer on channel 0.
    ...
    // Pump until the client receives it (keep servicing the server so its
    // retransmit/ack timers run).
...
async fn disconnect_with_reason_tears_down_the_peer() {
    ...
    // Server disconnects the peer with an AUTH_TIMEOUT-style reason code (2). The
    // graceful path means both ends eventually surface an Event::Disconnect.
    const REASON: u32 = 2;
    server
