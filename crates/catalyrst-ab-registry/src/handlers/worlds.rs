...
mod tests {
    ...
    fn world_name_validation_matches_upstream_regex() {
        ...
        for ok in ["foo.eth", "foo.dcl.eth", "my-world.dcl.eth", "UPPER.eth", "a.eth", "1.dcl.eth"] {
            assert!(is_world_name_valid(ok), "should be valid: {ok}");
        }
        for bad in [
            "invalid", "notaworld.com", "", "bad_name.eth", "foo.bar.eth",
            ".eth", ".dcl.eth", "foo.dcl.dcl.eth", "foo..eth",
        ] {
    ...
    fn parse_coordinate_handles_signs_whitespace_and_garbage() {
        ...
        assert_eq!(parse_coordinate("-5,-9").map(|c| (c.x, c.y)), Some((-5, -9)));
        assert_eq!(parse_coordinate(" 3 , 4 ").map(|c| (c.x, c.y)), Some((3, 4)));
    ...
    fn bounds_are_min_max_over_all_parcels() {
        assert_eq!(bounds_from_parcels(&v(&["0,0", "10,4", "-2,6"])), (-2, 0, 10, 6));
