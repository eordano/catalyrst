...
/// Fails closed: when `API_ADMIN_TOKEN` is unset (`expected` is None/empty) the
/// bearer is never accepted.
...
/// Gate a route on a valid admin bearer token, 403 otherwise.
...
mod tests {
    ...
    fn fails_closed_when_token_unset() {
        // No admin token configured -> bearer never accepted (would 403).
