...
fn stubscan(task: &serde_json::Value) -> serde_json::Value {
    let ours_path = task["ours"].as_str().unwrap();
    let up_path = task["upstream"].as_str().unwrap();
    let mut row = serde_json::json!({"pair": task["pair"]});
    let check = |path: &str| -> serde_json::Value {
        match extract_textures(path) {
            Err(e) => serde_json::json!({"error": e}),
            Ok(texes) => {
                let mut consts = Vec::new();
                for t in &texes {
                    if let Decoded::Rgba { rgba, .. } = decode_mip0(t.fmt, &t.payload, t.w, t.h) {
                        let first: [u8; 4] = [rgba[0], rgba[1], rgba[2], rgba[3]];
                        let is_const = rgba.chunks_exact(4).all(|p| p == first);
                        consts.push(serde_json::json!({
                            "name": t.name, "constant": is_const,
                            "px": if is_const { serde_json::json!(first.to_vec()) } else { serde_json::Value::Null },
                        }));
                    }
                }
                serde_json::json!(consts)
            }
        }
    };
    row["ours"] = check(ours_path);
    row["upstream"] = check(up_path);
    row
}

fn main() {
    let mut args: Vec<String> = std::env::args().skip(1).collect();
    let stub_mode = args.first().map(|a| a == "--stubscan").unwrap_or(false);
    if stub_mode {
        args.remove(0);
    }
    if args.len() < 2 {
        eprintln!("usage: texcmp [--stubscan] <tasks.jsonl> <out.jsonl> [threads]");
        std::process::exit(2);
    }
    ...
    tasks.par_iter().for_each(|task| {
        if stub_mode {
            let row = std::panic::catch_unwind(|| stubscan(task))
                .unwrap_or_else(|_| serde_json::json!({"pair": task["pair"], "error": "panic"}));
