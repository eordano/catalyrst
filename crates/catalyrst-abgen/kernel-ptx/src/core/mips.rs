...
use super::bc7::{Bc7Profile, OptTables, Params};
...
pub fn linearize_pixel(src: &[u8], srgb: bool, dst: &mut [f32]) {
    let r = src[0];
    let g = src[1];
    let b = src[2];
    let a = src[3] as f32;
...
pub fn quantize_pixel(src: &[f32], srgb: bool, dst: &mut [u8]) {
    if srgb {
        dst[0] = linear_to_srgb_u8(src[0]);
        dst[1] = linear_to_srgb_u8(src[1]);
        dst[2] = linear_to_srgb_u8(src[2]);
    } else {
        dst[0] = round_half_up_u8(src[0]);
        dst[1] = round_half_up_u8(src[1]);
        dst[2] = round_half_up_u8(src[2]);
    }
    dst[3] = round_half_up_u8(src[3]);
}

pub fn box_halve_dims(w: usize, h: usize) -> (usize, usize) {
    ((w / 2).max(1), (h / 2).max(1))
}

pub fn box_halve_cell(src: &[f32], w: usize, h: usize, nx: usize, ny: usize, out: &mut [f32]) {
    let c = 4usize;
...
pub fn box_halve_into(src: &[f32], w: usize, h: usize, dst: &mut [f32]) -> (usize, usize) {
    let (nw, nh) = box_halve_dims(w, h);
    for ny in 0..nh {
        for nx in 0..nw {
            let o = (ny * nw + nx) * 4;
            box_halve_cell(src, w, h, nx, ny, &mut dst[o..o + 4]);
        }
    }
    (nw, nh)
}

pub fn level_block_dims(w: usize, h: usize) -> (usize, usize) {
    (w.div_ceil(4), h.div_ceil(4))
}

pub fn quantize_pack_block(
    level: &[f32],
    w: usize,
    h: usize,
    srgb: bool,
    ...
    for r in 0..4 {
        ...
        for dx in 0..4 {
            ...
            let s = (sy * w + sx) * 4;
            let d = r * 16 + dx * 4;
            quantize_pixel(&level[s..s + 4], srgb, &mut out[d..d + 4]);
        }
    }
}
...
#[cfg(not(target_arch = "nvptx64"))]
pub fn scanline_to_blocks(rgba: &[u8], width: usize, height: usize) -> (Vec<u8>, usize) {
    let bw = width / 4;
    let bh = height / 4;
...
#[cfg(not(target_arch = "nvptx64"))]
pub fn pad_to_block_size(rgba: &[u8], w: usize, h: usize) -> (Vec<u8>, usize, usize) {
    let pw = (w + 3) & !3;
    let ph = (h + 3) & !3;
    if pw == w && ph == h {
...
pub fn box_halve(arr: &[f32], w: usize, h: usize) -> (Vec<f32>, usize, usize) {
    ...
    box_halve_into(arr, w, h, &mut out);
    (out, nw, nh)
}
...
pub fn encode_bc7_mip_chain_with_profile(
    ...
    for m in 0..mip_count {
        ...
        let comp = super::bc7::encode_blocks(&blocks, n, &params, tables);
