...
// Lazily runs the one-time crnlib global init (ryg_dxt / etc1 table
// builds, threading init, decomp memory callbacks). Upstream ran this
// from a namespace-scope static initializer, charging every process
// ~67M instructions at startup even when crnlib is never used. The FFI
// wrapper (cpp/crn_wrapper.cc) calls this before entering crnlib; a
// C++11 magic static makes it thread-safe and once-only.
extern "C" void crn_ffi_ensure_global_init(void) {
  static crnlib::crnlib_global_initializer s_crnlib_initializer;
  (void)s_crnlib_initializer;
}
