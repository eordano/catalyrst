...
bool elemental_vector::increase_capacity(uint min_new_capacity, bool grow_hint, uint element_size, object_mover pMover, bool nofail) {
  ...
  // Explicit uint64 casts: size_t is `unsigned long` on LP64 Linux, which is
  // neither crnlib uint32 nor uint64 (`unsigned long long`) -> ambiguous
  // overload under modern GCC.
  if ((grow_hint) && (!math::is_power_of_2((uint64)new_capacity)))
    new_capacity = (size_t)math::next_pow2((uint64)new_capacity);
