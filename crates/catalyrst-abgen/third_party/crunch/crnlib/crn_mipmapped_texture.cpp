...
bool mip_level::unpack_from_dxt(bool uncook) {
  ...
  // Upstream unity-branch bug: the get_unpacked_image() call lived inside
  // CRNLIB_ASSERT, so NDEBUG builds never unpacked and returned an empty
  // image. Hoist the side-effecting call out of the assert.
