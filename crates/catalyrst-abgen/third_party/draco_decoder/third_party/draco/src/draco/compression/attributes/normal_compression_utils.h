...
class OctahedronToolBox {
 ...
 private:
  inline void OctahedralCoordsToUnitVector(float in_s_scaled, float in_t_scaled,
    ...
    //
    // [abgen patch — faithful to draco 1.4.1 binaries] Upstream 1.4.1 wrote
    // `1.f - abs(y) - abs(z)` with the C integer abs() in scope, so y and z
    // were truncated toward zero before the magnitude was taken: x evaluates
    // to 1.f for every interior point (|y|,|z| < 1), the diamond fold below
    // never triggers, and the vector normalizes with the raw (y, z).
    // DracoUnity 4.0.2 (the version pinned by the asset-bundle-converter)
    // ships native libraries built from draco 1.4.1, so production asset
    // bundles carry normals decoded through that truncated form. Draco 1.5.x
    // fixed the call to `std::abs`, which changes nearly every decoded
    // normal. abgen reproduces the 1.4.1 arithmetic to stay byte-identical
    // with the reference output (verified exact across whole meshes).
    // The correct 1.5.7 form is `1.f - std::abs(y) - std::abs(z)`.
    const float x = 1.f - std::trunc(std::abs(y)) - std::trunc(std::abs(z));
