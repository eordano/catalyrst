...
/// Raw-chunk -> lz4hc-output memo for the fused serialize-per-platform pass:
/// sibling platforms of one built bundle share most 128KB chunks byte-for-byte
/// (identical .resS payloads, name-length-invariant CABs), so the second
/// serialize reuses the first's compressed bytes. Keyed on the full raw chunk,
/// so a hit returns exactly what lz4hc_compress would have produced — output
/// bytes are identical by construction. Scope one memo to one built bundle.
#[derive(Default)]
...
fn lz4_cache_dir() -> Option<&'static std::path::Path> {
    ...
    DIR.get_or_init(|| {
        let d = std::env::var("ABGEN_BC7_CACHE")
            .ok()
            .filter(|s| !s.is_empty())?;
