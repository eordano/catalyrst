...
/// Encode-once, serialize-per-platform build: one parse, object-graph build and
/// texture encode, then one commit/save per bundle name. Sound ONLY for
/// name sets whose targets are encode-identical — {windows, mac} share every
/// encode-affecting parameter (1024px cap, BC7 profile, stream split rule,
/// externals positions) and were measured payload-identical; linux/webgl
/// change the encode itself. Any non-shareable request (other targets,
/// expect_hash retry ladder, lod) falls back to independent full builds.
