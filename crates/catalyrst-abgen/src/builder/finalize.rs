...
impl<'a> Builder<'a> {
    ...
    /// Re-point an already-built (post-finalize_pathids) Builder at a sibling
    /// platform's bundle name so `commit` can serialize the SAME object graph
    /// (path ids, GUIDs, texture encodes, resS payload) for another target.
    /// Only the platform-derived bytes are recomputed: the "_<target>" suffix
    /// of ext_bundle_files / metadata_dependencies (strip_suffix, never blind
    /// replace), the metadata TextAsset m_Script, and the AssetBundle object
    /// (rebuilt via fill_assetbundle — preload/container ordering consumes CAB
    /// strings, so string-patching would be unsound). Callers must guarantee
    /// the old and new targets are encode-identical (windows<->mac).
