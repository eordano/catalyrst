...
/// Parse from already-materialized inputs (post `load_gltf_inputs`), so callers that
/// need both the raw gltf JSON/buffers and the Scene load (and Draco-decode) only once.
...
/// Classification-only parse: image_uri / materials / samplers come out identical to
/// `parse`, but image pixel decode and mesh-primitive construction are skipped
/// (`images`/`image_bytes` entries are None, node primitives empty).
