...
fn check(b: &Bundles, ctx: &ValidateCtx, out: &mut Vec<Finding>) {
    ...
    let ext_names: Vec<String> = sf
        .externals
        .iter()
        .map(|e| e.path.rsplit('/').next().unwrap_or(&e.path).to_lowercase())
        .collect();
    ...
    for o in &sf.objects {
        match o.class_id {
            ...
            C_MESHFILTER => {
                check_meshfilter(b, o, &internal_exists, &ext_names, &ab_deps, ctx, out)
            }
...
fn check_gameobject(
    ...
    for (i, c) in comps.iter().enumerate() {
        let pptr = c
            .get("component")
            .or_else(|| c.as_array().and_then(|a| a.get(1).or_else(|| a.first())));
...
fn resolve_external(
    ...
    let Some(name) = ext_names.get(idx) else {
        out.push(finding(
            ...
            b,
            format!(
                "{what}: external fileID={fid} out of range (externals len {})",
                ext_names.len()
            ),
        ));
        return;
