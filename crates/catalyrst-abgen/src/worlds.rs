...
fn guard_base_url(url: &str) -> Result<()> {
    if crate::clihelp::env_bool("ABGEN_ALLOW_PRIVATE_BASE_URL", false) {
        return Ok(());
    }
...
mod tests {
    ...
    fn guard_base_url_rejects_private_and_honors_escape() {
        ...
        for url in [
            ...
            assert!(guard_base_url(url).is_err(), "{url} should be refused");
        }
        for url in [
            ...
            assert!(guard_base_url(url).is_ok(), "{url} should pass");
        }
        std::env::set_var("ABGEN_ALLOW_PRIVATE_BASE_URL", "1");
        assert!(guard_base_url("http://127.0.0.1:8000/contents/").is_ok());
        std::env::remove_var("ABGEN_ALLOW_PRIVATE_BASE_URL");
    }
