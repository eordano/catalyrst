...
async fn resolve_entities(state: &AppState, pointers: Vec<String>) -> Vec<ResolvedEntity> {
    let ents = if let Some(cdb) = &state.content_db {
...
async fn eager_build_index(state: &AppState, entities: &[ResolvedEntity]) {
    ...
    for e in entities {
        ...
        for platform in &ib.platforms {
            ...
            if ib.max_queue > 0
                && ib.pending.load(std::sync::atomic::Ordering::Relaxed) >= ib.max_queue
            {
    ...
    let deadline = tokio::time::Instant::now() + ib.deadline;
    for h in handles {
...
pub async fn post_entities_versions(
    ...
    eager_build_index(&state, &ents).await;
