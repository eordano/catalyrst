...
const BIN_NAME: &str = "abgen-verify";
...
fn usage_text() -> &'static str {
    "usage: abgen-verify <ours-dir> <reference-dir> [-j JOBS] [--json PATH] [--tolerant]\n\
        \x20      abgen-verify gpu <diff|bench|corpus> [flags]   (needs --features gpu)\n\
         \n\
         ...
         \n\
         Exit codes: 0 = clean (non-tolerant: every pair byte-identical;\n\
         tolerant: no structural pair). 1 = byte diffs found in non-tolerant\n\
         mode, or bundle read/compare errors on either side. 2 = usage error.\n\
         3 = --tolerant found structural pairs (the D6 loop gates on it).\n\
         --help/-h prints this help; --version/-V prints the version."
}

fn main() {
    let argv: Vec<String> = std::env::args().skip(1).collect();
    if argv.first().map(String::as_str) == Some("gpu") {
        std::process::exit(run_gpu(&argv[1..]));
    }
    if let Err(e) = run() {
        eprintln!("error: {e:#}");
        std::process::exit(1);
...
fn run() -> Result<()> {
    ...
    let read_errs = std::sync::atomic::AtomicUsize::new(0);
    pairs
        .par_iter()
        .for_each(|(ref_path, ours_path, ent, name)| {
            match stat_pair(ref_path, ours_path, tolerant) {
                ...
                Err(e) => {
                    read_errs.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
                    eprintln!("err {ent}/{name}: {e}");
                }
            }
        });
    ...
    let n_read_errs = read_errs.into_inner();
    if n_read_errs > 0 {
        eprintln!("{n_read_errs} pair(s) failed to read/compare (counted as failures)");
    }
    ...
    if !tolerant {
        let n_diff = stats.iter().filter(|s| !s.byte_identical).count();
        if n_diff > 0 {
            eprintln!("{n_diff} of {} pair(s) differ byte-wise", stats.len());
            std::process::exit(1);
        }
    }
    Ok(())
