...
pub(crate) fn manifest_from_ids(
    ...
    // Single derive path (shared with the fused streaming build): one
    // derive_one_entity call per id. Entities with no json / no buildable
    // content resolve to None and are counted as missing.
