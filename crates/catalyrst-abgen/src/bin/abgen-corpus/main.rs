...
use sources::{
    from_collection_urn, from_entity_ids, from_live_reference, from_reference, manifest_from_ids,
};
...
fn run() -> Result<()> {
    ...
    let (manifest, out_root) = if let Some(ref live_ref) = live_mode {
        ...
        let cdir = content_dir
            ...
            .unwrap_or_else(|| DEFAULT_CONTENT_ROOT.to_string());
        if cdn_layout {
            // Fused streaming path: derive+build+manifest per entity in one pass
            // so encode work overlaps the scan of later entities (see
            // run_fused_entity_ids). Byte-identical to the phased path below.
            let raw =
...
mod tests {
    ...
    use super::sources::{parse_live_manifest, sample_stride};
