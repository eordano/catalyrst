...
| Var | Default | Meaning |
|---|---|---|
| `HTTP_SERVER_PORT` | `5152` | listen port |
...
map (a `nginx`/gateway rewrite of the prod host to the local bundle):

```csharp
{ "profile-images", ("/profile-images", 5152) },
```
...
which proxies to this service on `:5152`. See the integration note in the
deploy runbook.
