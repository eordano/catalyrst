...
pub struct AppStateInner {
    ...
    /// The resolved API base this service calls (room admin, ingress,
    /// participants), scheme included; never hand it to explorers -- that is
    /// `livekit_ws_url`, which may point at a different edge.
...
pub async fn build_state(cfg: &Config) -> Result<AppState> {
    ...
    let livekit_endpoints = cfg.livekit_endpoints();
    ...
    Ok(Arc::new(AppStateInner {
        ...
        pool: pool.clone(),
        livekit_api_url: livekit_endpoints.api_url,
        livekit_ws_url: livekit_endpoints.client_url,
