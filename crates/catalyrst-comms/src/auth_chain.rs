...
pub fn verify_signed_fetch(
    ...
    let metadata: serde_json::Value = serde_json::from_str(raw_metadata)
        .map_err(|_| SignedFetchError::new(400, format!("Invalid chain metadata: \"{raw_metadata}\"")))?;

    if !allowed_signers.is_empty() {
        let signer_field = metadata.get("signer").and_then(|v| v.as_str()).unwrap_or("");
        if !allowed_signers.iter().any(|s| *s == signer_field) {
            return Err(SignedFetchError::new(400, "Invalid metadata"));
        }
    }

    let ts = header_str(headers, AUTH_TIMESTAMP_HEADER).unwrap_or("0").to_string();
    let payload = build_payload(method, path, &ts, raw_metadata);
    let now = chrono::Utc::now().timestamp();

    let signer = validate_signature(&chain, &payload, &ts, DEFAULT_EXPIRATION_SECS, now).map_err(|e| {
        tracing::warn!(error = ?e, %payload, signer = %chain.signer, "signed-fetch rejected");
        match e {
            AuthChainError::Expired { .. } => {
                SignedFetchError::new(401, "Expired signature")
            }
