...
//!
//! Peer state records what this replica actually told a wallet to join, and its only consumer is
//! the `from_island_id` of the next publish, which needs the exact previous room name. The
//! assignment mirror records what Pulse last published for a wallet regardless of which replica
//! minted it, and its only consumer is the reconnect path.
//!
