...
fn pagination(limit: Option<i64>, offset: Option<i64>) -> (i64, i64) {
    ...
    (limit, offset.filter(|o| *o >= 0).unwrap_or(0))
}
...
pub async fn presign_evidence(
    ...
    let reporter = require_wallet_signer(&headers, "post", PRESIGN_PATH)?;
...
pub async fn upload_evidence(
    ...
    let uploader = require_wallet_signer(&headers, "put", &path)?;
...
pub async fn create_report(
    ...
    let reporter = require_wallet_signer(&headers, "post", CREATE_PATH)?;
...
mod tests {
    ...
    #[test]
    fn pagination_clamps_to_the_page_cap() {
        assert_eq!(pagination(None, None), (MAX_LIMIT, 0));
        assert_eq!(pagination(Some(10), Some(20)), (10, 20));
        assert_eq!(pagination(Some(0), Some(-5)), (MAX_LIMIT, 0));
        assert_eq!(pagination(Some(MAX_LIMIT + 1), None), (MAX_LIMIT, 0));
    }

    #[test]
    fn address_check_is_strict() {
        assert!(is_address("0x1111111111111111111111111111111111111111"));
        assert!(!is_address("0x111"));
        assert!(!is_address("1111111111111111111111111111111111111111"));
        assert!(!is_address("0xzzzz111111111111111111111111111111111111"));
    }
}
