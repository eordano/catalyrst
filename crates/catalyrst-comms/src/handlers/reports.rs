...
use crate::auth_chain::verify_signed_fetch_gated;
use crate::extract::{validate_body, SchemaValidate};
use crate::http::{auth_error, conflict, forbidden, not_found_labeled, ApiError};
...
/// Reads `signer` through the canonical-field gate, never a plain
/// `get("signer")`: a scene that signs `{"Signer":...}` carries a valid
/// signature over the re-spelled key, and an exact-key read would see no
/// scene signer at all and file the report as the wallet that signed it.
/// Applied before signature verification, so the refusal is a 400 that costs
/// no crypto, matching upstream's `metadataValidator` placement.
...
async fn require_wallet_signer(
    ...
    let sf = verify_signed_fetch_gated(headers, method, path, &[], Some(scene_signer_gate()))
        .await
...
mod wallet_signer_gate_tests {
    ...
    fn signed(metadata: &str) -> HeaderMap {
        ...
        let payload = build_payload("post", PRESIGN_PATH, &timestamp, metadata);
    ...
    async fn verify(metadata: &str) -> Result<String, u16> {
        require_wallet_signer(&signed(metadata), "post", PRESIGN_PATH)
            .await
            .map_err(|e| e.code)
    }
    ...
    async fn a_scene_signed_chain_is_refused() {
        assert_eq!(
            verify(r#"{"signer":"decentraland-kernel-scene"}"#).await,
            Err(400)
        );
    }

    #[tokio::test]
    async fn a_respelled_scene_signer_key_is_refused_not_filed_as_the_wallet() {
        assert_eq!(
            verify(r#"{"Signer":"decentraland-kernel-scene"}"#).await,
            Err(400)
        );
    ...
    #[tokio::test]
    async fn an_ordinary_wallet_chain_still_reports() {
        let wallet = Wallet::from_hex(KEY).unwrap();
        assert_eq!(
            verify("{}").await.as_deref(),
            Ok(wallet.address().to_lowercase().as_str())
        );
        assert!(verify(r#"{"signer":"dcl:explorer"}"#).await.is_ok());
    }
}
