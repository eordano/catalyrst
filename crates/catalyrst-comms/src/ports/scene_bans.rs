...
impl SceneBansComponent {
    ...
    pub async fn count(&self, place_id: &str) -> Result<i64, ApiError> {
        let n: i64 =
            sqlx::query_scalar("SELECT COUNT(*) FROM scene_bans WHERE place_id = $1")
                .bind(place_id)
                .fetch_one(&self.pool)
                .await?;
        Ok(n)
    ...
    pub async fn ban(&self, place_id: &str, banned_address: &str, banned_by: &str) -> Result<Uuid, ApiError> {
