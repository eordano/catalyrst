...
- `place_id` is computed as `realm_name` for worlds and `scene_id` for scenes — upstream additionally resolves world-name → content-hash via the worlds content server (`worlds.fetchWorldSceneId`). Once `catalyrst-worlds` lands, plumb that lookup in.
