...
const K: usize = 50;
// Rooms 0..MOD_ROOMS have an active moderator; the rest have connected
// non-moderators but no moderator.
...
async fn setup() -> Option<ScratchSchema> {
    ...
    scratch
        ...
        .await;
    // Adds the `sid` column the pre-change per-id query selects.
    scratch
