use catalyrst_comms::ports::player_reports::{
    ...
    MAX_PENDING_EVIDENCE_SLOTS, MAX_REPORTS_PER_HOUR,
};
...
async fn concurrent_submissions_cannot_overshoot_the_hourly_cap() {
    let Some(scratch) = setup().await else {
        return;
    };
    let reports = PlayerReportsComponent::new(scratch.pool.clone());

    for _ in 0..MAX_REPORTS_PER_HOUR - 1 {
        sqlx::query(
            "INSERT INTO player_reports (id, reporter_address, reported_address, reason, description) \
             VALUES ($1, $2, $3, 'harassment', 'seeded')",
        )
        ...
        .await
        .expect("seed the reporter one slot below the cap");
    }

    const RACERS: usize = 6;
    let mut drafts = Vec::with_capacity(RACERS);
    for _ in 0..RACERS {
        let (report_id, slots) = reports
            .create_evidence_slots(REPORTER, &one_file())
            .await
        ...
        reports
            ...
            .await
            .expect("upload");
        drafts.push((report_id, slots[0].key.clone()));
    }

    let mut racers = Vec::with_capacity(RACERS);
    for (report_id, key) in drafts {
        let component = PlayerReportsComponent::new(scratch.pool.clone());
        racers.push(tokio::spawn(async move {
            component.create_report(draft(report_id, vec![key])).await
        }));
    }
    ...
    for racer in racers {
        match racer.await.expect("racer task") {
            Ok(_) => accepted += 1,
            Err(ReportWriteError::RateLimited) => rate_limited += 1,
            Err(other) => panic!("expected RateLimited, got {other:?}"),
        }
    }
    assert_eq!(accepted, 1, "only one racer may take the last slot");
    assert_eq!(rate_limited, RACERS - 1);
    assert_eq!(
        reports.count_reports(None, None).await.unwrap(),
        MAX_REPORTS_PER_HOUR,
        "the cap must hold under concurrent submissions"
    );
