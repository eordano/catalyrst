...
async fn report_round_trips_from_presign_through_moderator_read() {
    let Some(scratch) = setup().await else {
        eprintln!("skipping report_round_trips: set CATALYRST_COMMS_TEST_PG to run");
        return;
    };
...
async fn report_without_uploaded_evidence_is_rejected() {
    let Some(scratch) = setup().await else {
        eprintln!("skipping report_without_uploaded_evidence: set CATALYRST_COMMS_TEST_PG to run");
        return;
    };
    let reports = PlayerReportsComponent::new(scratch.pool.clone());
...
async fn evidence_slots_are_bound_to_the_signer_that_presigned_them() {
    let Some(scratch) = setup().await else {
        eprintln!("skipping evidence_slots_are_bound: set CATALYRST_COMMS_TEST_PG to run");
        return;
    };
    let reports = PlayerReportsComponent::new(scratch.pool.clone());
...
async fn evidence_upload_enforces_declared_size_type_and_single_write() {
    let Some(scratch) = setup().await else {
        eprintln!("skipping evidence_upload_enforces: set CATALYRST_COMMS_TEST_PG to run");
        return;
    };
    let reports = PlayerReportsComponent::new(scratch.pool.clone());
...
async fn a_report_id_can_only_be_submitted_once() {
    let Some(scratch) = setup().await else {
        eprintln!(
            "skipping a_report_id_can_only_be_submitted_once: set CATALYRST_COMMS_TEST_PG to run"
        );
        return;
    };
    let reports = PlayerReportsComponent::new(scratch.pool.clone());
...
async fn unclaimed_evidence_slots_are_capped_per_reporter() {
    let Some(scratch) = setup().await else {
        eprintln!(
            "skipping unclaimed_evidence_slots_are_capped: set CATALYRST_COMMS_TEST_PG to run"
        );
        return;
    };
    let reports = PlayerReportsComponent::new(scratch.pool.clone());
