...
async fn seed(pool: &sqlx::PgPool) {
    ...
    // p2: 20 parcels on row y=1; all owned by 0xother but each carries estate
    // 'est1', which is owned by 0xestate.
...
async fn parcel_ownership_is_one_round_trip() {
    ...
    // Direct owner on the LAST coordinate — worst case for the old per-coord loop.
    assert!(
