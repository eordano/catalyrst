...
async fn body_community_id_is_not_stamped_on_a_freshly_minted_group() {
    let Some(scratch) = setup().await else {
        eprintln!("skipping body_community_id_is_not_stamped_on_a_freshly_minted_group: set CATALYRST_COMMS_TEST_PG to run");
        return;
    };
...
async fn service_token_may_bind_a_community_channel() {
    let Some(scratch) = setup().await else {
        eprintln!("skipping service_token_may_bind_a_community_channel: set CATALYRST_COMMS_TEST_PG to run");
        return;
    };
    let pool = scratch.pool.clone();
    let state = test_state(pool.clone(), Some(SERVICE_TOKEN.into()));
...
async fn plain_dm_group_creation_is_unaffected() {
    let Some(scratch) = setup().await else {
        eprintln!(
            "skipping plain_dm_group_creation_is_unaffected: set CATALYRST_COMMS_TEST_PG to run"
        );
        return;
    };
    let pool = scratch.pool.clone();
