...
/// Upstream builder-server refuses the scene signer on the delivered metadata
/// header before any signature is checked (`src/middleware/authentication.ts`
/// `isNotSceneSigner`), so a request a scene runtime signed on a visiting player's
/// behalf is refused rather than served as that player.
...
pub async fn try_extract_signer(headers: &HeaderMap, method: &str, path: &str) -> Option<Signer> {
    verify(headers, method, path, FIVE_MINUTES).await.ok()
}
...
pub async fn require_signer(
    ...
    verify(headers, method, path, THIRTY_MINUTES).await
}
...
mod scene_signer_gate_tests {
    ...
    async fn an_ordinary_signer_still_verifies_under_either_payload_shape() {
        ...
        for v6 in [true, false] {
            let signer = verify(&signed(metadata, v6), METHOD, PATH, THIRTY_MINUTES)
                .await
    ...
    async fn a_scene_signed_request_is_refused() {
        for metadata in [
            ...
            for v6 in [true, false] {
                let err = verify(&signed(metadata, v6), METHOD, PATH, THIRTY_MINUTES)
                    .await
