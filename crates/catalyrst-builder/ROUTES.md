...
deployment's assigned port (`5144`; see `umbrella/env/catalyrst-builder.env`)
and points at a PostgreSQL instance (`<DB_HOST>:5433`) with a dedicated
`builder` database.
...
| POST | `/v1/newsletter` | none | done | `{email,source?}` (source defaults to `Builder`). Email is trimmed + lowercased and validated server-side: missing/invalid -> `400 {ok:false,error}`. Valid -> durable upsert into `newsletter_subscriptions` (PK email; conflict updates `source`) -> `200 {ok:true}`; DB failure -> `500` (no silent success). Optional SaaS forward stays best-effort. |
...
The curation routes (`GET /v1/collections/curation`, `PATCH
/v1/collections/{id}/items/{item}/status`, `PATCH
/v1/collections/{id}/items/status`) use a separate `authorize_admin` gate in
`handlers/curation.rs`: either a timing-safe-compared `Authorization: Bearer
<CATALYRST_BUILDER_ADMIN_TOKEN>`, or a signed-fetch (AuthChain) request from an
address in `BUILDER_ADMIN_ADDRESSES`. The sites committee page reads the queue
server-side with that bearer (shared via `sites.env`).
