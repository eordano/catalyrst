...
fn numbers_equal(a: &Value, b: &Value) -> bool {
    match (a, b) {
        ...
        (Value::Object(xo), Value::Object(yo)) => {
            let mut xk: Vec<&String> = xo.keys().collect();
            let mut yk: Vec<&String> = yo.keys().collect();
            xk.sort();
            yk.sort();
            xk == yk
                && xo
...
const LEGACY_WIRE_ORDER: [&str; 11] = [
    "type",
    "x",
    "y",
    "top",
...
fn legacy_mapping_parity_against_upstream_snapshot() {
    ...
    for (id, v2obj) in v2 {
        ...
        let legacy = to_legacy(&tile);
        let got = serde_json::to_value(&legacy).unwrap();
        let want = v1
            .get(id)
        ...
        assert_eq!(
            key_set(&got),
            key_set(want),
            "legacy key set drifted for {id}"
        );
        let wire = wire_keys(&serde_json::to_string(&legacy).unwrap());
        let expected: Vec<String> = LEGACY_WIRE_ORDER
            .iter()
            .filter(|k| got.as_object().unwrap().contains_key(**k))
            .map(|k| k.to_string())
            .collect();
        assert_eq!(wire, expected, "legacy wire key order drifted for {id}");
        assert!(
...
fn nft_metadata_envelope_parity() {
    ...
    assert_eq!(
        key_set(&got),
        key_set(&want),
        "NFT top-level key set drifted"
    );
    assert_eq!(
        wire_keys(&serde_json::to_string(&nft).unwrap()),
        [
            "id",
            "name",
            "description",
        ...
        "NFT wire key order drifted"
    );
    assert_eq!(
        wire_keys(&serde_json::to_string(&nft.attributes[0]).unwrap()),
        ["trait_type", "value", "display_type"],
        "NFT attribute wire key order drifted"
    );
    ...
    for attr in got["attributes"].as_array().unwrap() {
        assert_eq!(key_set(attr), vec!["display_type", "trait_type", "value"]);
    }
