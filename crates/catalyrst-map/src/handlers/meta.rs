...
async fn build_estate_nft(state: &AppState, id: &str) -> Result<Option<Value>, sqlx::Error> {
    ...
    let coords_sql = format!(
        "SELECT x::int4, y::int4 FROM {schema}.parcel WHERE estate_id = $1"
    );
    ...
    let mut attributes: Vec<Value> = vec![
        json!({ "trait_type": "Size", "value": size.unwrap_or(0), "display_type": "number" }),
    ];
...
async fn get_token_inner(state: &AppState, address: String, id: String) -> (Response, bool) {
    ...
    if addr == state.map.land_contract().to_lowercase() {
        ...
        if let Some((x, y)) = row {
            return (get_parcel_inner(state, x.to_string(), y.to_string()).await, true);
        }
...
pub async fn get_contributions(
    ...
    let resp =
        Json(json!({ "ok": true, "data": crate::districts::contributions_for(&address) })).into_response();
    finalize(resp, last)
}
