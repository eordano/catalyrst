...
fn finalize_etag(mut resp: Response, last: i64, key: &str) -> Response {
    ...
    cache::apply_etag(&mut resp, last, &etag, cache::DEFAULT_MAX_AGE, cache::DEFAULT_SWR);
    resp
}
...
const VALID_FIELDS: &[&str] = &[
    "id", "x", "y", "type", "name", "top", "left", "topLeft", "updatedAt", "owner", "estateId",
    "tokenId", "price", "expiresAt", "rentalListing", "estateSize", "rentalPricePerDay",
    "rentalExpiresAt",
];
...
pub async fn get_tiles(
    ...
    if !state.map.is_ready() {
        return finalize((StatusCode::SERVICE_UNAVAILABLE, "Not ready").into_response(), last);
    }
    ...
    let body = serde_json::to_vec(&json!({ "ok": true, "data": Value::Object(map) }))
        .unwrap_or_default();
    let body = Arc::new(body);
    state.map.store_tiles_response(cache_key.clone(), body.clone());
