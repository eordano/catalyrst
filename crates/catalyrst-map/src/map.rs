...
impl MapComponent {
    pub fn new(pool: PgPool, schema: String, land_contract: String, estate_contract: String) -> Self {
    ...
    pub fn last_updated_at(&self) -> i64 {
        self.data.read().as_ref().map(|d| d.last_updated_at).unwrap_or(0)
    }
    ...
    async fn build(&self) -> anyhow::Result<MapData> {
        ...
        let mut tiles: HashMap<String, Tile> = HashMap::with_capacity(rows.len() + self.special_tiles.len());
        ...
        for r in &rows {
            ...
            let (price_str, expires) = if r.estate_full_id.as_deref().map(|s| !s.is_empty()).unwrap_or(false)
