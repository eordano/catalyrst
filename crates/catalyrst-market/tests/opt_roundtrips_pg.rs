// DB-round-trip optimization regression tests for catalyrst-market.
//
...
async fn trades_membership_probe_is_batch_scoped() {
    ...
    // Seed N=300 rows with distinct lowercase hashed_signatures.
    let n = 300usize;
    for i in 0..n {
    ...
    // Set-equality: exactly the K=4 seeded∩batch signatures — not a superset.
    assert_eq!(
        known.len(),
        4,
        "returns exactly the batch∩seeded intersection"
    );
    ...
    // None of the other 296 seeded-but-not-in-batch sigs leaked in.
    for i in 4..n {
        assert!(
...
async fn snapshot_hash_queries_run_concurrently() {
    ...
    // Seed known signature_hash rows in each local log table.
    let bids = ["0bbb", "1bbb"];
    ...
    // 3×0.4s. MAX(seq) join adds ~0.4s. New total ≈0.8s; old ≥1.6s.
    assert!(
...
async fn create_order_auth_reads_overlap_with_identical_verdicts() {
    ...
    // AUTHORIZED wallet owns an nft for the item.
