...
use axum::Router;
use catalyrst_contract_gate::{signed_fetch_headers, test_wallet, Case, Gate, Wallet};
...
impl Scratch {
    async fn create() -> Option<Self> {
        let admin_url = std::env::var("CATALYRST_MARKET_TEST_PG").ok()?;
...
async fn every_spec_route_answers_its_contract() {
    let Some(scratch) = Scratch::create().await else {
        eprintln!("skipping market contract gate: set CATALYRST_MARKET_TEST_PG to run");
        return;
    };
    scratch
    ...
    gate.hit(
        &app,
        with_signed_for(Case::new("get", "/v1/activity"), &user, "get", "/v1/other").expect(401),
    )
    .await;
    gate.hit(
        &app,
        Case::new("get", "/v1/activity").signed(&user).expect(400),
    )
    .await;
    gate.waive_success(
        "get",
        "/v1/activity",
        "a 200 aggregates the squid sales/bids/orders analytical tables plus the marketplace.trades / mv_trades reader (~10 subgraph-synced tables and materialized views) which the federation-scoped scratch database does not reproduce; unsigned->401, wrong-path->401 and missing-address->400 auth cases are asserted",
    );
