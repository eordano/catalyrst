...
fn land_search_ors_whole_string_and_word_similarity() {
    let land = NftFilters {
        category: Some(crate::dcl_schemas::NftCategory::Parcel),
        search: Some("genesis".to_string()),
        ..Default::default()
    };
    let sql = nfts_sql(&land);
    assert!(
        sql.contains("(search_text % $2 OR $2 <% search_text)"),
        "{sql}"
    );
    let estate = NftFilters {
        is_land: true,
        search: Some("genesis".to_string()),
        ..Default::default()
    };
    assert!(nfts_sql(&estate).contains("<% search_text"));

    let wearable = NftFilters {
        category: Some(crate::dcl_schemas::NftCategory::Wearable),
        search: Some("genesis".to_string()),
        ..Default::default()
    };
    let sql = nfts_sql(&wearable);
    assert!(sql.contains(" search_text % $2 "), "{sql}");
