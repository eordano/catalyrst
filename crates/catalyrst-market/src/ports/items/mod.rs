...
// A DbItem row from the catalog-items query: the same columns plus the SQL-computed whole-credit
// price (CEIL of the USD-wei-equivalent; 0 for not-for-sale items).
#[derive(Debug, sqlx::FromRow)]
...
// USD-pegged trades pass through (already USD wei), MANA-priced trades and classic store-minter
// prices are converted with the live MANA/USD rate, everything else prices at 0.
#[derive(Debug, Serialize)]
...
impl ItemsComponent {
    ...
    // The credit-aware catalog-items feed backing GET /v3/catalog/items: same items as get_items
    // (full catalog, incl. not-on-sale) but each item carries a server-computed, asset-type-aware
    // priceCredits. `mana_usd_rate` is USD per MANA, used to convert MANA-priced items (classic
    // store minter / ERC20 trades) to credits; USD-pegged trades are already in USD and are NOT
    // rate-adjusted.
