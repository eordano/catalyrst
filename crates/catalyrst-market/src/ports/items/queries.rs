...
mod tests {
    ...
    fn items_search_matches_name_words_and_collection_words() {
        ...
        assert!(sql.contains("lower(search_word.text) % lower($1)"), "{sql}");
        assert!(
            sql.contains("search_collection.id = item.collection_id"),
            "{sql}"
        );
        assert!(!sql.contains("search_text %"), "{sql}");
    ...
    fn only_the_catalog_items_feed_hides_unapproved_collections() {
        ...
        assert!(
            browse.contains("item.search_is_collection_approved = true"),
            "{browse}"
        );
        let (by_id, _) = build_items_query(&ItemFilters {
            item_id: Some("7".to_string()),
            ..Default::default()
        });
        assert!(!by_id.contains("search_is_collection_approved"), "{by_id}");
    }
