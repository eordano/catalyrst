...
fn catalog_search_prepass_matches_name_and_collection_words() {
    let f = CatalogFilters {
        search: Some("Pirate Hat".into()),
        ..Default::default()
    };
    let (sql, _) = queries::build_search_query(&f);
    assert!(
        sql.contains("SELECT DISTINCT ON (matched.id) matched.id, 'name'::text AS match_type, matched.word, matched.word_similarity"),
        "{sql}"
    );
    assert!(
        sql.contains("unnest(string_to_array(COALESCE(wearable.name, emote.name), ' ')) AS search_word(text)"),
        "{sql}"
    );
    assert!(
        sql.contains("unnest(string_to_array(search_collection.name, ' ')) AS search_word(text)"),
        "{sql}"
    );
    assert_eq!(
        sql.matches("lower(search_word.text) % lower($1)").count(),
        2,
        "{sql}"
    );
    assert_eq!(
        sql.matches("similarity(lower(search_word.text), lower($1))")
            .count(),
        2,
        "{sql}"
    );
    assert!(
        sql.contains("ORDER BY matched.id, matched.word_similarity DESC"),
        "{sql}"
    );
