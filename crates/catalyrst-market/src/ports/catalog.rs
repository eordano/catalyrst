    ...
    b.push_sql(
        " LEFT JOIN (
            SELECT
                ...
                MAX(assets -> 'sent' ->> 'token_id') AS token_id,
                ...
                json_agg(assets) AS aggregated_assets
            FROM unified_trades
...
fn build_get_min_price_case(b: &mut Builder, f: &CatalogFilters) {
    ...
    b.push_sql(
        " THEN LEAST(items.price, nfts_with_orders.min_price) ELSE nfts_with_orders.min_price END AS min_price ",
    );
}
...
fn build_get_max_price_case(b: &mut Builder, f: &CatalogFilters) {
    if f.only_minting {
        ...
        b.push_sql(" THEN GREATEST(items.price, nfts_with_orders.max_price) ELSE nfts_with_orders.max_price END AS max_price ");
    }
}
...
fn build_get_min_price_case_with_trades(b: &mut Builder, f: &CatalogFilters) {
    ...
    if f.only_minting {
        b.push_sql(&format!(
            " THEN LEAST(COALESCE(items.price, '{n}'), COALESCE(offchain_orders.min_order_amount_received, '{n}'), COALESCE(offchain_orders.open_item_trade_price, '{n}')) ELSE LEAST(COALESCE(offchain_orders.min_order_amount_received, '{n}'), COALESCE(offchain_orders.open_item_trade_price, '{n}')) END AS min_price ",
            n = MAX_NUMERIC_NUMBER,
        ));
    } else {
        b.push_sql(&format!(
            " THEN LEAST(COALESCE(items.price, '{n}'), COALESCE(nfts_with_orders.min_price, '{n}'), COALESCE(offchain_orders.min_order_amount_received, '{n}'), COALESCE(offchain_orders.open_item_trade_price, '{n}')) ELSE LEAST(COALESCE(nfts_with_orders.min_price, '{n}'), COALESCE(offchain_orders.min_order_amount_received, '{n}'), COALESCE(offchain_orders.open_item_trade_price, '{n}')) END AS min_price ",
            n = MAX_NUMERIC_NUMBER,
        ));
    }
}
...
fn build_get_max_price_case_with_trades(b: &mut Builder, f: &CatalogFilters) {
    if f.only_minting {
        ...
        b.push_sql(" THEN GREATEST(items.price, offchain_orders.max_order_amount_received, offchain_orders.open_item_trade_price) ELSE GREATEST(offchain_orders.max_order_amount_received, offchain_orders.open_item_trade_price) END AS max_price ");
    } else {
        b.push_sql(" CASE WHEN items.available > 0 AND (items.search_is_store_minter = true OR items.search_is_marketplace_v3_minter = true) ");
        if let Some(mx) = &f.max_price {
            let bi = b.bind_string(mx.clone());
        ...
        b.push_sql(" THEN GREATEST(items.price, nfts_with_orders.max_price, offchain_orders.max_order_amount_received, offchain_orders.open_item_trade_price) ELSE GREATEST(nfts_with_orders.max_price, offchain_orders.max_order_amount_received, offchain_orders.open_item_trade_price) END AS max_price ");
    }
}
...
fn build_collections_items_catalog_query(f: &CatalogFilters) -> (String, PgArguments) {
    ...
    if f.only_minting {
        b.push_sql(" NULL AS min_listing_price, NULL AS max_listing_price, 0 AS listings_count, ");
    } else {
        b.push_sql(" nfts_with_orders.min_price AS min_listing_price, nfts_with_orders.max_price AS max_listing_price, COALESCE(nfts_with_orders.listings_count, 0) AS listings_count, ");
    }
    if f.is_on_sale == Some(false) {
        b.push_sql(" nfts.owners_count, ");
    } else {
...
fn build_collections_items_catalog_query_with_trades(f: &CatalogFilters) -> (String, PgArguments) {
    ...
    if f.is_on_sale == Some(true) {
        b.push_sql(" LEAST(items.first_listed_at, ROUND(EXTRACT(EPOCH FROM ut_min_item.min_item_created_at))) as first_listed_at, ");
    } else {
        b.push_sql(" items.first_listed_at as first_listed_at, ");
    }
    ...
    if f.only_minting {
        b.push_sql(&format!(
            " items.urn,
              CASE WHEN offchain_orders.min_order_amount_received IS NULL THEN NULL
                   ELSE LEAST(COALESCE(offchain_orders.min_order_amount_received, '{n}')) END AS min_listing_price,
        ...
        b.push_sql(" items.urn,
              CASE WHEN offchain_orders.min_order_amount_received IS NULL AND nfts_with_orders.min_price IS NULL THEN NULL
                   ELSE LEAST(COALESCE(offchain_orders.min_order_amount_received, nfts_with_orders.min_price), COALESCE(nfts_with_orders.min_price, offchain_orders.min_order_amount_received)) END AS min_listing_price,
              ...
              GREATEST(ROUND(EXTRACT(EPOCH FROM offchain_orders.max_created_at)), nfts_with_orders.max_order_created_at) AS max_order_created_at,
            ");
    }
    if f.is_on_sale == Some(false) {
        b.push_sql(" nfts.owners_count, ");
