...
const NON_EDITABLE_LISTS_SQL: &str = "SELECT favorites.lists.id::text AS id FROM favorites.lists \
     WHERE favorites.lists.id = ANY($1::uuid[]) \
     AND favorites.lists.user_address != $2 \
     AND favorites.lists.user_address != $3 \
     AND NOT EXISTS (\
       SELECT 1 FROM favorites.acl \
       WHERE favorites.acl.list_id = favorites.lists.id \
       AND favorites.acl.permission = 'edit' \
       AND favorites.acl.grantee IN ($2, $4)\
     )";

/// Binds: $1 item id, $2 caller, $3 list ids, $4 default-list owner,
/// $5 grantee wildcard.
const PICK_IN_LISTS_SQL: &str = "INSERT INTO favorites.picks (item_id, user_address, list_id) \
...
impl ListsComponent {
    ...
    pub async fn check_non_editable_lists(
        ...
        let rows = sqlx::query(sqlx::AssertSqlSafe(NON_EDITABLE_LISTS_SQL.to_string()))
            .bind(list_ids)
...
mod tests {
    ...
    fn non_editable_lists_sql_tests_the_absence_of_an_edit_grant() {
        ...
        assert!(NON_EDITABLE_LISTS_SQL.contains("AND favorites.lists.user_address != $2"));
        assert!(NON_EDITABLE_LISTS_SQL.contains("AND favorites.lists.user_address != $3"));
        assert!(NON_EDITABLE_LISTS_SQL.contains("AND NOT EXISTS ("));
        assert!(NON_EDITABLE_LISTS_SQL.contains("SELECT 1 FROM favorites.acl"));
        assert!(NON_EDITABLE_LISTS_SQL.contains("AND favorites.acl.permission = 'edit'"));
        assert!(NON_EDITABLE_LISTS_SQL.contains("AND favorites.acl.grantee IN ($2, $4)"));
        assert!(!NON_EDITABLE_LISTS_SQL.contains("LEFT JOIN"));
        assert!(!NON_EDITABLE_LISTS_SQL.contains("!= 'edit'"));
        assert!(!NON_EDITABLE_LISTS_SQL.contains("NOT IN"));
    }

    /// Defence in depth: the INSERT itself only reaches lists the caller
    /// owns, the shared default list, or lists carrying an edit grant for
    /// them or for everyone.
    #[test]
    fn pick_in_lists_sql_scopes_the_insert_to_editable_lists() {
        assert!(PICK_IN_LISTS_SQL.contains("WHERE id = ANY($3::uuid[]) AND ("));
        assert!(PICK_IN_LISTS_SQL.contains("favorites.lists.user_address = $2"));
        assert!(PICK_IN_LISTS_SQL.contains("OR favorites.lists.user_address = $4"));
        assert!(PICK_IN_LISTS_SQL.contains("OR EXISTS ("));
        assert!(PICK_IN_LISTS_SQL.contains("AND favorites.acl.permission = 'edit'"));
        assert!(PICK_IN_LISTS_SQL.contains("AND favorites.acl.grantee IN ($2, $5)"));
        assert!(
