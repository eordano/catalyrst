...
// A small, fail-safe MANA/USD rate cache for server-side credit pricing. Instead of a dedicated
// on-chain client (upstream reads a Chainlink aggregator directly), it sources OUR price oracle:
// the catalyrst-price service's CoinGecko-shaped /api/v3/simple/price — the SAME feed the
// catalyrst-credits checkout settles at — so the credit price the catalog advertises stays
// coherent with what a purchase charges. It caches the last-known rate and refreshes on an
// interval; reads never fail.
pub const DEFAULT_PRICE_BASE_URL: &str = "http://127.0.0.1:5156";
pub const DEFAULT_REFRESH_INTERVAL_MS: u64 = 90_000;
// USD per MANA, used only before the first successful fetch; after that the last-known value is
// kept on failure.
pub const DEFAULT_FALLBACK_RATE: f64 = 0.02;
pub const DEFAULT_MAX_STALENESS_SECONDS: i64 = 86_400;
// Upper bound on how long the initial refresh may block startup. A slow/unreachable oracle must
// not wedge boot: after this the server serves the fallback and the interval keeps trying.
...
impl ManaUsdRateComponent {
    ...
    // Bounded initial refresh + background interval. If the oracle answers within the startup
    // timeout the server boots with a live rate; otherwise it boots on the fallback while the
    // still-running initial fetch (and the interval below) updates the cache as soon as the
    // oracle responds.
...
// Extracts USD-per-MANA from the CoinGecko-compatible simple/price body and applies the same
// completeness and staleness guards the credits checkout applies, so a stale or broken oracle
// answer is never advertised as a price.
...
// Format a MANA/USD rate (USD per MANA) as a bounded-precision decimal literal for Postgres
// numeric math. A non-positive/non-finite rate yields '0' so the callers' `usd_wei > 0` guards
// drop the rows rather than advertising a free item off a broken rate.
