...
fn top_creators_attributes_to_the_item_creator() {
    ...
    assert!(
        sql.contains("ORDER BY r.sales DESC, r.creator ASC"),
        "{sql}"
    );
...
/// Published-but-dormant is not "top" anything (the LEFT JOIN would let a zero
/// through), and a one-hit month over a near-empty catalogue is not somewhere
/// to send a shopper. Both floors are enforced in the query, next to each
/// other — a caller that has to re-filter is a caller that can forget to.
#[test]
fn top_creators_floors_out_dormant_and_barely_published_creators() {
    let (sql, binds) = build_top_creators_sql(None, None);
    assert!(sql.contains("WHERE r.sales > 0"), "{sql}");
    assert!(sql.contains("COALESCE(c.items, 0) >="), "{sql}");
    assert!(
        bind_ints(&binds).contains(&TOP_CREATORS_MIN_ITEMS),
        "{binds:?}"
    );
}
...
// ---- /v3/catalog/trending (marketplace-server #384) ----

#[test]
fn trending_windows_sales_over_the_shared_midnight_helper() {
    use super::types::TRENDING_DEFAULT_DAYS;
    use crate::ports::trendings::midnight_days_ago;
...
// ---- includeSocialEmotes on the shared unified feed (marketplace-server #384) ----
