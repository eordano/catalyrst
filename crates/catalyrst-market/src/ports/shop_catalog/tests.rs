...
use super::unified::{
    build_unified_items_sql, build_unified_listings_sql, parse_unified_filters,
    parse_unified_group_by, unified_min_price_bound_wei, ShopListingType, UnifiedCatalogFilters,
    UnifiedGroupBy, UnifiedSource,
};
...
fn unified_creator_filter_applies_inside_each_branch() {
    ...
    let (sql, binds) = build_unified_listings_sql(&filters, 0.5);
    assert_eq!(sql.matches(CREATOR_PREDICATE).count(), 2, "{sql}");
    assert_eq!(
        bind_texts(&binds)
            .iter()
...
fn unified_smart_wearable_filter_applies_inside_each_branch() {
    ...
    let (sql, _) = build_unified_listings_sql(&filters, 0.5);
    assert_eq!(sql.matches(SMART_WEARABLE_PREDICATE).count(), 2, "{sql}");
...
fn unified_feeds_select_seller_and_issued_id_in_each_branch() {
    let (sql, _) = build_unified_listings_sql(&UnifiedCatalogFilters::default(), 0.5);
    assert_eq!(sql.matches(SELLER_EXPR).count(), 2, "{sql}");
    assert_eq!(sql.matches(ISSUED_ID_EXPR).count(), 2, "{sql}");
...
#[test]
fn unified_items_source_filter_builds_a_single_branch() {
    let native_only = UnifiedCatalogFilters {
        source: Some(UnifiedSource::Native),
        ..Default::default()
    };
...
#[test]
fn unified_listing_type_filter_lands_in_both_union_branches() {
    ...
    assert_eq!(occurrences(&sql, PRIMARY), baseline + 2, "{sql}");
    ...
    assert_eq!(occurrences(&sql, SECONDARY), 2, "{sql}");
}
