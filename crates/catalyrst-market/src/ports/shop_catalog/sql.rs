...
/// #389/#390): creators ranked by how many of THEIR items sold in the window,
/// attributing each sale to `item.creator` via the `sale.item_id = item.id`
...
/// not still ranks; `r.sales > 0` then keeps published-but-dormant creators
/// out, and the [`TOP_CREATORS_MIN_ITEMS`] floor keeps out a one-hit month with
/// nothing to browse.
