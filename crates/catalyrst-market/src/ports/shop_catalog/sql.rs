...
// (search_wearable_body_shapes contains 'BaseMale'/'BaseFemale'). Both -> unisex, one -> that
// gender, neither/emote -> NULL. COALESCE(item_p, item_s) picks whichever side of the
// primary/secondary join is populated; ::text[] on both sides keeps the @> element types matching.
