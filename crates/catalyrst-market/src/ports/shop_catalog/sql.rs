...
pub(super) fn shop_search_where(
    ...
    let term_p = emit(Bind::Text(search.to_string()), binds, next_idx);
    let like_p = emit(
        Bind::Text(format!("%{}%", escape_like(search))),
        binds,
        next_idx,
    );
    crate::logic::search_match::shop_search_where(
        SHOP_ITEM_ID_EXPR,
        name_expr,
        SHOP_COLLECTION_ID_EXPR,
        SHOP_NON_ITEM_NAME_EXPR,
        &term_p,
        &like_p,
    )
}
...
pub fn build_shop_listings_sql(filters: &ShopCatalogFilters) -> (String, Vec<Bind>) {
    ...
    let mut wheres = vec![
        ...
        format!(" {APPROVED_COLLECTION_PREDICATE} "),
    ];
...
pub fn build_legacy_listings_sql(filters: &LegacyCatalogFilters) -> (String, Vec<Bind>) {
    ...
    if let Some(search) = filters.search.as_deref().filter(|s| !s.is_empty()) {
        let matched = shop_search_where(LEGACY_NAME_EXPR, search, &mut binds, &mut next_idx);
        wheres.push(format!(" {matched} "));
    }
