...
pub struct ItemFilters {
    ...
    /// Social emotes (those carrying an `outcome_type`) are included by default.
    /// Only `includeSocialEmotes=false` flips this off, adding the
    /// `emote.outcome_type IS NULL` exclusion (matches upstream getItemsParams).
...
impl Default for ItemFilters {
    fn default() -> Self {
        Self {
            ...
            ids: Vec::new(),
            sort_by: None,
            // Upstream getItemsParams default: social emotes included unless
            // explicitly excluded with includeSocialEmotes=false.
...
pub fn parse_filters(pairs: &[(String, String)]) -> Result<ItemFilters, ApiError> {
    ...
    Ok(ItemFilters {
        ...
        // Upstream: included by default; only `includeSocialEmotes=false` excludes.
        include_social_emotes: p.get_string("includeSocialEmotes", None).as_deref()
            != Some("false"),
    })
}

/// Parse a `minPrice`/`maxPrice` query value the way upstream getItemsParams
/// does: skip when absent or blank, otherwise `ethers.parseEther` the decimal
/// MANA string into a wei integer string (the unit the wei-scale `item.price`
/// and `assets->'received'->>'amount'` columns are compared against). Invalid
/// input yields a 400, mirroring upstream's `Invalid <param> value` HttpError.
...
/// Decimal MANA -> wei integer string, matching `ethers.parseEther` /
/// `parseUnits(value, 18)`: an optional leading `-`, an integer part, and an
/// optional fractional part of at most 18 digits; the fraction is right-padded
/// to 18 digits and concatenated, then leading zeros are trimmed. Any other
/// shape (empty, non-digit chars, multiple dots, >18 decimals) is rejected.
pub fn parse_ether(value: &str) -> Option<String> {
    ...
    // At least one digit must be present overall; both sides must be all digits.
    ...
    // ethers preserves a sign only for non-zero magnitudes.
    if neg && out != "0" {
        Some(format!("-{}", out))
    } else {
        Some(out.to_string())
...
pub fn build_items_query(filters: &ItemFilters) -> (String, Vec<Bind>) {
    ...
    // Social emotes (outcome_type set) are included by default; only excluded
    // when includeSocialEmotes=false (upstream EXCLUDE_SOCIAL_EMOTES). Passing
    // emoteOutcomeType together with this is contradictory and yields no emotes,
    // exactly as upstream notes.
...
mod parse_ether_tests {
    ...
    fn max_18_decimals_enforced() {
        // exactly 18 fractional digits is fine
        assert!(parse_ether("1.123456789012345678").is_some());
        // 19 fractional digits is rejected (matches ethers underflow error)
