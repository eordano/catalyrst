...
fn sort_items(ranks: &mut [ItemRank], sort_by: Option<RankingsSortBy>) {
    match sort_by {
        Some(RankingsSortBy::MostSales) => ranks.sort_by_key(|b| std::cmp::Reverse(b.sales)),
        _ => ranks.sort_by(|a, b| bn_cmp(&b.volume, &a.volume)),
    }
}

fn sort_creators(ranks: &mut [CreatorRank], sort_by: Option<RankingsSortBy>) {
    match sort_by {
        Some(RankingsSortBy::MostSales) => ranks.sort_by_key(|b| std::cmp::Reverse(b.sales)),
...
fn sort_collectors(ranks: &mut [CollectorRank], sort_by: Option<RankingsSortBy>) {
    match sort_by {
        Some(RankingsSortBy::MostSales) => ranks.sort_by_key(|b| std::cmp::Reverse(b.purchases)),
