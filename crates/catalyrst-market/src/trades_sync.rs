//! Upstream trades freshness poller (Marketplace v3 signed-trade book).
//!
//! The local `marketplace.trades` table was seeded by a one-shot import of the
//! upstream public book (GET https://marketplace-api.decentraland.org/v1/trades)
//! and nothing kept it fresh afterwards. This background task re-fetches the
//! upstream list every `TRADES_SYNC_INTERVAL_SECS` (default 900s) and inserts
//! any trade we do not hold yet, together with its `trade_assets` rows.
//!
//! Shape notes, derived from the live upstream API (2026-07):
//!   * the LIST endpoint returns `{ok, data: {data: [row...], count}}` where each
//!     row carries id/network/chain_id/signature/hashed_signature/checks/signer/
//!     type/expires_at/effective_since/created_at/contract — but NO assets;
//!   * the DETAIL endpoint (`GET .../v1/trades/{id}`, looked up by the LIST id)
//!     returns the camelCase trade with `sent`/`received` asset arrays
//!     (assetType/contractAddress/extra + amount|tokenId|itemId + beneficiary).
//!     Its inner `id` field does NOT always echo the list id, so the LIST id is
//!     the stable identity — it is also what the original snapshot stored.
//!
//! Trades are immutable once signed (the contract enforces expiry/owner/uses
//! lifecycle on-chain and mv_trades derives status locally), so the sync is
//! INSERT ... ON CONFLICT (hashed_signature) DO NOTHING — no updates ever.
...
/// A trade head row as parsed from the upstream LIST response.
#[derive(Debug, Clone)]
...
/// Sub-table routing for one asset, by the contract's asset type.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum AssetValue {
    /// asset_type 1 (ERC20) or 2 (USD-pegged MANA) -> trade_assets_erc20.amount
    Erc20 { amount: String },
    /// asset_type 3 (ERC721) -> trade_assets_erc721.token_id
    Erc721 { token_id: String },
    /// asset_type 4 (collection item) -> trade_assets_item.item_id
    Item { item_id: String },
}

/// One `marketplace.trade_assets` row parsed from the upstream DETAIL response.
#[derive(Debug, Clone, PartialEq, Eq)]
...
/// Parse one row of the upstream LIST response into the columns
/// `marketplace.trades` expects (0002_trades_schema.sql).
...
/// Normalize an upstream `extra` bytes field to the local storage convention:
/// the snapshot stores `''` for empty (never a bare `'0x'`) and keeps 0x-hex
/// payloads verbatim.
...
/// Parse the `sent`/`received` arrays of the upstream DETAIL response into
/// `marketplace.trade_assets` (+ per-type sub-table) rows.
...
async fn run_sweep(
    ...
    // Multi-MB body: buffer then parse once.
...
mod tests {
    ...
    fn list_row_maps_to_trades_columns() {
        ...
        // signature/hash/signer are stored lowercase like the snapshot rows
        ...
        // the checks jsonb is stored verbatim (same keys the snapshot holds)
        assert_eq!(head.checks["salt"], "0x988c1638e5");
        assert_eq!(head.checks["uses"], 1);
        // ISO timestamps parse to the exact millisecond
        ...
        // the trade contract keeps upstream casing (snapshot convention)
    ...
    fn asset_missing_its_typed_value_is_refused() {
        // an ERC721 asset without tokenId must not silently map
        let detail = serde_json::json!({
            "sent": [{"contractAddress": "0xaaa", "assetType": 3, "extra": ""}],
            "received": []
        });
        assert!(parse_detail_assets(&detail).is_err());
        // an ERC20 asset without amount must not silently map
        let detail = serde_json::json!({
            "sent": [],
