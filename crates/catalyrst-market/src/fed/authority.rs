...
pub async fn lookup_bid_item_id(
    pool: &PgPool,
    bid_sig_hash: &str,
) -> Result<Option<String>, ApiError> {
    let row: Option<(String,)> =
        sqlx::query_as("SELECT item_id FROM market_bids_local WHERE signature_hash = $1")
            .bind(bid_sig_hash)
            .fetch_optional(pool)
            .await
            .map_err(ApiError::from)?;
...
pub async fn lookup_bid_signer(
    pool: &PgPool,
    bid_sig_hash: &str,
) -> Result<Option<String>, ApiError> {
    let row: Option<(String,)> =
        sqlx::query_as("SELECT signer FROM market_bids_local WHERE signature_hash = $1")
            .bind(bid_sig_hash)
            .fetch_optional(pool)
            .await
            .map_err(ApiError::from)?;
...
pub async fn lookup_order_signer(
    pool: &PgPool,
    order_sig_hash: &str,
) -> Result<Option<String>, ApiError> {
    let row: Option<(String,)> =
        sqlx::query_as("SELECT signer FROM market_orders_local WHERE signature_hash = $1")
            .bind(order_sig_hash)
            .fetch_optional(pool)
            .await
            .map_err(ApiError::from)?;
...
pub async fn order_exists(pool: &PgPool, order_sig_hash: &str) -> Result<bool, ApiError> {
    let row: Option<(i32,)> =
        sqlx::query_as("SELECT 1 FROM market_orders_local WHERE signature_hash = $1")
            .bind(order_sig_hash)
            .fetch_optional(pool)
            .await
            .map_err(ApiError::from)?;
