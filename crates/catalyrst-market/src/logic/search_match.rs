...
//! similarity_threshold) to some word of the item's name or of its collection's name. A
//! substring over the whole name required the words in the typed order ("hat pirate" found
//! nothing named "pirate hat") and never saw the collection, which is where brand and
//! collaboration names live; whole-string similarity over `search_text` scored a short term
//! against the description too and made items with rich descriptions unsearchable by their
//! own name. Multi-word input compares each word against the WHOLE phrase, the rule
//! /v2/catalog has always applied, so every feed agrees on what a query matches. Upstream
//! serves the words from a pre-split table; splitting per row instead keeps the semantics and
//! needs no rebuild pipeline. Tags are not matched: no tag source is wired into this schema.
//!
//! LAND matches its `search_text` (coordinates, name, description) on the whole string OR on
//! its best run of words (`<%`): the whole-string measure is the forgiving one for multi-word
//! input, the word measure catches a term buried in a long description, and OR-ing them loses
//! nothing either side found.
...
/// `name_expr`/`collection_id_expr` are in the caller's own aliases; `term_param` is the
/// placeholder bound to the raw term.
...
/// `item_search_where` for feeds that also carry non-collection rows (LAND, estates, names).
/// The word arms are keyed on the row having an item, as upstream keys its word-table lookups
/// on the item id: a row with none matches only through the substring on `non_item_name_expr`
/// via `like_param`, a placeholder bound to `%escape_like(term)%`.
