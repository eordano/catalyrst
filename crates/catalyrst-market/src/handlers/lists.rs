...
use crate::auth_chain::{self, AuthChainError, AuthChainErrorExt, AUTH_CHAIN_HEADER_PREFIX};
...
/// Optional signed-fetch auth, mirroring upstream's `wellKnownComponents({
/// optional: true })` on this route: no auth-chain headers means an anonymous
/// caller (who can still read a public / ACL-granted list), while
/// present-but-invalid credentials are rejected outright.
fn optional_signer(
    headers: &HeaderMap,
    method: &str,
    path: &str,
...
pub async fn get_list_picks(
    ...
    let user_address = optional_signer(&headers, "get", &format!("/v1/lists/{id}/picks"))?;
