...
pub async fn create_order(
    ...
    match signer_has_active_lease_for_item(&state.pool, signer.as_str(), item_id).await {
        Ok(true) => {
            return err_json(
                StatusCode::FORBIDDEN,
    ...
    match signer_owns_any_nft_for_item(&state.pool, signer.as_str(), item_id).await {
        Ok(true) => {}
        Ok(false) => {
            return err_json(
                StatusCode::FORBIDDEN,
