...
/// than the seller, so primary mints count (see `TopCreator`). Ranking stays on
/// the windowed count; each row also carries the lifetime sales it displays and
/// its approved catalogue counts, and creators with almost nothing published
/// are floored out (see `build_top_creators_sql`). `first` (rows) and `days`
