...
impl<T: TypedMessage> Signed<T> {
    ...
    pub fn verify(&self, expected_signer: &str, now: i64) -> Result<(), FedError> {
        ...
        if skew > MAX_SKEW_PAST_SECS || skew < -MAX_SKEW_FUTURE_SECS {
            return Err(FedError::SkewedTimestamp { signed_at: self.signed_at, now, skew });
        }
...
fn decode_signature(sig: &str) -> Result<Vec<u8>, FedError> {
    ...
    if s.len() != 130 {
        return Err(FedError::InvalidSignature(format!("expected 132 hex chars (with 0x), got {}", sig.len())));
    }
