...
impl TypedMessage for SessionDelegation {
    ...
    fn encode_struct(&self) -> Vec<u8> {
        ...
        for s in &self.scope { out.push(*s as u8); }
...
pub fn check_delegation(delegation: &Signed<SessionDelegation>, required_scope: Scope, now: u64)
    -> Result<(), FedError>
{
    if delegation.message.expires_at <= now {
        return Err(FedError::SessionExpired { expires_at: delegation.message.expires_at, now });
    }
    if delegation.message.expires_at > delegation.message.signed_at.saturating_add(MAX_SESSION_LIFETIME_SECS) {
        return Err(FedError::Malformed("delegation lifetime exceeds 24h cap".into()));
    }
    if !delegation.message.scope.contains(&required_scope) {
        return Err(FedError::SessionScope {
            ...
            have: delegation.message.scope.iter().map(|s| s.as_str().to_string()).collect(),
        });
    }
    Ok(())
}
