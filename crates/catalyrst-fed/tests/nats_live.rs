//! Live NATS JetStream transport test. Exercises the real `NatsPublisher`
//! publish -> JetStream -> consume path end to end. Gated two ways: compiled
//! only under `--features nats` (the broker transport), and skipped at runtime
//! unless `FED_NATS_URL` points at a reachable broker — mirroring the env-gated
//! Postgres tests elsewhere in the workspace.
//!
//! Run with:  FED_NATS_URL=nats://127.0.0.1:4222
...
async fn nats_publish_then_consume_roundtrip() {
    ...
    // two peers on the same broker: A publishes, B consumes.
    ...
    // unique place id so reruns against a persistent stream stay distinguishable.
    ...
    // wait for the envelope to come back through JetStream.
