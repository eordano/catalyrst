...
fn check_admin(expected: Option<&str>, headers: &HeaderMap) -> Result<(), ApiError> {
    ...
    let token =
        bearer_token(headers).ok_or_else(|| ApiError::forbidden("missing or invalid bearer token"))?;
...
mod tests {
    ...
    fn missing_or_wrong_bearer_forbidden() {
        ...
        assert!(is_forbidden(check_admin(Some("secret"), &hm("Bearer wrong"))));
        assert!(is_forbidden(check_admin(Some("secret"), &hm("Basic secret"))));
    }
