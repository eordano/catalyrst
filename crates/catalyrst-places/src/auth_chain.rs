...
pub fn require_signer(
    ...
    validate_signature(&chain, &payload, &ts, FIVE_MINUTES, now).map(|a| a.to_lowercase())
}
...
mod tests {
    ...
    fn stale_timestamp_rejected_even_with_colon_in_path() {
        let chain = AuthChain { links: vec![], signer: String::new() };
        ...
        assert!(matches!(r, Err(AuthChainError::Expired { .. })), "stale ts must be Expired");
    }
}
