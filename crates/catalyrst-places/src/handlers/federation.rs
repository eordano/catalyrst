...
use crate::auth::{
    auth_address_verified, require_admin_bearer, require_admin_token_for_highlighted,
    require_bearer_token, require_ranking_token,
};
...
pub async fn put_place_ranking(
    ...
    let mut place = fetch_place(&state, &place_id).await?;
    require_admin_token_for_highlighted(
        &headers,
        state.admin_auth_token.as_deref(),
        place.highlighted,
    )?;
...
mod tests {
    ...
    use crate::auth::{
        require_admin_token_for_highlighted, require_ranking_token,
    ...
    fn data_team_token_cannot_move_a_highlighted_ranking() {
        let headers = bearer("data-team");
        assert!(require_ranking_token(&headers, Some("data-team"), Some("admin")).is_ok());
        let err = require_admin_token_for_highlighted(&headers, Some("admin"), true).unwrap_err();
    ...
    fn data_team_token_still_moves_an_uncurated_ranking() {
        let headers = bearer("data-team");
        assert!(require_admin_token_for_highlighted(&headers, Some("admin"), false).is_ok());
        assert!(require_admin_token_for_highlighted(&headers, None, false).is_ok());
    }

    #[test]
    fn admin_token_moves_a_highlighted_ranking() {
        let headers = bearer("admin");
        assert!(require_admin_token_for_highlighted(&headers, Some("admin"), true).is_ok());
    }

    #[test]
    fn unconfigured_admin_token_refuses_every_highlighted_write() {
        let err = require_admin_token_for_highlighted(&bearer("admin"), None, true).unwrap_err();
        assert_eq!(forbidden_message(err), HIGHLIGHTED_RANKING_IS_EDITORIAL);
        let err = require_admin_token_for_highlighted(&HeaderMap::new(), Some("admin"), true)
            .unwrap_err();
        assert_eq!(forbidden_message(err), HIGHLIGHTED_RANKING_IS_EDITORIAL);
    }
