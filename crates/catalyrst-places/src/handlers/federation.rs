...
async fn preflight<T: TypedMessage + DeserializeOwned>(
    ...
    let signed: Signed<T> = serde_json::from_value(raw)
        .map_err(|e| ApiError::bad_request(format!("invalid Signed<{}>: {}", T::PRIMARY_TYPE, e)))?;
    ...
    if let Some(addr) = crate::auth::auth_address_optional(headers) {
        if !addr.eq_ignore_ascii_case(&signer) {
            return Err(ApiError::unauthorized("auth-chain signer != envelope signer"));
        }
    }
    ...
    replay::check_and_record(state.places.writer_pool(), &signer, &signed.nonce, signed.signed_at)
        .await
...
async fn fed_patch_favorites(
    ...
    if signed.message.place_id != entity_id {
        return Err(ApiError::bad_request("place_id in body does not match path"));
    }
...
pub async fn patch_place_likes(
    ...
    let user = auth_address_verified(&headers, method.as_str(), uri.path())?;
    let like_req = body_like(&body)
        .ok_or_else(|| ApiError::bad_request("Invalid likes body. Expected { like: boolean|null }."))?;
...
pub async fn patch_world_favorites(
    ...
    patch_place_favorites(state, method, uri, headers, path, body).await
}
...
pub async fn patch_world_likes(
    ...
    patch_place_likes(state, method, uri, headers, path, body).await
}
...
fn require_admin(
    ...
    if state.admin_addresses.contains(&user) {
        ...
        Err(ApiError::forbidden(format!("Only admin allowed to update {action}")))
    }
}
...
fn body_ranking(body: &Option<Json<Value>>) -> Result<Option<f64>, ApiError> {
    let v = body
        ...
        .ok_or_else(|| ApiError::bad_request("Invalid ranking body. Expected { ranking: number|null }."))?;
    if v.is_null() {
        Ok(None)
    } else {
        v.as_f64()
...
pub async fn put_place_highlight(
    ...
    let highlighted = body
        ...
        .and_then(|v| v.as_bool())
        .ok_or_else(|| ApiError::bad_request("Invalid highlight body. Expected { highlighted: boolean }."))?;
    let mut place = fetch_place(&state, &place_id).await?;
