...
pub async fn inject_place_metadata(
    ...
    if let Some(place) = place {
        ...
        let title = format!("{} | Decentraland Place", place.title.clone().unwrap_or_default());
        let html = render(
            &title,
...
pub async fn inject_world_metadata(
    ...
    let world = if !world_id.is_empty() {
        state.places.find_world_by_id(&world_id).await.ok().flatten()
    } else {
        None
    };
    ...
    if let Some(world) = world {
        ...
        let title = format!("{} | Decentraland Place", world.title.clone().unwrap_or_default());
        let html = render(
            &title,
