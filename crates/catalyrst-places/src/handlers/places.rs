...
/// Upstream gates the `realms_detail` enrichment on the `with_realms_detail`
/// query flag (truthy "true"/"1"). Shared by the place GET, list, and map paths.
...
mod tests {
    ...
    fn with_realms_detail_is_truthy_only() {
        assert!(with_realms_detail(&pairs(&[("with_realms_detail", "true")])));
        assert!(with_realms_detail(&pairs(&[("with_realms_detail", "1")])));
        assert!(!with_realms_detail(&pairs(&[("with_realms_detail", "false")])));
