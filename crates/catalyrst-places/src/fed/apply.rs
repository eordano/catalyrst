...
pub async fn apply_vote(
    ...
    // Capture the signer's Snapshot voting power so the Wilson ranking weights
    // federated votes identically to local ones (port of updateLike's
    // `fetchScore`). Only fetched when storing a vote; a withdraw deletes the row.
