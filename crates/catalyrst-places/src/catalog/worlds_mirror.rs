...
mod tests {
    ...
    #[test]
    fn an_upstream_ranking_exclusion_rides_in_raw() {
        let (_, raw) = extract_fields(&json!({
            "id": "a.dcl.eth",
            "world_name": "a.dcl.eth",
            "exclude_from_ranking": true
        }))
        .unwrap();
        assert_eq!(raw["exclude_from_ranking"], json!(true));
        let (_, raw) =
            extract_fields(&json!({"id": "a.dcl.eth", "world_name": "a.dcl.eth"})).unwrap();
        assert_eq!(raw.get("exclude_from_ranking"), None);
    }
