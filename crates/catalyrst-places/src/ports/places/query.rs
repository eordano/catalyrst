...
// boolean, not just that row. Mirrored as a literal by PLACE_COLUMNS in
// rows.rs and by OVERLAPPING_PLACES in catalog/sync.rs, neither of which can
// interpolate a const; change the three together.
pub(super) const EXCLUDE_FROM_RANKING_SQL: &str =
...
pub(super) fn build_order_by(
    ...
    format!(
        "{highlighted_prefix}{live_prefix}{ranking_prefix}{rank_prefix}{order_column} {dir} NULLS LAST, deployed_at DESC, id ASC"
    )
}
