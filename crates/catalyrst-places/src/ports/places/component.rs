...
impl PlacesComponent {
    ...
    // to be curated -- a row this leg cannot write at all was already a silent
    // no-op before the condition existed.
    pub async fn set_ranking_from_score(
        ...
        let curated = sqlx::query(sqlx::AssertSqlSafe(probe))
            .bind(entity_id)
            .fetch_optional(writer)
            .await?
            ...
            .unwrap_or(false);
        if curated {
            Ok(ScoreRankingOutcome::RefusedCurated)
        } else {
            Ok(ScoreRankingOutcome::Stored)
        }
    }
