...
// places and the editorial admin ordering the highlighted shelf. While an
// entity is highlighted its ranking is editorial, so only the admin token may
// move it; an unconfigured admin token refuses every highlighted write.
pub fn require_admin_token_for_highlighted(
    headers: &HeaderMap,
    admin: Option<&str>,
    highlighted: bool,
) -> Result<(), crate::http::errors::ApiError> {
    ...
    secret_matches(ADMIN_TOKEN_ENV, admin, bearer_token(headers).as_deref())
        .map_err(|_| crate::http::errors::ApiError::forbidden(HIGHLIGHTED_RANKING_IS_EDITORIAL))
}
