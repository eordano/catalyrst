//! DB-backed coverage for the places federation reconciliation pull endpoints
//! (`/federation/places/{snapshot,changes}`).
//!
//! Gated on a reachable Postgres (`CATALYRST_PLACES_TEST_PG`, default
//! `postgres://postgres:postgres@127.0.0.1:5432/places`). Each test runs in its
//! own throwaway schema and skips cleanly when no Postgres is reachable, so the
//! suite is green on a DB-less CI box. Pure tests live inline in the module.
...
use catalyrst_places::clients::{CommsGatekeeper, Events};
...
/// `None` when no Postgres is reachable, so callers skip gracefully.
...
/// A component whose writer points at the temp schema; reader pool is the same
/// (the reconciliation reads only touch the local writer tables).
...
async fn changes_pages_by_seq_and_clamps_limit() {
    ...
    // Five actions => seq 1..=5 (bigserial, monotonic per-table).
    for i in 0..5 {
        seed_action(
            &places,
            &format!("{:064x}", i),
    ...
    // since=0, limit=2 returns the first two in seq order.
    ...
    // Paging from the high-water seq returns the remaining rows, no overlap.
    ...
    // since past the end yields an empty page.
...
async fn snapshot_shape_is_deterministic() {
    ...
    seed_action(&places, &format!("{:064x}", 1), "place-a", "favorite", "0xa").await;
    ...
    // Identical state => identical hash (order-independent, total ordering on
    // signature_hash). A re-read must not perturb the summary.
    let snap2 = snapshot_view(pool).await.unwrap();
    assert_eq!(snap["log_hash"], snap2["log_hash"]);

    // Mutating the log changes the deterministic hash.
    seed_action(&places, &format!("{:064x}", 4), "place-c", "vote_down", "0xd").await;
...
async fn endpoints_reachable_without_auth() {
    ...
    // No Authorization header on either request: both must 200.
    for path in [
        ...
        assert_eq!(resp.status(), StatusCode::OK, "{path} should 200 without auth");
    }
