...
async fn every_spec_route_answers_its_contract() {
    ...
    let place_exclusion_path = format!("/api/places/{}/ranking-exclusion", PLACE_ID);
    let excluded = gate
        .hit(
            &app,
            Case::new("put", "/api/places/{place_id}/ranking-exclusion")
                .path(&place_exclusion_path)
                .bearer(ADMIN_TOKEN),
        )
        .await;
    assert_eq!(
        (
            excluded["data"]["exclude_from_ranking"].clone(),
            excluded["data"]["ranking"].clone(),
        ),
        (json!(true), json!(0.0)),
        "excluding a place answers with the flag set and the score's last word cleared: {excluded}"
    );
    gate.hit(
        &app,
        Case::new("put", "/api/places/{place_id}/ranking-exclusion")
    ...
    let included = gate
        .hit(
            &app,
            Case::new("delete", "/api/places/{place_id}/ranking-exclusion")
                .path(&place_exclusion_path)
                .bearer(ADMIN_TOKEN),
        )
        .await;
    assert_eq!(
        included["data"]["exclude_from_ranking"],
        json!(false),
        "clearing the flag answers with it cleared: {included}"
    );
    gate.hit(
        &app,
        Case::new("delete", "/api/places/{place_id}/ranking-exclusion")
    ...
    let world_exclusion_path = format!("/api/worlds/{}/ranking-exclusion", WORLD_NAME);
    let wexcluded = gate
        .hit(
            &app,
            Case::new("put", "/api/worlds/{world_id}/ranking-exclusion")
                .path(&world_exclusion_path)
                .bearer(ADMIN_TOKEN),
        )
        .await;
    assert_eq!(
        (
            wexcluded["data"]["exclude_from_ranking"].clone(),
            wexcluded["data"]["ranking"].clone(),
        ),
        (json!(true), json!(0.0)),
        "excluding a world answers with the flag set and the ranking cleared: {wexcluded}"
    );
    gate.hit(
        &app,
        Case::new("put", "/api/worlds/{world_id}/ranking-exclusion")
    ...
    gate.hit(
        &app,
        Case::new("delete", "/api/worlds/{world_id}/ranking-exclusion")
            .path(&world_exclusion_path)
            .bearer(ADMIN_TOKEN),
    )
    .await;
