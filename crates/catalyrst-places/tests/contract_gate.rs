...
async fn every_spec_route_answers_its_contract() {
    ...
    let Some(scratch) = ScratchSchema::create("CATALYRST_PLACES_TEST_PG", "cg_places").await else {
        eprintln!("skipping places contract gate: set CATALYRST_PLACES_TEST_PG to run");
        return;
    };
    scratch
    ...
    gate.hit(
        ...
        Case::new("put", "/api/report/upload/{filename}")
            .path(&format!("/api/report/upload/{}", filename))
    ...
    gate.waive_error(
        ...
        "handler is tolerant by design: no auth gate and unknown filenames are accepted",
    );
