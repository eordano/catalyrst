...
mod tests {
    ...
    fn oracle_auth_chain_verification() {
        ...
        for (i, v) in vectors.iter().enumerate() {
            ...
            let result = catalyrst_crypto::verify::verify_auth_chain(
                &chain,
                &v.entity_id,
                Some(now_ms),
            );
        ...
        assert!(verified > 0, "All auth chains were skipped — no ECDSA chains found");
    }
    ...
    fn oracle_deployment_content_files() {
        ...
        for (i, v) in vectors.iter().enumerate() {
            ...
            for cf in &v.content_files {
                ...
                let gzip_path =
                    PathBuf::from(format!("{}.gzip", file_path.display()));
