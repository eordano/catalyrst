...
pub struct Config {
    ...
    /// Public, client-facing base for profile snapshot URLs (face/body/thumbnail)
    /// emitted in /profiles responses — the SAME public CDN base the content core
    /// uses (`PROFILE_CDN_BASE_URL`), so one value configures both services. MUST be
    /// publicly reachable (a gateway base that proxies to the profile-images
    /// service), NOT that service's loopback, or clients can't fetch the pictures.
    /// `PROFILE_IMAGES_URL` is the legacy ab-registry name; default is the public host.
...
impl Config {
    pub fn from_env() -> Result<Self> {
        Ok(Self {
            ...
            profile_images_url: env::var("PROFILE_CDN_BASE_URL")
                .or_else(|_| env::var("PROFILE_IMAGES_URL"))
