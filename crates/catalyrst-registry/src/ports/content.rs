...
impl ContentComponent {
    ...
    pub async fn resolve_one(&self, id: &str) -> Result<Option<ActiveEntity>, sqlx::Error> {
        let mut all = self
    ...
    pub async fn active_entity_ids_of_types(
        ...
        // One ORDER BY + LIMIT subquery per type, UNION ALL'd together. Each
        // branch is served by deployments_type_entity_ts_idx (entity_type,
        // entity_timestamp DESC) and Postgres streams the already-sorted
        // branches through a MergeAppend. The flat `entity_type = ANY($1)`
        // form instead walked the timestamp-only index and discarded every
        // row of the wrong type (~360k rows removed by filter -> 258ms); this
        // touches only ~`limit` rows per type (~5ms warm). Result is identical:
        // the global newest-`limit` set is a subset of the union of each type's
        // newest-`limit` rows, since every per-type LIMIT equals the outer one.
