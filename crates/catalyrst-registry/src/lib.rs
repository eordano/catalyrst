...
pub async fn build_state(cfg: &Config) -> Result<AppState> {
    ...
    // The profile-images base is emitted to clients (face/body/thumbnail URLs); a
    // loopback base means clients can't fetch the pictures. Warn loudly so the
    // misconfig is caught at boot — set PROFILE_IMAGES_PUBLIC_URL to the public base.
...
pub fn api_router() -> Router<AppState> {
    index_router().merge(extra_router())
}

/// The AB-availability index routes. The unified content/AB server now owns these
/// (servability-derived + JIT-aware, reading the corpus it serves), so it does NOT
/// mount these from here — it provides its own. Kept here for the standalone
/// binary until it is retired.
...
/// Every registry route EXCEPT the AB-availability index (active/versions). The
/// unified server mounts this (via its own AppState) to subsume the rest of the
/// registry — profiles, world manifest, status, queues, denylist, admin — so the
/// standalone ab-registry service can be fully retired.
pub fn extra_router() -> Router<AppState> {
    Router::new()
        .route(
