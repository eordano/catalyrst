...
use crate::{AuthChain, ContentStorage, DeploymentContext, Deployer, SyncError};
...
async fn download_file_with_retries(
    ...
    Err(last_error.unwrap_or_else(|| {
        SyncError::Other(format!("Failed to download {} after retries", hash))
    }))
}
