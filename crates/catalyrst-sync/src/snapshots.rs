...
use crate::{
    ContentStorage, ProcessedSnapshotStore, SnapshotMetadata, SnapshotStorageCheck,
...
pub async fn fetch_snapshots(
            ...
            Ok(resp) => {
                if resp.status().is_success() {
                    ...
                    last_error = Some(SyncError::Other(format!(
                        "HTTP {} fetching snapshots from {}", status, server
                    )));
                }
            }
            Err(e) => {
    ...
    Err(last_error.unwrap_or_else(|| {
        SyncError::Other(format!("Failed to fetch snapshots from {} after {} retries", server, max_retries))
    }))
}
    ...
    for (hash, servers) in snapshots {
        ...
        tasks.spawn(async move {
            if let Err(e) = download_snapshot_file(
                &client, storage.as_ref(), &hash, &servers, max_retries, retry_wait_ms,
            ).await {
...
pub async fn should_deploy_snapshot(
    ...
    let a_replaced_group_was_processed = replaced_snapshot_hash_groups.iter().any(|group| {
        !group.is_empty() && group.iter().all(|h| processed.contains(h))
    });
    ...
    D: crate::batch_deployer::DeploymentScheduler,
{
    ...
    download_snapshot_file(client, storage, snapshot_hash, servers, max_retries, retry_wait_ms).await?;
...
async fn download_snapshot_file(
        ...
        match client.get(&url).send().await {
            Ok(resp) => {
                if resp.status().is_success() {
                    ...
                    last_error = Some(SyncError::Other(format!(
                        "HTTP {} downloading snapshot {} from {}", status, snapshot_hash, server
                    )));
                }
            }
            Err(e) => {
    ...
    Err(last_error.unwrap_or_else(|| {
        SyncError::Other(format!("Failed to download snapshot {} after {} retries", snapshot_hash, max_retries))
    }))
}
