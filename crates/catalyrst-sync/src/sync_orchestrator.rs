...
use crate::{
    ContentStorage, DeploymentRepository, ProcessedSnapshotStore,
    SnapshotStorageCheck, SyncError, SyncState, TimeRange, Timestamp,
};
use crate::batch_deployer::{BatchDeployer, DeploymentScheduler};
use crate::pointer_changes::{self, PointerChangesOptions};
use crate::snapshots;
...
pub struct SyncOrchestrator {
    ...
    processed_store: Arc<dyn ProcessedSnapshotStore>,
    snapshot_store: Arc<dyn SnapshotStorageCheck>,
...
impl SyncOrchestratorRefs {
    ...
    async fn run_phased_bootstrap(&self) -> Result<(), SyncError> {
        ...
        let non_profile_filter: HashSet<String> =
            crate::NON_PROFILE_TYPES.iter().map(|s| s.to_string()).collect();
        let profile_filter: HashSet<String> =
            ["profile".to_string()].into_iter().collect();

        info!(types = ?non_profile_filter, "Phase 1: Bootstrap non-profile entities from snapshots");
        self.bootstrap_from_snapshots(Some(&non_profile_filter), false).await?;

        info!("Phase 2: Non-profile pointer-changes catch-up");
        self.bootstrap_from_pointer_changes(Some(&non_profile_filter)).await?;
        self.save_frontier().await;
        ...
        self.bootstrap_from_snapshots(Some(&profile_filter), true).await?;

        info!("Phase 5: Profile pointer-changes catch-up");
        self.bootstrap_from_pointer_changes(Some(&profile_filter)).await?;
        self.save_frontier().await;
    ...
    async fn save_frontier(&self) {
        let ts = {
            ...
            servers.values().map(|s| s.last_snapshot_timestamp).max().unwrap_or(0)
        };
        if ts > 0 {
    ...
    async fn bootstrap_from_snapshots(
        ...
        let mut snapshots_by_hash: HashMap<String, (crate::SnapshotMetadata, HashSet<String>)> =
            HashMap::new();
        ...
        if !snapshots_to_process.is_empty() {
            ...
            for item in &snapshots_to_process {
                if !self.storage.exists(&item.0).await.unwrap_or(true) {
                    need_download.push(item.clone());
                }
            }
            if !need_download.is_empty() {
                info!(count = need_download.len(), "Pre-downloading snapshot files in parallel");
