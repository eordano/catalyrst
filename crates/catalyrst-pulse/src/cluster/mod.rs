...
/// Cadence of the clustering pass. It is not a bound on broker delivery or client convergence.
...
struct PeerClusterState {
    ...
    published_owner: Option<(String, String, u64)>,
...
impl ClusterTracker {
    pub fn new(
        ...
        Self {
            ...
            peer_states: vec![
                PeerClusterState::default();
                max_peers + control::DEFAULT_CONTROL_POSITION_CAPACITY
            ],
            cluster_records: HashMap::new(),
            assignment_by_wallet: HashMap::new(),
            control: control::ControlSnapshot::default(),
            actual_peer_capacity: max_peers,
            pass_number: 0,
    ...
    pub fn run_pass(
        ...
        self.pass_number += 1;
        self.control = self.publisher.control_snapshot();
    ...
    fn collect_and_union_realms(
        ...
        let mut controls: BTreeMap<String, BTreeMap<i64, Vec<control::ControlMember>>> =
            BTreeMap::new();
        let native_winners = self.native_control_winners(grids, board, identity);
        for member in self.control.members.iter().cloned() {
            if native_winners.contains(&member.wallet) {
                continue;
            }
            controls
                .entry(member.realm.clone())
                .or_default()
                .entry(grids.cell_key(member.position.x, member.position.z))
                .or_default()
                .push(member);
        }
        let grids_by_realm: HashMap<&str, &SpatialGrid> = grids.realm_grids().collect();
        let mut realms: BTreeSet<&str> = grids_by_realm.keys().copied().collect();
        realms.extend(controls.keys().map(String::as_str));
        ...
        for realm in realms {
            ...
            self.collect_realm_nodes(
                realm_index,
                grids_by_realm.get(realm).copied(),
                controls.get(realm),
                board,
                identity,
            );
    ...
    fn native_control_winners(
        ...
        self.control
            ...
            .iter()
            .filter_map(|member| {
                let peer = identity.peer_by_wallet(&member.wallet)?;
                let session = identity.session_by_peer(peer).unwrap_or(&member.wallet);
                (session.eq_ignore_ascii_case(&member.session)
                    && board.try_read(peer).is_some()
                    && grids.realm_of(peer).is_some())
                .then(|| member.wallet.clone())
            })
            .collect()
    }
    ...
    fn collect_realm_nodes(
        ...
        controls: Option<&BTreeMap<i64, Vec<control::ControlMember>>>,
        board: &SnapshotBoard,
        identity: &IdentityBoard,
    ) {
        ...
        if let Some(controls) = controls {
            self.pass_cell_keys.extend(controls.keys().copied());
        }
        self.pass_cell_keys.sort_unstable();
        self.pass_cell_keys.dedup();
        ...
        for index in 0..self.pass_cell_keys.len() {
            ...
            if let Some(members) = controls.and_then(|controls| controls.get(&key)) {
                for member in members {
                    self.try_collect_control_member(member);
                }
            }
    ...
    fn try_collect_member(&mut self, peer: u32, board: &SnapshotBoard, identity: &IdentityBoard) {
        ...
        if self
            ...
            .is_some_and(|owner| owner.closed || !owner.session.eq_ignore_ascii_case(&session))
        {
            return;
        }
    ...
    fn try_collect_control_member(&mut self, member: &control::ControlMember) {
        let peer = self.actual_peer_capacity + member.slot as usize;
        if peer >= self.peer_states.len()
            || self.peer_states[peer].last_seen_pass == self.pass_number
        {
            return;
        }
        self.peer_states[peer].last_seen_pass = self.pass_number;
        if let Some(seen) = self.assignment_by_wallet.get_mut(&member.wallet) {
            seen.last_seen_pass = self.pass_number;
        }
        self.members.push(PassMember {
            peer: peer as u32,
    ...
    fn union_realm_neighbors(&mut self, first_node: usize) {
        for node in first_node..self.nodes.len() {
            ...
            for i in 0..NEIGHBOR_DX.len() {
                let Some(neighbor_x) = x.checked_add(NEIGHBOR_DX[i]) else {
                    continue;
                };
                let Some(neighbor_z) = z.checked_add(NEIGHBOR_DZ[i]) else {
                    continue;
                };
                let key = SpatialGrid::pack_key(neighbor_x, neighbor_z);
    ...
    /// Emits a feed event for one peer if its assignment -- owner, cluster and realm -- differs
    /// from the last one published, and either the change is exempt from the debounce or the peer
    /// has dwelled long enough. The debounce is bypassed on first assignment, owner change, teleport, realm
    ...
    fn try_publish_assignment(&mut self, member: usize, cluster_id: &str, realm: &str) -> bool {
        ...
        let owner_changed = !self.peer_states[peer].published_owner.as_ref().is_some_and(
            |(wallet, session, epoch)| {
                wallet.eq_ignore_ascii_case(&self.members[member].wallet)
                    && session.eq_ignore_ascii_case(&self.members[member].session)
                    && *epoch == self.members[member].owner_epoch
            },
        );
        ...
        let immediate = owner_changed
            || previously_published.is_none()
        ...
        self.peer_states[peer].published_owner = Some((
            self.members[member].wallet.clone(),
            self.members[member].session.clone(),
            self.members[member].owner_epoch,
        ));
        self.peer_states[peer].candidate_cluster_id = None;
        self.peer_states[peer].candidate_streak = 0;
    ...
    fn refresh_assignment_view(&self) {
        let assignments = self
            .members
            .iter()
            .filter_map(|member| {
                let state = &self.peer_states[member.peer as usize];
                let (wallet, session, epoch) = state.published_owner.as_ref()?;
                if !wallet.eq_ignore_ascii_case(&member.wallet)
                    || !session.eq_ignore_ascii_case(&member.session)
                    || *epoch != member.owner_epoch
                {
                    return None;
                }
                Some(feed::CurrentAssignment {
                    wallet: wallet.clone(),
                    session: session.clone(),
                    cluster_id: state.published_cluster_id.clone()?,
                    realm: state.published_realm.clone()?,
                })
            })
            .collect();
        self.publisher.replace_assignments(
            self.pass_number,
            std::time::Duration::from_millis(
                self.options.pass_interval_ms.saturating_mul(2).max(1000),
            ),
            assignments,
        );
    }
