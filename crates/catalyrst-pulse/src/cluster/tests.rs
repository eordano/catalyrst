...
use super::*;
use crate::cluster::control::{ControlMember, ControlOwner, ControlSnapshot};
...
fn control_owner(session: &str, closed: bool) -> ControlOwner {
    ControlOwner {
        session: session.to_string(),
        closed,
    }
...
#[test]
fn a_control_only_member_gets_a_real_cluster_and_current_assignment() {
    let mut w = World::new();
    w.feed.set_control_snapshot(ControlSnapshot {
        members: vec![controlled(
            0,
            "0xa",
            "0xaa",
            7,
        ...
        owners: HashMap::from([("0xa".into(), control_owner("0xaa", false))]),
    });
    w.pass();

    assert_eq!(w.cluster_count(), 1);
    assert_eq!(w.tracker.board().clusters[0].realm, "control-realm");
    assert_eq!(w.tracker.board().clusters[0].count, 1);
    let current = w.feed.current("0xa", "0xaa").unwrap();
    assert_eq!(current.realm, "control-realm");
    assert_eq!(w.feed.changes().len(), 1);
}

#[test]
fn control_and_native_members_share_the_existing_realm_cell_cluster() {
    let mut w = World::new();
    w.place_session(0, "0xa", "0xaa", "main", v3(1.0, 1.0), false);
    w.feed.set_control_snapshot(ControlSnapshot {
        members: vec![controlled(0, "0xb", "0xbb", 3, "main", v3(2.0, 2.0))],
        owners: HashMap::from([("0xb".into(), control_owner("0xbb", false))]),
    });
    w.pass();
    ...
    assert_eq!(
        w.feed.current("0xa", "0xaa").unwrap().cluster_id,
        w.feed.current("0xb", "0xbb").unwrap().cluster_id
    );
}

#[test]
fn native_position_wins_for_the_exact_control_session_without_double_counting() {
    let mut w = World::new();
    w.place_session(0, "0xa", "0xaa", "native", v3(1.0, 1.0), false);
    w.feed.set_control_snapshot(ControlSnapshot {
        members: vec![controlled(0, "0xa", "0xaa", 9, "control", v3(900.0, 900.0))],
        owners: HashMap::from([("0xa".into(), control_owner("0xaa", false))]),
    });
    w.pass();

    assert_eq!(w.cluster_count(), 1);
    assert_eq!(w.tracker.board().clusters[0].realm, "native");
    assert_eq!(w.tracker.board().clusters[0].count, 1);
    assert_eq!(w.tracker.board().peers[0].peer, 0);
}

#[test]
fn newer_control_owner_and_its_tombstone_suppress_a_stale_native_session() {
    let mut w = World::new();
    w.place_session(0, "0xa", "0xold", "main", v3(1.0, 1.0), false);
    w.feed.set_control_snapshot(ControlSnapshot {
        members: vec![controlled(0, "0xa", "0xnew", 10, "main", v3(2.0, 2.0))],
        owners: HashMap::from([("0xa".into(), control_owner("0xnew", false))]),
    });
    w.pass();
    assert_eq!(w.tracker.board().clusters[0].count, 1);
    assert!(w.tracker.board().peers[0].peer as usize >= CAP);
    assert!(w.feed.current("0xa", "0xnew").is_some());
    assert!(w.feed.current("0xa", "0xold").is_none());

    w.feed.set_control_snapshot(ControlSnapshot {
        members: Vec::new(),
        owners: HashMap::from([("0xa".into(), control_owner("0xnew", true))]),
    });
    w.pass();
    assert!(w.tracker.board().clusters.is_empty());
    assert!(w.feed.current("0xa", "0xold").is_none());

    w.place_session(0, "0xa", "0xnew", "main", v3(1.0, 1.0), false);
    w.pass();
    assert!(w.tracker.board().clusters.is_empty());
}

#[test]
fn a_lapsed_control_heartbeat_allows_the_exact_native_session_only() {
    let mut w = World::new();
    w.place_session(0, "0xa", "0xaa", "main", v3(1.0, 1.0), false);
    w.feed.set_control_snapshot(ControlSnapshot {
        members: Vec::new(),
    ...
    w.pass();
    assert_eq!(w.tracker.board().clusters.len(), 1);
    assert_eq!(w.tracker.board().peers[0].peer, 0);
}

#[test]
fn a_new_durable_control_epoch_republishes_an_unchanged_same_session_assignment() {
    let mut w = World::new();
    let snapshot = |epoch| ControlSnapshot {
        members: vec![controlled(0, "0xa", "0xaa", epoch, "main", v3(1.0, 1.0))],
        owners: HashMap::from([("0xa".into(), control_owner("0xaa", false))]),
    };
    w.feed.set_control_snapshot(snapshot(1));
    w.pass();
    w.feed.set_control_snapshot(snapshot(2));
    w.pass();

    assert_eq!(w.feed.changes().len(), 2);
}
...
#[test]
fn extreme_finite_control_coordinates_do_not_overflow_neighbor_traversal() {
    let mut w = World::new();
    w.feed.set_control_snapshot(ControlSnapshot {
        members: vec![controlled(
            0,
...
fn an_assignment_is_published_once_and_not_republished_while_it_holds() {
    ...
    let current = w.feed.current("0xa", "0xa").unwrap();
    assert_eq!(current.pass, 3);
    assert_eq!(current.cluster_id, w.cluster_of(0).unwrap());
}
...
fn a_move_between_clusters_waits_out_the_dwell_before_publishing() {
    ...
    let old = w.feed.current("0xb", "0xb").unwrap().cluster_id;
    w.feed.clear();

    w.place(1, "0xb", "r", v3(cell * 8.7, 1.0));
    w.pass();
    assert!(w.feed.changes().is_empty(), "pass 1 of the dwell is silent");
    assert_ne!(w.cluster_of(1).unwrap(), old);
    assert_eq!(
        w.feed.current("0xb", "0xb").unwrap().cluster_id,
        old,
        "lookup must not expose a pre-debounce candidate"
    );
    w.pass();
    ...
    assert_eq!(
        w.feed.current("0xb", "0xb").unwrap().cluster_id,
        w.cluster_of(1).unwrap()
    );
}

#[test]
fn a_departed_session_is_absent_from_the_current_view_despite_takeover_retention() {
    let mut w = World::new();
    w.place_session(0, "0xa", "0xs1", "r", v3(1.0, 1.0), false);
    w.pass();
    assert!(w.feed.current("0xa", "0xs1").is_some());
    assert!(w.feed.current("0xa", "0xother").is_none());
    w.remove(0);
    w.pass();
    assert!(w.tracker.assignment_by_wallet.contains_key("0xa"));
    assert!(w.feed.current("0xa", "0xs1").is_none());
}

#[test]
fn slot_reuse_between_passes_publishes_for_the_new_owner_even_in_the_same_cluster() {
    for (wallet, session) in [("0xa", "0xs2"), ("0xb", "0xs1")] {
        let mut w = World::new();
        w.place_session(0, "0xa", "0xs1", "r", v3(1.0, 1.0), false);
        w.pass();
        let old_cluster = w.cluster_of(0).unwrap();
        w.feed.clear();
        w.remove(0);
        w.place_session(0, wallet, session, "r", v3(1.0, 1.0), false);
        w.pass();
        assert_eq!(w.cluster_of(0).as_deref(), Some(old_cluster.as_str()));
        let changes = w.feed.changes();
        assert_eq!(
            changes.len(),
            1,
            "the recycled slot must not inherit the prior owner's silence"
        );
        assert_eq!(changes[0].wallet, wallet);
        assert_eq!(changes[0].session.session, session);
        assert!(w.feed.current("0xa", "0xs1").is_none());
        assert_eq!(
            w.feed.current(wallet, session).unwrap().cluster_id,
            old_cluster
        );
        if wallet == "0xa" {
            assert_eq!(
                changes[0].session.displaced_session.as_deref(),
                Some("0xs1")
            );
        } else {
            assert!(!changes[0].session.is_takeover());
        }
    }
}
