//! The Pulse server: drives the ENet host, routes the real protobuf
//! `ClientMessage`/`ServerMessage` envelopes the Unity client speaks, and runs the per-tick
//! simulation that fans interest-managed state out to observers.
//!
//! Wire: each ENet packet body is a prost-encoded
//! [`ClientMessage`](crate::decentraland::pulse::ClientMessage) (C→S) or
//! [`ServerMessage`](crate::decentraland::pulse::ServerMessage) (S→C) — a proto3 `oneof`,
//! byte-identical to upstream's `Google.Protobuf`. Channel 0 is reliable
//! (handshake, teleport, resync, joins/leaves, full state); channel 1 carries
//! unreliable-sequenced movement/state deltas; channel 2 is unreliable-unsequenced.
//!
//! Pipeline (ports `Messaging/*Handler.cs` + `Peers/Simulation/PeerSimulation.cs`):
//!   - handshake (signed DCL auth chain) authenticates a peer ([`crate::handshake`]);
//!   - movement/teleport/emote/profile inputs *publish snapshots* into the
//!     [`SnapshotBoard`](crate::snapshot::SnapshotBoard) (per-peer sequence numbers + server
//!     ticks, not hardcoded zero);
//!   - `ResyncRequest` records the client's known seq so the next tick replays state;
//!   - every [`tick_ms`](PulseServer::tick) the [`PeerSimulation`] iterates observers, queries
//!     the spatial area of interest, and sends `PlayerJoined` (full state on first visibility)
//!     then `PlayerStateDelta` / discrete events afterwards, recovering out-of-sync clients via
//!     targeted-delta-or-full resync.
...
/// ENet channel assignments (mirrors `PacketMode` / `ENetChannel`).
pub mod channel {
    /// Reliable ordered: handshake, teleport, resync, joins/leaves, full state.
    pub const RELIABLE: u8 = 0;
    /// Unreliable sequenced: high-frequency movement input + state deltas.
    pub const UNRELIABLE_SEQUENCED: u8 = 1;
    /// Unreliable unsequenced: low-frequency emotes / profile announcements.
    pub const UNRELIABLE_UNSEQUENCED: u8 = 2;
}

/// Default simulation tier steps in ms (`PeerOptions.SimulationSteps`): TIER_0 every tick,
/// TIER_1 every 2nd, TIER_2 every 4th.
pub const DEFAULT_SIMULATION_STEPS: [u32; 3] = [50, 100, 200];
/// Default per-peer snapshot ring depth (`PeerOptions.SnapshotHistoryCapacity`).
pub const DEFAULT_RING_CAPACITY: usize = 10;
const DEFAULT_MAX_PEERS: usize = 4096;

/// What a decoded [`ClientMessage`] should cause the server to do. Decoupled from transport so
/// the handshake/auth protocol logic is unit-testable without a live socket.
#[derive(Debug, Clone, PartialEq)]
pub enum Action {
    /// Reply to the sender on the reliable channel with this server message.
    Reply(ServerMessage),
    /// Authenticated successfully; bind the session to `wallet`. Any prior peer holding the
    /// same wallet should be disconnected (duplicate session). `initial_state` carries the
    /// asserted starting snapshot the client signed through the handshake, seeded into the
    /// board on bind (`HandshakeHandler.SeedInitialState`); already field-validated.
    Authenticated {
        wallet: String,
        duplicate_of: Option<u32>,
        initial_state: Option<Box<PlayerInitialState>>,
    },
    /// The gameplay message mutated the snapshot board (already applied); nothing to send now.
    /// Fan-out happens on the next simulation tick.
    Applied,
    /// A hardening policy rejected the message: optionally reply (e.g. a handshake
    /// failure body), flip the peer to PENDING_DISCONNECT, and tear the transport down
    /// with `reason` so the client reads a reason code (`PeerDefense.Reject` /
    /// `HandshakeHandler.RejectBanned`). Mirrors upstream's reject-then-disconnect.
    Reject {
        reply: Option<ServerMessage>,
        reason: DisconnectReason,
    },
    /// Nothing to do (e.g. a gameplay message from an unauthenticated peer).
    Ignore,
}
...
/// Field-level input hardening (`Messaging/Hardening/FieldValidator.cs`). A malformed
/// client field (NaN/Inf floats, unset position/velocity, out-of-range parcel) would
/// poison the snapshot ring and break observer interpolation, so it is rejected before
/// any publish. Upstream disconnects the peer; the catalyrst transport instead drops the
/// offending message (`Action::Ignore`) — the snapshot ring stays clean either way.
mod validate {
    use super::Vector3;
    ...
    /// `FieldValidator.IsValidPlayerStateFloats` + the parcel-range check: position and
    /// velocity must be present and finite, scalar floats finite, optional head IK /
    /// point-at finite when present, and the parcel index in range.
    ...
    /// Config-gated emote caps (`FieldValidator.ValidateEmoteStart` /
    /// `ValidateHandshakeInitialState`): an oversized `emote_id` or an excessive
    /// declared `duration_ms` is rejected. A zero cap disables that check (upstream's
    /// `maxEmoteIdLength > 0` / `maxEmoteDurationMs > 0` guards), so length is measured
    /// in chars (UTF-16 code units upstream — close enough for the URN inputs in play).
    pub fn emote_caps(
        emote_id: Option<&str>,
...
/// Holds every per-peer board + the simulation. Owned by the server loop (single-writer), so
/// the upstream lock-free machinery collapses to plain `&mut` access while preserving semantics.
pub struct PulseServer {
    ...
    // ── Anti-abuse hardening (`Transport/Hardening`, `Messaging/Hardening`) ──
    /// Anti-replay cache rejecting reused `(wallet, timestamp)` within the PENDING_AUTH window.
    pub replay_policy: HandshakeReplayPolicy,
    /// Platform-wide wallet blocklist, rejected at handshake.
    pub ban_list: BanList,
    /// Per-peer handshake-attempt throttle.
    pub attempt_policy: HandshakeAttemptPolicy,
    /// Pre-auth admission budget (global + per-IP) capping unauthenticated connections.
    pub pre_auth: PreAuthAdmission,
    /// `FieldValidatorOptions.MaxEmoteIdLength` (0 disables).
    pub max_emote_id_length: usize,
    /// `FieldValidatorOptions.MaxEmoteDurationMs` (0 disables).
    pub max_emote_duration_ms: u32,
    /// Per-peer corrupt-packet token bucket (oversized / protobuf parse-fail) → PacketCorrupted.
...
impl PulseServer {
    ...
    pub fn with_config(
        ...
        Self {
            ...
            // Replay TTL tracks PendingAuthCleanTimeoutMs; memory cap tracks MaxPeers
            // (the two are derived, not duplicated — see HandshakeReplayPolicy docs).
    ...
    /// Decode + route one client packet. Handshake yields an [`Action`] the caller applies;
    /// gameplay messages publish snapshots into the board (returning [`Action::Applied`]).
    /// `now_ms` gates handshake freshness; `now` is the monotonic server tick stamped on
    /// published snapshots.
    pub fn dispatch(
        &mut self,
        peer: u32,
        ...
        let _ = channel; // the oneof tag, not the channel, selects the message variant
        let Ok(msg) = ClientMessage::decode(data) else {
            // Malformed protobuf — debit the per-peer corruption budget. Exhausting it
            // disconnects the peer with PACKET_CORRUPTED (upstream CorruptedPacketLimiter);
            // otherwise drop the packet. `now` is the monotonic ms clock.
            if self
                .corrupted_limiter
    ...
    /// The handshake hardening pipeline, ordered exactly like `HandshakeHandler.Handle`:
    /// single-shot gate → per-peer attempt throttle → parse/verify → ban check →
    /// anti-replay → initial-state field validation → AUTHENTICATED. `now_ms` is the
    /// wall-clock skew gate; `now` is the monotonic clock for the replay TTL.
    fn handle_handshake(
        &mut self,
        peer: u32,
        ...
        // Single-shot: once a slot is AUTHENTICATED a second HandshakeRequest must not
        // silently rebind it.
        if self.is_authenticated(peer) {
            return Action::Ignore;
        }

        // Throttle before any parsing/crypto — the per-peer counter lives on PeerState.
        // On overflow the policy disconnects with AUTH_FAILED (no reply body upstream).
        ...
        // Ban list — checked before the replay cache so a banned wallet doesn't consume
        // an anti-replay slot (`RejectBanned`: reply "banned" + disconnect BANNED).
        ...
        // Anti-replay — a valid signature alone isn't enough; reject a reused
        // (wallet, timestamp) within the window (`PeerDefense.Reject`).
        ...
        // Initial-state field validation before the AUTHENTICATED transition
        // (`ValidateHandshakeInitialState`): a malformed asserted state aborts cleanly
        // with INVALID_HANDSHAKE_FIELD — no half-authenticated slot survives.
    ...
    /// `FieldValidator.ValidateHandshakeInitialState`: the embedded player state must
    /// pass float/parcel checks AND the optional emote fields must respect the caps.
    ...
    /// Mutate the snapshot board for an authenticated gameplay message (ports the
    /// `Messaging/*Handler.cs` bodies — minus the rate-limit/field-validation hardening, which
    /// the catalyrst transport does not yet model). Fan-out is deferred to the next tick.
        ...
        match msg {
            client_message::Message::Input(input) => {
                ...
                if !validate::player_state(&state, &self.encoder) {
                    return Action::Ignore; // INVALID_INPUT_FIELD
                }
            ...
            client_message::Message::Teleport(t) => {
                // realm non-empty, parcel in range, position finite (INVALID_TELEPORT_FIELD).
                if t.realm.is_empty()
            ...
            client_message::Message::EmoteStart(e) => {
                // Emote field caps (`ValidateEmoteStart`): oversized id / excessive
                // declared duration is INVALID_EMOTE_FIELD.
                ...
                if !validate::player_state(&state, &self.encoder) {
                    return Action::Ignore; // INVALID_EMOTE_FIELD
                }
                PeerSnapshotPublisher::publish_from_player_state(
                    &mut self.board,
                    &mut self.grid,
            ...
            client_message::Message::ProfileAnnouncement(p) => {
                // Monotonic, non-negative (`ProfileAnnouncementHandler`).
    ...
    /// Publish a cancel stop snapshot for the peer's active emote (`EmoteStopHandler`).
    ...
    /// Apply a dispatched [`Action`] (binding/duplicate teardown) against the live host.
    /// `now` is the monotonic server clock, stamped on the seeded initial snapshot.
    async fn apply(
        &mut self,
        host: &mut Host,
        peer: u32,
        ...
        match action {
            ...
            Action::Reject { reply, reason } => {
                // Send any failure body first (e.g. the "banned" / "Invalid initial
                // state" handshake reply), then tear the transport down carrying the
                // reason code so the client reads it (`PeerDefense.Reject`).
            ...
            Action::Authenticated {
                ...
                if let Some(dup) = duplicate_of {
                    // Evict the prior session with a DUPLICATE_SESSION reason so its
                    // client learns why (`transport.Disconnect(dup, DUPLICATE_SESSION)`).
                ...
                // Promotion out of PENDING_AUTH frees the global + per-IP pre-auth slots.
                self.pre_auth.release_on_promotion(peer);
                self.identity.set(peer, wallet.clone());
                self.board.set_active(peer);
                // Seed the asserted starting state the client signed through the handshake
                // (`HandshakeHandler.SeedInitialState`). Already field-validated in dispatch.
    ...
    /// Publish the handshake's asserted initial snapshot into the board
    /// (`HandshakeHandler.SeedInitialState`). The emote start tick is backdated by the
    /// reported offset so observers scrub a mid-flight emote forward on the next
    /// `EmoteStarted`; realm stays unset until the first `TeleportRequest`.
    ...
    /// Run one simulation tick and flush every queued [`OutgoingMessage`] to the host.
    /// `now` is the monotonic server clock, feeding the connection-lifecycle sweep
    /// (PENDING_AUTH / DISCONNECTING timeouts) inside `simulate_tick`.
    async fn run_tick(&mut self, host: &mut Host, now: u32) -> anyhow::Result<()> {
        ...
        // Reap the peers the lifecycle sweep flagged (upstream's post-loop
        // `peersToBeRemoved` drain + the in-loop `transport.Disconnect` calls). A
        // PENDING_AUTH peer past the budget is disconnected with AUTH_TIMEOUT so its
        // client reads a reason code rather than seeing a silent slot reclaim; the
        // DISCONNECTING grace path is board-cleanup only (the transport teardown that
        // moved it to DISCONNECTING already carried its reason). Either way we wipe the
        // server-side boards so the slot is immediately reusable.
    ...
    /// Drive the host with a simulation tick every `tick_ms` (default 50ms = TIER_0 base rate).
                ...
                serviced = host.service() => {
                    match serviced? {
                        Some(Event::Connect { peer }) => {
                            ...
                            // Pre-auth admission control: cap the unauthenticated connections
                            // a single IP — and the whole server — may hold in PENDING_AUTH
                            // (`TryAdmitOrRefuse`). On refusal, hard-disconnect with the
                            // specific budget/IP reason instead of admitting the slot.
    ...
    /// Begin tearing down a peer (duplicate session): clean up its state and notify observers.
    /// The transport-level disconnect is driven by `rusty_enet`'s own ENet event flow; this
    /// only wipes the server-side boards so the slot is immediately reusable, matching
    /// `PeerSimulation.CleanupDisconnectedPeer`.
    ...
    async fn on_disconnect(&mut self, host: &mut Host, peer: u32) -> anyhow::Result<()> {
        // Release the pre-auth slot on the lifecycle Disconnected event (idempotent w.r.t.
        // promotion — `ReleaseOnDisconnect`). Covers client drop, handshake failure, and
        // auth-timeout.
        self.pre_auth.release_on_disconnect(peer);
    ...
    /// Wipe every per-peer board, drop observer views, and tell remaining observers the peer
    /// left (`PeerSimulation.CleanupDisconnectedPeer`).
    async fn cleanup_peer(&mut self, host: &mut Host, peer: u32) -> anyhow::Result<()> {
        ...
        // Tell remaining authenticated observers the peer left.
    ...
    /// Send a server message to one peer on `channel` as prost bytes.
    async fn send(
        &self,
        host: &mut Host,
        peer: u32,
...
mod tests {
    ...
    fn corrupt_packets_exhaust_budget_then_disconnect() {
        ...
        // Truncated length-delimited field → ClientMessage::decode fails (corrupt packet).
        let bad: &[u8] = &[0x0A, 0xFF];
        // Default budget tolerates a burst of 5 corrupt packets…
        for _ in 0..5 {
            assert_eq!(
        ...
        // …the 6th exhausts it and disconnects with PACKET_CORRUPTED.
        assert_eq!(
            srv.dispatch(7, channel::RELIABLE, bad, 1000, 1000),
            Action::Reject {
                reply: None,
    ...
    /// A field-valid PlayerState (finite position + velocity present) at `parcel`, the
    /// minimum the FieldValidator accepts.
    ...
    fn authenticated_input_publishes_snapshot_with_real_sequence() {
        ...
        // seq advanced 0 (not hardcoded), server_tick captured.
        ...
        // A second input advances the per-peer sequence.
        let bytes = client_msg(client_message::Message::Input(PlayerStateInput {
    ...
    /// Build a real, verifiable signed-fetch auth-chain header bag and return it plus the
    /// expected lower-cased wallet and the `now_ms` it must be verified at.
    ...
    /// Re-encode a signed handshake's bytes with a different `initial_state` (the auth
    /// chain bytes are unaffected by the optional asserted state).
    ...
    async fn handshake_seeds_valid_initial_state() {
        ...
        // Drive the bind path's seed publish directly (apply() needs a live host).
    ...
    async fn handshake_with_malformed_initial_state_is_rejected() {
        ...
        // NaN position -> ValidateHandshakeInitialState fails -> handshake aborts with a
        // failure reply AND a transport disconnect carrying INVALID_HANDSHAKE_FIELD
        // (FieldValidator.Reject), so no half-authenticated slot survives.
    ...
    fn input_without_position_is_rejected() {
        let mut srv = PulseServer::new();
        authed(&mut srv, 7, "0xabc");
        // proto default state: no position/velocity -> upstream IsValidPlayerStateFloats fails.
        let bytes = client_msg(client_message::Message::Input(PlayerStateInput {
    ...
    async fn valid_handshake_authenticates_and_binds_wallet() {
        ...
        // Simulate apply() binding: wallet -> session + identity + active board slot.
    ...
    async fn banned_wallet_is_rejected_at_handshake() {
        ...
        // Ban the very wallet the signed chain authenticates as.
        srv.ban_list.replace([wallet.clone()]);

        match srv.dispatch(1, channel::RELIABLE, &bytes, now_ms, 0) {
            Action::Reject { reply, reason } => {
                assert_eq!(reason, DisconnectReason::Banned);
                // Banned reply body carries the "banned" error (RejectBanned).
                match reply.and_then(|m| m.message) {
                    Some(server_message::Message::Handshake(h)) => {
                        assert!(!h.success);
        ...
        assert!(!srv.is_authenticated(1));
        // The peer is flipped to PENDING_DISCONNECT (PeerDefense.Reject semantics).
        assert_eq!(
    ...
    async fn replayed_handshake_pair_is_rejected() {
        ...
        // First handshake on peer 1 authenticates and records (wallet, ts) in the cache.
        ...
        // A replay of the SAME captured packet on a fresh peer (same wallet+timestamp,
        // within the TTL window) is rejected with HANDSHAKE_REPLAY_REJECTED.
    ...
    async fn handshake_attempts_are_throttled() {
        let mut srv = PulseServer::new();
        // MaxAttempts default is 2: two bad attempts allowed, the third is throttled.
        srv.peers
            .insert(1, PeerState::new(PeerConnectionState::PendingAuth, 0));
        ...
        // Attempts 1 and 2: parse fails -> failure Reply (counter increments 0->1->2).
        for _ in 0..2 {
            assert!(matches!(
        ...
        // Attempt 3: throttled before any parse -> Reject(AUTH_FAILED), no reply.
    ...
    async fn handshake_initial_state_emote_cap_is_enforced() {
        ...
        // Valid player state but an oversized emote id in the asserted initial state.
        let init = PlayerInitialState {
            state: Some(valid_state(7)),
