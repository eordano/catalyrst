...
mod tests {
    ...
    #[test]
    fn require_signer_recovers_the_wallet_address() {
        let wallet = Wallet::from_hex(TEST_KEY).unwrap();
        let now_ms = chrono::Utc::now().timestamp_millis();
        let headers = signed_headers(&wallet, "post", "/v1/rentals-listings", now_ms);
        let signer =
            require_signer(&headers, "post", "/v1/rentals-listings", FIVE_MINUTES).unwrap();
        assert_eq!(signer, wallet.address().to_lowercase());
    }

    #[test]
    fn require_signer_rejects_a_rebound_path() {
        let wallet = Wallet::from_hex(TEST_KEY).unwrap();
        let now_ms = chrono::Utc::now().timestamp_millis();
        let headers = signed_headers(&wallet, "post", "/v1/rentals-listings", now_ms);
        let err = require_signer(&headers, "post", "/v1/other", FIVE_MINUTES).unwrap_err();
        assert!(matches!(err, AuthChainError::InvalidSignature(_)));
    }

    #[test]
    fn require_signer_rejects_outside_the_symmetric_window() {
        let wallet = Wallet::from_hex(TEST_KEY).unwrap();
        for skew_ms in [-(FIVE_MINUTES + 60) * 1000, (FIVE_MINUTES + 60) * 1000] {
            ...
            let err =
                require_signer(&headers, "post", "/v1/rentals-listings", FIVE_MINUTES).unwrap_err();
