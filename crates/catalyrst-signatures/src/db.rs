...
impl Database {
    ...
    pub async fn get_listings(&self, q: &ListingQuery) -> Result<PaginatedListings, sqlx::Error> {
        ...
        if let Some(n) = &f.network {
            qb.push("AND rentals.network = ")
                .push_bind(n.clone())
                .push(" ");
        }
        if let Some(l) = &f.lessor {
            qb.push("AND rentals_listings.lessor = ")
                .push_bind(l.clone())
                .push(" ");
        }
        if let Some(t) = &f.tenant {
            qb.push("AND rentals_listings.tenant = ")
                .push_bind(t.clone())
                .push(" ");
        }
        ...
        if let Some(min) = f.min_distance_to_plaza {
            qb.push("AND metadata.distance_to_plaza >= ")
                .push_bind(min)
                .push(" ");
        }
        ...
        if let Some(adj) = f.adjacent_to_road {
            qb.push("AND metadata.adjacent_to_road = ")
                .push_bind(adj)
                .push(" ");
        }
        if let Some(min) = f.min_estate_size {
            if min >= 0 {
                qb.push("AND metadata.estate_size >= ")
                    .push_bind(min)
                    .push(" ");
            }
        }
