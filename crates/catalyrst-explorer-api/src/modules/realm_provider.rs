...
async fn validate_external_catalyst(base: &str) -> Option<(String, SocketAddr)> {
    ...
    let addr = catalyrst_commons::http::resolve_and_pin(&parsed).await?;
    Some((trimmed.to_string(), addr))
}

fn catalyst_client_cfg() -> catalyrst_commons::http::HttpClientCfg {
...
fn no_redirect_client() -> &'static reqwest::Client {
    ...
    C.get_or_init(|| {
        catalyrst_commons::http::http_client("catalyst-status", &catalyst_client_cfg())
    })
}
...
mod tests {
    ...
    #[tokio::test]
    async fn validate_external_catalyst_refuses_internal_and_non_https_targets() {
        for url in [
            "https://the cloud-metadata IMDS/latest/meta-data/",
            "https://127.0.0.1/",
            "https://localhost/",
            "https://10.0.0.1/",
            "https://192.168.1.1/",
            ...
            assert!(
                validate_external_catalyst(url).await.is_none(),
    ...
    #[tokio::test]
    async fn validate_external_catalyst_pins_a_public_https_literal() {
        let (trimmed, addr) = validate_external_catalyst("https://8.8.8.8:8443/")
            .await
            .expect("public https literal must be accepted");
        assert_eq!(trimmed, "https://8.8.8.8:8443");
        assert_eq!(addr, "8.8.8.8:8443".parse::<SocketAddr>().unwrap());
    }
