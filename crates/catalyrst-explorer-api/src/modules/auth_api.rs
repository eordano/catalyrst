...
async fn create_request(
    ...
    let code = (now.timestamp_subsec_nanos() % 100) as u32;
...
async fn get_request_validation(
    State(state): State<AppState>,
    Path(id): Path<String>,
) -> Response {
    let now = Utc::now();
...
async fn get_identity(
    ...
    let Some(record) = state.auth_api.identities.get(&id).map(|e| e.value().clone()) else {
    ...
    if record.expiration < now {
        ...
        update_identity_status(&state, &id, false, Some(DeletionReason::Expired), &signer, now);
        return (
            StatusCode::GONE,
            Json(ErrorBody {
    ...
    {
        state.auth_api.identities.remove(&id);
        update_identity_status(&state, &id, false, Some(DeletionReason::IpMismatch), &signer, now);
        return (
    ...
    state.auth_api.identities.remove(&id);
    update_identity_status(&state, &id, true, Some(DeletionReason::Consumed), &signer, now);
    (
        StatusCode::OK,
...
fn identity_status_response(state: &AppState, id: &str) -> Response {
    let Some(status) = state.auth_api.identity_status.get(id).map(|e| e.value().clone()) else {
        return (
