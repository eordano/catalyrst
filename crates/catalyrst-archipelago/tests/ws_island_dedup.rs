...
async fn connect_and_handshake(port: u16, wallet: &LocalWallet) -> WsStream {
    ...
    ws.send(WsMessage::Binary(encode(client_packet::Message::ChallengeRequest(
        ChallengeRequestMessage { address: address.clone() },
    ))))
    .await
    ...
    ws.send(WsMessage::Binary(encode(client_packet::Message::SignedChallenge(
        SignedChallengeMessage { auth_chain_json: chain.to_string() },
    ))))
    .await
...
async fn send_heartbeat(ws: &mut WsStream) {
    ws.send(WsMessage::Binary(encode(client_packet::Message::Heartbeat(Heartbeat {
        position: Some(Position { x: 1.0, y: 0.0, z: 1.0 }),
        desired_room: None,
    }))))
    .await
...
async fn island_changed_is_delivered_exactly_once() {
    ...
    assert_eq!(n, 1, "fresh peer must receive IslandChanged exactly once, got {n}");
}
...
async fn second_socket_same_wallet_gets_one_island_and_survives_stale_close() {
    ...
    assert_eq!(count_island_changed(&mut ws_a, Duration::from_millis(3000)).await, 1);
    ...
    assert_eq!(n_b, 1, "reconnect socket must receive its inherited island exactly once, got {n_b}");
