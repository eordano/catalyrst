...
pub struct ServerConfig {
    ...
    #[serde(default)]
    pub control_v4_audience: String,
}
...
impl Default for ServerConfig {
    fn default() -> Self {
        Self {
            ...
            control_v4_audience: String::new(),
        }
    }
}
...
fn default_lk_ttl_secs() -> i64 {
    crate::livekit::MAX_TOKEN_TTL_SECS
}
...
impl Config {
    pub fn from_env() -> Result<Self> {
        ...
        let (cluster, mut server, mut auth, mut livekit, mut nats) = match path {
            Some(p) if p.exists() => {
        ...
        if let Ok(v) = env::var("ARCHIPELAGO_CONTROL_V4_AUDIENCE") {
            if !v.trim().is_empty() {
                server.control_v4_audience = v.trim().to_string();
            }
        }
        if nats.url.is_none() {
            tracing::warn!(
        ...
        let control_database_url = env::var("ARCHIPELAGO_CONTROL_PG_CONNECTION_STRING")
            .ok()
            .filter(|s| !s.is_empty());
