...
struct HealthResp {
    ...
    feed_assignment_coalesced: u64,
    feed_assignment_oversized: u64,
    socket_write_timeout: u64,
    socket_assignments_written: u64,
}
...
async fn stats_health(State(s): State<AppState>) -> Json<HealthResp> {
    ...
    Json(HealthResp {
        ...
        feed_assignment_coalesced: s.feed.coalesced_count(),
        feed_assignment_oversized: s.feed.assignment_oversized_count(),
        socket_write_timeout: s.feed.socket_write_timeout_count(),
        socket_assignments_written: s.feed.socket_assignments_written_count(),
    })
}
...
async fn auth_challenge(State(s): State<AppState>, body: Bytes) -> impl IntoResponse {
    ...
    if !crate::session::is_address(&req.address.to_ascii_lowercase()) {
        return (
            StatusCode::BAD_REQUEST,
            Json(ErrorResp {
...
async fn livekit_token(State(s): State<AppState>, body: Bytes) -> impl IntoResponse {
    ...
    let session = crate::session::session_key_of(&req.auth_chain, &addr);
    match s.control_v4.realm_holder(&addr).await {
        Ok(Some(holder)) if holder != session => {
            return (
                StatusCode::FORBIDDEN,
                Json(ErrorResp {
                    error: "another session holds this wallet".into(),
                }),
            )
                .into_response();
        }
        ...
        Err(_) => {
            return (
                ...
                Json(ErrorResp {
                    error: "authority unavailable".into(),
                }),
            )
                .into_response();
        }
