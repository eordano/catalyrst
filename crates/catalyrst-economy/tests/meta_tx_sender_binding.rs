...
async fn forged_from_cannot_burn_a_victims_relayer_quota() {
    let Some(scratch) = support::setup_db().await else {
        eprintln!("skipping forged_from_cannot_burn_a_victims_relayer_quota: set CATALYRST_ECONOMY_TEST_PG to run");
        return;
    };
...
async fn quota_is_keyed_on_the_signed_user_address_for_both_overloads() {
    let Some(scratch) = support::setup_db().await else {
        eprintln!("skipping quota_is_keyed_on_the_signed_user_address_for_both_overloads: set CATALYRST_ECONOMY_TEST_PG to run");
        return;
    };
    support::seed_collection(&scratch.pool, COLLECTION).await;
    let base = support::spawn_app(&scratch, 10).await;
...
async fn undecodable_meta_tx_calldata_is_rejected() {
    let Some(scratch) = support::setup_db().await else {
        eprintln!(
            "skipping undecodable_meta_tx_calldata_is_rejected: set CATALYRST_ECONOMY_TEST_PG to run"
        );
        return;
    };
    support::seed_collection(&scratch.pool, COLLECTION).await;
