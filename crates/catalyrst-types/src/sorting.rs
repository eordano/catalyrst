...
pub fn happened_before(a: &impl IntoEntityComparable, b: &impl IntoEntityComparable) -> bool {
    ...
    a.timestamp < b.timestamp
        || (a.timestamp == b.timestamp
            && a.entity_id.to_lowercase() < b.entity_id.to_lowercase())
}
...
mod tests {
    ...
    fn happened_before_by_timestamp() {
        let a = EntityComparable { timestamp: 100, entity_id: "bbb".into() };
        let b = EntityComparable { timestamp: 200, entity_id: "aaa".into() };
    ...
    fn happened_before_tiebreak_by_entity_id() {
        let a = EntityComparable { timestamp: 100, entity_id: "aaa".into() };
        let b = EntityComparable { timestamp: 100, entity_id: "bbb".into() };
        assert!(happened_before(&a, &b));
        assert!(!happened_before(&b, &a));
    }
    ...
    fn happened_before_equal_is_false() {
        let a = EntityComparable { timestamp: 100, entity_id: "aaa".into() };
        let b = EntityComparable { timestamp: 100, entity_id: "aaa".into() };
        assert!(!happened_before(&a, &b));
    }
}
