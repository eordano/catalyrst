...
impl AppStateInner {
    ...
    /// Background job mirroring upstream's `expirePrivateVoiceChatJob`: every
    /// `PRIVATE_VOICE_CHAT_JOB_INTERVAL` ms, drain expired 1:1 call intents and
    /// notify both parties. Without this, stale rows would wedge users behind
    /// the unique caller/callee constraints and surface false busy conflicts.
