...
fn page(p: &Option<Pagination>) -> (i64, i64) {
    match p {
        Some(p) => {
            let mut limit = if p.limit <= 0 { DEFAULT_LIMIT } else { p.limit as i64 };
            if limit > MAX_LIMIT {
                limit = MAX_LIMIT;
            }
...
fn page_number(p: &Option<Pagination>) -> i32 {
    match p {
        Some(p) => {
            let limit = if p.limit <= 0 { DEFAULT_LIMIT } else { p.limit as i64 };
            let offset = if p.offset < 0 { 0 } else { p.offset as i64 };
...
fn friendship_status_invalid(msg: impl Into<String>) -> GetFriendshipStatusResponse {
    GetFriendshipStatusResponse {
        response: Some(get_friendship_status_response::Response::InvalidRequest(invalid_req(
            msg,
        ))),
    }
}
...
fn upsert_internal_error(msg: impl Into<String>) -> UpsertFriendshipResponse {
    UpsertFriendshipResponse {
        response: Some(upsert_friendship_response::Response::InternalServerError(internal_err(
            msg,
        ))),
    }
}
...
fn transition_valid(from: Option<Action>, to: Action) -> bool {
    let allowed: &[Option<Action>] = match to {
        Action::Request => &[Some(Action::Cancel), Some(Action::Reject), Some(Action::Delete), None],
...
impl SocialServiceServer<Context, SocialError> for SocialServiceImpl {
    async fn get_friends(
        ...
        let profiles = context.server_context.profiles().friend_profiles(&friends).await;
    ...
    async fn get_friendship_status(
        ...
        let other = match raw {
            Some(a) if !a.trim().is_empty() => {
                let n = normalize(&a);
                if !is_eth_address(&n) {
                    return Ok(friendship_status_invalid("Invalid user address in the request payload"));
                }
                n
            }
            _ => {
        ...
        match resolved {
            ...
            Err(e) => Ok(GetFriendshipStatusResponse {
                response: Some(get_friendship_status_response::Response::InternalServerError(
    ...
    async fn block_user(
        ...
        if let Err(e) = db.block_user(&me, &other).await {
            return Ok(BlockUserResponse {
                response: Some(block_user_response::Response::InternalServerError(internal_err(
                    e.to_string(),
                ))),
            });
        }
        ...
        match db.last_friendship_action(&me, &other).await {
            Ok(Some(last)) => {
                let _ = db
                    .apply_friendship_action(&me, &other, "block", false, Some(last.friendship_id), None)
                    .await;
            }
            Ok(None) => {}
            Err(e) => {
            ...
            SocialEvent::Friendship(FriendshipUpdate {
                update: Some(friendship_update::Update::Block(
                    friendship_update::BlockResponse {
                        user: Some(User { address: me.clone() }),
                    },
                )),
            }),
        );
    ...
    async fn unblock_user(
        ...
        let other = match request.user.as_ref() {
            ...
            None => {
                return Ok(UnblockUserResponse {
                    response: Some(unblock_user_response::Response::InvalidRequest(invalid_req(
                        "missing user",
                    ))),
                })
            }
        ...
        if let Err(e) = db.unblock_user(&me, &other).await {
            return Ok(UnblockUserResponse {
                response: Some(unblock_user_response::Response::InternalServerError(internal_err(
                    e.to_string(),
                ))),
            });
        }
        ...
        if let Some(last) = last {
            if db
                .apply_friendship_action(&me, &other, "delete", false, Some(last.friendship_id), None)
                .await
                .is_ok()
            {
                pubsub.publish(
        ...
        let profile = context.server_context.profiles().blocked_profile(&other, None).await;
        Ok(UnblockUserResponse {
            response: Some(unblock_user_response::Response::Ok(unblock_user_response::Ok {
                profile: Some(profile),
            })),
        })
    }
    ...
    async fn get_blocking_status(
        ...
        let me = Self::caller(&context)?;
        let (blocked_users, blocked_by_users) =
            match context.server_context.db().get_blocking_status(&me).await {
                Ok(v) => v,
    ...
    async fn upsert_social_settings(
            ...
            .await
        {
            Ok(row) => Ok(UpsertSocialSettingsResponse {
                response: Some(upsert_social_settings_response::Response::Ok(settings_to_proto(
                    &row,
                ))),
            }),
            Err(e) => Ok(UpsertSocialSettingsResponse {
                response: Some(upsert_social_settings_response::Response::InternalServerError(
                    internal_err(e.to_string()),
                )),
            }),
        }
    ...
    async fn get_private_messages_settings(
        ...
        if targets.len() > MAX_USER_ADDRESSES {
            return Ok(GetPrivateMessagesSettingsResponse {
                response: Some(get_private_messages_settings_response::Response::InvalidRequest(
                    invalid_req(format!("Too many user addresses: {}", targets.len())),
                )),
            });
        }
    ...
    async fn subscribe_to_friendship_updates(
        ...
        let me = Self::caller(&context)?;
        Ok(stream_for(context.server_context.pubsub(), &me, |e| match e {
    ...
    async fn start_private_voice_chat(
        ...
        if callee == me {
            return Ok(StartPrivateVoiceChatResponse {
                response: Some(start_private_voice_chat_response::Response::InvalidRequest(
                    invalid_req("cannot call yourself"),
                )),
            });
        }
        let db = context.server_context.db();
        match db.start_private_voice_chat(&me, &callee, VOICE_TTL_SECS).await {
            Ok(id) => {
                context.server_context.pubsub().publish(
                    &callee,
                    SocialEvent::PrivateVoice(PrivateVoiceChatUpdate {
                        call_id: id.to_string(),
                        status: PrivateVoiceChatStatus::VoiceChatRequested as i32,
                        caller: Some(User { address: me.clone() }),
                        callee: Some(User { address: callee.clone() }),
                        credentials: None,
                    }),
                );
                Ok(StartPrivateVoiceChatResponse {
                    response: Some(start_private_voice_chat_response::Response::Ok(
                        start_private_voice_chat_response::Ok { call_id: id.to_string() },
                    )),
                })
            }

            Err(_) => Ok(StartPrivateVoiceChatResponse {
                response: Some(start_private_voice_chat_response::Response::ConflictingError(
                    ConflictingError {
                        message: Some("user already has an active voice chat".into()),
                    },
                )),
            }),
        }
    ...
    async fn accept_private_voice_chat(
        ...
        let id = match Uuid::parse_str(&request.call_id) {
            ...
            Err(_) => {
                return Ok(AcceptPrivateVoiceChatResponse {
                    response: Some(accept_private_voice_chat_response::Response::InvalidRequest(
                        invalid_req("invalid call_id"),
                    )),
                })
            }
        };
            ...
            None => {
                return Ok(AcceptPrivateVoiceChatResponse {
                    response: Some(accept_private_voice_chat_response::Response::NotFound(
                        NotFoundError { message: Some("call not found".into()) },
                    )),
                })
            }
        };
        if chat.callee_address != me {
            return Ok(AcceptPrivateVoiceChatResponse {
                response: Some(accept_private_voice_chat_response::Response::ForbiddenError(
                    ForbiddenError { message: Some("not the callee".into()) },
                )),
            });
        }
        ...
        ctx.pubsub().publish(
            ...
            SocialEvent::PrivateVoice(PrivateVoiceChatUpdate {
                ...
                caller: Some(User { address: chat.caller_address.clone() }),
                callee: Some(User { address: chat.callee_address.clone() }),
    ...
    async fn reject_private_voice_chat(
        ...
        let id = match Uuid::parse_str(&request.call_id) {
            ...
            Err(_) => {
                return Ok(RejectPrivateVoiceChatResponse {
                    response: Some(reject_private_voice_chat_response::Response::InvalidRequest(
                        invalid_req("invalid call_id"),
                    )),
                })
            }
        ...
        Ok(RejectPrivateVoiceChatResponse {
            response: Some(reject_private_voice_chat_response::Response::Ok(
                reject_private_voice_chat_response::Ok { call_id: request.call_id },
            )),
        })
    }
    ...
    async fn end_private_voice_chat(
            ...
            Err(_) => {
                return Ok(EndPrivateVoiceChatResponse {
                    response: Some(end_private_voice_chat_response::Response::NotFound(
                        NotFoundError { message: Some("invalid call_id".into()) },
                    )),
                })
            }
        };
        ...
        Ok(EndPrivateVoiceChatResponse {
            response: Some(end_private_voice_chat_response::Response::Ok(
                end_private_voice_chat_response::Ok { call_id: request.call_id },
            )),
        })
    }
    ...
    async fn get_incoming_private_voice_chat_request(
        ...
        let me = Self::caller(&context)?;
        match context.server_context.db().incoming_private_voice_chat(&me).await? {
            Some(chat) => Ok(GetIncomingPrivateVoiceChatRequestResponse {
                response: Some(get_incoming_private_voice_chat_request_response::Response::Ok(
                    get_incoming_private_voice_chat_request_response::Ok {
                        caller: Some(User { address: chat.caller_address }),
            ...
            None => Ok(GetIncomingPrivateVoiceChatRequestResponse {
                response: Some(
                    get_incoming_private_voice_chat_request_response::Response::NotFound(
                        NotFoundError { message: Some("no incoming call".into()) },
                    ),
                ),
            }),
        }
        ...
        let role = match db.community_role(&request.community_id, &me).await? {
            ...
            _ => {
                return Ok(StartCommunityVoiceChatResponse {
                    response: Some(start_community_voice_chat_response::Response::ForbiddenError(
                        ForbiddenError { message: Some("requires moderator or owner role".into()) },
                    )),
                })
            }
        };
        ...
        Ok(StartCommunityVoiceChatResponse {
            response: Some(start_community_voice_chat_response::Response::Ok(
                start_community_voice_chat_response::Ok {
                    credentials: Some(CommunityVoiceChatCredentials { connection_url: conn }),
                },
            )),
        })
    }
        ...
        let role = match db.community_role(&request.community_id, &me).await? {
            ...
            None => {
                return Ok(JoinCommunityVoiceChatResponse {
                    response: Some(join_community_voice_chat_response::Response::ForbiddenError(
                        ForbiddenError { message: Some("not a community member".into()) },
                    )),
                })
            }
        };
    ...
    async fn request_to_speak_in_community_voice_chat(
        ...
        let me = Self::caller(&context)?;
        let db = context.server_context.db();
        if db.community_role(&request.community_id, &me).await?.is_none() {
        ...
        Ok(RequestToSpeakInCommunityVoiceChatResponse {
            response: Some(request_to_speak_in_community_voice_chat_response::Response::Ok(
                request_to_speak_in_community_voice_chat_response::Ok { message: "ok".into() },
            )),
        })
    }
    ...
    async fn promote_speaker_in_community_voice_chat(
        ...
        Ok(PromoteSpeakerInCommunityVoiceChatResponse {
            response: Some(promote_speaker_in_community_voice_chat_response::Response::Ok(
                promote_speaker_in_community_voice_chat_response::Ok { message: "ok".into() },
            )),
        })
    }
    ...
    async fn demote_speaker_in_community_voice_chat(
        ...
        Ok(DemoteSpeakerInCommunityVoiceChatResponse {
            response: Some(demote_speaker_in_community_voice_chat_response::Response::Ok(
                demote_speaker_in_community_voice_chat_response::Ok { message: "ok".into() },
            )),
        })
    }
    ...
    async fn kick_player_from_community_voice_chat(
        ...
        Ok(KickPlayerFromCommunityVoiceChatResponse {
            response: Some(kick_player_from_community_voice_chat_response::Response::Ok(
                kick_player_from_community_voice_chat_response::Ok { message: "ok".into() },
            )),
        })
    }
    ...
    async fn reject_speak_request_in_community_voice_chat(
        ...
        Ok(RejectSpeakRequestInCommunityVoiceChatResponse {
            response: Some(reject_speak_request_in_community_voice_chat_response::Response::Ok(
                reject_speak_request_in_community_voice_chat_response::Ok { message: "ok".into() },
            )),
        })
    }
    ...
    async fn end_community_voice_chat(
        ...
        if let Err(f) = require_moderator(db, &request.community_id, &me).await? {
            return Ok(EndCommunityVoiceChatResponse {
                response: Some(end_community_voice_chat_response::Response::ForbiddenError(f)),
            });
        }
        let _ = context
            .server_context
        ...
        Ok(EndCommunityVoiceChatResponse {
            response: Some(end_community_voice_chat_response::Response::Ok(
                end_community_voice_chat_response::Ok { message: "ok".into() },
            )),
        })
    }
    ...
    async fn mute_speaker_from_community_voice_chat(
        ...
        Ok(MuteSpeakerFromCommunityVoiceChatResponse {
            response: Some(mute_speaker_from_community_voice_chat_response::Response::Ok(
                mute_speaker_from_community_voice_chat_response::Ok { muted: request.muted },
            )),
        })
    }
}
...
fn friendship_update_for(
    ...
    let user = Some(User { address: from.to_string() });
    ...
    FriendshipUpdate { update: Some(update) }
}
...
async fn friendship_requests(
    ...
    let (limit, offset) = page(&request.pagination);
    let fetched = async {
        let rows = db.get_friendship_requests(&me, incoming, limit, offset).await?;
...
async fn fan_community_voice(
    ...
    let db = ctx.db();
    let community_name = db.community_name(community_id).await.ok().flatten().unwrap_or_default();
    let members = db.community_member_addresses(community_id).await.unwrap_or_default();
    let now_ms = chrono::Utc::now().timestamp_millis();
    let ended_at = matches!(status, CommunityVoiceChatStatus::CommunityVoiceChatEnded).then_some(now_ms);
