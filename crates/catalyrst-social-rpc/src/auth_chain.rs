...
pub fn require_signer(
    ...
    let timestamp = obj_str(&value, AUTH_TIMESTAMP_HEADER)
        .ok_or(AuthChainError::MissingHeader(AUTH_TIMESTAMP_HEADER))?
        .to_string();
    let metadata = obj_str(&value, AUTH_METADATA_HEADER).unwrap_or("{}").to_string();
    let payload = build_payload(method, path, &timestamp, &metadata);
...
mod tests {
    ...
    async fn verify_handshake_rejects_method_path_mismatch() {
        ...
        assert!(matches!(err, AuthChainError::InvalidSignature(_)), "{err:?}");
    }
    ...
    fn verify_handshake_rejects_malformed_envelope() {
        ...
        assert!(matches!(err, AuthChainError::MalformedChain { .. }), "{err:?}");
        let err2 = verify_handshake("[]", "get", "/", FIVE_MINUTES_SECS, 0).unwrap_err();
        assert!(matches!(err2, AuthChainError::EnvelopeNotObject), "{err2:?}");
    }
