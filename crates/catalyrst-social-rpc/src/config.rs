...
pub struct Config {
    ...
    /// 1:1 voice-chat call intents older than this (ms) are reclaimed by the
    /// background sweep. Upstream `PRIVATE_VOICE_CHAT_EXPIRATION_TIME` (60000).
    pub private_voice_chat_expiration_ms: i64,
    /// Interval (ms) between sweep passes. Upstream
    /// `PRIVATE_VOICE_CHAT_JOB_INTERVAL` (1000).
    pub private_voice_chat_job_interval_ms: u64,
    /// Max rows reclaimed per sweep pass. Upstream
