...
impl Gatekeeper {
    ...
    /// GET /users/:addr/voice-chat-status — true if the user is currently in any
    /// LiveKit voice chat (private or community). Mirrors upstream
    /// `commsGatekeeper.isUserInAVoiceChat`: reads the boolean
    /// `is_user_in_voice_chat` field off the JSON response. Propagates the error
    /// (rather than defaulting to false) so the caller can fail closed and not
    /// silently let a second concurrent call slip through.
    ...
    /// GET /users/:addr/community-voice-chat-status — true if the user is
    /// currently in a community voice chat. Mirrors upstream
    /// `commsGatekeeper.isUserInCommunityVoiceChat`: reads the boolean
    /// `isInCommunityVoiceChat` (camelCase) field off the JSON response.
