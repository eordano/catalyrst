...
impl Db {
    ...
    /// Symmetric busy check mirroring upstream `voiceDb.areUsersBeingCalledOrCallingSomeone`
    /// (social-service-ea/src/adapters/voice-db.ts). Returns true if ANY of the
    /// given addresses already appears as either the caller OR the callee of an
    /// existing 1:1 voice chat. This covers all four caller/callee column ×
    /// new-caller/new-callee address combinations, so a user can be neither
    /// calling nor being called by a second person at the same time.
    ///
    /// This port now follows upstream's FINAL schema (migration
    /// `1749835946066_expire-private-voice-chats`): `expires_at`/`updated_at`
    /// were dropped and liveness is keyed off `created_at` + the configured TTL.
    /// Upstream's own busy query is unfiltered (it trusts the sweep to keep the
    /// table clean), but to avoid a false `ConflictingError` from a stale row in
    /// the race window before the next sweep pass, this port applies the SAME
    /// `created_at < now() - PRIVATE_VOICE_CHAT_EXPIRATION_TIME` predicate the
    /// sweep uses — i.e. a row is "busy" only while it is still live. This is
    /// upstream-equivalent: any row the busy check skips is exactly a row the
    ...
    /// Active-friendship lookup mirroring upstream `friendsDb.getFriendship([a,b])`:
    /// returns the friendship's `is_active` flag (None when no friendship row
    /// exists at all). Used by the voice eligibility gate to decide whether two
    /// non-`ALL`-privacy users are allowed to call each other.
    ...
    /// Mirrors upstream `voiceDb.createPrivateVoiceChat`: insert id/caller/callee
    /// only. `created_at` defaults to `now()`; liveness/TTL is derived from it by
    /// the busy check and the sweep (no `expires_at` column post-alignment).
    ...
    /// Background expiry sweep mirroring upstream `voiceDb.expirePrivateVoiceChat`
    /// (social-service-ea/src/adapters/voice-db.ts): delete the oldest voice
    /// chats whose `created_at` is older than `expiration_ms`, up to `limit`
    /// rows, returning the deleted (id, caller, callee) tuples so callers can
    /// fan an EXPIRED update to both parties. Ordered `created_at ASC` so the
    /// oldest are reclaimed first, matching upstream's batched drain.
    ...
    /// Admin read: every still-live 1:1 voice chat, newest first. Backs the
    /// admin "active voice calls" view.
    ///
    /// Post-alignment the table no longer stores `expires_at`; liveness is keyed
    /// off `created_at` + the configured TTL, exactly like the busy check and the
    /// sweep. The admin wire still carries an `expires_at` for operator
    /// convenience, so it is COMPUTED here as `created_at + expiration_ms` rather
    /// than read from a column — the value is identical to what the old
    /// `expires_at` column held (both were `created_at + TTL`).
