...
async fn ws_handshake_valid_chain_receives_transport_frame() {
    ...
    match msg {
        ...
        other => panic!("expected binary transport frame (no text welcome), got {:?}", other),
    }
}
...
async fn ws_handshake_garbage_frame_gets_close_3003() {
    ...
    match msg {
        Message::Close(Some(frame)) => {
            assert_eq!(frame.code, tokio_tungstenite::tungstenite::protocol::frame::coding::CloseCode::from(3003u16));
        }
