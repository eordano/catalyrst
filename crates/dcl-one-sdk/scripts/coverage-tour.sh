...
c43_start_flag_compat_deeplink() {
  STARTDIR=$SCENE start_and_wait 61017 --ci --no-watch --skip-build --asset-bundles \
    --mcp --mcp-port 61098 --no-browser --mini-comms --skip-install --multi-instance \
  ...
  grep -q -- '--mini-comms has no effect' "$log"
...
# --- round 3: prompts, grants, sidecars
