...
`@dcl/asset-packs`, with the SDK7 they compile down to. Verified against `@dcl/sdk` 7.25.
...
`pointerFilter: 'block'` is required on the clickable element, and the containing
...
scale). The events arrive through `triggerAreaEventsSystem`.
...
- Do better than the smart item here. Asset-packs calls `TriggerArea.setBox(entity)` with
  ...
  non-local ones in JS. `CL_MAIN_PLAYER` is the local player only, is short-circuited by
  the physics engine, and needs no filtering.
...
Systems can be prioritised: `engine.addSystem(fn, 10, 'name')`, higher runs first.
...
the tween finishes; poll it in a system.
...
`wearables`, `emotes`, `position`, `entity`) or null.

## ON_STATE_CHANGE

Fires when SET_STATE changes the value. Route every write through one function and call the
listeners there - see [SET_STATE](actions.md#set_state).
...
[Damage and health](actions.md#damage-and-health).
