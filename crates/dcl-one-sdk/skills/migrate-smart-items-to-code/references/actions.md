...
All 63 `ActionType` values from `@dcl/asset-packs`, with the SDK7 they compile down to.
Every snippet below type-checks against `@dcl/sdk` 7.25.
...
Requires a `GltfContainer` whose glb contains the named clips.
...
External hosts: see `ALLOW_MEDIA_HOSTNAMES` in SKILL.md.
...
Requires a `LightSource` on the entity.
...
`Transform.position` is relative to the parent. See `worldPosition()` in SKILL.md for
scene-space.
...
All of these are restricted actions. See the permissions section of SKILL.md.
...
Both need `ALLOW_TO_TRIGGER_AVATAR_EMOTE` and only fire while the player is inside the
scene bounds.
...
screen naming the destination domain. `OPEN_EXTERNAL_LINK` in `requiredPermissions`.
...
one function. See [triggers](triggers.md#on_counter_change).
...
The palette instantiates an editor-authored composite. `Composite.instance` exists in
`@dcl/ecs` but is marked deprecated and unsupported. Write a factory:
...
The palette's SHOW_TEXT / SHOW_IMAGE draw a screen-space overlay. SDK7 does that with
`@dcl/sdk/react-ecs`: one renderer for the whole scene, re-rendered when module state
changes. There is no imperative "show a toast" call.
...
a real function. Outside the editor, that module does not exist.
...
Not an SDK feature. It is an HTTP conversation with the Decentraland Rewards service
