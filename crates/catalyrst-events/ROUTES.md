...
- **Total upstream routes:** 29 (includes `DELETE /api/events/{event_id}`,
  which the original count of this table omitted)
- **Ported as GET (parity-target):** 11
- **Ported as writes** (federation-envelope or admin/signer-gated; see per-route
  notes below): 10
- **Deprecated upstream (410 Gone):** 3 (`ProfileSubscription/*`)
- **Still 501 (not implemented):** 1 (`DELETE /api/events/{event_id}`)
- **Ported (sitemap):** 4
- **Local addition (not upstream):** 1 (`GET /api/events/moderation`)
...
> _Repair pass 2026-07-03: Sitemap section was still marked "out of scope" despite
> `handlers/sitemap.rs` (4 real handlers, wired in `src/lib.rs`) being fully ported;
> corrected above to match `TODO.md`'s "ported" note._
