...
use catalyrst_events::content_store::ContentStore;
...
fn build_state(pool: PgPool, content_dir: std::path::PathBuf) -> AppState {
    Arc::new(AppStateInner {
        ...
        content_store: Arc::new(ContentStore::new(content_dir)),
...
async fn every_spec_route_answers_its_contract() {
    let Some(scratch) = ScratchSchema::create("CATALYRST_EVENTS_TEST_PG", "cg_events").await else {
        eprintln!("skipping events contract gate: set CATALYRST_EVENTS_TEST_PG to run");
        return;
    };
    scratch
    ...
    let created = gate
        ...
        .await;
    assert_eq!(
        created["data"]["local"]["description"],
        json!("x y"),
    ...
    let patched = gate
        ...
        .await;
    assert_eq!(
        patched["data"]["local"]["description"],
        json!("x y"),
