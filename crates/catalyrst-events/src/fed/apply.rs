//! Events federation write path (events.md §3).
//!
//! Lifecycle for a moderator action (profile-settings update / schedule upsert):
//!   1. verify the `Signed<T>` envelope (00-primitives.md §2.1) — caller,
//!   2. replay-check the nonce (§2.2) — [`crate::fed::replay`] — caller,
//!   3. authority: the signer is a local moderator (or, for a self profile
//!      edit, `target == signer`) — caller via [`crate::fed::authority`],
//!   4. append to `signed_actions_events` (dedup on signature_hash) +
//!      materialise the current view (`event_profile_settings` / `schedules_local`),
//!   5. emit the action to gossip — caller.
//!
//! This module owns step 4. `origin_peer` is None for local clients.
...
pub struct Applied {
    ...
    /// false if this signature_hash was already logged (re-delivered gossip).
    pub fresh: bool,
}
...
/// Apply a signed profile-settings update. Returns the materialised settings in
/// the upstream `ProfileSettingsAttributes` shape.
...
/// Apply a signed schedule upsert. Returns the materialised schedule in the
/// upstream `ScheduleAttributes` shape.
...
/// Materialised profile settings in the upstream `ProfileSettingsAttributes`
/// shape; returns upstream defaults when the user has no row yet.
