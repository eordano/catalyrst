...
/// Federation-signed moderator update of another user's profile settings
/// (upstream `PATCH /api/profiles/{id}/settings`). The signer must be a local
/// moderator; `target` is the address whose settings/permissions are written.
/// `PATCH /api/profiles/me/settings` reuses this with `target == signer` and is
/// open to any wallet (self-edit) — see handlers::profile_settings.
#[derive(Debug, Clone, Serialize, Deserialize)]
...
/// Federation-signed admin schedule upsert (upstream `POST /api/schedules` and
/// `PATCH /api/schedules/{id}`). The signer must be a local moderator. A `None`
/// `schedule_id` is a create (a fresh id is derived from signer+name+nonce);
/// `Some` is an update of an existing schedule.
#[derive(Debug, Clone, Serialize, Deserialize)]
...
mod tests {
    ...
    fn profile_settings_minimal_body_defaults() {
        // upstream PATCH bodies are sparse; only `target` + `signed_at` required.
