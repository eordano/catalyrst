//! Per-catalyst content-addressed image cache for poster uploads (events.md §3).
//!
//! Replaces the upstream S3 bucket: a poster is stored under its sha256 hash in
//! a sharded directory tree, deduplicated by content. The handler shapes the
//! upstream `PosterAttributes` response from the returned hash. Mirrors the
//! communities `ContentStore` layout, sized for 500 KB poster images.
...
/// Upstream `POSTER_FILE_SIZE` / `POSTER_VERTICAL_FILE_SIZE` (500 KB).
...
impl ContentStore {
    ...
    /// Store `body` content-addressed; returns its sha256 hex hash. Idempotent.
...
mod tests {
    ...
    fn tmpdir(tag: &str) -> PathBuf {
        ...
        let nanos = std::time::SystemTime::now()
            ...
            .map(|d| d.as_nanos())
            .unwrap_or(0);
        p.push(format!("evt-content-{}-{}-{}", tag, std::process::id(), nanos));
        p
    }
