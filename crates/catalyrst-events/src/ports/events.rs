...
impl EventsComponent {
    ...
    fn build_where(&self, f: &EventListFilters, binds: &mut Vec<EventBind>) -> String {
        ...
        if !f.positions.is_empty() {
            ...
            for (x, y) in &f.positions {
                ...
                clauses.push(format!("(coordinates_x = {} AND coordinates_y = {})", px, py));
            }
    ...
    pub async fn query(
        ...
        let order_clause = if let Some(s) = &f.search {
            let dir = if matches!(f.order, SortOrder::Asc) { "ASC" } else { "DESC" };
    ...
    pub async fn exists_visible(&self, event_id: &str, signer: &str) -> Result<bool, ApiError> {
        ...
        let row: Option<(bool, Option<String>, Value)> = sqlx::query_as(
            "SELECT approved, user_creator, raw FROM event WHERE id = $1",
        )
        .bind(event_id)
        .fetch_optional(&self.pool)
        .await?;
...
fn event_row_to_record(
    ...
    let user_name = raw.get("user_name").and_then(|v| v.as_str()).map(String::from);
    let estate_id = raw.get("estate_id").and_then(|v| v.as_str()).map(String::from);
    let scene_name = raw.get("scene_name").and_then(|v| v.as_str()).map(String::from);
    let estate_name = raw
        .get("estate_name")
        .and_then(|v| v.as_str())
        .map(String::from)
    ...
    let all_day = raw.get("all_day").and_then(|v| v.as_bool()).unwrap_or(false);
    let world = raw.get("world").and_then(|v| v.as_bool()).unwrap_or(false);
    let duration = r.duration_ms.or_else(|| raw.get("duration").and_then(|v| v.as_i64()));
        ...
        Some(u) => {
            local_attending.iter().any(|id| id == &r.id)
                || raw
                    ...
                    .map(|arr| {
                        arr.iter().any(|x| {
                            x.as_str().map(|s| s.eq_ignore_ascii_case(u)).unwrap_or(false)
                        })
                    })
                    .unwrap_or(false)
        }
    ...
    EventRecord {
        ...
        description: r
            .description
            .or_else(|| raw.get("description").and_then(|v| v.as_str()).map(String::from)),
        start_at: r.start_at,
        ...
        scene_name,
        approved: r.approved,
        rejected: raw.get("rejected").and_then(|v| v.as_bool()).unwrap_or(false),
        highlighted: r.highlighted,
        trending: r.trending,
        world,
        recurrent: r.recurrent,
        recurrent_frequency: raw.get("recurrent_frequency").and_then(|v| v.as_str()).map(String::from),
        recurrent_weekday_mask: raw.get("recurrent_weekday_mask").and_then(|v| v.as_i64()).unwrap_or(0),
        recurrent_month_mask: raw.get("recurrent_month_mask").and_then(|v| v.as_i64()).unwrap_or(0),
        recurrent_interval: raw.get("recurrent_interval").and_then(|v| v.as_i64()).unwrap_or(0),
        ...
        schedules: raw
            ...
            .map(|arr| arr.iter().filter_map(|x| x.as_str().map(String::from)).collect())
            .unwrap_or_default(),
        total_attendees: raw.get("total_attendees").and_then(|v| v.as_i64()).unwrap_or(0),
        ...
        live,
        attending,
        place_id: raw.get("place_id").and_then(|v| v.as_str()).map(String::from),
