...
const ENV_DOCS: &[(&str, &str)] = &[
    ...
    (
        "PLACES_API_URL",
        "places API base URL: resolves an event's place_id on create and location edits (unreachable = place_id stays null) and embeds the destination on GET /v1/events/:id (default http://127.0.0.1:5134)",
    ),
    (
        "EVENTS_BASE_URL",
