...
/// Upstream `POSTER_FILE_TYPES` (horizontal poster allows GIF).
const POSTER_FILE_TYPES: [&str; 4] = ["image/jpeg", "image/png", "image/gif", "image/webp"];
/// Upstream `POSTER_VERTICAL_FILE_TYPES` (no GIF).
...
/// Read the `poster` file field from the multipart body (upstream
/// `req.files.poster`). Rejects missing / empty / duplicate files.
...
/// Store the poster in the per-catalyst content cache and shape the upstream
/// `PosterAttributes` response (`{ filename, url, size, type }`). Replaces the
/// upstream S3 upload (events.md §3).
async fn store_and_respond(
    ...
    // Upstream filename layout was `<userHash><timeHash><ext>`; under the content
    // cache the hash is the stable address. We keep the `/<dir>/` prefix so the
    // URL shape matches what clients expect.
