...
pub struct EventListQuery {
    ...
    pub allow_deleted: Option<String>,
}
    ...
    fn from_pairs(pairs: &[(String, String)]) -> Self {
        ...
        for (k, v) in pairs {
            match k.as_str() {
                ...
                "allow_deleted" => q.allow_deleted = Some(v.clone()),
...
fn admin_token_matches(configured: Option<&str>, presented: Option<&str>) -> bool {
    let Some(token) = configured.filter(|t| !t.is_empty()) else {
        return false;
    };
    presented
...
pub(crate) fn is_admin_request(headers: &HeaderMap, state: &AppState) -> bool {
    ...
    admin_token_matches(state.admin_token.as_deref(), presented)
}
...
pub(crate) async fn list_events(
    ...
    let user = optional_user(headers, "get", signed_path).await?;
    ...
    let include_unapproved = viewer_is_admin(state, headers, user.as_deref()).await?;
    let Some(filters) = parse_filters(&q, Vec::new(), None, user, admin, include_unapproved)?
    else {
...
pub async fn post_event_search(
    ...
    let admin = is_admin_request(&headers, &state);
    let include_unapproved = viewer_is_admin(&state, &headers, user.as_deref()).await?;
    let Some(filters) = parse_filters(
        &q,
        body.place_ids,
        body.community_id,
        user,
...
pub async fn get_event(
    ...
    let evt = load_event_for_viewer(&state, &headers, &event_id, &path).await?;
    Ok(Json(ApiOk::new(evt)))
}

fn is_soft_deleted(evt: &EventRecord) -> bool {
...
pub(crate) fn visible_to_viewer(evt: &EventRecord, viewer: Option<&str>, admin: bool) -> bool {
    ...
    let owner =
        matches!((viewer, evt.user.as_deref()), (Some(v), Some(u)) if v.eq_ignore_ascii_case(u));
    if owner {
        return true;
    }
...
pub(crate) async fn load_event_for_viewer(
    state: &AppState,
    headers: &HeaderMap,
    event_id: &str,
    signed_path: &str,
...
mod envelope_tests {
    ...
    fn plain_query_serializes_flat_list() {
        ...
        let q = EventListQuery::from_pairs(&pairs);
        let filters = parse_filters(&q, Vec::new(), None, None, false, false)
...
#[cfg(test)]
mod admin_auth_probe_tests {
    use super::{admin_token_matches, timing_safe_eq};
    ...
    fn admin_auth_probe_empty_configured_token_must_reject() {
        ...
        assert!(!admin_token_matches(Some(""), Some("anything")));
    }

    #[test]
    fn admin_auth_probe_empty_presented_vs_set_secret_rejects() {
        assert!(!admin_token_matches(Some("the-real-token"), Some("")));
        assert!(!admin_token_matches(Some("the-real-token"), None));
