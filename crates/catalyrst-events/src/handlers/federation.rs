...
/// True when the JSON body is a `Signed<T>` envelope (`{ domain, message,
/// signature, ... }`).
...
/// Verify envelope signature + auth-chain cross-check + skew + domain + replay.
/// Returns the recovered wallet signer.
pub async fn preflight<T: TypedMessage + DeserializeOwned>(
    state: &AppState,
    headers: &HeaderMap,
    body: Value,
) -> Result<(Signed<T>, String), ApiError> {
    let signed: Signed<T> = serde_json::from_value(body)
...
/// Best-effort gossip of a locally-applied signed events action. Never fails the
/// request; the action is durable in `signed_actions_events`.
...
mod tests {
    ...
    fn rejects_legacy_or_partial_bodies() {
        ...
        assert!(!is_federation_envelope(&json!({ "domain": {}, "message": {} })));
