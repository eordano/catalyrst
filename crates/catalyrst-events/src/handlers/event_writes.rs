...
pub struct UpdateEventBody {
    ...
    pub world: Option<Option<bool>>,
    #[serde(default)]
    pub community_id: Option<String>,
    #[serde(default, deserialize_with = "deserialize_some")]
    #[schema(value_type = Option<String>)]
    pub featured_item: Option<Option<String>>,
    #[serde(default)]
...
fn normalize_featured_item(value: Option<&str>) -> Result<Option<String>, ApiError> {
    match value.filter(|v| !v.is_empty()) {
        Some(urn) => {
            validate_featured_item(urn).map_err(ApiError::bad_request)?;
            Ok(Some(urn.to_owned()))
        }
        None => Ok(None),
    }
}
...
fn validate_create(body: &CreateEventBody) -> Result<DateTime<Utc>, ApiError> {
    ...
    if let Some(u) = &body.url {
        validate_event_url(u).map_err(ApiError::bad_request)?;
    }
    if let Some(img) = &body.image {
        sanitize_asset_url(img).map_err(ApiError::bad_request)?;
    }
    if let Some(img) = &body.image_vertical {
        sanitize_asset_url(img).map_err(ApiError::bad_request)?;
    }
    normalize_featured_item(body.featured_item.as_deref())?;
    if body.duration < 0 {
...
fn build_create_raw(
    ...
    let attendance: Value = json!({
        ...
        "featured_item": body.featured_item.as_deref().filter(|v| !v.is_empty()),
...
async fn resolve_location(places: &Places, raw: &mut Map<String, Value>) {
    ...
    let (place_id, world) = places
        .resolve_location(world, server.as_deref(), x, y)
        .await;
    raw.insert(
        "place_id".into(),
        place_id.map(Value::String).unwrap_or(Value::Null),
    );
    raw.insert("world".into(), json!(world));
}
...
fn featured_item_changed(raw: &Map<String, Value>, body: &UpdateEventBody) -> bool {
    let Some(next) = &body.featured_item else {
        return false;
    };
    let next = next.as_deref().filter(|v| !v.is_empty());
    let current = raw.get("featured_item").and_then(|v| v.as_str());
    next != current
}
...
fn apply_owner_edit(raw: &mut Map<String, Value>, body: &UpdateEventBody) -> Result<(), ApiError> {
    ...
    if let Some(img) = &body.image {
        let img = sanitize_asset_url(img).map_err(ApiError::bad_request)?;
        raw.insert("image".into(), json!(img));
    }
    if let Some(img) = &body.image_vertical {
        let img = sanitize_asset_url(img).map_err(ApiError::bad_request)?;
        raw.insert("image_vertical".into(), json!(img));
    }
    if let Some(server) = &body.server {
        raw.insert("server".into(), json!(server));
    }
    ...
    if let Some(item) = &body.featured_item {
        let item = normalize_featured_item(item.as_deref())?;
        raw.insert("featured_item".into(), json!(item));
    }
    set_opt_bool(raw, "all_day", body.all_day);
    set_opt_bool(raw, "recurrent", body.recurrent);
    if let Some(world) = body.world {
        raw.insert("world".into(), json!(world.unwrap_or(false)));
    }
...
pub async fn patch_event(
    ...
    if was_approved && (content_touched(&body) || featured_item_changed) {
        raw.insert("approved".into(), json!(false));
    }
    raw.insert("updated_at".into(), json!(Utc::now().to_rfc3339()));
