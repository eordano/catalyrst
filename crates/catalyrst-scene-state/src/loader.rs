//! Scene load/reload control. Port of `loadOrReload` in
//! `src/controllers/handlers/debugging-handler.ts`.
//!
//! Loading a scene: stop+drop any existing scene under that name, acquire the
//! source (local or world), construct a runtime, and register it. Upstream then
//! kicks off the scene's `onStart`/`onUpdate` game loop; here we build a
//! [`RelayRuntime`] (the JS game loop is the deferred state-core — see
//! `runtime.rs`).
...
pub async fn load_or_reload(state: &AppState, name: &str) -> Result<()> {
    if let Some(prev) = state.scenes.remove(name) {
        ...
        // The JsRuntime aborts its onUpdate loop + tears down the isolate when
        // the Scene (and thus the Arc<dyn SceneRuntime> -> JsRuntimeHandle) is
        // dropped (see JsRuntimeHandle::drop -> shutdown). Drop it explicitly.
        drop(prev);
    }
    ...
    // Default: run the scene's real game.js inside V8 (JsRuntime). The scene
    // declares its own entity-range policy via registerScene(...). When V8 is
    // explicitly disabled (DISABLE_JS_RUNTIME=1) or the source is empty, fall
    // back to the scene-logic-free RelayRuntime with the default 512/512/512
    // policy.
    let runtime: Arc<dyn SceneRuntime> = if state.cfg.disable_js_runtime || source.trim().is_empty()
    {
        ...
        Arc::new(JsRuntime::new(hash.clone(), source, realm, limits, static_crdt))
    };
