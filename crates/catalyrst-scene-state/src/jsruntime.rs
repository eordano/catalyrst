...
impl HostState {
    ...
    ) -> (Arc<Mutex<CrdtEngine>>, Arc<Mutex<Vec<u8>>>, Arc<Mutex<ServerTransportConfig>>) {
        Self::with(isolate, |c| {
            let h = c.borrow();
            (
                Arc::clone(&h.engine),
...
fn build_system_module<'s>(
    ...
    match module {
        ...
        "RestrictedActions" => {
            for m in [
                "movePlayerTo", "teleportTo", "triggerEmote", "triggerSceneEmote",
                "openExternalUrl", "openNftDialog", "changeRealm",
            ] {
...
mod tests {
    ...
    fn scene_onstart_writes_to_engine() {
        ...
        let handle = spawn("test-scene".into(), js, "dcl-test".into(), RuntimeLimits::default(), Vec::new());
        let ok = wait_for(|| {
    ...
    fn scene_relays_client_messages_via_observer() {
        ...
        let handle = spawn("echo-scene".into(), js, "dcl-test".into(), RuntimeLimits::default(), Vec::new());
    ...
    fn client_close_reclaims_range() {
        ...
        "#
        .to_string();
        let handle = spawn("close-scene".into(), js, "dcl-test".into(), RuntimeLimits::default(), Vec::new());
        ...
        handle
            .tx
            .send(Command::ClientCrdt { index, body })
            .unwrap();
    ...
    fn infinite_onupdate_is_terminated_and_shutdown_does_not_hang() {
        ...
        let handle = spawn("spin-scene".into(), js, "dcl-test".into(), limits, Vec::new());
        ...
        assert!(stopped, "watchdog should have terminated the infinite onUpdate");
