...
impl CrdtEngine {
    ...
    // Tombstones mask late ops for deleted ids; unbounded DELETE_ENTITY streams
    // from an authed client would otherwise grow this set forever. FIFO-evict past
    // MAX_DELETED_ENTITIES like upstream: a stale PUT for an evicted id becomes a
    // ghost bounded by max_components — the accepted trade-off.
