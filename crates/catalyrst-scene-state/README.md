...
by default — **note:** as of 2026-07-03 this crate has no
`umbrella/env/catalyrst-scene-state.env` and is not currently deployed; port
5153 is already live-bound by Metabase (`umbrella/env/metabase.env`,
`MB_JETTY_PORT=5153`). Pick an unused port from the deployment's port map
before wiring this service up.
