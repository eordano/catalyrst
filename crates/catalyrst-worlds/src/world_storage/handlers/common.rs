...
pub fn parse_pagination(params: &HashMap<String, String>) -> Result<Pagination, ApiError> {
    let pairs: Vec<(String, String)> = ["limit", "offset"]
        .into_iter()
        .filter_map(|k| params.get(k).map(|v| (k.to_string(), v.clone())))
        .collect();
    let page = catalyrst_types::get_pagination_params(&pairs, MAX_LIMIT);
...
mod tests {
    ...
    #[test]
    fn pagination_ignores_a_page_parameter() {
        let p = parse_pagination(&params(&[("limit", "10"), ("page", "3")])).unwrap();
        assert_eq!(
            (p.limit, p.offset),
            (10, 0),
            "world-storage listing paginates on limit/offset only"
        );
    }
