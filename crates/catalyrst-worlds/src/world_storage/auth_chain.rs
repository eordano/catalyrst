...
pub use catalyrst_crypto::signed_fetch::{
    ...
    PayloadOverPaths, AUTH_CHAIN_HEADER_PREFIX, AUTH_METADATA_HEADER, AUTH_TIMESTAMP_HEADER,
...
pub fn check_freshness(
    ...
    catalyrst_crypto::check_symmetric_skew(timestamp, now, expiration_secs)
}
...
pub async fn verify_request(
    ...
    let signer = match v6
        .validate(|payload| {
            validate_signature(&chain, payload, &ts, ONE_MINUTE, now, eip1654_validator)
        })
        .await
    {
        Ok(signer) => signer,
