...
/// A scene-signed request (`metadata.signer == "decentraland-kernel-scene"`) must never
/// reach a player-only surface. Signature verification checks only the signed bytes, so a
/// mixed-case spelling verifies yet would slip past a case-sensitive gate; match
/// case-insensitively, exactly like the sibling in `auth_chain::require_verified`.
fn is_kernel_scene_signer(signer: Option<&str>) -> bool {
    signer
        .map(|s| s.eq_ignore_ascii_case(crate::auth_chain::KERNEL_SCENE_SIGNER))
        .unwrap_or(false)
}
...
pub async fn verify_request(
    ...
    if is_kernel_scene_signer(metadata.signer.as_deref()) {
...
mod tests {
    ...
    #[test]
    fn scene_signer_rejection_is_case_insensitive() {
        assert!(is_kernel_scene_signer(Some("decentraland-kernel-scene")));
        assert!(is_kernel_scene_signer(Some("Decentraland-Kernel-Scene")));
        assert!(is_kernel_scene_signer(Some("DECENTRALAND-KERNEL-SCENE")));
        assert!(!is_kernel_scene_signer(Some("0xabc")));
        assert!(!is_kernel_scene_signer(None));
    }
