        ...
        match self {
            ...
            Self::ZeroGossipPubkey { peer_id } => write!(
                ...
                "peer {peer_id}: gossip_pubkey is 32 zero bytes, which is a placeholder and not \
                 a key. This slice reads the key for nothing else — there is no signed channel \
            ...
            Self::PinnedRootOnCleartextUrl { peer_id, url } => write!(
                ...
                "peer {peer_id}: mtls_root_pem is set, but worlds_url is cleartext ({url}), so \
                 the pinned root authenticates nothing — there is no TLS handshake for it to \
...
/// A peer that is in the file, is valid, and is simply not a *worlds* peer.
///
...
impl AdmittedPeer {
    ...
    pub fn admit(
        ...
        // --- 1. dao_proposal is still the template -------------------------------
        ...
        // --- 3. gossip_pubkey is 32 zero bytes -----------------------------------
        //
        ...
        // --- 4. peer_id is a reserved example name -------------------------------
        ...
        // --- 5. no worlds_url: still needs a pinned root, then omit --------------
        ...
        if !url.username().is_empty() || url.password().is_some() {
            tracing::warn!(
                peer_id = %canonical_id,
                "worlds_url carries userinfo; dropping it — worlds federation sends no \
        ...
        if url.query().is_some() || url.fragment().is_some() {
            tracing::warn!(
                peer_id = %canonical_id,
                "worlds_url carries a query or fragment; dropping it — the request path is \
        ...
        // --- 7. the pinned root, which IS the admission decision ----------------
