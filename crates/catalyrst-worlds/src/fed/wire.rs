...
mod tests {
    ...
    /// §3's grep gate, run as a test so it runs wherever tests run.
    #[test]
                ...
                if rel.contains("fed/") && !rel.ends_with(local_ctor_home) {
                    assert!(
                        ...
                        "{rel}:{} mints a local world name inside the federation module. \
                         ...
                         this constructor — and a peer-reported name reaching it is the \
                ...
                for table in ["worlds", "world_scenes", "world_permissions"] {
                    ...
                    assert!(
                        ...
                        "fed/{name}:{} writes to the authoritative table `{table}`. \
                         The mirror path writes zero columns of it — see migration 0005.",
                        i + 1
                    );
                }
            }
