...
#[cfg_attr(feature = "ts", derive(ts_rs::TS), ts(export, export_to = "worlds/"))]
#[serde(rename_all = "camelCase")]
pub struct ServerAboutConfigurations {
    #[cfg_attr(feature = "ts", ts(type = "number"))]
    pub network_id: i64,
    pub realm_name: String,
    pub global_scenes_urn: Vec<String>,
...
pub struct ServerAboutResponse {
    pub healthy: bool,
    pub accepting_users: bool,
    pub configurations: ServerAboutConfigurations,
    pub content: AboutContentStatus,
    pub lambdas: AboutServiceStatus,
    pub comms: AboutComms,
    #[cfg_attr(feature = "ts", ts(type = "Record<string, unknown>"))]
...
pub async fn get_server_about(State(state): State<AppState>) -> Json<ServerAboutResponse> {
    ...
    Json(server_about(
        &cfg.http_base_url,
        cfg.network_id,
        &cfg.lambdas_public_url,
        cfg.global_scenes_urn.as_deref(),
        db_ok,
        state.sfu.is_alive(),
    ))
}
...
fn server_about(
    base_url: &str,
    network_id: i64,
    lambdas_public_url: &str,
    global_scenes_urn: Option<&str>,
    db_ok: bool,
    sfu_alive: bool,
) -> ServerAboutResponse {
    ServerAboutResponse {
        healthy: db_ok,
        accepting_users: db_ok,
        configurations: ServerAboutConfigurations {
            network_id,
            realm_name: host_of(base_url).to_string(),
            global_scenes_urn: global_scenes_urn
                .map(|s| s.split_whitespace().map(|x| x.to_string()).collect())
                .unwrap_or_default(),
            scenes_urn: Vec::new(),
        },
        ...
        lambdas: AboutServiceStatus {
            healthy: true,
            public_url: lambdas_public_url.to_string(),
        },
        comms: AboutComms {
            healthy: sfu_alive,
            protocol: "v3".to_string(),
            adapter: "offline:offline".to_string(),
        },
        catalyrst: catalyrst_extensions(base_url),
    }
}
...
#[cfg(test)]
mod tests {
    use super::{host_of, server_about};
    use super::{is_locally_published, resolve_fixed_adapter, resolve_realm_name, ZERO_ADDRESS};
    use crate::ports::worlds::WorldScene;
    use serde_json::json;

    #[test]
    fn the_server_about_names_this_server_as_the_content_server() {
        let about = server_about(
            "https://worlds.example.org",
            1,
            "https://peer.example.org/lambdas",
            Some("urn:a  urn:b"),
            true,
            false,
        );
        let v = serde_json::to_value(&about).unwrap();
        assert_eq!(v["content"]["publicUrl"], "https://worlds.example.org");
        assert_eq!(v["content"]["healthy"], true);
        assert_eq!(
            v["lambdas"]["publicUrl"],
            "https://peer.example.org/lambdas"
        );
        assert_eq!(v["configurations"]["realmName"], "worlds.example.org");
        assert_eq!(v["configurations"]["networkId"], 1);
        assert_eq!(
            v["configurations"]["globalScenesUrn"],
            serde_json::json!(["urn:a", "urn:b"])
        );
        assert_eq!(v["configurations"]["scenesUrn"], serde_json::json!([]));
        assert_eq!(v["comms"]["adapter"], "offline:offline");
        assert_eq!(v["comms"]["healthy"], false);
        assert_eq!(v["healthy"], true);
        assert_eq!(v["acceptingUsers"], true);
        assert_eq!(
            v["catalyrst"]["sceneAdapter"]["publicUrl"],
            "https://worlds.example.org/get-scene-adapter"
        );

        let down = server_about("http://127.0.0.1:5139", 1, "x", None, false, true);
        assert!(!down.healthy && !down.accepting_users && !down.content.healthy);
        assert!(down.comms.healthy);
        assert_eq!(down.configurations.realm_name, "127.0.0.1:5139");
        assert!(down.configurations.global_scenes_urn.is_empty());

        assert_eq!(host_of("http://127.0.0.1:5139/x?y#z"), "127.0.0.1:5139");
        assert_eq!(host_of("worlds.example.org"), "worlds.example.org");
    }
