...
async fn mint(
    ...
    let auth = require_verified(headers, "post", path, EXPLORER_METADATA_KEYS)
        .await
    ...
    let rate_limit_key = format!("{}:{}", world_name, subject);

    if is_shared_secret && state.rate_limiter.is_rate_limited(&rate_limit_key) {
    ...
    if !allowed {
        if is_shared_secret {
            state.rate_limiter.record_failed_attempt(&rate_limit_key);
            if state.rate_limiter.is_rate_limited(&rate_limit_key) {
                return Err(ApiError::too_many(
                    "Too many shared-secret attempts. Try again later.",
                    RATE_LIMIT_WINDOW_SECONDS,
                ));
