...
async fn mint(
    ...
    let access = state.worlds.get_access(world_name).await?;
    ...
    if is_shared_secret && state.rate_limiter.is_rate_limited(world_name, &subject).await {
    ...
    let allowed = access.check_access(&identity, secret.as_deref());
    if !allowed {
        if is_shared_secret {
            let tripped = state
                .rate_limiter
    ...
    if is_shared_secret {
        state.rate_limiter.clear_attempts(world_name, &subject).await;
    }
