...
fn skipped(property: &str) {
    eprintln!(
        "SKIPPED (no CATALYRST_WORLDS_TEST_PG): unverified — {property}. \
...
async fn an_unauthenticated_refresh_is_refused_before_any_peer_is_contacted() {
    let Some(scratch) = setup_db().await else {
        return skipped(
            "an unauthenticated POST to the refresh route reaches zero peers — the \
    ...
    // above would be satisfied by a route that simply never works.
...
/// produces. Same defect class as the zod laundering in `sites/wcs.ts`: a failure
