...
fn skipped(property: &str) {
    eprintln!(
        "SKIPPED-AUDIT (no CATALYRST_WORLDS_TEST_PG): unverified — {property}. \
...
// revocation mechanism is a restart (§2.5: "registry reload at runtime: not built ...
...
async fn a_revoked_peers_worlds_stop_being_published_and_the_two_routes_agree() {
    ...
    // than simply emptying the table.
...
async fn the_sweep_spares_an_unconfigured_server_and_spares_a_local_veto() {
    ...
    assert!(
        rows.is_empty() && total == 0,
        "with no adjudicated allowlist, nothing is publishable — that is why retaining \
         the rows is safe"
    );
...
async fn the_veto_route_authorizes_before_any_extractor_can_answer() {
    ...
    for (label, content_type, body) in [
        ...
        (
            "well-formed and valid — the one that was already 403",
            Some("application/json"),
            r#"{"hidden":true}"#,
        ),
    ] {
...
//
// Before the boot sweep existed that was harmless: federation simply did not happen
...
async fn a_peer_omitted_for_having_no_worlds_url_is_swept_as_an_omission_not_a_de_admission() {
    ...
    // Same entry, still adjudicated, simply not a worlds peer any more.
    ...
    assert_eq!(
        ...
        1,
        "the rows go, which is correct — an omitted peer is not a worlds peer"
    );
    assert_eq!(
...
fn only_our_own_database_text_is_withheld_from_the_published_reason() {
    ...
    assert!(
        ...
        "Display stays verbatim — the log line is the whole point of keeping it"
    );
