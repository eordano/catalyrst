...
fn skipped(invariant: &str) {
    eprintln!(
        "SKIPPED-INVARIANT {}: not verified on this run — {}",
...
async fn every_forbidden_key_from_a_hostile_peer_payload_is_rejected_by_the_database() {
    ...
    for (col, val) in [
        ...
        let err = sqlx::query(sqlx::AssertSqlSafe(sql))
            ...
            .await
            .expect_err(&format!(
                "INSERT naming `{col}` succeeded against remote_worlds — a peer-supplied \
...
async fn an_acl_row_cannot_attach_to_a_remote_only_world() {
    ...
    .await
    .expect_err(
        "an ACL row was created against a world that exists only as a peer's claim — \
