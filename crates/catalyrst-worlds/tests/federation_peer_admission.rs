...
fn unset_peers_file_is_not_configured_and_is_not_an_error() {
    ...
    assert!(
        !peers.is_configured(),
        "NotConfigured must never report itself as configured — the two states are the \
...
fn zero_worlds_peers_is_configured_with_an_empty_list_not_not_configured() {
    ...
    assert!(
        ...
        "'federation was requested and yielded nothing' must be DISTINGUISHABLE from \
         'federation was never requested' — the routes answer 200 with an empty list in one \
...
async fn pinned_client_trusts_only_its_own_root() {
    ...
    // And the inverse, so the test cannot pass by the server simply being broken.
    ...
    let err = match result {
        ...
        Ok(resp) => panic!(
            "a client pinned to a PRIVATE throwaway root successfully reached {WEBPKI_HOST} \
             ...
             pinned root is not an admission decision — any WebPKI-valid host that wins a \
...
mod tls {
    ...
    async fn spawn(ca: &Arc<Ca>, response: String) -> Server {
        ...
        let leaf = params.signed_by(&leaf_key, &ca.cert, &ca.key).unwrap();

        let cfg = rustls::ServerConfig::builder()
            .with_no_client_auth()
            .with_single_cert(
                vec![leaf.der().clone(), ca.cert.der().clone()],
