...
pub type GenerateFn = Arc<
    dyn Fn(f64, f64) -> futures::future::BoxFuture<'static, Result<Vec<SnapshotMetadata>, Box<dyn std::error::Error + Send + Sync>>>
        + Send
        + Sync,
>;
...
pub async fn generate_snapshots_multi(
    ...
    for interval in intervals {
        let saved = snapshots_repository::find_snapshots_strictly_contained_in_time_range(
            pool, interval,
        )
        .await?;
...
mod tests {
    ...
    fn division_matches_reference_shape() {
        ...
        let (intervals, remainder) = divide_time_in_years_months_weeks_and_days(TimeRange::new(init, end));

        assert!(!intervals.is_empty());
        let mut prev_end = init;
        for iv in &intervals {
            assert!((iv.init_timestamp - prev_end).abs() < 1.0, "intervals must be contiguous");
        ...
        assert!((first_span - MS_PER_YEAR).abs() < 1.0, "first interval should be a year");
    }
    ...
    fn division_genesis_to_fixed_end_exact_counts() {
        ...
        assert_eq!(count_of(MS_PER_MONTH), 1, "expected exactly 1 month interval");
    ...
    fn entity_on_shared_boundary_is_counted_in_both_windows_inclusive() {
        ...
        let only_first = intervals
            ...
            .count();
        assert_eq!(only_first, 1, "a non-boundary entity is in exactly one window");
    }
}
