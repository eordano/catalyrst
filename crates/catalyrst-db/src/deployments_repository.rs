...
pub async fn deployment_exists(pool: &PgPool, entity_id: &str) -> Result<bool, sqlx::Error> {
    let row: Option<(i32,)> = sqlx::query_as(
        "SELECT 1 FROM deployments WHERE entity_id = $1",
    )
    .bind(entity_id)
    .fetch_optional(pool)
    .await?;
...
pub async fn stream_all_distinct_entity_ids(pool: &PgPool) -> Result<Vec<String>, sqlx::Error> {
    let rows: Vec<(String,)> =
        sqlx::query_as("SELECT DISTINCT entity_id FROM deployments")
            .fetch_all(pool)
            .await?;
...
pub async fn save_deployment(
    ...
    let wrapped_metadata: Option<serde_json::Value> = metadata.map(|m| {
        serde_json::json!({ "v": m })
    });
...
pub async fn batch_save_deployments(
    ...
    for d in deployments {
        ...
        entity_metadatas.push(d.entity_metadata.as_ref().map(|m| serde_json::json!({ "v": m })));
    ...
    let pointer_jsons: Vec<serde_json::Value> = deployments
        .iter()
        .map(|d| serde_json::Value::Array(
            d.entity_pointers.iter().map(|p| serde_json::Value::String(p.clone())).collect()
        ))
        .collect();
