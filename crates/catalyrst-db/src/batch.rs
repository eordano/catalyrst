...
impl BatchDeploymentPipeline {
    ...
    pub async fn flush(&mut self) -> Result<(), sqlx::Error> {
        ...
        for d in &self.pending_deployments {
            ...
            entity_metadatas
                .push(d.entity_metadata.as_ref().map(|m| serde_json::json!({ "v": m })));
        ...
        if !generated_ids.is_empty() {
            ...
            let batch_entity_types: Vec<&str> = self.pending_deployments.iter().map(|d| d.entity_type.as_str()).collect();
            let batch_entity_ids: Vec<&str> = self.pending_deployments.iter().map(|d| d.entity_id.as_str()).collect();
            let batch_timestamps: Vec<f64> = self.pending_deployments.iter().map(|d| d.entity_timestamp_ms).collect();
        ...
        debug!("Batch flush complete: {} deployments committed", deployment_count);
