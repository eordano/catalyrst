...
impl SquidBlockchainChecker {
    ...
    async fn third_party_root_from_squid(
        ...
        .map_err(|e| ValidatorError::BlockchainQuery(format!("third-party root query failed: {e}")))?;
...
async fn check_parcel_access(pool: &PgPool, address: &str, x: i32, y: i32) -> Result<bool, ValidatorError> {
    let parcel_owner: Option<String> = sqlx::query_scalar(
        "SELECT owner_id FROM squid_marketplace.parcel WHERE x = $1 AND y = $2"
    )
    .bind(x)
    .bind(y)
    .fetch_optional(pool)
...
async fn check_name_ownership(pool: &PgPool, address: &str, name: &str) -> Result<bool, ValidatorError> {
    let owner: Option<String> = sqlx::query_scalar(
        "SELECT owner_id FROM squid_marketplace.ens WHERE subdomain = $1"
    )
    .bind(name)
    .fetch_optional(pool)
    .await
...
async fn check_collection_access_query(
    ...
    let row = match row {
        ...
        None => {
            ...
            .fetch_optional(pool)
            .await
            .map_err(|e| ValidatorError::BlockchainQuery(format!("collection query (ci) failed: {e}")))?;
...
async fn check_nft_ownership(pool: &PgPool, address: &str, urn: &str) -> Result<bool, ValidatorError> {
...
impl BlockchainChecker for SquidBlockchainChecker {
    ...
    async fn check_third_party_access(
        ...
        if !self.tp_root_via_squid && self.tp_subgraph.is_none() {
            warn!(asset_urn, "no third-party root source configured; rejecting (fail-closed)");
            return Ok(false);
        }
        ...
        let root = if self.tp_root_via_squid {
            ...
            warn!(asset_urn, "could not resolve L2 block for registry-subgraph root lookup");
            None
        };

        let Some(root) = root else {
            warn!(tp_id, ?block, "third-party not approved or root unavailable");
            return Ok(false);
        };
...
mod tests {
    ...
    async fn decentraland_address_check() {
        ...
        assert!(checker.is_address_owned_by_decentraland(
