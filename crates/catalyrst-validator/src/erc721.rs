...
fn extract_body_shapes(metadata: Option<&serde_json::Value>) -> Vec<String> {
    let data = metadata.and_then(|m| {
        m.get("data")
            .or_else(|| m.get("emoteDataADR74"))
    });

    let representations = data.and_then(|d| d.get("representations")).and_then(|r| r.as_array());
...
mod tests {
    ...
    fn format_minimal_entity() {
        ...
        assert!(result.attributes.iter().any(|a| a.trait_type == "Rarity" && a.value == "epic"));
        assert!(result.attributes.iter().any(|a| a.trait_type == "Category" && a.value == "hat"));
    }
}
