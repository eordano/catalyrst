...
pub async fn validate_profile_access(
    ...
    if !is_valid_eth_address(&pointer) {
        return ValidationResponse::fail("The given pointer is not a valid ethereum address.".to_string());
    }
    ...
    if let Some(metadata) = &entity.metadata {
        ...
        if !item_urns.is_empty() {
                ...
                .await
            {
                ...
                Err(e) => {
                    return ValidationResponse::fail(format!(
                        "Error checking item ownership: {e}"
                    ));
                }
            }
        }
...
mod tests {
    ...
    fn eth_address_validation() {
        ...
        assert!(!is_valid_eth_address("1234567890abcdef1234567890abcdef12345678"));
    }
