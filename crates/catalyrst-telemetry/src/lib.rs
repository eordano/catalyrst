...
pub async fn build_state(cfg: &Config) -> Result<AppState> {
    ...
    if let Ok(Some((v,))) =
        sqlx::query_as::<_, (String,)>("SELECT value FROM admin_settings WHERE key = 'ingest_enabled'")
            .fetch_optional(&pool)
            .await
    {
...
pub fn api_router() -> Router<AppState> {
    Router::new()
        ...
        .route("/dash/issue/state", post(handlers::dashboard::set_issue_state))
        ...
        .route("/dash/admin/bulk-delete", post(handlers::admin::bulk_delete))
