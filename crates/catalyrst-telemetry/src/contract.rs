...
pub fn load_from_env() -> Option<Arc<Contract>> {
    let path = match std::env::var("TELEMETRY_CONTRACT_PATH") {
        ...
        _ => {
            tracing::warn!(
                "TELEMETRY_CONTRACT_PATH unset — telemetry contract validation DISABLED (accepting all events)"
            );
            return None;
        }
    };
    match std::fs::read_to_string(&path) {
        Ok(text) => match serde_json::from_str::<Contract>(&text) {
            Ok(contract) => {
                tracing::info!(
                    ...
                    "telemetry contract loaded — validation ENABLED"
                );
            ...
            Err(err) => {
                tracing::warn!(
                    path = %path,
                    error = %err,
                    "telemetry contract unparseable — validation DISABLED (accepting all events)"
                );
                None
            }
        },
        Err(err) => {
            tracing::warn!(
                path = %path,
                error = %err,
                "telemetry contract file missing/unreadable — validation DISABLED (accepting all events)"
            );
            None
        }
    }
