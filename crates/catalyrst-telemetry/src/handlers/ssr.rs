...
fn parse_filters(q: &[(String, String)], surface: Surface) -> Filters {
    ...
    Filters {
        ...
        kind: query_get(q, "kind").or_else(|| {
            if surface == Surface::Errors { Some("event".to_string()) } else { None }
        }),
        env: query_get(q, "env"),
        release: query_get(q, "release"),
        tag: query_get(q, "tag"),
        sort: query_get(q, "sort").or_else(|| {
            Some(if metrics { "frequent" } else { "recent" }.to_string())
        }),
        status: query_get(q, "status").or_else(|| {
            Some(if metrics { "all" } else { "unresolved" }.to_string())
        }),
    }
}
...
fn qs(f: &Filters, surface: Surface, fingerprint: &Option<String>, extra: &[(&str, String)]) -> String {
    let metrics = surface == Surface::Metrics;
    let mut p: Vec<(String, String)> = Vec::new();
    p.push(("hours".into(), f.hours.to_string()));
    p.push(("source".into(), if metrics { "segment" } else { "sentry" }.into()));
    if let Some(v) = nonempty(&f.q) { p.push(("q".into(), v)); }
    if let Some(v) = nonempty(&f.level) { p.push(("level".into(), v)); }
    if let Some(v) = nonempty(&f.kind) { p.push(("kind".into(), v)); }
    if let Some(v) = nonempty(&f.env) { p.push(("environment".into(), v)); }
    if let Some(v) = nonempty(&f.release) { p.push(("release".into(), v)); }
    if let Some(v) = nonempty(&f.tag) { p.push(("tag".into(), v)); }
    if let Some(v) = nonempty(&f.sort) {
        if v != "recent" { p.push(("sort".into(), v)); }
    }
    if !metrics {
        if let Some(v) = nonempty(&f.status) {
            if v != "all" { p.push(("status".into(), v)); }
        }
    }
    if let Some(fp) = nonempty(fingerprint) { p.push(("fingerprint".into(), fp)); }
    for (k, v) in extra { p.push((k.to_string(), v.clone())); }
    p.iter()
...
pub async fn page(State(st): State<AppState>, OriginalUri(uri): OriginalUri) -> Html<String> {
    ...
    let raw_query: Vec<(String, String)> = uri
        .query()
        .map(parse_query)
        .unwrap_or_default();
    ...
    match route.surface {
        Surface::Errors => {
            ...
            cache.insert(
                format!("/dash/stats?{}", qs(&filters, route.surface, &route.fingerprint, &[])),
                stats.clone(),
            );
            ...
            if let Some(fp) = &route.fingerprint {
                ...
                cache.insert(
                    format!(
                        "/dash/events?{}",
                        qs(&filters, route.surface, &route.fingerprint, &[("group", "1".into()), ("limit", "1".into())])
                    ),
                    issue.clone(),
                );
                let meta = issue
                ...
                let events = call_events(
                    ...
                    route.surface,
                    &route.fingerprint,
                    &[("group", "0".into()), ("limit", "100".into()), ("offset", "0".into())],
                )
                .await;
                cache.insert(
                    format!(
                        "/dash/events?{}",
                        qs(&filters, route.surface, &route.fingerprint, &[("group", "0".into()), ("limit", "100".into()), ("offset", "0".into())])
                    ),
                    events.clone(),
                );
                ...
                let events = call_events(
                    ...
                    route.surface,
                    &None,
                    &[("group", group.into()), ("limit", "100".into()), ("offset", "0".into())],
                )
                .await;
                cache.insert(
                    format!(
                        "/dash/events?{}",
                        qs(&filters, route.surface, &None, &[("group", group.into()), ("limit", "100".into()), ("offset", "0".into())])
                    ),
                    events.clone(),
                );
        ...
        Surface::Metrics if matches!(route.tab, "funnel" | "breakdown") => {
            ...
            cache.insert(format!("/dash/metrics?hours={}", filters.hours), metrics.clone());
    ...
    html = html.replace("<!--SSR:tableview-style-->", if panel_surface { "display:none" } else { "" });
    html = html.replace("<!--SSR:panel-style-->", if panel_surface { "display:block" } else { "display:none" });
    html = html.replace("<!--SSR:stats-style-->", if no_chrome { "display:none" } else { "" });
    html = html.replace("<!--SSR:toolbar-style-->", if no_chrome { "display:none" } else { "" });
    html = html.replace("<!--SSR:ihead-show-->", if ihead_show { "show" } else { "" });
...
async fn call_stats(st: &AppState, f: &Filters, fp: &Option<String>, source: &str) -> Value {
    ...
    if let Some(fp) = nonempty(fp) { body["fingerprint"] = json!(fp); }
...
async fn call_events(
    ...
    let source = if surface == Surface::Metrics { "segment" } else { "sentry" };
    let mut body = json!({ "hours": f.hours, "source": source });
    // Mirror qs() field names so the synthesized ListParams matches the request.
    if let Some(v) = nonempty(&f.q) { body["q"] = json!(v); }
    if let Some(v) = nonempty(&f.level) { body["level"] = json!(v); }
    if let Some(v) = nonempty(&f.kind) { body["kind"] = json!(v); }
    if let Some(v) = nonempty(&f.env) { body["environment"] = json!(v); }
    if let Some(v) = nonempty(&f.release) { body["release"] = json!(v); }
    if let Some(v) = nonempty(&f.tag) { body["tag"] = json!(v); }
    if let Some(v) = nonempty(&f.sort) { if v != "recent" { body["sort"] = json!(v); } }
    if let Some(v) = nonempty(&f.status) { if v != "all" { body["status"] = json!(v); } }
    if let Some(v) = nonempty(fp) { body["fingerprint"] = json!(v); }
    for (k, v) in extra {
...
async fn call_flags(st: &AppState) -> Value {
    dashboard::flags(State(st.clone())).await.map(|j| j.0).unwrap_or(json!({}))
}
...
fn metrics_total(m: &Value, hours: i64) -> String {
    ...
    format!("<b>{}</b> events · {} users / {}h", commas(total), commas(users), h)
}

fn errors_thead(issues: bool, f: &Filters) -> String {
    let freq = if f.sort.as_deref() == Some("frequent") { " ▾" } else { "" };
    let recent = if f.sort.as_deref() == Some("recent") { " ▾" } else { "" };
    if issues {
        format!(
...
fn render_rows(data: &Value, issues: bool, metrics: bool) -> String {
    ...
    items.iter().map(|it| row_html(it, issues, metrics)).collect::<Vec<_>>().join("")
}
...
fn row_html(it: &Value, issues: bool, metrics: bool) -> String {
    if metrics {
        let title = esc(&strip_tags(&{
            let t = vstr(it, "title");
            if t.is_empty() { "(event)".to_string() } else { t }
        }));
        if issues {
    ...
    let title = esc(&strip_tags(&{
        let t = vstr(it, "title");
        if t.is_empty() { "(no message)".to_string() } else { t }
    }));
    if issues {
        let fp = esc(&vstr(it, "fingerprint"));
        let status = {
            let s = vstr(it, "status");
            if s.is_empty() { "unresolved".to_string() } else { s }
        };
...
fn issue_head(meta: &Value, fp: &str) -> (String, String) {
    ...
    let badge_txt = if !level.is_empty() { level.clone() } else { kind.clone() };
    let title = {
        let t = vstr(meta, "title");
        if !t.is_empty() { strip_tags(&t) } else { fp.to_string() }
    };
    ...
    let assignee_block = if !assignee.is_empty() {
        format!("<span class=\"chip\" title=\"assignee\">@<b>{}</b></span>", esc(&assignee))
    } else {
        "<span class=\"chip\" style=\"opacity:.55\" title=\"assignee\">unassigned</span>".to_string()
    };
...
fn col(v: f64) -> &'static str {
    if v >= 99.0 { "var(--ok)" } else if v >= 95.0 { "var(--warn)" } else { "var(--err)" }
}
...
fn render_health(h: &Value) -> String {
    ...
    let empty = vec![];
    let by_status = h.get("by_status").and_then(|x| x.as_array()).unwrap_or(&empty);
    ...
    let mx = series.iter().filter_map(|p| p.get(1).and_then(|x| x.as_i64())).max().unwrap_or(1).max(1);
    let bars: String = series.iter().map(|p| {
        let b = p.get(0).and_then(|x| x.as_str()).unwrap_or("");
        let c = p.get(1).and_then(|x| x.as_i64()).unwrap_or(0);
        format!("<div class=\"bar\" style=\"height:{}%\" title=\"{} — {}\"></div>", (c as f64 / mx as f64 * 100.0).round() as i64, esc(b), c)
    }).collect();
    let by_release = h.get("by_release").and_then(|x| x.as_array()).unwrap_or(&empty);
...
fn render_flags(f: &Value) -> String {
    ...
    let flags = config.get("flags").and_then(|x| x.as_object()).cloned().unwrap_or_default();
    let variants = config.get("variants").and_then(|x| x.as_object()).cloned().unwrap_or_default();
    let empty = vec![];
    let observed = f.get("observed").and_then(|x| x.as_array()).unwrap_or(&empty);
    let obs: std::collections::HashMap<String, i64> = observed.iter().filter_map(|p| {
        Some((p.get(0)?.as_str()?.to_string(), p.get(1)?.as_i64()?))
    }).collect();
...
fn render_session(s: &Value, session_id: &str) -> String {
    ...
    let dur = if !first.is_empty() && !last.is_empty() {
        let parse = |t: &str| chrono::DateTime::parse_from_rfc3339(t).ok().map(|d| d.timestamp());
    ...
    let fmt_dur = |d: i64| -> String {
        ...
        else { format!("{}h {}m", d / 3600, (d % 3600) / 60) }
    };
    let empty = vec![];
    let by_level = s.get("by_level").and_then(|x| x.as_array()).unwrap_or(&empty);
    ...
    let started = {
        let src = if !app_start.is_empty() { &app_start } else { &first };
        let head: String = src.chars().take(16).collect();
        head.replace('T', " ")
    };
    let tl_block = if tl.is_empty() { "<div class=\"empty\">no events</div>".to_string() } else { tl };
    format!(
