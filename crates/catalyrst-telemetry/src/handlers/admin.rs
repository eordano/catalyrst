...
mod tests {
    ...
    async fn telemetry_admin_extractor_fails_closed_when_token_unset() {
        ...
        let mut parts = parts_with_authorization(Some("Bearer anything"));
        let rejection = AuthenticatedAdminIdentity::from_request_parts(&mut parts, &secret)
            .await
            .err()
            .expect("unset secret must not establish an admin identity");
        let (status, body) = admin_rejection_as_legacy_forbidden(&rejection);
        assert_eq!(status, StatusCode::FORBIDDEN);
        assert_eq!(
            body,
            "admin disabled (CATALYRST_TELEMETRY_ADMIN_TOKEN unset)"
        );
    }

    #[tokio::test]
    async fn telemetry_admin_extractor_rejects_missing_wrong_and_non_bearer() {
        let secret = ConfiguredAdminBearerSecret {
            environment_variable: ADMIN_TOKEN_ENV,
            configured: Some("s3cret".to_string()),
        };
        for auth in [None, Some("Bearer wrong"), Some("Basic s3cret")] {
            let mut parts = parts_with_authorization(auth);
            let rejection = AuthenticatedAdminIdentity::from_request_parts(&mut parts, &secret)
                .await
                .err()
                .expect("missing/wrong/non-bearer must not establish an admin identity");
            let (status, body) = admin_rejection_as_legacy_forbidden(&rejection);
            assert_eq!(status, StatusCode::FORBIDDEN);
            assert_eq!(body, "invalid admin bearer");
        }
    }

    #[tokio::test]
    async fn telemetry_admin_extractor_accepts_matching_bearer() {
        let secret = ConfiguredAdminBearerSecret {
            environment_variable: ADMIN_TOKEN_ENV,
            configured: Some("s3cret".to_string()),
        };
        let mut parts = parts_with_authorization(Some("Bearer s3cret"));
        assert!(
            AuthenticatedAdminIdentity::from_request_parts(&mut parts, &secret)
                .await
                .is_ok()
        );
    }
