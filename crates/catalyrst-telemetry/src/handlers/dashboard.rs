...
fn split_tag(s: &Option<String>) -> (Option<String>, Option<String>) {
    match blank(s).and_then(|t| t.split_once(':').map(|(k, v)| (k.to_string(), v.to_string()))) {
...
type Norm = (Option<String>, Option<String>, Option<String>, Option<String>, Option<String>, i64, i64, i64);
...
pub async fn events(
    ...
    if p.group == 1 {
        ...
        let rows = sqlx::query_as::<_, IssueRow>(&sql)
            .bind(&source).bind(&kind).bind(&level).bind(&q)
            .bind(hours).bind(&fingerprint).bind(limit).bind(offset)
            .bind(&environment).bind(&release).bind(&status)
            .bind(&tag_key).bind(&tag_val)
        ...
        let rows = sqlx::query_as::<_, EventRow>(&sql)
            .bind(&source).bind(&kind).bind(&level).bind(&q)
            .bind(hours).bind(&fingerprint).bind(limit).bind(offset)
            .bind(&environment).bind(&release).bind(None::<String>)
            .bind(&tag_key).bind(&tag_val)
            .fetch_all(&st.pool).await.map_err(err)?;
...
pub async fn stats(
    ...
    async fn counts(pool: &sqlx::PgPool, sql: &str, hours: i64, fp: &Option<String>, src: &Option<String>) -> Result<Vec<(Option<String>, i64)>, sqlx::Error> {
        sqlx::query_as::<_, (Option<String>, i64)>(sql).bind(hours).bind(fp).bind(src).fetch_all(pool).await
    }
    let by_level = counts(&st.pool, &group_count("COALESCE(body->>'level','(none)')"), hours, &fp, &src).await.map_err(err)?;
    let by_kind = counts(&st.pool, &group_count("event_kind"), hours, &fp, &src).await.map_err(err)?;
    let by_source = counts(&st.pool, &group_count("source"), hours, &fp, &src).await.map_err(err)?;
    ...
    let pair = |v: Vec<(Option<String>, i64)>| -> Vec<Value> {
        v.into_iter().map(|(k, c)| json!([k.unwrap_or_default(), c])).collect()
    };
    Ok(Json(json!({
        "total": total,
        "hours": hours,
...
pub async fn health(
    ...
    let unhealthy: i64 = by_status.iter()
        .filter(|(k, _)| matches!(k.as_deref(), Some("crashed" | "abnormal" | "unhandled" | "errored")))
        .map(|(_, c)| c).sum();
    let crashed: i64 = by_status.iter().filter(|(k, _)| k.as_deref() == Some("crashed")).map(|(_, c)| c).sum();
    let crash_free = if total > 0 { (1.0 - crashed as f64 / total as f64) * 100.0 } else { 100.0 };
    let healthy_rate = if total > 0 { (1.0 - unhealthy as f64 / total as f64) * 100.0 } else { 100.0 };
    ...
    let crash_free_users = if total_users > 0 { (1.0 - crashed_users as f64 / total_users as f64) * 100.0 } else { 100.0 };
...
pub async fn funnel(
    ...
    let hours = p.hours.clamp(1, 24 * 365);
    let steps: Vec<String> = p.steps.unwrap_or_default().split('|')
        .map(|s| s.trim().to_string()).filter(|s| !s.is_empty()).collect();
    if steps.len() < 2 {
        return Err((StatusCode::BAD_REQUEST, "need >=2 steps (pipe-separated)".into()));
    }
    ...
    let rows = sqlx::query_as::<_, (Option<String>, String, String)>(&format!(
        "SELECT {USERKEY} AS uk, body->>'event' AS ev, \
         ...
         GROUP BY 1,2"))
        .bind(hours).bind(&steps).fetch_all(&st.pool).await.map_err(err)?;
    use std::collections::HashMap;
    let mut per_user: HashMap<String, HashMap<String, String>> = HashMap::new();
    for (uk, ev, t) in rows {
        if let Some(uk) = uk { per_user.entry(uk).or_default().insert(ev, t); }
    }
    ...
    for evs in per_user.values() {
        ...
        for (i, step) in steps.iter().enumerate() {
            match evs.get(step) {
                Some(t) if last.map_or(true, |l| t >= l) => { counts[i] += 1; last = Some(t); }
                _ => break,
            }
        }
    }
    let first = counts.first().copied().unwrap_or(0).max(1);
    let result: Vec<Value> = steps.iter().zip(&counts).enumerate().map(|(i, (step, &c))| {
...
pub async fn breakdown(
    ...
    let Some(prop) = prop else {
        ...
        return Ok(Json(json!({ "props": keys.into_iter().map(|(k,)| k).collect::<Vec<_>>(), "rows": [] })));
    };
    let rows = sqlx::query_as::<_, (Option<String>, i64, i64)>(&format!(
        ...
        .bind(hours).bind(&event).bind(&prop).fetch_all(&st.pool).await.map_err(err)?;
...
pub async fn story(
    ...
    .map_err(err)?
    .flatten();
    Ok(Json(json!({ "user": user_key, "utm": utm, "count": events.len(), "events": events })))
}
...
pub async fn metrics(
    ...
    let hours = p.hours.clamp(1, 24 * 365);
    let win = "source = 'segment' AND received_at > now() - make_interval(hours => $1::int)";
    async fn q(pool: &sqlx::PgPool, sql: &str, hours: i64) -> Result<Vec<(Option<String>, i64)>, sqlx::Error> {
        sqlx::query_as::<_, (Option<String>, i64)>(sql).bind(hours).fetch_all(pool).await
    }
    let by_event = q(&st.pool, &format!(
        "SELECT body->>'event' AS k, count(*) AS c FROM telemetry.telemetry_events \
         WHERE {win} AND body->>'event' IS NOT NULL GROUP BY 1 ORDER BY 2 DESC LIMIT 50"), hours).await.map_err(err)?;
    ...
    let users: i64 = sqlx::query_scalar(&format!(
        "SELECT count(DISTINCT COALESCE(body->>'userId', body->>'anonymousId')) \
         FROM telemetry.telemetry_events WHERE {win}"))
        .bind(hours).fetch_one(&st.pool).await.map_err(err)?;
    let bucket = if hours <= 48 { "hour" } else { "day" };
    let series = sqlx::query_as::<_, (String, i64)>(&format!(
        "SELECT to_char(date_trunc('{bucket}', received_at AT TIME ZONE 'UTC'),'YYYY-MM-DD\"T\"HH24:MI') AS b, \
           count(*) AS c FROM telemetry.telemetry_events WHERE {win} GROUP BY 1 ORDER BY 1"))
...
pub async fn flags(State(st): State<AppState>) -> Result<Json<Value>, (StatusCode, String)> {
    ...
    let config: Value = match reqwest::Client::new().get(&url).timeout(std::time::Duration::from_secs(5)).send().await {
    ...
    let observed = sqlx::query_as::<_, (Option<String>, i64)>(
        "SELECT f->>'flag' AS k, count(*) c FROM telemetry.telemetry_events, \
         ...
         WHERE source='sentry' GROUP BY 1 ORDER BY 2 DESC LIMIT 200")
        .fetch_all(&st.pool).await.map_err(err)?;
    Ok(Json(json!({
        "config": config,
...
/// the rest of the dashboard — keep behind the tailnet / front with auth before
/// any public exposure.
pub async fn sql_query(
    ...
    if !(low.starts_with("select") || low.starts_with("with")) {
        return Err((StatusCode::BAD_REQUEST, "only SELECT / WITH queries are allowed".into()));
    }
    if raw.contains(';') {
        return Err((StatusCode::BAD_REQUEST, "one statement only (no ';')".into()));
    }
    ...
    let run = async {
        ...
        sqlx::query("SET LOCAL statement_timeout = 15000").execute(&mut *tx).await?;
        sqlx::query_scalar::<_, Value>(&wrapped).fetch_all(&mut *tx).await
    }.await;
    ...
    let cols: Vec<String> = rows.first()
        ...
        .unwrap_or_default();
    Ok(Json(json!({ "columns": cols, "rows": rows, "truncated": truncated })))
}
...
/// existing status. Unauthenticated like the rest of this loopback/tailnet
...
pub async fn set_issue_state(
    ...
    if let Some(s) = b.status.as_deref() {
        if !matches!(s, "unresolved" | "resolved" | "ignored") {
            return Err((StatusCode::BAD_REQUEST, "status must be unresolved|resolved|ignored".into()));
        }
    }
