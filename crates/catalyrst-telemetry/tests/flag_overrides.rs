...
// Scratch database on :5434, created + dropped per run; skips silently without
// CATALYRST_TELEMETRY_TEST_PG. Mirrors production: schema `telemetry`,
// search_path=telemetry (the handlers reference telemetry.* explicitly).
struct Scratch {
    state: AppState,
    admin: PgPool,
    db: String,
...
async fn setup() -> Option<Scratch> {
    let base = std::env::var("CATALYRST_TELEMETRY_TEST_PG").ok()?;
    ...
    let state = build_state(&cfg).await.ok()?;
...
async fn flag_override_roundtrip_merge_clear_and_audit() {
    let Some(s) = setup().await else {
        eprintln!("skipping flag_override_roundtrip: CATALYRST_TELEMETRY_TEST_PG unset");
        return;
    };
    ...
    let flags = dashboard::flags(State(st.clone())).await.expect("flags").0;
    ...
    let flags2 = dashboard::flags(State(st.clone())).await.expect("flags2").0;
