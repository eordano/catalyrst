...
Rust port of decentraland/camera-reel-service. All under `/api`. CORS:
allow-any-origin, methods GET/POST/PATCH/DELETE. Plus `GET /health/live`.

Port: `5163` (see `umbrella/env/catalyrst-camera-reel.env`, which notes
"5149 is catalyrst-sync" — `5143` belongs to `catalyrst-world-storage`, not
this crate).
