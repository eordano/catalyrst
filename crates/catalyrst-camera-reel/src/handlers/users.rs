...
use crate::handlers::{default_compact, default_limit, default_offset, optional_auth};
...
fn only_public_for(headers: &HeaderMap, method: &str, path: &str, user_address: &str) -> bool {
    match optional_auth(headers, method, path) {
        Some(signer) if signer.eq_ignore_ascii_case(user_address) => false,
        _ => true,
    }
}
...
pub async fn get_user_images(
    ...
    let images = state
        .db
        .get_user_images(&user_address, q.offset as i64, q.limit as i64, only_public)
        .await
        .map_err(|_| ApiError::NotFound("user not found".to_string()))?;
    ...
    if q.compact {
        ...
        Ok((StatusCode::OK, Json(GetImagesResponse { images, user_data })).into_response())
    }
}
