...
pub async fn upload_image(
    ...
    let image_bytes = image_bytes.ok_or_else(|| ApiError::BadRequest("missing image".to_string()))?;
    let metadata_bytes =
...
pub async fn get_metadata(
    ...
    match state.db.get_image(&image_id).await {
        Ok(db_image) => {
        ...
        Err(sqlx::Error::ColumnDecode { source, .. }) => {
            ...
            Err(ApiError::Internal("couldn't decode image".to_string()))
        }
        Err(e) => {
