...
use crate::rest::handlers::roles::has_moderation_permission;
...
pub async fn get_community_requests(
    ...
    let role = state.communities.member_role(id, signer.as_str()).await?;
    ...
    for row in rows.iter_mut() {
        if let Some(map) = row.as_object_mut() {
            ...
            let has_thumb = map
                ...
                .unwrap_or(false);
            let thumb = if has_thumb {
                thumbnail_url(&state.cdn_url, &community_id)
            } else {
                "N/A".to_string()
            };
            map.insert("thumbnailUrl".to_string(), serde_json::Value::String(thumb));
