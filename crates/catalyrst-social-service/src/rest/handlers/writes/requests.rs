...
pub async fn create_request(
    ...
    let community: Option<(String, bool)> =
        sqlx::query_as("SELECT name, private FROM communities WHERE id = $1 AND active = TRUE")
            .bind(community_uuid)
            .fetch_optional(&state.pool)
            .await
            .ok()
            .flatten();
    let (community_name, private) = match community {
        Some((name, private)) => (name, private),
        None => {
            return err_json(
