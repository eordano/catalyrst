...
fn set_thumbnail_list(cdn_url: &str, item: &mut CommunityListItem) {
    if item.has_thumbnail {
        item.thumbnail_url = Some(thumbnail_url(cdn_url, &item.id.to_string()));
    }
}
...
mod tests {
    ...
    #[test]
    fn missing_thumbnail_omits_key() {
        let mut items = vec![sample_item(false)];
        enrich_list_v1("https://cdn.example", &mut items, &HashMap::new());
        let v = serde_json::to_value(&items[0]).unwrap();
        let m = v.as_object().unwrap();
        assert!(
            !m.contains_key("thumbnailUrl"),
            "no-thumbnail community must omit thumbnailUrl entirely"
        );
