...
use crate::rest::http::{
    get_all, get_bool, get_first, get_pagination_params, Paginated, Pagination,
};
    ...
    F: Fn(&str) -> Option<serde_json::Value>,
{
    for obj in results.iter_mut() {
        ...
        let friends: Vec<serde_json::Value> =
            addresses.iter().filter_map(|addr| render(addr)).collect();
        if insert_empty || !friends.is_empty() {
...
async fn enrich_with_friends(state: &AppState, user: &str, results: &mut [serde_json::Value]) {
    let by_community = friends_by_community(state, user, results).await;
    ...
    insert_friends(results, &by_community, false, |addr| {
        profiles
...
async fn enrich_with_friend_addresses(
    ...
    insert_friends(results, &by_community, true, |addr| {
        Some(serde_json::Value::String(addr.to_string()))
    });
}
