...
pub async fn remove_member(
    ...
    if target == signer {
        let role = load_role_uuid(&state, uuid, signer.as_str()).await;
        if role == Role::Owner {
            return err(
                StatusCode::UNAUTHORIZED,
        ...
        let kicker_role = load_role_uuid(&state, uuid, signer.as_str()).await;
...
pub async fn update_member_role(
    ...
    let updater_role = load_role_uuid(&state, uuid, signer.as_str()).await;
    let target_role = load_role_uuid(&state, uuid, &target).await;
...
pub async fn ban_member(
    ...
    let banner_role = load_role_uuid(&state, uuid, signer.as_str()).await;
    let target_role = load_role_uuid(&state, uuid, &target).await;
...
pub async fn unban_member(
    ...
    let unbanner_role = load_role_uuid(&state, uuid, signer.as_str()).await;
    let target_role = load_role_uuid(&state, uuid, &target).await;
