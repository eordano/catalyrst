...
fn require_admin(
    ...
    require_admin_with_token(state.admin_token.as_deref(), headers)
}

fn require_admin_with_token(
    expected: Option<&str>,
    headers: &HeaderMap,
) -> Result<String, (StatusCode, Json<serde_json::Value>)> {
...
mod tests {
    ...
    fn bearer(tok: Option<&str>) -> HeaderMap {
        let mut h = HeaderMap::new();
        if let Some(t) = tok {
    ...
    #[test]
    fn unset_configured_token_fails_closed() {
        assert!(require_admin_with_token(None, &bearer(Some("anything"))).is_err());
        assert!(require_admin_with_token(None, &bearer(None)).is_err());
    }
    ...
    #[test]
    fn set_secret_accepts_correct_rejects_others() {
        assert!(require_admin_with_token(Some("secret"), &bearer(Some("secret"))).is_ok());
        assert!(require_admin_with_token(Some("secret"), &bearer(Some("wrong"))).is_err());
        assert!(require_admin_with_token(Some("secret"), &bearer(Some(""))).is_err());
        assert!(require_admin_with_token(Some("secret"), &bearer(None)).is_err());
    }
}
