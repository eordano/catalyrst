...
///
/// Replaces `rest::fed::authority::load_role`. Behaviour-preserving: same SQL, same
/// `to_ascii_lowercase` on the member, same "missing row means not a member", and the
/// same propagation of a query failure.
