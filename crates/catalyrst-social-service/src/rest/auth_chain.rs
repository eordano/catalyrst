...
pub fn try_extract_signer(headers: &HeaderMap, method: &str, path: &str) -> Option<Signer> {
...
pub fn require_signer(
    ...
    let result = signed_fetch::verify_signed_fetch(headers, method, path, FIVE_MINUTES);
    if let Err(e) = &result {
