...
mod tests {
    ...
    #[tokio::test]
    async fn a_re_cased_metadata_key_answers_from_the_pre_gate_before_the_signature() {
        let now_secs = 1_700_000_000;
    ...
    #[tokio::test]
    async fn a_re_spelled_signer_key_signed_as_delivered_is_refused_by_the_pre_gate() {
        let now_secs = 1_700_000_000;
        let ts_ms = now_secs * 1000;
        ...
        assert!(
            verify_handshake(&frame, "get", "/", FIVE_MINUTES_SECS, now_secs)
                .await
                .is_ok(),
            "the signature alone accepts this frame, which is why the pre-gate must not"
        );
        assert!(frame_declares_refused_signer(&frame));
    }
    ...
    async fn the_header_path_refuses_a_re_spelled_signer_key_before_verification() {
        ...
        assert!(
            matches!(&err, AuthChainError::InvalidSignature(m)
                if m == "requests from scenes are not allowed"),
            "{err:?}"
        );
    }
