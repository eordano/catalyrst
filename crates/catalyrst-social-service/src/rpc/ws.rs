use crate::rpc::auth_chain::{
    frame_declares_scene_signer, handshake_path, verify_handshake, FIVE_MINUTES_SECS,
};
...
async fn auth_handshake(state: &AppState, path: &str, socket: &mut WebSocket) -> Result<Signer> {
    ...
    let signer = verify_handshake(&text, "get", path, FIVE_MINUTES_SECS, now_secs)
        .await
        .map_err(|e| anyhow!("handshake: {e}"))?;

    // ADR-44: a scene must not authenticate as a user on this surface, matching the HTTP routes
    // (upstream #440). The chain is genuinely valid — the scene signer rides a lowercased signing
    // payload — so the metadata is inspected after the signature verifies.
    if frame_declares_scene_signer(&text) {
