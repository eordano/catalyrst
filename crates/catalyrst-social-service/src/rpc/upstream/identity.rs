...
mod tests {
    ...
    #[test]
    fn auth_frame_verifies_against_the_servers_own_handshake_check() {
        let id = identity();
        let frame = id.ws_auth_frame_at("/", NOW_SECS * 1000).unwrap();
        let signer = verify_handshake(&frame, "get", "/", FIVE_MINUTES_SECS, NOW_SECS).unwrap();
        assert_eq!(signer, id.address());
    }

    #[test]
    fn auth_frame_signed_for_one_path_fails_another() {
        let frame = identity()
            .ws_auth_frame_at("/social-rpc", NOW_SECS * 1000)
            .unwrap();
        let err = verify_handshake(&frame, "get", "/", FIVE_MINUTES_SECS, NOW_SECS).unwrap_err();
        assert!(
