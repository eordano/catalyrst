...
`Sign` column: `optional` = `signedFetchMiddleware({ optional: true })` (handler runs with or
...
`Status` column: `GET` = implemented read path, queries the local `communities` DB; `WRITE` =
