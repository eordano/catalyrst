...
async fn deactivate_community(pool: &PgPool, id: Uuid) {
    sqlx::query("UPDATE communities SET active = FALSE WHERE id = $1")
        .bind(id)
        .execute(pool)
        .await
        .expect("deactivate community");
}
...
#[tokio::test]
async fn unauthorized_invite_answers_the_same_whatever_the_target_is() {
    let Some(scratch) = setup_db().await else {
        return;
    };
    let pool = scratch.pool.clone();
    ...
    let member_target = "0x00000000000000000000000000000000000000e1";
    let banned_target = "0x00000000000000000000000000000000000000e2";
    let stranger_target = "0x00000000000000000000000000000000000000e3";
    seed_member(&pool, cid, member_target, "member").await;
    seed_ban(&pool, cid, banned_target, owner).await;

    let mut answers: Vec<(StatusCode, serde_json::Value)> = Vec::new();
    for target in [member_target, banned_target, stranger_target] {
        let headers = signed_headers(
            &outsider,
            "post",
            &format!("/v1/communities/{}/requests", cid),
        ...
        .await;
        let (status, body) = writes::create_request(
            State(state.clone()),
            headers,
            Path(cid.to_string()),
            Bytes::from(
                serde_json::to_vec(&json!({"type": "invite", "targetedAddress": target})).unwrap(),
            ),
        )
        .await;
        assert_eq!(
            status,
            StatusCode::UNAUTHORIZED,
            "a caller without invite permission must be refused for {}",
            target
        );
        assert_eq!(
            body.0["message"].as_str().unwrap_or_default(),
            format!(
                "The user {} doesn't have permission to invite users",
                outsider_addr
            ),
            "the refusal must name the caller's missing permission, never the target's standing"
        );
        assert_eq!(
            pending_requests(&pool, cid, target).await,
            0,
            "the refused invite must not create a request for {}",
            target
        );
        answers.push((status, body.0));
    }

    assert_eq!(
        answers[0], answers[1],
        "a member target and a banned target must be indistinguishable to a caller with no standing"
    );
    assert_eq!(
        answers[1], answers[2],
        "a banned target and an address with no record must be indistinguishable"
    );
...
#[tokio::test]
async fn request_to_join_for_another_address_is_refused_before_reading_the_target() {
    let Some(scratch) = setup_db().await else {
        return;
    };
    let pool = scratch.pool.clone();
    ...
    let owner = "0x0000000000000000000000000000000000000001";
    let outsider = mk_wallet(121);

    let cid = rand_uuid();
    seed_community(&pool, cid, owner, true).await;
    seed_member(&pool, cid, owner, "owner").await;

    let member_target = "0x00000000000000000000000000000000000000f1";
    let stranger_target = "0x00000000000000000000000000000000000000f2";
    seed_member(&pool, cid, member_target, "member").await;

    let mut answers: Vec<(StatusCode, serde_json::Value)> = Vec::new();
    for target in [member_target, stranger_target] {
        let headers = signed_headers(
            &outsider,
            "post",
            &format!("/v1/communities/{}/requests", cid),
        ...
        let (status, body) = writes::create_request(
            ...
            Path(cid.to_string()),
            Bytes::from(
                serde_json::to_vec(&json!({"type": "request_to_join", "targetedAddress": target}))
                    .unwrap(),
            ),
        )
        .await;
        assert_eq!(
            ...
            "a request to join named for another address must be refused for {}",
            target
        );
        assert_eq!(
            body.0["message"].as_str().unwrap_or_default(),
            "User trying to impersonate another user",
            "the refusal must be the impersonation error, never the target's standing"
        );
        assert_eq!(
            pending_requests(&pool, cid, target).await,
            0,
            "the refused request must not be stored for {}",
            target
        );
        answers.push((status, body.0));
    }
    ...
    assert_eq!(
        answers[0], answers[1],
        "a member target and an address with no record must be indistinguishable"
    );
...
#[tokio::test]
async fn an_inactive_community_answers_like_one_that_never_existed() {
    let Some(scratch) = setup_db().await else {
        return;
    };
    let pool = scratch.pool.clone();
    ...
    let owner = "0x0000000000000000000000000000000000000001";
    let caller = mk_wallet(122);
    let caller_addr = wallet_addr(&caller);

    let inactive_id = rand_uuid();
    seed_community(&pool, inactive_id, owner, true).await;
    seed_member(&pool, inactive_id, owner, "owner").await;
    deactivate_community(&pool, inactive_id).await;
    let missing_id = rand_uuid();

    let mut answers: Vec<(StatusCode, String)> = Vec::new();
    for cid in [inactive_id, missing_id] {
        let headers = signed_headers(
            &caller,
            "post",
            &format!("/v1/communities/{}/requests", cid),
        ...
        let (status, body) = writes::create_request(
            ...
            Bytes::from(
                serde_json::to_vec(
                    &json!({"type": "request_to_join", "targetedAddress": caller_addr}),
                )
                .unwrap(),
            ),
        )
        ...
        assert_eq!(
            ...
            "a deactivated community must not be distinguishable from a missing one ({})",
            cid
        );
        assert_eq!(
            body.0["message"].as_str().unwrap_or_default(),
            format!("Community not found: {}", cid),
            "the refusal must be the not-found body for {}",
            cid
        );
        answers.push((
            status,
            body.0.to_string().replace(&cid.to_string(), "COMMUNITY"),
        ));
    }

    assert_eq!(
        answers[0], answers[1],
        "a deactivated community and one that never existed must answer identically"
    );
...
#[tokio::test]
async fn the_already_member_refusal_names_the_community() {
    let Some(scratch) = setup_db().await else {
        return;
    };
    let pool = scratch.pool.clone();
    ...
    let owner = "0x0000000000000000000000000000000000000001";
    let member = mk_wallet(123);
    let member_addr = wallet_addr(&member);
    ...
    let (status, body) = writes::create_request(
        ...
        Path(cid.to_string()),
        Bytes::from(
            serde_json::to_vec(&json!({"type": "request_to_join", "targetedAddress": member_addr}))
                .unwrap(),
        ),
    )
    .await;
    assert_eq!(
        status,
        StatusCode::BAD_REQUEST,
        "an existing member cannot request to join again"
    );
    assert_eq!(
        body.0["message"].as_str().unwrap_or_default(),
        format!(
            "User cannot join since it is already a member of the community: {} ({})",
            "Ban Integrity Community", cid
        ),
        "the refusal must name the community as upstream does"
    );
    assert_eq!(
