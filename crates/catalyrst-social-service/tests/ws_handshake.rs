...
use catalyrst_social_service::rpc::auth_chain::FIVE_MINUTES_SECS;
...
async fn signed_frame(ts_ms: i64, metadata: &str) -> (String, String) {
    ...
    let payload = build_payload_v6("get", "/", &ts_ms.to_string(), metadata);
...
#[tokio::test]
async fn ws_handshake_re_spelled_scene_signer_key_gets_close_3003() {
    let addr = spawn_server().await;
    tokio::time::sleep(Duration::from_millis(50)).await;
