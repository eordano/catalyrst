...
async fn setup_db() -> Option<ScratchSchema> {
    ...
    let scratch =
        ScratchSchema::create("CATALYRST_SOCIAL_SERVICE_TEST_PG", "cg_social_envelope").await?;
    for sql in [
...
async fn community_404_carries_unified_envelope() {
    let Some(scratch) = setup_db().await else {
        eprintln!("skipping community_404_carries_unified_envelope: no postgres on :5434");
        return;
    };
...
async fn mutes_error_carries_unified_envelope() {
    let Some(scratch) = setup_db().await else {
        eprintln!("skipping mutes_error_carries_unified_envelope: no postgres on :5434");
        return;
    };
    let (state, dir) = build_state(&scratch.pool).await;
