...
async fn setup_db() -> Option<ScratchSchema> {
    ...
    let scratch = ScratchSchema::create("CATALYRST_SOCIAL_SERVICE_TEST_PG", "cg_social_v2").await?;
    apply_migration(
        &scratch.pool,
...
async fn v2_minimal_runs_the_same_branch_as_v1() {
    let Some(scratch) = setup_db().await else {
        eprintln!("skipping v2_minimal_runs_the_same_branch_as_v1: no postgres");
        return;
    };
    let pool = scratch.pool.clone();
    let (state, dir) = build_state(&pool).await;
