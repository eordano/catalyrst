...
//! `rest::handlers::client::communities::delete` and `rest::fed::apply::apply_delete` only
//! `UPDATE communities SET active = FALSE`. RPC-side twin:
//! `tests/community_voice_soft_delete.rs`.
...
/// BC-1 regression. Needs no database -- an unreachable pool is the fault.
#[tokio::test]
