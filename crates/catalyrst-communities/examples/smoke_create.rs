...
use catalyrst_communities::fed::messages::CommunityCreate;
...
async fn main() -> anyhow::Result<()> {
    let base = env::args().nth(1).unwrap_or_else(|| "http://127.0.0.1:5136".into());
    let mut key = [0u8; 32];
    ...
    let message = CommunityCreate {
        name: format!("smoke-{}", hex::encode(&nonce[..4])),
    ...
    if community_id.len() != 64 {
        anyhow::bail!("community_id wrong length");
    }
    println!("ok community_id={} sig_hash={}", community_id, sig_hash);
    Ok(())
}
