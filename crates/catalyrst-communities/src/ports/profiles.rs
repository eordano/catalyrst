...
impl ProfilesComponent {
    ...
    /// `catalystClient.getOwnedNames(address, { pageSize: '1' }).length > 0` —
    /// upstream's community-creation gate. True iff the address has at least one
    /// claimed DCL name (`hasClaimedName` on its active profile entity).
    ///
    /// Returns `None` when the content DB is unavailable so callers can decide
    /// whether to fail closed; the enrichment query is the same source upstream
    /// `getOwnedNames` ultimately resolves against.
