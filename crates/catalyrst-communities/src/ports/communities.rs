...
impl CommunitiesComponent {
    ...
    pub async fn is_private(&self, id: Uuid) -> Result<bool, ApiError> {
        let row: Option<bool> = sqlx::query_scalar(
            "SELECT private FROM communities WHERE id = $1 AND active = TRUE",
        )
        .bind(id)
    ...
    pub async fn get_by_id(
        ...
        let Some((id, name, description, owner_address, private, active, unlisted, created_at, updated_at)) = row else {
            return Ok(None);
        };
        let privacy = if private { "private" } else { "public" };
        let visibility = if unlisted { "unlisted" } else { "all" };

        let members_count: i64 = sqlx::query_scalar(
    ...
    pub async fn list(
        ...
        if as_user.is_none() {
            ...
            where_clauses.push("(c.unlisted = FALSE OR EXISTS (SELECT 1 FROM community_members m WHERE m.community_id = c.id AND m.member_address = $USER))".to_string());
        }
        ...
        if only_with_active_voice_chat {
            where_clauses.push("EXISTS (SELECT 1 FROM community_voice_chats v WHERE v.community_id = c.id)".to_string());
        }
        ...
        let mut q = sqlx::query_as::<_, (Uuid, String, String, String, bool, bool, bool, NaiveDateTime, NaiveDateTime, i64, String, bool)>(&select_sql);
        for p in &params {
            q = q.bind(p);
        }
        ...
        let results: Vec<serde_json::Value> = rows
            .into_iter()
            .map(|(id, name, description, owner_address, private, active, unlisted, created_at, _updated_at, members_count, role, has_thumbnail)| {
                let privacy = if private { "private" } else { "public" };
                let visibility = if unlisted { "unlisted" } else { "all" };
                serde_json::json!({
                    "id": id,
    ...
    pub async fn member_communities(
        ...
        let limit_idx = if roles.map(|r| !r.is_empty()).unwrap_or(false) { 3 } else { 2 };
        ...
        let mut q = sqlx::query_as::<_, (Uuid, String, String, String, NaiveDateTime)>(&select_sql).bind(&lower);
        ...
        let results = rows
            .into_iter()
            .map(|(id, name, owner_address, role, joined_at)| MemberCommunity {
                id, name, owner_address, role, joined_at,
            })
            .collect();
        Ok((results, total))
    }
