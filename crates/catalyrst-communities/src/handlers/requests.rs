...
pub async fn get_member_requests(
    ...
    let owner_addrs: Vec<String> = rows
        .iter()
        .filter_map(|r| r.get("ownerAddress").and_then(|v| v.as_str()).map(str::to_string))
        .collect();
    let owner_names = state.profiles.get_owner_names(&owner_addrs).await;

    for row in rows.iter_mut() {
        if let Some(map) = row.as_object_mut() {
            let id = map.get("communityId").and_then(|v| v.as_str()).map(str::to_string).unwrap_or_default();
            let has_thumb = map.remove("_hasThumbnail").and_then(|v| v.as_bool()).unwrap_or(false);
            ...
            let owner = map.get("ownerAddress").and_then(|v| v.as_str()).map(|s| s.to_lowercase()).unwrap_or_default();
            let owner_name = owner_names.get(&owner).cloned().unwrap_or_default();
            map.insert("ownerName".to_string(), serde_json::Value::String(owner_name));
        }
    }
