...
use crate::http::{get_all, get_first, get_pagination_params, ApiError, EnvelopeData, Paginated};
use crate::AppState;
...
pub async fn get_mutes(
    ...
    let filter: Option<Vec<String>> = if filter.is_empty() { None } else { Some(filter) };
