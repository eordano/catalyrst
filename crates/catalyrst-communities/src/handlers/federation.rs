...
pub async fn snapshot(
    ...
    let role_hashes: Vec<(String,)> = sqlx::query_as(
        "SELECT signature_hash FROM community_role_log ORDER BY signature_hash ASC",
    )
    .fetch_all(&state.pool)
    .await
    .map_err(ApiError::from)?;
...
pub async fn changes(
    ...
    let role: Vec<(String, String, String, String, String, i64, serde_json::Value, i64)> =
        sqlx::query_as(
    ...
    let role_json: Vec<serde_json::Value> = role
        .into_iter()
        .map(|(sig, cid, signer, target, role, signed_at, payload, seq)| {
            serde_json::json!({
                "kind": "role",
                "signature_hash": sig,
                "community_id": cid,
