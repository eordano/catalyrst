...
pub async fn apply_create(
    ...
    let existed: Option<(String,)> = sqlx::query_as(
        "SELECT community_id FROM communities_local WHERE community_id = $1",
    )
    .bind(&community_id)
...
pub async fn apply_place_remove(
    ...
    .await?;
    sqlx::query(
        "DELETE FROM community_places WHERE id = $1 AND community_id = $2",
    )
    .bind(&signed.message.place_id)
    .bind(uuid)
    .execute(&mut *tx)
...
pub async fn apply_request_status(
    ...
    if let Ok(uuid) = Uuid::parse_str(&signed.message.request_id) {
        sqlx::query(
            "UPDATE community_requests SET status = $2, updated_at = now() WHERE id = $1",
        )
        .bind(uuid)
        .bind(&signed.message.status)
        .execute(pool)
