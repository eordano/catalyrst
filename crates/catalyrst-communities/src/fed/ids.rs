...
pub fn community_uuid_from_hex(hex_id: &str) -> Uuid {
    // Never panic on attacker-controlled input: a *valid but short* hex string
    // (e.g. "abcd" -> 2 bytes) made `bytes[..16]` index out of bounds. Decode
    // failures fall back to zero; short decodes left-pad. Valid 64-char ids
    // (>=16 bytes) keep the original bytes[..16] behavior.
