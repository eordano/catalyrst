...
impl Replay {
    pub async fn new(pool: PgPool) -> Result<Arc<Self>, sqlx::Error> {
        ...
        let rows: Vec<(String, String)> = sqlx::query_as(
            "SELECT signer, nonce FROM seen_nonces WHERE expires_at >= $1",
        )
        .bind(now)
        .fetch_all(&pool)
        .await?;
    ...
    pub async fn check_and_record(
        ...
        // Atomic claim: the row's existence IS the dedup. The previous SELECT-then-
        // INSERT was a TOCTOU — two concurrent requests with the same (signer,nonce)
        // both saw an empty SELECT and both returned Ok. INSERT ... ON CONFLICT DO
        // NOTHING with rows_affected()==0 means someone (possibly a concurrent txn)
        // already claimed it -> reject. (Matches the places fed replay.)
        ...
        map.entry(signer_key).or_insert_with(PerSigner::new).insert(nonce_hex);
