        ...
        match op().await? {
            ...
            RetryDecision::Retry {
                ...
                tracing::warn!(
                    retry = label,
                    attempt,
                    of = if rate_limited { max_attempts } else { attempts },
                    rate_limited,
...
/// Parses a `Retry-After` header, in either the delta-seconds or the HTTP-date form.
///
/// An unparseable value still yields a short wait rather than `None`: the header's
/// presence is itself the signal to slow down.
...
mod tests {
    ...
    async fn operation_errors_propagate() {
        let r: Result<Option<u32>> = retry_with_backoff("test", 3, 1000, || async {
            Err(anyhow::anyhow!("hard failure"))
        })
        .await;
        assert_eq!(r.unwrap_err().to_string(), "hard failure");
    }
}
