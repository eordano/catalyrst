...
fn bench_content_hash_validation(c: &mut Criterion) {
    ...
    group.bench_function("valid_cidv0", |b| {
        b.iter(|| catalyrst_hashing::is_canonical_cid(black_box(valid_cidv0)))
    });
    group.bench_function("valid_cidv1", |b| {
        b.iter(|| catalyrst_hashing::is_canonical_cid(black_box(valid_cidv1)))
    });
    group.bench_function("valid_hex_sha256", |b| {
        b.iter(|| catalyrst_hashing::is_canonical_cid(black_box(valid_hex)))
    });
    group.bench_function("invalid_path_traversal", |b| {
        b.iter(|| catalyrst_hashing::is_canonical_cid(black_box(invalid)))
    });
    ...
    group.bench_function("batch_1000_mixed", |b| {
        b.iter(|| {
            for h in &hashes {
                black_box(catalyrst_hashing::is_canonical_cid(black_box(h)));
            }
        })
    });
    ...
    fn setup() -> BenchState {
        ...
        rt.block_on(async {
            ...
            let state = Arc::new(AppState {
                ...
                deployments_cache: TtlMap::bounded(
                    "deployments",
                    DEPLOYMENTS_CACHE_TTL,
                    DEPLOYMENTS_CACHE_MAX_ENTRIES,
                ),
