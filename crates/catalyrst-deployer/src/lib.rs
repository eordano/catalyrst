...
pub use garbage_collection::{GarbageCollectionConfig, GarbageCollectionManager};
pub use parallel_pipeline::{
    EntityFetcher, FetchError, FetchedEntity, HttpEntityFetcher, ParallelDeploymentPipeline,
    PipelineConfig, PipelineResult, SyncTask,
};
pub use pointer_manager::{DeltaPointerResult, PointerManager};
pub use sequential_task_executor::{ISequentialTaskExecutor, SequentialTaskExecutor};
...
pub trait DatabaseBackend: Send + Sync {
    ...
    async fn calculate_overwrote(&self, entity: &Entity)
    ...
    async fn find_unreferenced_content_hashes_batch(
        &self,
        since: DateTime<Utc>,
        after: Option<&str>,
        limit: usize,
    ) -> Result<Vec<ContentHash>, BackendError> {
        let mut all = self.find_unreferenced_content_hashes(since).await?;
        all.sort();
        all.dedup();
        let start = match after {
            Some(cursor) => all.partition_point(|h| h.as_str() <= cursor),
            None => 0,
        };
