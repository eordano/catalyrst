...
use crate::{
    ContentHash, DeploymentContext, DeploymentResult, DeploymentService, Entity, EntityType,
...
impl HttpEntityFetcher {
    async fn try_fetch_from_server(
        ...
        Ok(FetchedEntity {
            ...
            entity_bytes: content_files.get(entity_id).cloned().unwrap_or_default(),
            content_files,
            auth_chain,
            total_bytes,
        })
                ...
                match fetcher.fetch_entity(&entity_id, &servers).await {
                    Ok(fetched) => {
                        ...
                        if tx.send(fetched).await.is_err() {
                            debug!(
                                entity_id,
                                "validate channel closed, dropping fetched entity"
                            );
                        }
                    }
                    Err(e) => {
            ...
            set.spawn(async move {
                    ...
                    .await
                {
                    ...
                    Err(errors) => {
                        ...
                        metrics.failed.fetch_add(1, Ordering::Relaxed);
                        let _ = failed_tx.send(FailedEntity { entity_id, errors }).await;
                        return;
                    }
                };
                ...
                if tx.send(validated).await.is_err() {
                    debug!(
                        entity_id,
                        "persist channel closed, dropping validated entity"
                    );
                }
            });
        }
