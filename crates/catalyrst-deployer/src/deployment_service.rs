...
impl DeploymentService {
    pub async fn deploy_entity(
        ...
        if entity.pointers.is_empty() {
            return DeploymentResult::Invalid(vec!["The entity does not have any pointer.".into()]);
        }
    ...
    async fn deploy_entity_inner(
        ...
        let is_content_unchanged = self.check_content_unchanged(entity, context).await;
        ...
        let checks = match self
            .build_validation_checks(entity, is_content_unchanged, context)
            .await
        {
            Ok(c) => c,
            Err(e) => {
        ...
        if let Err(e) = self.store_content(hashes).await {
            ...
            return DeploymentResult::Invalid(vec!["Failed to store entity content".into()]);
        }
        ...
        match self.persist_deployment(entity, audit_info, hashes).await {
            ...
            Err(e) => {
                ...
                DeploymentResult::Invalid(vec!["There was an error deploying the entity".into()])
            }
        }
    }
    ...
    async fn check_content_unchanged(&self, entity: &Entity, context: DeploymentContext) -> bool {
        ...
        match self
            ...
            .await
        {
            Ok(active) if !active.is_empty() => active[0].metadata == entity.metadata,
            Ok(_) => false,
            Err(e) => {
    ...
    async fn build_validation_checks(
        ...
        let is_already_deployed = self.database.deployment_exists(&entity.id).await?;
    ...
    async fn persist_deployment(
        ...
        let pointer_results = self
            .pointer_manager
            .reference_entity_from_pointers(&*self.database, entity, &overwrote_set, is_overwritten)
            .await?;
