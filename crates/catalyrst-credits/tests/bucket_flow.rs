...
async fn expired_history_wallet_refunds_normally_without_expiry() {
    ...
    // This assertion was previously inverted (it asserted applied == 5.0),
    // encoding the unclamped-refund hole: `refund_in_tx` only applied its
    // cumulative clamp when spend rows existed, so any tx_ref without them was
    // refunded for whatever the caller asked. `ports/packs.rs` hit that path on
    // every Stripe reversal by passing a Stripe EVENT ID as tx_ref.
    let outcome = credits
