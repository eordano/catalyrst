//! Round-trip test for the external captcha provider against an in-process mock
//! siteverify endpoint. No live hCaptcha/reCAPTCHA service is contacted: the mock
//! mimics the provider contract (form-POST `secret`+`response`, JSON `success`)
//! and asserts the secret is forwarded and the verdict parsed.
...
async fn siteverify(Form(form): Form<HashMap<String, String>>) -> Json<Value> {
    // The provider trusts the secret; reject if the server forwarded the wrong one.
...
async fn invalid_token_rejected() {
    let addr = spawn_mock().await;
    let provider = provider_for(addr);
    assert!(!provider.verify("wrong-token", Some("1.2.3.4")).await.unwrap());
}
...
async fn unreachable_provider_errors() {
    // A provider that can't be reached must error, never silently pass the gate.
    let provider = CaptchaProvider::new(
        SECRET.to_string(),
