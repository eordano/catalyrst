...
pub fn test_state_with_market(
    ...
    Arc::new(AppStateInner {
        ...
        credits_onchain_authorize_enabled: false,
        checkout_success_url: String::new(),
        checkout_cancel_url: String::new(),
    })
}

pub fn test_state_onchain(
    pool: sqlx::PgPool,
    enabled: bool,
    signer_key: Option<&str>,
    manager_contract: Option<&str>,
) -> AppState {
    let http = reqwest::Client::new();
    Arc::new(AppStateInner {
        credits: CreditsComponent::new(pool.clone()),
        ...
        checkout_fulfillment_mode: "secondary".into(),
        require_purchase_intent: false,
        economy_base_url: "http://127.0.0.1:1".into(),
        economy_admin_token: Some("test-token".into()),
        escrow_address: Some("0x0000000000000000000000000000000000000001".into()),
        ...
        economy_http: http,
        quote_cache: Default::default(),
        credits_signer_key: signer_key.map(|s| s.to_string()),
        credits_manager_contract: manager_contract.map(|s| s.to_string()),
        credits_onchain_authorize_enabled: enabled,
        checkout_success_url: String::new(),
        checkout_cancel_url: String::new(),
    })
}
