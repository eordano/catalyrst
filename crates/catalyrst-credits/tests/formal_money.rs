...
//! ```
//!
//! Without the env var every test short-circuits to a pass, matching the rest
//! of this crate's PG-gated suites.
...
async fn charge_refund_revokes_and_reports_the_unrecoverable_shortfall() {
    ...
    credits
        ...
        .await
        .unwrap();
    // Buyer spends 70 before disputing.
    credits
...
async fn revoke_debits_the_paid_bucket_first() {
    ...
    // ...and the ledger says the same thing: the debit landed in `paid`.
...
async fn revoke_without_a_wallet_row_is_still_audited_as_a_total_loss() {
    ...
    // A paid purchase whose wallet row never existed.
    seed_paid_purchase(&pool, &addr, &pi, "100", 1000).await;
    ...
    // No balance moved, so no ledger row and no invented wallet row.
...
async fn pack_reversal_is_bounded_by_the_purchase_across_replays() {
    ...
    // Partial: 300 of 1000 cents -> 30 credits.
    credits
    ...
    // Redelivery of the same cumulative amount changes nothing.
    assert!(matches!(
        credits
    ...
    // Remaining cumulative refund settles on the exact remainder, no dust.
    credits
    ...
    // A dispute arriving after the full refund adds nothing more.
    let after = credits
...
async fn zero_total_checkout_completes_and_moves_nothing() {
    ...
    // (a) a wallet WITH a balance: the checkout runs and the balance is intact.
    let addr = scratch_addr();
    seed_wallet(&pool, &addr, "20", "80").await;
    ...
    // (b) a wallet with NO `user_credits` row: zero is still affordable.
    let fresh = scratch_addr();
    credits
...
// ------------------------------------- 4. f64 filter vs NUMERIC effect ------
...
async fn sub_f64_amounts_keep_ledger_and_balance_in_lockstep() {
    ...
    // (a) spend: the debit comes out of the EARNED bucket, so the old fallback
    // wrote it to `paid` and desynchronised the earned invariant.
    let addr = scratch_addr();
    ...
    // (b) refund: no fallback existed there, so both rows were dropped while
    // the balance moved.
    credits
    ...
    // (c) admin revoke: same filter, same fallback, same divergence.
    credits
...
// ------------------------------------ 5. ledger replay vs 0012 backfill -----
...
async fn clamped_backfill_origin_is_recorded_as_an_adjustment_row() {
    ...
    // The adjustment is explicit and greppable, not a silent balance rewrite.
    ...
    // Idempotent: nothing divergent is left, so a second pass is a no-op.
    let second = credits
...
async fn realignment_batches_the_walk_and_stays_idempotent() {
    ...
    // The default entry point walks the same wallets to the same fixed point.
    let third = credits
...
async fn refund_restores_earned_first_by_design() {
    ...
    // Spend 25: earned-first -> 20 earned + 5 paid.
    credits
    ...
    // Refunding the remainder returns the wallet exactly to its opening state.
    credits
...
// ------------------------------- rescued from the formal credits bridge -----
