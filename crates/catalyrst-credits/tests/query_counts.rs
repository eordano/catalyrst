...
//! a scratch PostgreSQL, counts the sqlx statements it emitted (via the scoped
//! `common::sql_capture()` subscriber), and asserts BOTH the collapsed count and
//! that the observable result (money strings, ordering, state, response shape) is
...
//! the pool's query futures are polled on the thread that owns the capture
//! subscriber.
...
async fn spend_in_tx_locks_user_credits_once() {
    ...
    // Funded wallet: earned 2 / paid 3 (available 5).
    sqlx::query(
    ...
    // (a) funded spend: shape + money parity.
    ...
    // (b) never-seeded address: payment_required.
...
async fn get_cart_total_in_single_query() {
    ...
    // Mixed lines: "1.50" x2, "0.00" x1, "2" x3.
    ...
    // Order preserved (added_at, id) and per-line price strings unchanged.
    ...
    // Empty cart: still one statement, empty items, literal "0" total.
...
async fn quote_amounts_batched_single_query() {
    ...
    // 60 amounts interleaving ~40 valid weis with invalid entries at known slots.
    let valid_weis = [
        "1000000000000000000",
        "2500000000000000000",
        "500000000000000000",
...
async fn captcha_generate_single_statement() {
    ...
    // Post-state parity with the old UPDATE-then-INSERT sequence.
