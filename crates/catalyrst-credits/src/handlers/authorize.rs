...
use crate::handlers::{csprng_32_hex, signer_from};
...
pub(crate) fn derive_idempotency_key(
    ...
    h.update([0x1f]);
    h.update(item_key.as_bytes());
    h.update([0x1f]);
    h.update(usd_price_cents.to_string().as_bytes());
    h.update([0x1f]);
    h.update(auth_ts.as_bytes());
    format!("authz:{}", hex::encode(h.finalize()))
}
...
pub async fn authorize(
    ...
    if !state.credits_onchain_authorize_enabled {
        return Err(ApiError::service_unavailable(
            "on-chain credit authorization is disabled; set CREDITS_ONCHAIN_AUTHORIZE_ENABLED=true \
             to enable treasury-claim minting",
        ));
    }
    ...
    let id = format!(
        "0x{}",
        csprng_32_hex().ok_or_else(|| ApiError::Internal("CSPRNG unavailable".into()))?
    );
    ...
    let item_key = if has_trade {
        format!("trade:{}", body.trade_id.as_deref().unwrap_or_default())
    } else {
        format!(
            "mint:{}:{}",
            body.contract_address.as_deref().unwrap_or_default(),
            body.item_id.as_deref().unwrap_or_default()
        )
    };
    let auth_ts = headers
        .get("x-identity-timestamp")
    ...
    let issued_expires_ms = reserved.expires_at.timestamp_millis();
    ...
    Ok(Json(AuthorizeCreditOut {
        credit: AuthorizedCreditOut {
            id: reserved.id,
            amount: reserved.amount_wei.clone(),
            available_amount: reserved.amount_wei.clone(),
            expires_at: issued_expires_ms,
            signature,
            contract: contract.clone(),
        },
        max_credited_value: reserved.amount_wei,
        usd_cents: reserved.usd_cents,
        oracle_rate,
    }))
}
...
mod tests {
    ...
    fn idempotency_key_is_deterministic_and_input_sensitive() {
        let base = derive_idempotency_key("0xABC", "trade:t1", 500, "1690000000000");
        assert_eq!(
            base,
            derive_idempotency_key("0xabc", "trade:t1", 500, "1690000000000"),
            "signer case is normalized, so the key is stable"
        );
        assert!(base.starts_with("authz:"));
        assert_ne!(
            base,
            derive_idempotency_key("0xABC", "trade:t1", 501, "1690000000000"),
            "a different price is a different request"
        );
        assert_ne!(
            base,
            derive_idempotency_key("0xABC", "mint:0xc:9", 500, "1690000000000"),
            "a different item is a different request"
        );
        assert_ne!(
            base,
            derive_idempotency_key("0xABC", "trade:t1", 500, "1690000000001"),
            "a different auth timestamp is a different request"
        );
    }
