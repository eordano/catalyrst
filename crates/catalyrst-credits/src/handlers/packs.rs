...
/// and purchase_intent.rs). This is the ONLY one of the five with no
/// digit/grammar validation at all -- pure length/lexicographic comparison
/// against the cap `"10000"` -- and it is only ever reached after
/// `validate_positive_amount` (handlers/admin/common.rs) has already rejected
/// anything containing `e`/`E`/`-`/repeated `.`/whitespace. Standalone on
/// ungated input it silently mis-reads scientific notation and surrounding
/// whitespace as "small".
#[cfg(test)]
