...
/// Listing-backed bases (a specific V2 order OR a specific signed trade)
/// sell exactly one token per checkout. Trade lines are qty=1 by design:
/// the unified /v1/orders scan only surfaces public_nft_order trades (one
/// token per signature); item-order trades (uses>1) are not served through
/// this path, so no multi-use fill is attempted even though the contract's
/// signatureUses accounting would technically allow one accept per use.
...
mod tests {
    ...
    fn trade_basis_is_one_per_checkout_too() {
        // a signed nft order sells exactly one token; qty is capped at 1
