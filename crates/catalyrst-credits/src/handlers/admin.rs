...
pub fn router() -> Router<AppState> {
    Router::new()
        .route(
            "/admin/seasons",
            get(list_seasons).post(create_season),
        )
        .route(
            "/admin/seasons/{id}",
...
fn validate_goal_common(
    ...
    if total_steps < 1 || total_steps > 100_000 {
...
async fn revoke_credits(
    ...
    let outcome = state
        .credits
        .admin_revoke_credits(&address, &amount, reason.as_deref(), actor.as_deref(), &detail)
        .await?;
    Ok(Json(GrantOut {
        address,
        applied: outcome.applied,
...
async fn block_user(
    ...
    let blocked = state
        .credits
        .admin_set_blocked(&address, b.blocked, reason.as_deref(), actor.as_deref(), &detail)
        .await?;
