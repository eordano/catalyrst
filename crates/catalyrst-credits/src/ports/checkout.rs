...
pub struct RepricedLine {
    ...
    /// mode "trade": the marketplace.trades uuid the charge was pinned to.
...
impl OutboxWorker {
    ...
    async fn attempt_broker_buy(&self, row: &OutboxRow, token: &str, escrow: &str) -> BrokerResult {
        ...
        let (price_wei, token_id, trade_payload): (String, Option<String>, Option<serde_json::Value>) = if row.mode == "trade" {
            // Trade lines are ALWAYS pinned at checkout (trade uuid + basis);
            // there is no fresh-selection fallback for a signed listing.
            ...
            // Fail-closed pre-flight (same pattern as the primary
            // store_mintable gate): the trade must still be status='open' in
            // the unified mv before we pay for the accept() attempt.
        ...
        if let Some(trade) = trade_payload {
            // the FULL signed-trade payload — economy has no market-DB access
            body["trade"] = trade;
        }
