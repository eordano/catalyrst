...
pub struct ReconcileReport {
    ...
    #[serde(rename = "creditUsagesReconciled")]
    pub credit_usages_reconciled: Vec<CreditUsageDiff>,
}
...
impl CreditsComponent {
    pub async fn reconcile(
        ...
        let credit_usage_drift = credit_usages_reconciled
            .iter()
            .filter(|d| !d.consumed)
            .count();
        if credit_usage_drift > 0 {
            tracing::error!(
                invariant = "credit_used==debited",
                count = credit_usage_drift,
                credit_ids = ?credit_usages_reconciled
                    .iter()
                    .filter(|d| !d.consumed)
                    .map(|d| d.credit_id.as_str())
                    .collect::<Vec<_>>(),
                "RECONCILE ALERT: on-chain CreditUsed events could not debit the wallet balance \
                 (insufficient balance or spend failure); the authorization stays 'authorized'"
            );
        }
        ...
        Ok(ReconcileReport {
            ...
            credit_usages_reconciled,
        })
    }

    /// Debit balances for authorizations redeemed on-chain (a `CreditUsed`
    /// event in `credit_usages`). The row is flipped to 'consumed' BEFORE the
    /// debit so the spend's reservation-aware sufficiency check does not count
    /// the amount it is about to take; on failure the whole transaction rolls
    /// back and the row stays 'authorized' for the next pass.
    async fn reconcile_credit_authorizations(&self) -> Result<Vec<CreditUsageDiff>, ApiError> {
        let candidates: Vec<(String, String, i64)> = sqlx::query(
            "SELECT a.id, a.address, a.usd_cents \
             FROM credit_authorizations a \
             JOIN credit_usages u ON u.credit_id = a.id \
        ...
        for (credit_id, address, usd_cents) in candidates {
                ...
                .await
            {
                Ok(spent) => {
                    tx.commit().await?;
                    out.push(CreditUsageDiff {
                        credit_id,
                        address,
                        debited: amount,
                        consumed: true,
                        replayed: spent.replayed,
                        new_balance: spent.available,
                    });
                }
                Err(e) => {
                    tx.rollback().await?;
                    tracing::error!(
                        ...
                        address = %address,
                        error = %e,
                        "credit-used debit FAILED; authorization stays 'authorized' for the next pass"
                    );
                    out.push(CreditUsageDiff {
                        credit_id,
                        address,
