...
/// Which venue fulfils a resale listing surfaced by the unified /v1/orders.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum ListingVenue {
    /// on-chain MarketplaceV2 order — fulfilled by executeOrder(tokenId)
    V2 { token_id: String },
    /// Marketplace v3 off-chain signed trade — fulfilled by
    /// DecentralandMarketplacePolygon.accept(trade), pinned by the trade uuid
...
/// What a checkout line's charge was pinned to.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum BasisKind {
    /// collection-store mint (no specific token)
    Primary,
    /// a specific on-chain MarketplaceV2 listing
    Secondary { token_id: String },
    /// a specific off-chain signed trade (Marketplace v3)
    Trade { trade_id: String },
}

impl BasisKind {
    /// Listing-backed bases sell ONE specific token/signature per checkout.
...
/// DecentralandMarketplacePolygon 1.0.0 — the off-chain signed-trade executor.
/// Unified /v1/orders rows whose marketplaceAddress is this contract are
/// Marketplace v3 trade listings (mv_trades), pinned by trade uuid.
/// The OTHER off-chain venue (OffChainMarketplaceV2, 0xa40b1d12...) has a
/// different EIP-712 domain and stays rejected until the economy broker
/// supports it.
...
impl PricingClient {
    ...
    /// Cheapest fulfillable on-chain MarketplaceV2 listing ONLY (used by the
    /// outbox worker's legacy unpinned-secondary fallback, which can only
    /// fulfil via executeOrder).
    ...
    /// Cheapest fulfillable listing across the supported venues
    /// (MarketplaceV2 orders, and — when `include_trades` — Marketplace v3
    /// signed trades).
    ...
    /// Full trade JSON (signature + checks + assets + mv status) from
    /// catalyrst-market GET /v1/trades/{id} — the payload the economy
    /// broker's mode "trade" needs.
...
pub fn resolve_basis(
    ...
    // FREE mints (price 0) are NOT a checkout basis: the grid already treats
    // them as not-buyable (isCatalogItemBuyable), and a 0-Credit checkout
    // would bypass the payment step entirely while spending relayer gas on
    // every "purchase" — an unmetered grief vector. Caught live by the
    // money-path smoke: a 1-Credit listed item priced at 0 because its
    // collection also had a free mint.
    ...
    match mode {
        ...
        // auto = CHEAPEST fulfillable among {V2 listing, trade listing,
        // available mint > 0}. Tie order: on-chain V2, then trade, then mint —
        // the V2-vs-trade tie is settled inside select_cheapest_listing, and
        // the mint only wins when STRICTLY cheaper than the chosen listing.
...
/// Venue preference on an exact price tie: on-chain V2 first, then trade.
...
fn select_cheapest_listing(
    ...
    for o in orders {
        ...
        if !is_v2 && !is_trade {
            // every other venue (incl. the 0xa40b... OffChainMarketplaceV2 and
            // the Ethereum trade contracts) stays rejected
            continue;
        }
        ...
        let venue = if is_v2 {
            ...
            // unified trade rows carry the trade uuid as both id and tradeId
...
mod tests {
    ...
    fn trade_order(
        ...
        serde_json::json!({
            "id": trade_id,
            "tradeId": trade_id,
            // the mv_trades branch surfaces the checksummed trade contract
    ...
    fn select_cheapest_listing_accepts_trade_venue_pinned_by_trade_id() {
        let now = 1_000i64;
        let orders = vec![
            order(MARKETPLACE_V2_POLYGON, "2901", "20000000000000000", "open", 9_999),
            trade_order("1bbe7d78-dd71-4cbe-9085-70d679d3ad11", "2902", "10000000000000000", "open", 9_999),
        ];
        let got = select_cheapest_listing(&orders, "0", now, true).expect("a match");
        assert_eq!(
            got.venue,
    ...
    fn price_tie_prefers_the_onchain_v2_listing_over_the_trade() {
        ...
        let orders = vec![
            trade_order("1bbe7d78-dd71-4cbe-9085-70d679d3ad11", "2902", "10000000000000000", "open", 9_999),
            order(MARKETPLACE_V2_POLYGON, "2901", "10000000000000000", "open", 9_999),
        ];
        let got = select_cheapest_listing(&orders, "0", now, true).expect("a match");
        assert!(
    ...
    fn trades_are_excluded_when_the_caller_cannot_fulfil_them() {
        // the outbox worker's legacy unpinned-secondary path can only
        // executeOrder on V2, so its selection must never return a trade
        let now = 1_000i64;
    ...
    fn auto_ignores_a_free_mint_and_charges_the_listing() {
        // Free mints are not a checkout basis (0-Credit checkout = payment
        // step bypassed + relayer gas grief). The real listing wins.
        let listing = OpenListing {
            venue: ListingVenue::V2 {
                token_id: "2901".into(),
            },
