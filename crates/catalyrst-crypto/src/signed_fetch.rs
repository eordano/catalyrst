...
//! call-site parameter. Current per-service windows, preserved verbatim from
//! the pre-consolidation copies and pending a product ruling on the outliers:
//!
//! | service | entry point | window |
//! |---|---|---|
//! | camera-reel, credits, events, market, notifications, places | require_signer / try_extract_signer | 5 min |
//! | builder | require_signer | 30 min |
//! | builder | try_extract_signer | 5 min |
//! | comms | require_signer / try_extract_signer | 5 min |
//! | comms | verify_signed_fetch (scene admin/bans/voice/adapter) | 1 min |
//! | social-service rest | require_signer / try_extract_signer | 5 min |
//! | social-service rpc, quests | handshake::verify_handshake | 5 min (quests: env-overridable) |
//! | scene-state | verify_auth_frame (service-local extraction) | 5 min |
//! | server, worlds | require_verified | 5 min |
//! | world-storage | verify_request (service-local freshness, accepts future timestamps) | 1 min |
