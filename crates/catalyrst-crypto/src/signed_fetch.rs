...
//! | signatures | `auth::require_signer` (own copy of the whole module) | AUTH_EXPIRATION_SECONDS, default +/- 5 min |
//! | world-storage | `auth_chain::verify_request` -> local `check_freshness` | 1 min in the past, UNBOUNDED in the future (no `.abs()`) |
