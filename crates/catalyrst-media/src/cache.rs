...
pub async fn fetch(
    ...
    Ok(rows
        .into_iter()
        .map(
            |(text_sha256, translated_text, detected_language, detected_confidence)| CachedRow {
                text_sha256,
                translated_text,
                detected_language,
                detected_confidence,
...
pub async fn store(
    ...
    let translated: Vec<String> = rows
        .iter()
        .map(|(_, i)| i.translated_text.clone())
        .collect();
    let languages: Vec<String> = rows
        .iter()
        .map(|(_, i)| i.detected_language.clone())
        .collect();
