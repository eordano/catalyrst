//! Fixed-window rate limits on POST /entities, keyed per client: a short burst window and a daily
//! quota, counted in independent buckets so a request must pass both.
//!
//! Env: `POST_ENTITIES_RATE_LIMIT_MAX` (default 200), `POST_ENTITIES_RATE_LIMIT_WINDOW_SECONDS`
//! (default 60), `POST_ENTITIES_DAILY_QUOTA_MAX` (default 300, always over a 24-hour window),
//! `TRUSTED_CLIENT_IP_HEADER` (unset by default, matching upstream). Unset keys on the socket peer,
//! which is correct only for a directly exposed process -- and this process never is: POST
//! /entities arrives through the nginx front, so the peer is the front for every request and the
//! per-client limits collapse into one global budget shared by every deployer. Deployment note: the
//! catalyrst-live unit environment must set `TRUSTED_CLIENT_IP_HEADER=x-real-ip`; the front already
//! writes `X-Real-IP $remote_addr` via its reverse-proxy config.
...
const DEFAULT_BURST_MAX: u64 = 200;
const DEFAULT_BURST_WINDOW_SECONDS: u64 = 60;
const DEFAULT_DAILY_QUOTA_MAX: u64 = 300;
const DAILY_QUOTA_WINDOW_SECONDS: u64 = 86_400;
...
pub const BURST_BUCKET: &str = "/entities burst";
pub const DAILY_QUOTA_BUCKET: &str = "/entities daily-quota";
...
pub struct PostEntitiesLimiters {
    burst: Arc<PostEntitiesRateLimiter>,
    daily_quota: Arc<PostEntitiesRateLimiter>,
}
...
impl PostEntitiesLimiters {
    pub fn from_env() -> Self {
        let burst_max = parse_positive(
        ...
        let burst_window_seconds = parse_positive(
        ...
        if burst_window_seconds > MAX_WINDOW_SECONDS {
            panic!("invalid POST_ENTITIES_RATE_LIMIT_WINDOW_SECONDS: {burst_window_seconds} exceeds a day and is almost certainly milliseconds by mistake");
        }
        let daily_quota_max = parse_positive(
            "POST_ENTITIES_DAILY_QUOTA_MAX",
            std::env::var("POST_ENTITIES_DAILY_QUOTA_MAX")
                .ok()
                .as_deref(),
            DEFAULT_DAILY_QUOTA_MAX,
        );
    ...
    fn new(
        ...
        Self {
            burst: Arc::new(PostEntitiesRateLimiter::new(
                BURST_BUCKET,
                burst_max,
                burst_window_seconds,
                trusted_client_ip_header.clone(),
            )),
            daily_quota: Arc::new(PostEntitiesRateLimiter::new(
                DAILY_QUOTA_BUCKET,
                daily_quota_max,
...
mod tests {
    ...
    fn real_ip_header() -> Option<HeaderName> {
    ...
    fn counts_reject_over_budget_and_reset_next_window() {
        let limiter = PostEntitiesRateLimiter::new(BURST_BUCKET, 2, 60, None);
    ...
    #[test]
    fn buckets_count_one_identity_independently() {
        let burst = PostEntitiesRateLimiter::new(BURST_BUCKET, 1, 60, None);
        let daily =
            PostEntitiesRateLimiter::new(DAILY_QUOTA_BUCKET, 3, DAILY_QUOTA_WINDOW_SECONDS, None);
        let t0 = 1_000_000_000_000;
        assert!(burst.decide_at("a", 1, t0).allowed);
        assert!(!burst.decide_at("a", 1, t0).allowed);
        assert!(daily.decide_at("a", 3, t0).allowed);
        assert!(daily.decide_at("a", 3, t0).allowed);
        assert!(daily.decide_at("a", 3, t0).allowed);
        let rejected = daily.decide_at("a", 3, t0);
        assert!(!rejected.allowed);
        assert!(
            rejected.retry_after_seconds >= 1
                && rejected.retry_after_seconds <= DAILY_QUOTA_WINDOW_SECONDS
        );
        assert!(burst.decide_at("a", 1, t0 + 2 * 60_000).allowed);
        assert!(!daily.decide_at("a", 3, t0).allowed);
    }

    #[test]
    fn phase_is_per_bucket_as_well_as_per_identity() {
        let window_ms = DAILY_QUOTA_WINDOW_SECONDS * 1000;
        assert_ne!(
            phase_offset(BURST_BUCKET, "203.0.113.7", window_ms),
            phase_offset(DAILY_QUOTA_BUCKET, "203.0.113.7", window_ms)
        );
    }

    #[test]
    fn daily_quota_window_is_fixed_and_tightens_the_fallback_bucket() {
        let limiters = PostEntitiesLimiters::new(200, 60, 300, None);
        assert_eq!(
            limiters.daily_quota.window_ms,
            DAILY_QUOTA_WINDOW_SECONDS * 1000
        );
        assert_eq!(limiters.daily_quota.max, 300);
        assert_eq!(limiters.daily_quota.fallback_max, 30);
        assert_eq!(limiters.burst.window_ms, 60_000);
        assert_eq!(limiters.burst.fallback_max, 20);
    }

    #[test]
    fn trusted_header_later_line_wins() {
        let limiter = PostEntitiesRateLimiter::new(BURST_BUCKET, 2, 60, real_ip_header());
    ...
    fn limited_app(limiter: Arc<PostEntitiesRateLimiter>) -> Router {
        Router::new()
            ...
            .route_layer(from_fn_with_state(limiter, post_entities_rate_limit))
    }

    fn stacked_app(burst_max: u64, daily_quota_max: u64) -> (Router, PostEntitiesLimiters) {
        let limiters = PostEntitiesLimiters::new(burst_max, 60, daily_quota_max, real_ip_header());
        let app = Router::new().route("/entities", limiters.guard(post(|| async { "deployed" })));
        (app, limiters)
    }
    ...
    async fn unidentified_clients_share_one_tightened_bucket() {
        let limiter = Arc::new(PostEntitiesRateLimiter::new(BURST_BUCKET, 30, 60, None));
        let app = limited_app(limiter);
    ...
    #[tokio::test]
    async fn exhausted_daily_quota_rejects_with_burst_budget_untouched() {
        let (app, limiters) = stacked_app(10, 1);
        let ok = app
            .clone()
            .oneshot(deploy_req("203.0.113.7"))
            .await
        ...
        assert!(limiters.burst.decide("203.0.113.7", 10).allowed);
        assert!(!limiters.daily_quota.decide("203.0.113.7", 1).allowed);
    ...
    #[tokio::test]
    async fn burst_rejection_never_spends_daily_quota() {
        let (app, limiters) = stacked_app(1, 2);
        ...
        assert!(limiters.daily_quota.decide("203.0.113.7", 2).allowed);
        assert!(!limiters.daily_quota.decide("203.0.113.7", 2).allowed);
    }
    async fn expose_handler_label(request: Request, next: Next) -> Response {
        let label = handler_label(&request);
    ...
    #[tokio::test]
    async fn handler_label_is_the_route_template_not_the_request_path() {
        let limiters = PostEntitiesLimiters::new(10, 60, 10, real_ip_header());
        let app = Router::new().nest(
            "/content",
            Router::new().route(
                "/entities/{id}",
                limiters
                    .guard(post(|| async { "deployed" }))
                    .route_layer(axum::middleware::from_fn(expose_handler_label)),
            ),
        );
        let request = Request::builder()
            .method("POST")
            .uri("/content/entities/bafkreiabc123")
        ...
        assert_eq!(
            response.headers().get("x-test-handler").unwrap(),
            "/content/entities/{id}"
        );

        assert_eq!(handler_label(&deploy_req("203.0.113.7")), "");
    }
}
