...
pub(crate) fn app_state_with_storage(storage: Arc<dyn ContentStorage>) -> Arc<AppState> {
    app_state_with(storage, Arc::new(StubDeployer))
}

pub(crate) fn app_state_with_deployer(deployer: Arc<dyn Deployer>) -> Arc<AppState> {
    app_state_with(Arc::new(EmptyStorage), deployer)
}

fn app_state_with(storage: Arc<dyn ContentStorage>, deployer: Arc<dyn Deployer>) -> Arc<AppState> {
    Arc::new(AppState {
        storage,
