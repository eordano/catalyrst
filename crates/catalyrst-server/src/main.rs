...
impl Database for StubDatabase {
    async fn active_entities_by_pointers(&self, _pointers: &[String]) -> Result<Vec<Value>, DatabaseError> {
        Ok(vec![])
    }
    ...
    async fn active_entity_ids_by_content_hash(&self, _hash: &str) -> Result<Vec<String>, DatabaseError> {
        Ok(vec![])
    }
    async fn get_deployments(&self, _options: &DeploymentQueryOptions) -> Result<DeploymentQueryResult, DatabaseError> {
    ...
    async fn get_audit_info(&self, _entity_type: &str, _entity_id: &str) -> Result<Option<Value>, DatabaseError> {
        Ok(None)
    }
...
async fn main() {
    ...
    if read_only {
        tracing::info!("Content Server running in read-only mode. POST /entities will not be exposed");
    }

    let content_public_url = env_or(
        "CONTENT_URL",
        &format!("http://{}:{}/content", host, port),
    );
    let lambdas_public_url = env_or(
        "LAMBDAS_URL",
        &format!("http://{}:{}/lambdas", host, port),
    );
