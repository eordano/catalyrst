...
/// tailnet (no TLS terminator) can opt out with `ADMIN_COOKIE_INSECURE=1`.
...
fn parse_message(message: &str) -> Option<ParsedMessage> {
    ...
    let exp = chrono::DateTime::parse_from_rfc3339(exp_raw).ok()?.timestamp();
    Some(ParsedMessage {
        host,
        address,
        nonce,
...
mod tests {
    ...
    fn valid_eth_address_checks_shape() {
        assert!(valid_eth_address("0xaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"));
        assert!(valid_eth_address("0xAbC0000000000000000000000000000000000000"));
        assert!(!valid_eth_address("0x123")); // too short
        assert!(!valid_eth_address("aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa")); // no 0x
        assert!(!valid_eth_address("0x../../etc/passwd")); // path injection attempt
        assert!(!valid_eth_address("0xzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzz")); // non-hex
    }

    #[test]
    fn url_host_extracts_host() {
        assert_eq!(url_host("https://realm.example.com/admin").as_deref(), Some("realm.example.com"));
        assert_eq!(url_host("http://127.0.0.1:5141").as_deref(), Some("127.0.0.1:5141"));
        assert_eq!(url_host("https://h.test/a/b?c=d").as_deref(), Some("h.test"));
    }
    ...
    fn cookie_value_finds_named_cookie() {
        let mut h = HeaderMap::new();
        h.insert(header::COOKIE, "other=1; cat_admin=the-value; x=2".parse().unwrap());
