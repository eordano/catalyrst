...
impl SyncOrchestratorRefs {
    ...
    async fn run_bootstrap(&self) -> Result<(), SyncError> {
        ...
        loop {
            let result = if self.config.phased_sync {
    ...
    /// with exponential backoff, until none remain (or the orchestrator stops). Runs
    /// unfiltered on purpose: a straggler may have been held back by either phased slice, and
    /// only a full pass proves its snapshots complete so they can be marked processed.
    async fn retry_bootstrap_stragglers(&self) -> Result<(), SyncError> {
        ...
        loop {
            ...
            match self.bootstrap_from_snapshots(None, true).await {
                Ok(()) => {}
                Err(SyncError::Stopped) => return Err(SyncError::Stopped),
                Err(e) => warn!(error = %e, "Snapshot bootstrap retry failed"),
            }
            match self.bootstrap_from_pointer_changes(None).await {
                Ok(()) => {}
                Err(SyncError::Stopped) => return Err(SyncError::Stopped),
    ...
    async fn save_frontier(&self) {
        // The persisted frontier is max-over-servers: every pointer-changes poll boundary
        // writes its confirmed cursor through the GREATEST-monotonic advance_sync_frontier,
        // and on restart that one global value fast-forwards EVERY server — including servers
        // that never completed bootstrap. A min-over-servers floor offered here can never bind
        // against that max, so this method records liveness only. Expressing "never run ahead
        // of the slowest bootstrapped server" requires per-server persisted cursors
        // (upstream's lastEntityTimestampFromSnapshotsByServer shape), which this port does
        // not have.
        let _ = self
            .deployment_repo
...
/// ran without an entity-type filter, and the snapshot's cross-pass bookkeeping (failed in an
/// earlier pass / cleanly deployed by an earlier non-marking pass).
fn snapshot_pass_verdict(
    ...
    if failed_this_pass {
        ...
        // A clean unfiltered pass proves the whole snapshot deployed, whatever any earlier
        // pass saw.
        SnapshotPassVerdict::Retire
...
mod tests {
    ...
    use super::{apply_snapshot_fetch, snapshot_pass_verdict, SnapshotPassVerdict};
