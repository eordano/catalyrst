...
impl LiveDeploymentRepository {
    ...
    pub async fn advance_sync_frontier(&self, timestamp: Timestamp) -> Result<(), SyncError> {
        sqlx::query(
            "INSERT INTO system_properties (key, value) VALUES ('sync_frontier', $1) ON CONFLICT (key) DO UPDATE SET value = GREATEST(system_properties.value::bigint, EXCLUDED.value::bigint)::text",
        ).bind(timestamp.to_string())
         .execute(&self.pool).await.map_err(|e| SyncError::Storage(e.to_string()))?;
        let previous = self
            .gauges
