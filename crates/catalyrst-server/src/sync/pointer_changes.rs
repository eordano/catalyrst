...
///
/// These values become the stream's high-water mark, and the persisted frontier only ever moves
/// forward (`advance_sync_frontier` is GREATEST-monotonic), so a single bad one is permanent:
