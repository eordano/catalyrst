        ...
        if let Some(repo) = &heartbeat_repo {
            if last_beat.is_none_or(|t| t.elapsed() >= std::time::Duration::from_secs(10)) {
                let _ = repo
                    ...
                    .await;
                if greatest_timestamp > last_persisted {
                    let _ = repo.advance_sync_frontier(greatest_timestamp).await;
                    last_persisted = greatest_timestamp;
                }
        ...
        for item in items {
            ...
            if let Some(local_ts) = deployment.local_timestamp {
                if local_ts >= options.from_timestamp {
                    ...
                    progress.fetch_max(greatest_timestamp, std::sync::atomic::Ordering::Relaxed);
                }
            }
