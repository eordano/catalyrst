...
pub fn bad_request(message: &str) -> Response {
    (
        ...
        Json(ApiErrorBody::labeled("Bad request", message)),
    )
        .into_response()
}
...
pub fn not_found(message: &str) -> Response {
    (
        ...
        Json(ApiErrorBody::labeled("Not Found", message)),
    )
        .into_response()
}
...
pub fn service_unavailable(message: &str) -> Response {
    (
        ...
        Json(ApiErrorBody::labeled("Service Unavailable", message)),
    )
        .into_response()
}
...
pub fn internal_server_error() -> Response {
    (
        StatusCode::INTERNAL_SERVER_ERROR,
        Json(ApiErrorBody::new("Internal Server Error")),
    )
        .into_response()
}
...
mod tests {
    ...
    async fn error_envelope_wire_shape() {
        ...
        assert_eq!(
            v,
            json!({ "ok": false, "error": "Not Found", "message": "Entity not found" })
        );
    }
