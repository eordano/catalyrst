...
impl TtlBucket {
    fn new(ttl: Duration, max_size: usize) -> Self {
        Self { ttl, max_size, entries: HashMap::new() }
    }
...
impl DeployRateLimiter {
    ...
    fn is_rate_limited(&self, etype: &str, pointers: &[String]) -> bool {
        ...
        b.get_mut(etype).map(|bk| bk.is_limited(pointers)).unwrap_or(false)
    }
...
impl WriteDeployer {
    ...
    pub fn new(
        ...
        let eip1654: Arc<dyn Eip1654Validator> =
            Arc::new(ValidationCache::new(Arc::new(RpcEip1654Validator::new(eth_rpc_url))));
        ...
        let tp_subgraph = match (tpr_subgraph_url, blocks_l2_subgraph_url) {
            (Some(tpr), Some(blocks)) => {
                Some(catalyrst_validator::tp_subgraph::TpSubgraph::new(blocks, tpr))
            }
            _ => None,
        };
...
impl Deployer for WriteDeployer {
    async fn deploy_entity(
        ...
        self.rate_limiter.record(entity.entity_type.as_str(), &pointers_lc);
...
mod tests {
    ...
    fn rate_limiter_ttl_and_size() {
        ...
        assert!(!bucket.is_limited(&p("0,0")), "entry should expire after ttl");

        let mut small = TtlBucket::new(Duration::from_secs(3600), 2);
        small.record(&p("a"));
        small.record(&p("b"));
        small.record(&p("c"));
        assert!(small.is_limited(&p("unrelated")), "over max_size trips bucket");
    }
