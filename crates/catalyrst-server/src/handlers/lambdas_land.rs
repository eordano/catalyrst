...
pub async fn user_lands(
    ...
    let elements: Vec<Value> = rows
        .into_iter()
        .map(
            |(name, contract_address, token_id, category, x, y, description, has_data, price, image)| {
...
pub async fn user_lands_permissions(
    ...
    let elements = match external_graph::parcels_by_update_operator(&state.eth_network, &update_operator)
        .await
    {
        Ok(e) => e,
