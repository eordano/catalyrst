...
//! it on the loopback port or over the private network, and front it with auth before
//! any public exposure.
...
/// Liveness path probed for a single-service bundle's overall health. Every
/// bundle answers `/health` except scene-state, which has no `/health` route —
/// its liveness endpoint is `/status` (already listed in its `detail`). Without
/// this, a healthy scene-state probes `/health` → 404 → renders falsely "down".
/// Multi-service bundles are unaffected (they always fetch `/health` as JSON).
