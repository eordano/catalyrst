...
pub(crate) fn extract_auth_chain_from_fields(fields: &BTreeMap<String, String>) -> Result<Option<Value>, AppError> {
    ...
    for i in 0..=biggest_index {
        ...
        let payload = fields
            .get(&payload_key)
            .ok_or_else(|| InvalidRequestError::new(format!("Missing auth chain element at index {i}")))?;
        ...
        .map_err(|e| InvalidRequestError::new(format!("Failed to read multipart field: {e}")))?
    {
        ...
        if field.file_name().is_some() {
            ...
            while let Some(chunk) = field.chunk().await.map_err(|e| {
                InvalidRequestError::new(format!("Failed to read file data: {e}"))
            })? {
            ...
            let value = field
                .text()
                .await
                .map_err(|e| InvalidRequestError::new(format!("Failed to read field value: {e}")))?;
...
mod tests {
    ...
    fn extract_auth_chain_caps_at_max_links() {
        ...
        let out = extract_auth_chain_from_fields(&f).expect("parser ok");
        let arr = out.expect("Some(array)").as_array().cloned().expect("is array");
        assert_eq!(
            arr.len(),
            101,
            "parser must currently include every consecutive index from 0..=biggest_index"
        );
    }
    ...
    fn extract_auth_chain_empty_returns_empty_vec() {
        ...
        let out = extract_auth_chain_from_fields(&f).expect("parser ok");
        assert!(out.is_none(), "expected Ok(None) when no authChain keys present");
    }
