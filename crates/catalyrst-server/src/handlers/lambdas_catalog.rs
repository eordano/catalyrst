...
fn parse_catalog_query(
    ...
    if let Some(ref t) = text_search {
        if t.chars().count() < 3 {
            return Err(bad_request("The text search must be at least 3 characters long"));
        }
    }
    let items_label = if id_param_name == "wearableId" { "wearables" } else { "emotes" };
    ...
    Ok(CatalogQuery {
        filters: CatalogFilters {
            collection_ids: if collection_ids.is_empty() { None } else { Some(collection_ids) },
            item_ids: if item_ids.is_empty() { None } else { Some(item_ids) },
            text_search,
        },
...
async fn fetch_item_urns(
    ...
    if let Some(ref cids) = filters.collection_ids {
        ...
        for c in cids {
            ...
            binds.push(Bind::Text(format!("{}:%", cursor_to_squid(&c.to_lowercase()))));
            idx += 1;
        }
    ...
    if let Some(ref iids) = filters.item_ids {
        ...
        binds.push(Bind::TextArray(
            iids.iter().map(|s| cursor_to_squid(&s.to_lowercase())).collect(),
        ));
        idx += 1;
    }
...
fn paginate(mut definitions: Vec<Value>, limit: i64) -> (Vec<Value>, Option<String>) {
    definitions.sort_by(|a, b| {
        let ai = a.get("id").and_then(|v| v.as_str()).unwrap_or("").to_lowercase();
        let bi = b.get("id").and_then(|v| v.as_str()).unwrap_or("").to_lowercase();
        ai.cmp(&bi)
    });
...
fn filter_and_extract_base_wearables(
    ...
    let mut matched: Vec<&crate::handlers::base_wearables::BaseWearable> = base
        ...
        .collect();
    matched.sort_by(|a, b| a.urn.to_lowercase().cmp(&b.urn.to_lowercase()));
...
async fn catalog_wearables_with_base(state: &AppState, query: CatalogQuery) -> Response {
    ...
    let next = next_last_id
        .map(|nl| format!("?{}", build_next_query(filters, limit, &nl, "wearableId")));
...
async fn catalog(
    ...
    let urns = fetch_item_urns(pool, item_type_prefix, &query.filters, query.limit, &query.last_id).await;
    ...
    let next = next_last_id.map(|nl| {
        format!("?{}", build_next_query(&query.filters, query.limit, &nl, id_param_name))
    });
...
mod tests {
    ...
    fn extract_wearable_rewrites_contents_and_images() {
        ...
        assert_eq!(def["thumbnail"], "https://c.example/content/contents/QmThumb");
        let contents = &def["data"]["representations"][0]["contents"];
        assert_eq!(contents[0]["key"], "male/x.glb");
        assert_eq!(contents[0]["url"], "https://c.example/content/contents/QmGlb");
    }
    ...
    fn parse_rejects_short_text_search() {
        let p = CatalogParams { text_search: Some("ab".into()), ..Default::default() };
        assert!(parse_catalog_query(&p, &[], "wearableId").is_err());
    }

    #[test]
    fn parse_accepts_collection_id_and_lowercases() {
        let p = CatalogParams { collection_id: vec!["URN:Decentraland".into()], ..Default::default() };
        let q = parse_catalog_query(&p, &[], "wearableId").unwrap();
        assert_eq!(q.filters.collection_ids.unwrap(), vec!["urn:decentraland".to_string()]);
    }
    ...
    fn build_next_query_orders_params() {
        ...
        assert_eq!(q, "collectionId=urn%3Ac1&textSearch=hat&limit=50&lastId=urn%3Ac1%3A5");
    }
    ...
    fn extract_emote_legacy_builds_adr74() {
        ...
        assert_eq!(def["emoteDataADR74"]["representations"][0]["contents"][0]["url"], "https://c/content/contents/QmE");
