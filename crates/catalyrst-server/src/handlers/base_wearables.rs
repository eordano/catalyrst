...
pub const BASE_AVATARS_COLLECTION_ID: &str = "urn:decentraland:off-chain:base-avatars";

pub const BASE_WEARABLES: &[&str] = &[
...
fn name_from_metadata(metadata: &Value, urn: &str) -> String {
    match metadata.get("name").and_then(|n| n.as_str()) {
        Some(n) if !n.is_empty() => n.to_string(),
        _ => urn.split(':').nth(4).unwrap_or("").to_string(),
    }
}

fn english_name_from_metadata(metadata: &Value) -> Option<String> {
    metadata
        .get("i18n")
        .and_then(|v| v.as_array())
        .and_then(|arr| {
            arr.iter().find_map(|entry| {
                let code = entry.get("code").and_then(|c| c.as_str())?;
                if code == "en" {
...
pub async fn fetch_base_wearables(state: &AppState) -> Vec<BaseWearable> {
    let pointers: Vec<String> = BASE_WEARABLES.iter().map(|u| u.to_string()).collect();
    let entities = state
        .database
    ...
    let mut out = Vec::with_capacity(BASE_WEARABLES.len());
    for urn in BASE_WEARABLES {
        let entity = match by_pointer.get(&urn.to_lowercase()) {
            Some(e) => e.clone(),
            None => continue,
        };
        let metadata = entity.get("metadata").cloned().unwrap_or(Value::Null);

        let real_urn = metadata
            .get("id")
            .and_then(|v| v.as_str())
            .map(|s| s.to_string())
            .unwrap_or_else(|| urn.to_string());
        let name = name_from_metadata(&metadata, &real_urn);
        let category = metadata
            ...
            .unwrap_or("")
            .to_string();
        let english_name = english_name_from_metadata(&metadata);
        out.push(BaseWearable {
            urn: real_urn,
            name,
            category,
            english_name,
