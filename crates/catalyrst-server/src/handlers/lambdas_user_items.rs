...
fn validate_and_sort(
    ...
    match (sort.as_str(), direction.as_str()) {
        ...
        ("name", "DESC") => {
            ...
            items.sort_by(|a, b| {
                locale_cmp(&a.name, &b.name).then_with(|| by_urn(a, b)).reverse()
            });
        }
        ("date", "ASC") => {
...
async fn owned_items_response(
    ...
    if include_definitions || include_entities {
        ...
        definitions = page
            .iter()
            .map(|i| by_pointer.get(&i.urn.to_lowercase()).cloned().unwrap_or(Value::Null))
            .collect();
    }
...
pub async fn user_third_party_wearables(
    ...
    let direction = qs_get_string(&params, "direction")
        .map(|s| s.to_uppercase())
        .unwrap_or_else(|| if sort == "name" { "ASC".into() } else { "DESC".into() });
    if !(sort == "name" && (direction == "ASC" || direction == "DESC")) {
        return bad_request(&format!("Invalid sorting requested: {} {}", sort, direction));
    }
...
mod tests {
    ...
    fn sort_least_rare_is_full_reverse() {
        ...
        validate_and_sort(&mut rarest, &parse_query_string("orderBy=rarity&direction=DESC")).unwrap();
        validate_and_sort(&mut least, &parse_query_string("orderBy=rarity&direction=ASC")).unwrap();
    ...
    fn sort_date_oldest_has_reversed_urn_tiebreak() {
        ...
        validate_and_sort(&mut oldest, &parse_query_string("orderBy=date&direction=ASC")).unwrap();
        ...
        validate_and_sort(&mut newest, &parse_query_string("orderBy=date&direction=DESC")).unwrap();
    ...
    fn sort_date_newest_orders_by_max_desc() {
        ...
        validate_and_sort(&mut items, &parse_query_string("orderBy=date&direction=DESC")).unwrap();
    ...
    fn invalid_sort_combination_errors_with_reference_message() {
        ...
        let err = validate_and_sort(&mut items, &parse_query_string("orderBy=bogus&direction=ASC"))
            .unwrap_err();
        assert_eq!(err, "Invalid sorting requested: bogus ASC");

        let err2 = validate_and_sort(&mut items, &parse_query_string("orderBy=rarity&direction=sideways"))
            .unwrap_err();
