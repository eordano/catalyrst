...
use crate::query_params::{camel_to_snake, parse_query_string, qs_get_array, qs_get_bool, qs_get_number, qs_get_string, to_query_string};
use crate::state::{AppState, CacheEntry, DeploymentQueryOptions, DEPLOYMENTS_CACHE_MAX_ENTRIES, DEPLOYMENTS_CACHE_TTL};
...
pub async fn get_deployments(
    ...
    .map_err(|_| AppError::ServiceUnavailable(
        "deployments query exceeded server-side time budget; narrow the time range or filters".into(),
    ))?
    ...
    if state.deployments_cache.len() >= DEPLOYMENTS_CACHE_MAX_ENTRIES {
        ...
        if do_sweep {
            state.deployments_cache.retain(|_, v| !v.is_expired(DEPLOYMENTS_CACHE_TTL));
        }
    }
    if state.deployments_cache.len() < DEPLOYMENTS_CACHE_MAX_ENTRIES {
...
fn filter_deployment_fields(dep: &Value, fields: &[String]) -> Value {
    ...
    for key in &["entityType", "entityId", "entityTimestamp", "deployedBy", "entityVersion"] {
...
fn calculate_next_relative_path(options: &DeploymentQueryOptions, last_deployment: &Value) -> String {
    let field = options
        .sorting_field
        .as_deref()
    ...
    if !options.pointers.is_empty() {
        next_params.insert(
            "pointer".to_string(),
            options.pointers.clone(),
        );
    }

    if !options.deployed_by.is_empty() {
        next_params.insert(
            "deployedBy".to_string(),
            options.deployed_by.clone(),
        );
    }

    if options.only_currently_pointed == Some(true) {
        next_params.insert(
            "onlyCurrentlyPointed".to_string(),
            vec!["true".to_string()],
        );
    }
    ...
    next_params.insert(
        "sortingField".to_string(),
        vec![field.to_string()],
    );
    next_params.insert(
        "sortingOrder".to_string(),
        vec![order.to_string()],
    );
