...
fn build_urn(protocol: &str, contract: &str, option: &str) -> String {
    let version = if contract.starts_with("0x") {
        "v2"
    } else {
        "v1"
    };
...
pub async fn get_erc721_entity(
    ...
    let protocol = get_urn_protocol(chain_id).ok_or_else(|| {
        InvalidRequestError::new(format!("Invalid chainId '{}'", params.chain_id))
    })?;
    ...
    let entity_id = entity_value
        .get("id")
        .and_then(|v| v.as_str())
        .unwrap_or("");
    if state.denylist.is_denylisted(entity_id) {
        return Err(NotFoundError::new("Entity does not exist").into());
    }
