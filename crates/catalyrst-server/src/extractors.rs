...
/// `Json<T>` wrapper whose rejection is a JSON `{"error":...}` body
/// (status preserved) instead of axum's default text/plain.
...
/// `Multipart` wrapper whose extraction-time rejection (e.g. bad boundary)
/// is a JSON body instead of text/plain.
