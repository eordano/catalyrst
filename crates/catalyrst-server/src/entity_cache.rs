...
pub async fn load_entity_cache(
    pool: &PgPool,
    cache: &LiveEntityCache,
) -> Result<CacheStats, sqlx::Error> {
    #[derive(sqlx::FromRow)]
    struct Row {
        entity_id: String,
        entity_type: String,
...
async fn handle_entity_invalidation(
    ...
    for displaced_id in &displaced_ids {
        let still_active = current_mappings
            .iter()
            .any(|m| m.entity_id == *displaced_id);
        if still_active {
    ...
    for mapping in &current_mappings {
        ...
        ec.pointer_to_id
            .insert(lower.clone(), mapping.entity_id.clone());
        let owned = ec
            .entity_to_pointers
