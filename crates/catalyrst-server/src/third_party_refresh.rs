...
pub fn spawn(squid_pool: PgPool, tp: TpSubgraph, interval: Duration) -> tokio::task::JoinHandle<()> {
    tokio::spawn(async move {
        loop {
            match refresh(&squid_pool, &tp).await {
                Ok(n) => info!(count = n, "third-party roots refreshed from registry subgraph"),
