...
fn content_routes(
    state: &Arc<AppState>,
    post_entities_limiters: &crate::rate_limit::PostEntitiesLimiters,
) -> Router<Arc<AppState>> {
    Router::new()
...
pub fn build_router(state: Arc<AppState>) -> Router {
    ...
    let mut app = top
        .merge(content_routes(&state, &post_entities_limiters))
        .merge(lambdas_routes())
        .nest("/content", content_routes(&state, &post_entities_limiters))
        .fallback(not_found)
