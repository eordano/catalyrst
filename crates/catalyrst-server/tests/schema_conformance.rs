...
fn push_if_sql(out: &mut Vec<String>, candidate: &str) {
    ...
    if !SQL_VERBS.iter().any(|v| upper.starts_with(v)) { return; }
    if !CONTENT_TABLES.iter().any(|t| upper.contains(&t.to_uppercase())) { return; }
...
async fn every_embedded_statement_prepares_against_migrated_schema() {
        ...
        .await
    {
        Ok(pool) => pool,
        Err(e) => {
            let _: Option<()> = catalyrst_testgate::pg_unusable(
                PG_VAR,
