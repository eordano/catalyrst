...
async fn same_hash_in_two_files_returns_entity_twice() {
    ...
    // Baseline (TS catalyst) semantics: one row per content_files usage, no
    // dedup — an entity referencing the same hash from two files appears twice.
    assert_eq!(
    ...
    // Once superseded, all of the old entity's file usages disappear (the
    // handler then 404s on the empty result).
