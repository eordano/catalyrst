...
fn the_admin_session_field_is_not_public() {
    ...
    assert!(
        !code_lines(declaration).any(|line| line.starts_with("pub ") && line.ends_with(",")),
...
fn the_admin_session_derives_nothing() {
    ...
    let preceding = lines[..declaration]
        ...
        .unwrap_or("");
    assert!(
        !preceding.starts_with("#[derive"),
        "AdminSession gained a derive ({preceding}); Deserialize/Clone-style derives widen \
