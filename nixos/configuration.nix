...
let
  ...
  bundleCommonEnv = {
    ...
    # catalyrst-worlds builds every URL it advertises in a world's /about from
    # this — scene content, the comms adapter. Left unset it defaults to a
    # loopback address no visitor can reach.
    HTTP_BASE_URL = cfg.publicUrl;

    # The comms member of the social bundle refuses to start without this
    # rather than defaulting to a production Decentraland endpoint. Unset, the
    # bundle logs "member unavailable, serving without it" and every
    # scene-adapter route 404s — which is what kept an authoritative scene
    # server from ever minting itself into a room here.
    LAMBDAS_URL = "http://127.0.0.1:5141/lambdas";

    # The one identity /get-server-scene-adapter will mint a scene server for.
    # Unset, that route 401s every caller, the scene's server branch never boots
    # and clients show "Server Disconnected" while the game runs with no rules —
    # and nothing observable from outside distinguishes that from a healthy node.
    # This is the address of the flagtag scene runner's signing key; changing the
    # key means changing this.
  ...
  bundleProxyLocations =
    lib.optionalAttrs cfg.subServices.explore {
      ...
      "/world/" = exploreLoc;
      "/wallet/" = exploreLoc;
      # A world's /about advertises ${HTTP_BASE_URL}/contents/<hash> for scene
      # content and .../worlds/<name>/comms for the adapter. Without these a
      # deployed client resolves the realm and then cannot fetch a single byte
      # of it. The comms route must be a regex: "/worlds/" (prefix) is taken by
      # the abgen image server, and a regex beats a prefix match.
      "/contents/" = exploreLoc // { extraConfig = "proxy_read_timeout 600s;"; };
      "/available-content" = exploreLoc;
      "~ ^/worlds/[^/]+/comms$" = exploreLoc;
    }
