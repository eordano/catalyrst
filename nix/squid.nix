...
let
  ...
  # squid-eth/polygon crash-looped on `SlowBuffer.prototype` from 2026-08-04
  # (1425 restarts) until pinned back to an LTS that ships it. Raise the pin
  # only after that dep chain is patched to crypto.timingSafeEqual.
  nodejs = nodejs_24;
  src = ../contracts/squid;
in
buildNpmPackage {
