...
Funnel rollup events use the persona prefix (e.g. `creator_publish_step_viewed`) and feed the dashboards; fine-grained surface interactions keep their screen prefix (e.g. `bd_collections_card_clicked`) for drill-down.
...
| --- | --- | --- |
| collection viewed | `creator_collection_viewed` | `builder.collections.tsx`, `builder.collection_.$id.tsx` |
| item added/edited | `creator_item_edited` | `builder.item-editor.tsx` |
| publish started | `creator_publish_started` | `builder.publish-collection.tsx` |
| curation submitted | `creator_curation_submitted` | `builder.publish-collection.tsx` (commit SIMULATED) |
| published / on sale | `creator_collection_published` | `builder.publish-collection.tsx` (success step) |
...
| --- | --- | --- |
| listed | `creator_item_listed` | `marketplace.sell.tsx` |
| store viewed | `creator_store_viewed` | `marketplace.collection.tsx` |
| sale realized | `creator_sale_completed` | `marketplace.buy.tsx` (on-chain SIMULATED) |
...
New route `creator-hub.operator-metrics.tsx` (Creator Hub Chrome). Panels: per-scene/world occupancy (presence), deploy funnel conversion, ban/admin activity. Emits `operator_dashboard_viewed` + `operator_visits_viewed` (`peers`, `scenes`, `worlds` from the presence snapshot) + `operator_dashboard_funnel_clicked`. Data: catalyrst-presence `/current` (+ `/current/scenes`, `/current/worlds`); deploy/ban funnels from emitted events. Fixture fallback when presence is empty/unreachable.
...
| Step | Event | Emitting route(s) / data |
| --- | --- | --- |
| queue viewed | `admin_queue_viewed` | `builder.curation.tsx` ({ pending count }) |
...
| Step | Event | Emitting route(s) / data |
| --- | --- | --- |
| queue viewed | `admin_events_queue_viewed` | `landings.whatson-admin.tsx` |
...
| Step | Event | Emitting route(s) |
| --- | --- | --- |
| console opened | `cast_console_opened` | `landings.cast-stream.tsx` |
...
CAST keeps its instrumentation inside its own story routes (already an XState console); no separate dashboard - its metrics may roll up alongside CREATOR on the creator dashboard.
## 6. Builder -> Creator Hub integration plan
The Builder is standalone `bd_*` screens (`builder.collections.tsx`, `builder.scenes.tsx`, `builder.curation.tsx`, `builder.worlds.tsx`, etc.) while the Creator Hub (`ch_*`) owns scene create/deploy/manage - two front doors to the same wallet's content. Unify under one chrome and one analytics rollup:

1. Single chrome: wrap the Builder screens in `CreatorHubChrome` (as `creator-hub.manage.tsx` does) so Collections / Items / Scenes / Worlds / Curation become Creator Hub tabs. ui3 stays untouched - compose on the consumer side.
...
3. Analytics rollup: keep `bd_*`/`ch_*` screen events for drill-down; ADD the persona rollup events (`creator_*`, `operator_*`, `admin_*`) at the same call sites. Map: `bd_collections_*`, `bd_item_*`, publish -> `creator_*` (F-CRE-1/2); `bd_scenes_*`, `bd_worlds_*`, `ch_deploy_*` -> `operator_*` (F-OP-1); `bd_curation_*` -> `admin_*` (F-ADM-1).
4. Shared session id: all surfaces already use `ensureSid` - one `sid` cookie makes Builder -> Creator Hub -> Marketplace one funnel, not three anonymous users.
...
- Dashboards are loader-rendered (SSR, no client query lib): the loader computes the summary from the catalyst/presence/builder data source; the dashboard's own `*_dashboard_viewed` event fires on mount.
