...
Reads are real across the site; the write / action paths below are disclosed simulations - each returns `{ simulated: true }` / `{ stub: true }` and surfaces a user-facing note (e.g. "no on-chain transaction or Snapshot proposal was submitted"). Each item is `simulateFn` in `catalyrst/sites/packages/data/src/lib/catalyst/<file>` -> the real backend it needs; tick when the live write path replaces the `simulate*` export.
## Governance (DAO)
...
_Generated from a `grep` of `export ... simulate*` in `catalyrst/sites/packages/data/src/lib/catalyst/` (9 exports remain). Regenerate after wiring any path._
