...
// The foundry surface provisions itself through the same code path a real
// database takes — migrations, then the v3 schema — and is left EMPTY. Nothing
// is seeded: the honesty suite asserts every route is truthful with zero rows,
// and the suite that wants rows imports the real fixtures itself.
...
async function applySchemaAndSeed(url: string): Promise<void> {
  ...
  try {
    ...
    const migrations = await provisionFoundry(pool);
    // eslint-disable-next-line no-console
    console.log(
      `[e2e] provisioned foundry v3 (${migrations.length} migration(s), 0 rows — ` +
        "the suite imports the real fixtures itself)",
    );
  } finally {
    await pool.end();
  }
...
export async function setup(): Promise<void> {
  ...
  process.env.FOUNDRY_DATABASE_URL = state.url;
