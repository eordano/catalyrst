...
-- The foundry surface is deliberately NOT described here. globalSetup calls
-- provisionFoundry() — the same migrations-then-schema path a real database
-- takes — and seeds no rows. A copy of that DDL in this file would be a second
-- source of truth that drifts, and an empty foundry schema is exactly the state
-- the honesty suite asserts against: every route must render truthfully with
-- zero rows in it.
