...
`GET /dash/flags` returns `{config:{flags:{"explorer-...":bool},variants:{...}}}` - not keyed by a sites `experiment.key`, so the (2) deterministic local hash of `(sid + experiment.key)` (bucketed by `story.md` weights) decides every assignment; the app never breaks.
