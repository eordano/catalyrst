...
if (!snapshot && args.runner === "dclbots") {
  fail(`${snapshotFile} not found \u{2014} a dclbots run without its snapshot is not evidence`);
}
...
try {
  ...
  if (ingest.report.sceneId) {
    ...
    if (known.rowCount === 0) {
      console.log(
        `foundry-ingest-bench: scene "${ingest.report.sceneId}" is not in the registry \u{2014} recording the run without a scene link`,
      );
      ingest = {
        ...ingest,
  ...
  console.log(
    `foundry-ingest-bench: ${slug} \u{2192} report ${out.reportId}, trajectory ${out.trajectoryId}, ${out.events} event(s)` +
  ...
  if (isPermissionDenied(e)) {
    ...
    console.error(`foundry-ingest-bench: permission denied \u{2014} payload parked at ${file}`);
    exitCode = 3;
  } else {
    console.error("foundry-ingest-bench: failed \u{2014}", (e as Error).message);
    exitCode = 1;
  }
} finally {
  await pool.end();
