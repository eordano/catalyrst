...
function main(): void {
  ...
  if (index.length === 0) {
    fail(
      "no z.object shapes found in generated-schemas/ — a scan with an empty index cannot flag anything and must not report success",
    );
  }
  ...
  if (writeBaseline) {
    ...
    console.log(
      `Every entry needs a one-line "reason" saying why it cannot be fixed by\n` +
        `importing the generated schema — --check fails on reasonless entries.`,
    );
    return;
  }
