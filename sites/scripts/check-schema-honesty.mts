...
/**
 * A value invented on the spot, as opposed to one that came from somewhere.
 *
 * A call or an identifier is not judged: `catch { return cached }` may well be
 * handing back something real, and this check does not guess. Booleans are left
 * out for the same reason `false` is a poor lie — it asserts nothing a reader
...
// while the gate stays green. That drift already happened once, and it
// presented as two unrelated red findings rather than as what it was.
