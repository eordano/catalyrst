    ...
    while (f < bodyEnd) {
      ...
      for (const g of group) {
        ...
        if (omitting.length > 0 && !hasTsOptional) {
          violations.push(
            `${path}:${g.line}: ${omitting[0]} on a ts-rs field without #[ts(optional)] — generated TS declares the field present while serde omits it`,
          );
        }
        if (bad.length > 0 && good.length > 0) {
          violations.push(
            `${path}:${g.line}: serde attr mixes ${good.join("/")} with ${bad.join("/")} — ts-rs 10 drops the whole attr, silently ignoring ${good.join("/")}; split into separate #[serde(...)] attrs`,
          );
        }
        if (keys.includes("serialize_with") && !hasTsShape) {
          violations.push(
            `${path}:${g.line}: serialize_with on a ts-rs field without #[ts(type/as)] — the custom serializer's wire shape is not reflected in the generated TS`,
          );
        }
      }
      f = gl + 1;
