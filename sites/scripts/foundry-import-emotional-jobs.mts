...
function park(e: unknown): never {
  ...
  console.error(
    `foundry-import-emotional-jobs: refused by the database \u{2014} ${(e as Error).message}`,
  );
...
try {
  ...
  console.log(
    `foundry-import-emotional-jobs: ${total} job rows across ${scenes_read} scenes read (${none_rows} read as serving none); job A \u{2014} the deck's mandatory persistence job \u{2014} is served by ${job_a} of them`,
  );
} catch (e) {
  if (isPermissionDenied(e)) park(e);
  console.error("foundry-import-emotional-jobs: failed \u{2014}", (e as Error).message);
  process.exitCode = 1;
} finally {
  await pool.end();
}
