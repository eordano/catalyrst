...
function park(e: unknown): never {
  ...
  console.error(`foundry-import-gdd: refused by the database \u{2014} ${(e as Error).message}`);
...
try {
  for (const doc of docs) {
    ...
    console.log(
      `foundry-import-gdd: ${doc.id} \u{2014} ${doc.honesty.sections.length} sections, ` +
  ...
  console.error("foundry-import-gdd: failed \u{2014}", (e as Error).message);
  process.exitCode = 1;
} finally {
  await pool.end();
}
