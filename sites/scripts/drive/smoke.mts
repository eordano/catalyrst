...
async function visit(tab: Tab, url: string, waitMs: number, allowConsole: RegExp[]): Promise<VisitResult> {
  ...
  try {
    ...
    // is loaded) used to escape visit() entirely and abort the whole sweep --
    // observed 2026-07-27 at 08:10 and 11:07, both times after every route so
    // far had already passed, so the run reported "a route regressed" when none
    // had. It gets the same single retry any other failure gets. A retry is safe
    // here: cmd() matches strictly on message id so a late response cannot be
...
async function main(): Promise<void> {
  ...
  const routes: Route[] = opts.routes
    ...
    : ROUTES;
  console.log(
    `[smoke] ${routes.length} routes vs ${opts.base} · ${git.stamp}`,
  );
  ...
  try {
    ...
    if (authed.length) {
      ...
      if (!signed) {
        console.log(
          "[smoke] burner unavailable (no __DCL_DEV__ — not a dev server?): skipping authed pass",
        );
      } else {
  ...
  console.log(`[smoke] GREEN — ${results.length} route-states clean`);
}
