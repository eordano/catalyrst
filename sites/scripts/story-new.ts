...
function parseArgs(argv: string[]): Args {
  ...
  if (!id || typeof id !== "string") {
    throw new Error("missing <id> (the story slug, e.g. `places-hero`)");
  }
  if (!/^[a-z0-9][a-z0-9-]*$/.test(id)) {
    throw new Error(`invalid id "${id}": use lowercase letters, digits and dashes`);
  }
  ...
  return {
    id,
    owner: (flags.owner as string) ?? "eordano@gmail.com",
    key: (flags.key as string) ?? id.replace(/-/g, "_"),
...
function renderStoryMd(meta: StoryMeta, multi: boolean): string {
  ...
  const body = multi
    ? [
        ...
        "",
        "- **control** — baseline flow.",
        "- **treatment** — the changed flow under test.",
        "",
...
function renderComponentTsx(comp: string): string {
  ...
  return `// ${comp} — interactive surface driving the \`${machineVar}\`.
//
...
describe("${machineVar} — model-based path coverage", () => {
...
describe("${machineVar} — telemetry events", () => {
