    ...
    const req = request({ socketPath, path, method: "GET", headers: headers() }, (res) => {
      ...
      res.on("end", () => {
        if ((res.statusCode ?? 0) < 200 || (res.statusCode ?? 0) >= 300) {
          reject(new Error(`${path} \u{2192} HTTP ${res.statusCode}`));
          return;
        }
        try {
          resolve(JSON.parse(body) as T);
        } catch (e) {
          reject(new Error(`${path} \u{2192} unparseable response: ${(e as Error).message}`));
        }
      });
    });
    req.on("error", reject);
...
async function api<T>(path: string): Promise<T> {
  ...
  if (!res.ok) throw new Error(`${path} \u{2192} HTTP ${res.status}`);
...
try {
  ...
  console.error(`foundry-ingest-llm: copilot not readable at ${where} \u{2014} ${(e as Error).message}`);
  console.error(
...
try {
  ...
  console.log(
    `foundry-ingest-llm: ${rows.length} row(s) upserted \u{2014} ${totals.input} in / ${totals.output} out`,
  );
} catch (e) {
  if (isPermissionDenied(e)) {
    ...
    console.error(`foundry-ingest-llm: refused by the database \u{2014} ${(e as Error).message}`);
    console.error(`foundry-ingest-llm: ${rows.length} row(s) parked at ${out}`);
    process.exit(EXIT_PARKED);
  }
  console.error("foundry-ingest-llm: failed \u{2014}", (e as Error).message);
  process.exitCode = 1;
} finally {
  await pool.end();
}
