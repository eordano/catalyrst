    ...
    for (const file of walk(p.absRoot)) {
      for (const spec of specifiers(fs.readFileSync(file, "utf8"))) {
        ...
        if (!allowed.has(target)) {
          out.push({
            key: `${rel(file)} -> ${spec}`,
            message: `${rel(file)}: ${p.name} may not import ${target} — "${spec}"`,
          });
        }
      }
    }
...
describe("package boundaries", () => {
  ...
  it("every waived upward import still exists (the waiver list only shrinks)", () => {
    ...
    expect(stale, "delete these from WAIVED — the import they excused is gone").toEqual([]);
  });
    ...
    for (const p of packages) {
      for (const file of walk(p.absRoot)) {
        for (const spec of specifiers(fs.readFileSync(file, "utf8"))) {
          ...
          violations.push(
            `${rel(file)}: relative import "${spec}" escapes ${p.name} into ${target?.name ?? "another package"} — use its alias`,
          );
        }
      }
    }
