...
function park(e: unknown): never {
  ...
  console.error(
    `foundry-import-market-cells: refused by the database \u{2014} ${(e as Error).message}`,
  );
...
try {
  ...
  console.error("foundry-import-market-cells: failed \u{2014}", (e as Error).message);
  process.exitCode = 1;
} finally {
  await pool.end();
}
