...
async function collectCounts(
  ...
  needed.add(rateNumerator(story.metric.primary));
...
function computeRows(story: StoryMeta, counts: Counts): VariantRow[] {
  const numEvent = rateNumerator(story.metric.primary);
...
function decide(
  ...
  if (!control || rows.length < 2) {
    reasons.push(
      "no control arm to test against (single-variant story) — cannot ship/kill",
    );
    return { verdict: "KEEP RUNNING", reasons, tests: [] };
  }
  ...
  if (winner) {
    reasons.push(
      `treatment "${winner.variant}" beats control by ${(winner.diff * 100).toFixed(2)}pp ` +
        `(z=${winner.z.toFixed(2)}, p=${winner.p.toFixed(4)} < α=${alpha})`,
    );
  ...
  reasons.push(
    `no treatment moved the primary significantly at α=${alpha} ` +
...
async function main(): Promise<void> {
  ...
  if (!collected) {
    const msg =
      ...
      `POST /dash/sql was unreachable or returned no rows. Check TELEMETRY_URL ` +
      `and that the experiment (exp_key=${story.experiment.key}) is emitting events.\n`;
    if (args.json) {
      process.stdout.write(
  ...
  const lines: string[] = [];
  lines.push(`Readout — ${story.id}  (key=${story.experiment.key}, source=${source})`);
  lines.push(`primary: ${story.metric.primary}   α=${args.alpha}   min_sample=${report.minSample}`);
  lines.push("");
  ...
  if (decision.tests.length) {
    ...
    for (const t of decision.tests) {
      lines.push(
        `vs control — ${t.variant}: Δ=${(t.diff * 100).toFixed(2)}pp  z=${t.z.toFixed(2)}  ` +
