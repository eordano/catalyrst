...
//
// The TS registry (events.ts) is compile-time-only; it is erased at runtime and
// unknown to the Rust ingester. This emits a checked-in JSON contract — event
// names + per-prop structured kinds/enums — that every downstream consumer reads
// at RUNTIME: the metric-ref gate resolves references against it, the emit-path
// validator (track.ts) shape-checks props against it, and catalyrst-telemetry
// validates ingested events against it. It is generated from the SAME harvest as
// events.ts, so the two never drift, and `--check` verifies it is in sync.
// ---------------------------------------------------------------------------
...
function buildContract({ events }: HarvestResult): {
  ...
  for (const name of [...events.keys()].sort()) {
    ...
    // forwarded/loose arg): don't over-validate — accept any props.
...
function contractCheck(data: HarvestResult): number {
  ...
  console.error(
    "telemetry contract out of date — run: node scripts/telemetry-catalog.mts --contract",
  );
  return 1;
}
