...
function park(e: unknown): never {
  ...
  console.error(`foundry-import-real: refused by the database \u{2014} ${(e as Error).message}`);
...
async function enrichSceneDisplay(
  ...
  for (const s of scenes) {
    ...
    try {
      ...
      console.error(
        `foundry-import-real: display fetch failed for ${s.id} \u{2014} ${(e as Error).message}`,
      );
      continue;
    }
    if (!display) continue;
...
try {
  ...
  if (migrateOnly) {
    ...
    console.log(
      `foundry-import-real: ${upserted} doc\u{2194}game readings upserted` +
  ...
  console.error("foundry-import-real: failed \u{2014}", (e as Error).message);
  process.exitCode = 1;
} finally {
  await pool.end();
}
