...
function finish(ok: boolean): never {
  ...
  console.log(`foundry-door-probe: ${ok ? "PASS" : "FAIL"} \u{2014} ${steps.map((s) => `${s.ok ? "+" : "!"}${s.name}`).join(" ")}`);
  process.exit(ok ? 0 : 1);
}
...
try {
  ...
  done("rotate-code", Boolean(next), next ? "next return code minted and saved" : "no fresh code on the page \u{2014} probe will be locked out; re-seed the code file");
  ...
  done("model", /llm.default/i.test(body), /llm.default/i.test(body) ? "model chip reads llm-default (self-hosted)" : body.includes("Big Pickle") ? "composer fell back to Big Pickle \u{2014} session is outside the workspace" : "model chip unreadable");
