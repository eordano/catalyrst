...
// The generated file must pass scripts/check-ascii.sh: non-ASCII runtime text rides \u{..} escapes.
function lit(s: string): string {
  return JSON.stringify(s).replace(/[^\x00-\x7f]/gu, (c) => `\\u{${c.codePointAt(0)!.toString(16)}}`);
}
  ...
  const body = rows
    .map((r) => {
      const members = r.members
        ? `\n    members: [${r.members.map((m) => lit(m)).join(", ")}],`
        : "";
      return `  {
    key: ${lit(r.key)},
    name: ${lit(r.name)},
    unit: ${lit(r.unit)},
    port: ${r.port},
    healthPath: ${lit(r.healthPath)},
    expect: ${lit(r.expect)},
    serves: ${lit(r.serves)},${members}
  },`;
    })
    .join("\n");
