...
const SKIP_UNITS: Record<string, string> = {
  ...
  "catalyrst-sites": "the /server page itself — if it renders, sites is up",
};
...
const DISPLAY: Record<string, { name: string; serves?: string }> = {
  content: {
    ...
    serves: "catalyst content/lambdas API — nearly every other surface reads through it",
  },
...
function buildRows(facts: Facts): Row[] {
  ...
  for (const [unitName, unit] of Object.entries(facts.units)) {
    ...
    if (primary) {
      ...
      fail(
        `unit ${unitName} is long-running but declares no health endpoint — declare healthPath in facts.nix or add it to SKIP_UNITS with a reason`,
      );
    }
...
function render(rows: Row[]): string {
  const body = rows
    ...
    .join("\n");
  return `// Generated by scripts/gen-operator-registry.mts from nixos/facts.nix — do not edit.
...
if (process.argv.includes("--check")) {
  ...
  try {
    ...
    fail(`${OUT} is missing — run npm run gen:operator`);
  }
  if (current !== next) {
    fail("services.generated.ts drifted from nixos/facts.nix — run npm run gen:operator");
  }
