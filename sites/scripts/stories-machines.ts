...
function prettyEvent(ev: string | undefined | null): string {
  ...
  if ((m = ev.match(/^xstate\.done\.actor\.(.+)$/))) return "✓ " + m[1];
  if ((m = ev.match(/^xstate\.error\.actor\.(.+)$/))) return "✗ " + m[1];
...
function buildMermaid(mod: any): Built {
  ...
  function edges(node: any) {
    ...
    for (const e of node.edges) {
      ...
      if (isRoot) {
        globals.push(tid ? `${label} → ${tid}` : `${label} (internal)`);
      } else {
...
writeFileSync(join(STORIES, "MACHINES.md"), md.join("\n"));

const cards = entries
  .map(
    (e) => `<article class="card" data-name="${e.name}" data-surface="${e.surface}">
  <h3>${e.name} <small>${e.built.exportName} · ${e.built.states} states</small></h3>
...
<body>
  ...
  <div class="meta">${entries.length} machines · ${surfaces.length} surfaces · generated by scripts/stories-machines.ts</div>
  <input id="q" type="search" placeholder="filter by name… (e.g. marketplace, vote, deploy)" autofocus />
  <div id="list">
${cards}
  </div>
  <script type="module">
...
writeFileSync(join(STORIES, "machines.html"), html);

console.log(
  `wrote app/stories/MACHINES.md + machines.html — ${entries.length} machines, ${surfaces.length} surfaces${failures.length ? `, ${failures.length} skipped` : ""}`,
);
