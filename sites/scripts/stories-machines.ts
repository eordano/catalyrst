...
for (const s of surfaces) {
  ...
  for (const e of bySurface.get(s)!) {
    md.push(`### ${e.name}`);
    md.push(
      `\`${e.built.exportName}\` \u{B7} ${e.built.states} states \u{B7} [source](./${e.name}/machine.ts)`,
    );
    md.push("");
    md.push("```mermaid");
...
console.log(
  `wrote app/stories/MACHINES.md + machines.html \u{2014} ${entries.length} machines, ${surfaces.length} surfaces${failures.length ? `, ${failures.length} skipped` : ""}`,
);
