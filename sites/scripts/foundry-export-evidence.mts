...
for (const scene of scenes) {
  ...
  if (reports.length === 0) {
    ...
    for (const r of reports) {
      ...
      lines.push("", `### ${r.ranAt} \u{2014} ${label}`, "");
      if (r.runner === "arena") {
        lines.push(
  ...
  for (const doc of linkedDocs) {
    ...
    if (full.groundingRequestIds.length > 0) {
      ...
      for (const a of asks) {
        lines.push(`- "${a.title}" \u{2014} ${byId.get(a.id) ?? 0} pledge(s)`);
      }
      lines.push("");
    }
  }
  ...
  if (!isResponseSignalsConfigured()) {
    ...
    try {
      ...
      lines.push(
        ...
        "A session is a browser cookie \u{2014} the program cannot yet tell a person from its own automation.",
        "",
      );
    } catch {
  ...
  indexRows.push(
    `| ${name} | ${scene.title.replace(/\|/g, "\\|")} | ${reports.length} | ${linkedDocs.map((d) => d.id).join("<br>") || "\u{2014}"} |`,
  );
}
