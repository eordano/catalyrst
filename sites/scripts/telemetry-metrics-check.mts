...
for (const [key, stories] of expKeyStories) {
  ...
  if (stories.length > 1) {
    expKeyFailures.push(
      `exp_key "${key}" is shared by ${stories.length} stories (${stories.join(", ")}) — their telemetry would mix`,
    );
  }
  const status = expKeyStatus.get(key);
  if ((status === "live" || status === "running") && !keyInCode(key)) {
    expKeyFailures.push(
      `exp_key "${key}" (${status}) appears in no code file — nothing emits it, so its readout joins on a key that never ships`,
    );
  }
}
...
if (shipping.length > 0) {
  ...
  for (const r of shipping) {
    const why = !r.bound ? "no numerator binding" : `unknown events: ${[...new Set(r.unresolved)].join(", ")}`;
    console.error(`  ${r.file} [${r.status}] — ${why}`);
  }
  console.error(
