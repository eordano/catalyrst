...
describe("sid cookie helpers", () => {
  ...
  it("serializes a safe Set-Cookie", () => {
    ...
    // This test claimed "safe" for a year-lived id while never checking the
    // one flag that keeps it off the wire in plaintext — production shipped
    // without it until 2026-08-06.
...
describe("sid cookie convergence helpers", () => {
  ...
  it("sharedSidDomain widens only hosts under the shared parent", () => {
    ...
    expect(at("https://foundry.catalyst.example.com/foundry")).toBe("catalyst.example.com");
  ...
  it("hasSplitSidCookie detects two same-named sid cookies and nothing else", () => {
    const mk = (cookie: string) =>
      new Request("https://foundry.catalyst.example.com/", { headers: { cookie } });
    expect(hasSplitSidCookie(mk("sid=a; sid=b"))).toBe(true);
    expect(hasSplitSidCookie(mk("sid=a; other=b"))).toBe(false);
    expect(hasSplitSidCookie(new Request("https://foundry.catalyst.example.com/"))).toBe(false);
  });
});
