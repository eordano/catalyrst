...
describe("sidLoader cookie scope", () => {
  it("first mint under *.catalyst.example.com is wide-scope from the start", () => {
    const base = sidLoader(req("https://foundry.catalyst.example.com/foundry"));
  ...
  it("a split jar under the shared parent converges: wide winner plus host-scope expiry", () => {
    const base = sidLoader(req("https://foundry.catalyst.example.com/foundry", "sid=stale; sid=winner"));
  ...
  it("a single existing sid sets nothing", () => {
    const base = sidLoader(req("https://foundry.catalyst.example.com/foundry", "sid=only"));
    expect(base.created).toBe(false);
    expect(cookies(base)).toHaveLength(0);
  });

  it("a split jar off the shared parent is left alone — no domain to converge onto", () => {
