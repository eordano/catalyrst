---
id: pixelwars-shortgdd-v2
title: Pixelwars shortGDD
kind: shortgdd
version: 2
supersedes: pixelwars-shortgdd-v1
source: slack-import
source_ref: https://decentralandteam.slack.com/archives/C0BDDMXGLU8/p1786569035319029
created_at: 2026-08-12T21:10:00.000Z
...
>
> **Document passes:** &#9312; 2026-08-12 -- AFK adaptation of the brief (markers above, `[agent-decided]`) &#183;
> &#9313; 2026-08-12 -- program-side review merge, everything it touched is tagged **`[review-pass]`** so the
> owner and the program team can see which iteration each line came from. Pass &#9313; changed &#167;10 to
> program-assessable criteria (why: decisions.md) and parked every other divergent suggestion in
> `ideas.md` -- no section text was silently rewritten. Pointers sit at &#167;4.3 and &#167;9.
...
`[review-pass]` *Alternative hook set parked in `ideas.md`: daily first-win bonus (a true Day-2 anchor -- the current Day-2 sentence leans on a weekly deadline) &#183; weekend Paint War with side-lock and badges &#183; badge collection. Owner picks the cadence mix before the Week-4 league build.*
...
`[review-pass]` *Sequencing alternatives parked in `ideas.md`: gate the combat build behind the H2-02/H2-03 verdicts instead of committing Weeks 2-3 &#183; move the hide spike to Week 1 (the brief's own "early spike" advice) &#183; a second standing non-goal candidate ("never a shooter first"). Owner decides before Week 1 starts.*

---

## 10. Success Criteria -- how v1 will be judged

`[review-pass]` *Reframed so the assessed criteria are the program's own -- studios are not required to build custom telemetry (the Foundation provides no tooling for it). We run our own multiplayer server, so the original instrumented targets stay below as internal extras, not commitments.*
...
**Assessed against the Creator Success 2026: What Makes a Great Experience guidance:**

- At least 80% of playtesters understand what to do within 5-10 seconds, without instructions or help (-> H2-08).
- Players have meaningful progression and a reason to return across multiple sessions. For v1, D7 retention should be greater than 10%, with a target of progressing toward 20%+ (-> H3-02; the log's >=8% honesty bar predates this program line -- the 10% v1 bar governs).
- Gameplay is visible and understandable to nearby users, creating memorable moments that players may want to share.
- The game meets the "Discoverability Bar" -- suitable for promotion by the Foundation Discoverability team; requires the criteria above and no unresolved IP or Content Policy issues (&#167;12 declaration).

**Program-level target:** more than 20% D7 retention and meeting the Discoverability Bar.

**Hypothesis failed (go/no-go):** if fewer than 10% of week-1 players return on another calendar day, the league/event meta lacks pull -- we stop and rethink the retention layer rather than request v2. `[review-pass]`

**Internal telemetry targets -- self-instrumented on our own server; extras, not commitments:**

1. % of new players who flip a tile within 10 s of spawn -- target >=80% (-> H2-08);
2. median completed rounds per session -- target >=2 (-> H3-03);
3. first-session funnel: spawn -> first tile painted -> first round completed -> second round started -> league board seen -> session end.

Pivot readings (2 weeks live): D7 low -> the league/event hooks failed (H3-02), redesign the meta before adding content. Median rounds/session < 2 -> loop-depth problem: rebalance combat/items, not visuals. First-tile-within-10s < 60% -> FTUE/spawn problem: fix spawn placement and readability first.
...
|---|---|---|---|---|---|---|
| H2-01 | IF the paint grid drops 2m->1m (4x tiles), THEN load time <=1.5x V0 and entity count within SDK7 limits | &#167;6 performance; &#167;9 W1 | Arithmetic in doc, then load test on Stom's branch (desktop) | parked | | -- |
...
| H2-05 | IF quick-move teleports along own paint at 2x speed, THEN owner self-test finds motion jitter-free | &#167;3; brief fallback named | Desktop, owner self-test, 1 corridor | parked | | -- |
...
| H3-04 | IF a weekly match night is announced, THEN event peak CCU >=3x weekday median | &#167;4.3 hook 2 | One announced event on live V0 vs baseline | parked | | -- |
