...
describe("parseHonesty", () => {
  it("counts plain and arrowed markers alike \u{2014} the arrow is a cross-reference, not a different marker", () => {
    const body = [
      "Preamble legend: [OPEN] means unreached \u{2014} never counted.",
      "",
      "## Core loop",
      "[OPEN] not interviewed yet.",
      "[OPEN \u{2192} \u{A7}4] deferred to the systems section.",
      "[HYPOTHESIS] players will trade.",
      "[HYPOTHESIS \u{2192} H1-02] retention holds a week.",
...
describe("readGddFile sources", () => {
  ...
  it("reads absent grounding keys as null and empty \u{2014} no invented ground", () => {
    const dir = mkdtempSync(join(tmpdir(), "gdd-src-"));
...
function stubGddDb() {
  ...
  const db = {
    query: vi.fn(async (sql: string, values: unknown[] = []) => {
      ...
      throw new Error(`stubGddDb: unmodelled query \u{2014} ${sql}`);
    }),
  };
...
describe("gdd grounding keys", () => {
  ...
  it("answers briefsQuotingAsk by stored key \u{2014} a grounded ask matches, any other id does not", async () => {
    const db = stubGddDb();
    await upsertGddDoc(db, readGddFile(fixture));
...
describe("parseAppendixHypotheses", () => {
  const body = [
    ...
    "",
    "## Appendix A \u{2014} Hypothesis Log",
  ...
  it("maps columns by header name: the seven-column hand-kept log parses", () => {
    const handKept = [
      ...
      "|---|---|---|---|---|---|---|",
      "| H1-01 | IF the core cycle runs as placeholder text THEN a first-time player continues \u{2265}5 cycles | \u{A7}3 Core Loop | Paper/text prototype | survived | kill-check held \u{B7} 2026-08-12 | paper (chat) |",
      "| H2-01 | IF card outcomes are deterministic THEN a third run is still completed | \u{A7}3 Core Loop | Paper prototype, 3 runs | parked | | \u{2014} |",
      "| H1-02 | IF the same consequence is staged vs text-only THEN staged reads as loop | \u{A7}3 Core Loop | Greybox, owner self-test | active | | \u{2014} |",
    ].join("\n");
    ...
    expect(rows[0]).toMatchObject({
      id: "H1-01",
      status: "survived",
      stage: "section \u{A7}3 Core Loop",
...
describe("changedSections", () => {
  ...
  it("returns [] for identical bodies \u{2014} a real no-text-change reading", () => {
