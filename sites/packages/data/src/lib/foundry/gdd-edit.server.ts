...
export async function editGddDoc(input: EditGddDocInput): Promise<EditedGddDoc> {
  ...
  if (successor) {
    throw new FoundryStateError(
      `This document was already superseded by v${successor.version} \u{2014} edit that one instead.`,
    );
  }
  ...
  if (section) {
    ...
    if (section.name !== undefined && section.name !== current.name) {
      throw new FoundryStateError(
        "The document changed while you were editing \u{2014} reload and try again.",
      );
    }
  ...
  if (bodyMd === old.bodyMd) {
    throw new FoundryStateError(
      "Nothing changed \u{2014} the edit matches the current version.",
    );
  }
  ...
  if (await getGddDoc(db, doc.id)) {
    throw new FoundryStateError(
      `A document with id ${doc.id} already exists \u{2014} this edit cannot supersede safely.`,
    );
  }
