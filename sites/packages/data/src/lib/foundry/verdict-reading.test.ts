...
describe("runVerdictReading", () => {
  ...
  it("turns harness-gap-only failures into a neutral verified count", () => {
    ...
    expect(r.detail).toBe(
      "4 of 6 checks could not be evaluated (harness gaps) \u{2014} no genuine failures",
    );
  });

  it("stays 'failed' when the counts are unrecorded \u{2014} never invents neutrality", () => {
    const r = runVerdictReading({
      verdict: "fail",
      checksFailed: null,
      checksTotal: null,
