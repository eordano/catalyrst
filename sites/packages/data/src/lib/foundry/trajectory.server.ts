...
function toEvent(trajectoryId: string, r: EventDbRow): TrajectoryEvent {
  if (!KNOWN_TYPES.has(r.type) && r.ignorable !== true) {
    throw new TrajectoryLogError(
      `trajectory ${trajectoryId} seq ${r.seq} has unknown event type "${r.type}" and is not marked ignorable \u{2014} refusing to reconstruct the log`,
    );
  }
...
export async function appendEvents(
  ...
  for (const e of events) {
    ...
    if (!KNOWN_TYPES.has(e.type) && e.ignorable !== true) {
      throw new TrajectoryLogError(
        `event type "${e.type}" is unknown and not marked ignorable \u{2014} a later reader would have to refuse the whole log`,
      );
    }
...
function assertClosedBrackets(events: readonly TrajectoryEvent[]): void {
  ...
  if (turns !== 0 || steps !== 0) {
    throw new TrajectoryForkError(
      `fork boundary falls inside an open ${turns !== 0 ? "turn" : "step"} \u{2014} pick a seq at a closed bracket`,
    );
  }
}
