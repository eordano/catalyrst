...
function validate(input: {
  ...
  if (id.length === 0) return "Give the game an id \u{2014} it becomes /foundry/play/<id>.";
  ...
  if (id === "register") return 'The id "register" is taken by this form \u{2014} pick another.';
  ...
  if (note.length === 0)
    return "Say where this game comes from \u{2014} the shelf shows it on the card.";
    ...
    if (doc !== "") {
      ...
      if (found.rowCount === 0) {
        throw new FoundryStateError(
          `No design doc has the id "${doc}" \u{2014} use an id from /foundry/gdd, or leave the field empty.`,
        );
      }
    }
