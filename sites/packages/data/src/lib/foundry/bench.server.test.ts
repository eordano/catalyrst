...
describe("buildBenchIngest \u{2014} dclbots", () => {
  it("maps one turn, the manifest, the snapshot and one event per verdict", () => {
    ...
    // A real failure, not a "cannot evaluate" one.
    expect(report.checksUnevaluable).toBe(0);
    expect(report.trajectoryId).toBe(trajectory.id);
  });

  it("counts the failures the harness could not evaluate", () => {
    const { report } = buildBenchIngest(
      evidence({
        logLines: [
          "  [PASS] log: 1 line matches 'round start'",
          "  [FAIL] component-present: cannot evaluate: entity listing unread  (pages truncated)",
          "  [FAIL] log: no line matches 'phase='  (ring evicted)",
          "flagtag: FAIL (2 of 3 checks)",
        ],
      }),
    );
    expect(report.checksFailed).toBe(2);
    expect(report.checksUnevaluable).toBe(1);
  });
...
describe("buildBenchIngest \u{2014} arena", () => {
