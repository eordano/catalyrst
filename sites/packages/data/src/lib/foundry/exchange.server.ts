...
function toBoardRow(r: RequestDbRow): RequestBoardRow {
  return {
    ...
    // read_at is NOT NULL, so a non-null reading_read_at iff a reading row
...
export const EDIT_NOT_AUTHOR =
  "Only this ask's author can edit it \u{2014} pledge to it, or post your own ask.";
export const EDIT_IMPORTED =
  "This ask is quoted verbatim from a public post and keeps its author's wording \u{2014} post your own ask instead.";
...
export async function editRequest({
  ...
  await withTx(async (client) => {
    ...
    if (title.trim() === row.title && body.trim() === row.body) {
      throw new FoundryStateError(
        "Nothing changed \u{2014} the ask already reads exactly like this.",
      );
    }
    await client.query(
