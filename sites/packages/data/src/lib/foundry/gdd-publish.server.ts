...
export async function publishCopilotDraft(
  ...
  if (!isCopilotSessionId(trimmed)) {
    throw new FoundryStateError(
      "That does not look like a copilot session id \u{2014} it reads ses_ followed by letters and digits, from the copilot's own address bar.",
    );
  }
  ...
  try {
    ...
    throw new FoundryStateError(
      `${why}. Ask the copilot to re-emit the document starting with a frontmatter block \u{2014} ---, title: <name>, --- \u{2014} then publish again.`,
    );
  }
