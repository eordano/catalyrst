...
export function validateDisplayName(name: string): string | null {
  ...
  if (trimmed.length < 2 || trimmed.length > 32) {
    return "2\u{2013}32 characters. Letters, numbers, spaces and . _ - only.";
  }
...
const CARRY_NO_PERSONA = "Claim a persona first \u{2014} a carry code moves one that exists.";
const CARRY_CODE_DEAD =
  "That code doesn't open anything. Codes are single-use \u{2014} mint a fresh one in the browser that holds the persona.";
