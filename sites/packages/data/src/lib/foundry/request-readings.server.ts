...
export async function upsertRequestReadings(
  ...
  for (const row of rows) {
    ...
    if ((ask.rowCount ?? 0) === 0) {
      console.warn(
        `request-readings: no foundry.request row for "${row.url}" \u{2014} reading skipped`,
      );
    ...
    if (row.shelfAnswer !== null) {
      ...
      if ((scene.rowCount ?? 0) === 0) {
        console.warn(
          `request-readings: shelf answer "${row.shelfAnswer}" is not in the scene registry \u{2014} reading for "${row.url}" skipped`,
        );
        skipped.push(row.url);
        continue;
      }
