...
import { DEPLOY_PROBE_SESSION_TITLE, LLM_REFERENCE_PRICING } from "./types";
...
export async function llmUsageSummary(
  ...
  const [totals, days, recent] = await Promise.all([
    db.query<TotalsDbRow>(
      // The deploy's own gateway probe is metered like everything else; the
      // FILTERed pair below lets the surface say how much of the headline is
      // that probe rather than hiding it or silently counting it as drafting.
      `SELECT count(*)::int AS messages,
              count(DISTINCT session_id)::int AS sessions,
              coalesce(sum(input_tokens), 0)::bigint AS input_tokens,
              coalesce(sum(output_tokens), 0)::bigint AS output_tokens,
              coalesce(sum(${COST_EXPR}), 0)::float8 AS cost_usd,
              count(*) FILTER (WHERE session_title = $1)::int AS probe_messages,
              coalesce(sum(input_tokens + output_tokens)
                FILTER (WHERE session_title = $1), 0)::bigint AS probe_tokens,
              coalesce(sum(${COST_EXPR})
                FILTER (WHERE session_title = $1), 0)::float8 AS probe_cost_usd
         FROM foundry.llm_usage`,
      [DEPLOY_PROBE_SESSION_TITLE],
    ),
    db.query<DayDbRow>(
  ...
  return {
    ...
    probeMessages: Number(t.probe_messages),
    probeTokens: Number(t.probe_tokens),
    probeCostUsd: Number(t.probe_cost_usd),
    byDay,
