...
const LANE_SQL: Record<TimelineLane, string> = {
  community: `
    SELECT 'community'::text AS lane, ('al-' || a.id::text) AS id, a.at AS at,
           ...
           -- build_status rows carry which step the ledger recorded; bodyFor
           -- turns it into the verb. Every other action keeps an empty body.
           CASE
             WHEN a.action = 'build_status' THEN COALESCE(a.detail->>'status', '')
             ELSE ''
           END AS body,
...
function bodyFor(r: TimelineDbRow): string {
  if (r.lane === "community") {
    switch (r.action) {
      ...
      case "request_build":
        return "asked for a design doc to be built";
      case "build_status":
        // The recorded step rode in on the row body (community lane SQL).
        switch (r.body) {
          case "building":
            return "started building a design doc";
          case "verifying":
            return "sent a build for verification";
          case "landed":
            return "landed a build of a design doc";
          case "failed":
            return "recorded a failed build of a design doc";
          default:
            return "recorded a build step for a design doc";
        }
      default:
        return `${r.action} on ${r.subject ?? "\u{2014}"} \u{2014} detail not rendered`;
    }
  }
  if (r.lane === "exchange") return "asked in public \u{2014} imported to the exchange";
  ...
  if (r.lane === "harness") {
    ...
    return "playtest ran \u{2014} no verdict recorded";
  }
