...
describe("listTimeline \u{2014} community subjects", () => {
  ...
  it("keeps a dangling subject nameless \u{2014} verb only, no invented label", async () => {
    state.rows = [
      row({
        sid: "sid-a",
        action: "rsvp_session",
  ...
  it("resolves doc subjects for approval/edit/publish/build actions from gdd_doc", async () => {
    await listTimeline({ lanes: ["community"] });
    const union = state.queries[0];
    expect(union).toContain("'approve_gdd','edit_gdd_doc','publish_gdd_draft'");
    expect(union).toContain("'request_build','build_status'");
  ...
  it("names the build actions, reading the recorded step off the row body", async () => {
    state.rows = [
      row({
        id: "al-7",
        sid: "sid-a",
    ...
    const { rows } = await listTimeline();
    expect(rows.map((r) => r.body)).toEqual([
      "asked for a design doc to be built",
      "landed a build of a design doc",
      "recorded a failed build of a design doc",
    ]);
    for (const r of rows) {
      expect(r.subjectKind).toBe("doc");
      expect(r.subjectLabel).toBe("Zoo");
    }
  });

  it("keeps session plumbing off the feed \u{2014} carry mints, redeems and rebinds are the holder's business", async () => {
    await listTimeline({ lanes: ["community"] });
    const union = state.queries[0];
  ...
  it("gives only request actions the request kind \u{2014} no fallback that would link a persona sid", async () => {
    await listTimeline({ lanes: ["community"] });
    const union = state.queries[0];
    expect(union).toContain(
...
describe("listTimeline \u{2014} docs lane", () => {
  it("moves the doc title into the subject label and keeps a label-free verb", async () => {
    state.rows = [
      row({
        ...
        action: "design_doc",
        subject: "flagtag",
        subject_label: "Flag Tag \u{2014} shortGDD",
        provenance: "import",
      }),
    ];
    const { rows } = await listTimeline();
    expect(rows[0].body).toBe("design doc imported");
    expect(rows[0].subjectLabel).toBe("Flag Tag \u{2014} shortGDD");
    expect(rows[0].actor).toEqual({ source: "design workspace" });
  });

  it("attributes a program-drafted doc to the program itself \u{2014} never 'imported'", async () => {
    state.rows = [
      row({
        lane: "docs",
        id: "gd-8",
...
describe("timelineStats", () => {
  it("dates the first memory from the instant the exchange lane renders \u{2014} the imported ask's original public date", async () => {
    state.statsRow = {
      events: 12,
      actors: 3,
