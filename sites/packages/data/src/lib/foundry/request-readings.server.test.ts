...
function row(overrides: Partial<RequestReadingRow>): RequestReadingRow {
  return {
    ...
    confidence: "inferred",
    readAt: "2026-08-17",
    basis: "This program's own reading \u{2014} not a fact the ask carries.",
    crowdRange: "6\u{2013}24 active players + spectators",
    ...overrides,
  };
}

describe("upsertRequestReadings", () => {
  it("upserts by the ask id derived from the permalink \u{2014} a re-run refreshes in place", async () => {
  ...
  it("skips a reading whose shelf answer names a scene the registry lacks \u{2014} never nulls a judgment", async () => {
...
describe("the readings fixture", () => {
  ...
  it("carries the deck's slide-09 range on every cell reading, and none on a cell-null one", () => {
    ...
    const DECK_RANGES: Record<string, string> = {
      "creator-led-social-competition": "6\u{2013}24 active players + spectators",
      "community-operated-game-clubs": "8\u{2013}50 recurring participants",
    };
    for (const r of readings) {
      if (r.cell === null) {
        expect(r.crowdRange, `${r.url} fits no cell \u{2014} no range applies`).toBeNull();
      } else {
