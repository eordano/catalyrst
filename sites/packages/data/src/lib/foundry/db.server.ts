...
export class FoundryRateLimitError extends Error {
  readonly status = 429;
  constructor(message = "Too many writes from this session \u{2014} try again shortly") {
    super(message);
...
// The principal set of one live sid: its canonical persona sid (resolved
// through foundry.sid_alias) plus every alias of that persona. Parameterized on
// a single $1; embed as `sid IN (${PRINCIPAL_SET_SQL})` so role, consent and
// attribution reads see everything the persona ever did under any of its sids.
// Writes never use this -- rows stay where they were written; only reads widen.
export const PRINCIPAL_SET_SQL = `
  WITH canon AS (
    SELECT COALESCE(
      (SELECT persona_sid FROM foundry.sid_alias WHERE alias_sid = $1), $1) AS sid)
  SELECT sid FROM canon
  UNION
  SELECT a.alias_sid FROM foundry.sid_alias a JOIN canon c ON a.persona_sid = c.sid`;

/** Every sid the given sid answers for -- `[sid]` when it has no aliases. */
export async function principalSids(sid: string): Promise<string[]> {
  const res = await getPool().query<{ sid: string }>(PRINCIPAL_SET_SQL, [sid]);
  const out = res.rows.map((r) => r.sid);
  return out.length > 0 ? out : [sid];
}

const CANONICAL_SQL = `SELECT COALESCE(
  (SELECT persona_sid FROM foundry.sid_alias WHERE alias_sid = $1), $1) AS sid`;

/** The persona sid this sid resolves to -- itself when it aliases nothing. */
export async function canonicalSid(sid: string): Promise<string> {
  const res = await getPool().query<{ sid: string }>(CANONICAL_SQL, [sid]);
  return res.rows[0]?.sid ?? sid;
}

/** In-transaction twin of canonicalSid, for writes that must land canonically. */
export async function canonicalSidTx(
  client: PoolClient,
  sid: string,
): Promise<string> {
  const res = await client.query<{ sid: string }>(CANONICAL_SQL, [sid]);
  return res.rows[0]?.sid ?? sid;
}
...
function rateKeys(sid: string, ip?: string | null): string[] {
  const keys = [`sid:${sid}`];
...
/**
 * Counts a write against the session/IP budget without ever refusing it -- the
 * exemption for the essential once-per-principal writes (a first persona claim,
 * a return-code redemption) that must not be stranded because the journey's own
 * earlier writes spent the budget.
 */
export function recordRate(sid: string, ip?: string | null): void {
  const now = Date.now();
  for (const k of rateKeys(sid, ip)) record(k, now);
...
export function assertRate(sid: string, ip?: string | null): void {
  const now = Date.now();
  if (rateKeys(sid, ip).some((k) => overLimit(k, now))) {
