...
export async function programChecks(
  ...
  return [
    ...
    },
    {
      ...
      source:
        "Verdicts parsed from dcl-scene-bots output, ingested with foundry:ingest-bench. Arena sandbox simulations are excluded \u{2014} a passing verdict here means a real run against the scene.",
    ...
    },
    {
      ...
      value: docs === 0 ? "\u{2014}" : `${group(Number(r?.docs_no_open ?? 0))} of ${group(docs)}`,
    ...
    // a stage nobody has reached yet, shown the way the console shows absence.
    {
      ...
      source:
        "Visitor-posted rows in foundry.request (origin = 'visitor') \u{2014} imported asks are counted under Open requests. Stage one: someone asks for what they want built.",
      href: "/foundry/exchange",
    },
    {
      id: "loop-pledge",
...
export async function copilotPipelineReceipts(
  ...
  const single = (kinds: string[]) => {
    ...
    return {
      ...
      note: ` \u{2014} ${d.title}${d.superseded ? ", superseded" : ""}`,
    };
  };
