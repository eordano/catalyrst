...
// Public URL of a deployment entity — the citation behind every "entity <id>"
// note. Verified resolving on the public worlds content server (2026-08-16).
const WORLDS_CONTENTS_BASE =
  process.env.FOUNDRY_WORLDS_CONTENTS_BASE ??
  "https://worlds-content-server.decentraland.org/contents";

/** The public content-server URL of a deployment entity, or null without one. */
export function entityHref(entityId: string | null): string | null {
  return entityId ? `${WORLDS_CONTENTS_BASE}/${entityId}` : null;
}
...
const inflight = new Map<string, Promise<ProbeMemo>>();

async function fetchProbe(realm: string): Promise<ProbeMemo> {
  const now = Date.now();
  ...
  const memo: ProbeMemo = { ...result, at: now };
  probes.set(realm, memo);
  if (probes.size > 200) {
...
/** One probe per realm at a time: concurrent visitors of a cold or stale realm
 *  share a single in-flight fetch instead of racing duplicates at it. */
function refreshProbe(realm: string): Promise<ProbeMemo> {
  const pending = inflight.get(realm);
  if (pending) return pending;
  const next = fetchProbe(realm).finally(() => inflight.delete(realm));
  inflight.set(realm, next);
  return next;
}
 ...
 *
 * Stale-while-revalidate: a memo past its window is answered immediately while a
 * background refresh replaces it — only a never-probed realm makes the page wait
 * on the network. The memo carries its own `at`, so the page stamps the real
 * probe time, never the render time.
 */
async function probeRealm(realm: string): Promise<ProbeMemo> {
  const memo = probes.get(realm);
  if (memo) {
    if (Date.now() - memo.at >= PROBE_MEMO_MS) void refreshProbe(realm);
    return memo;
  }
  return refreshProbe(realm);
}
...
export async function sceneEmbed(scene: FoundryScene): Promise<SceneEmbed | null> {
  ...
  const { reachable, status, probedUrl, at } = await probeRealm(realm);
  return {
    url,
    reachable,
