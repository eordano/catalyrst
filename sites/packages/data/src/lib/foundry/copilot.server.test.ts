...
describe("stuckFromSessionStatus", () => {
  ...
  it("ignores a first retry \u{2014} one attempt is a blip, not an outage", () => {
    expect(
...
describe("doorFunnelFrom", () => {
  ...
  it("counts only door-titled sessions inside the window and joins replies", () => {
    const sessions = [
      mk("ses_a", "nora \u{2014} from the site", t0 + 1000),
      mk("ses_b", "colda \u{2014} from the site", t0 + 2000),
      mk("ses_c", "pipeline smoke", t0 + 3000),
      mk("ses_d", "old \u{2014} from the site", t0 - 5000),
    ];
  ...
  it("skips malformed rows instead of counting them", () => {
    const f = doorFunnelFrom(
      [{ title: "ghost \u{2014} from the site" }, { id: "ses_y", title: 42 as unknown as string }],
      new Set(),
      0,
    );
