...
import { PRINCIPAL_SET_SQL } from "./db.server";
import type {
  GddDoc,
  GddHonesty,
  GddHonestySection,
...
// [OPEN] and [HYPOTHESIS] also appear arrowed -- "[OPEN -> sec. 4]",
...
function bullet(text: string, label: string): string | undefined {
  ...
  if (!value || value === "\u{2014}" || value === "-") return undefined;
  return value;
}
...
function withMobileGap(testedOn: string, mobileSensitive: string | undefined): string {
  ...
  return `${testedOn} \u{2014} mobile pending`;
}
...
/** Which sections differ between two versions of a doc \u{2014} names in the NEWER
  ...
  if (fm.source !== undefined && !GDD_SOURCES.includes(fm.source as GddSource)) {
    throw new Error(
      `${label}: frontmatter source "${fm.source}" is not one of ` +
        `${GDD_SOURCES.join(", ")} \u{2014} refusing to relabel it`,
    );
  }
...
export async function upsertGddSceneReadings(
  ...
  for (const row of rows) {
    const key = `${row.gddDocId} \u{2194} ${row.sceneId}`;
    const doc = await db.query("SELECT 1 FROM foundry.gdd_doc WHERE id = $1", [
      row.gddDocId,
    ]);
    if ((doc.rowCount ?? 0) === 0) {
      console.warn(`gdd-scene-readings: no foundry.gdd_doc row for "${row.gddDocId}" \u{2014} reading skipped`);
      skipped.push(key);
      continue;
    }
    ...
    if ((scene.rowCount ?? 0) === 0) {
      console.warn(`gdd-scene-readings: no foundry.scene row for "${row.sceneId}" \u{2014} reading skipped`);
      skipped.push(key);
      continue;
    }
    await db.query(
...
/** Whether this sid's PERSON made the doc chain: their principal set holds the
 *  sid behind the chain's latest edit/publish act. gdd_doc stores no author --
 *  the maker is resolved through action_log, chain-wide because an edit mints
 *  a new doc id. Presentation gate only: requestBuild re-checks in-tx. */
export async function isDocChainMaker(
  db: Pool,
  docId: string,
  sid: string,
): Promise<boolean> {
  const res = await db.query(
    `WITH RECURSIVE older AS (
       SELECT d.id, d.supersedes FROM foundry.gdd_doc d WHERE d.id = $2
       UNION
       ...
       SELECT a.sid FROM foundry.action_log a
        WHERE a.action IN ('edit_gdd_doc','publish_gdd_draft')
          AND a.subject IN (SELECT id FROM chain)
        ORDER BY a.at DESC, a.id DESC
        LIMIT 1
     )
     SELECT 1 FROM maker m WHERE m.sid IN (${PRINCIPAL_SET_SQL})`,
    [sid, docId],
  );
