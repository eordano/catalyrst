...
export function proposalDetailToMarkdown(p: ProposalDetail): string {
  const facts: Array<[string, string | number | null | undefined]> = [
    ...
    ["Voting", p.start && p.finish ? `${p.start} → ${p.finish}` : p.finish],
    ["Threshold", p.threshold ? `${p.threshold}${p.thresholdReached ? " (reached)" : ""}` : null],
    [
      "Budget",
      p.budget ? `${p.budget.size}${p.budget.beneficiary ? ` → ${p.budget.beneficiary}` : ""}` : null,
    ],
  ];
...
function eventLine(e: Event): string {
  ...
  return `- [${title}](/whats-on/${encodeURIComponent(e.id)})${meta ? ` — ${meta}` : ""}`;
}
...
export function projectDetailToMarkdown(p: ProjectDetail): string {
  ...
  const team = (p.personnel ?? []).length
    ? joinBlocks([
        heading(2, "Team"),
        p.personnel.map((m) => `- ${clean(m.name)}${m.role ? ` — ${clean(m.role)}` : ""}`).join("\n"),
      ])
    : "";
  const updates = (p.updates ?? []).length
    ? joinBlocks([
        heading(2, "Updates"),
        p.updates
          .map((u) => `- ${inlineParts([`#${u.index}`, u.status, u.created_at])}${u.introduction ? ` — ${clean(u.introduction)}` : ""}`)
          .join("\n"),
      ])
    : "";
...
export function assetDetailToMarkdown(nft: AssetDetail, listings: AssetListing[] = []): string {
  const facts: Array<[string, string | number | null | undefined]> = [
    ...
    [
      "Price",
      nft.order
        ? `${nft.order.price} MANA${nft.order.expiresLabel ? ` — ${nft.order.expiresLabel}` : ""}`
        : "not listed",
    ],
  ];
  const bullets = facts
...
export function discoverToMarkdown(content: HomeContent): string {
  ...
  if (hotspots.length)
    blocks.push(
      heading(2, "Hotspots"),
      hotspots
        .map((h) => `- ${h.href ? `[${clean(h.title)}](${h.href})` : clean(h.title)}${typeof h.online === "number" ? ` — ${h.online} online` : ""}`)
        .join("\n"),
    );
  if (rituals.length)
    blocks.push(
      heading(2, "Rituals"),
      rituals
        .map((r) => `- ${r.href ? `[${clean(r.title)}](${r.href})` : clean(r.title)}${r.day ? ` — ${clean(r.day)}` : ""}`)
        .join("\n"),
    );
  return joinBlocks(blocks);
}
        ...
        items
          .map((i) =>
            `- ${inlineParts([
              ...
              i.rarity,
              i.category,
              i.price && i.price !== "—" ? `${i.price} MANA` : null,
