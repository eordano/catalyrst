...
- Session lifetime: ephemeral keys expire (default 7 days); an expired identity reports `status: "expired"` and `isConnected: false` - call `connect()` again. catalyrst independently enforces the expiration and a 5-minute per-request timestamp window.
