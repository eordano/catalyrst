...
describe("wizardIssues", () => {
  ...
  it("requires the squid RPC endpoints on public-gateway only", () => {
    const missing = wizardIssues(
      answers({ domain: "example.org", acmeEmail: "ops@example.org" }),
    );
    expect(missing.map((i) => i.field)).toEqual(["squidEthRpc", "squidPolygonRpc"]);
...
describe("generateWizardOutput", () => {
  ...
  it("emits sites.env only when wallets are given", () => {
    const out = generateWizardOutput(VALID_GATEWAY);
    const sites = out.secrets.find((s) => s.path.endsWith("sites.env"));
    expect(sites?.body).toBe("ADMIN_WALLETS=0x1111111111111111111111111111111111111111\n");
    const none = generateWizardOutput({ ...VALID_GATEWAY, adminAddresses: "" });
    expect(none.secrets.some((s) => s.path.endsWith("sites.env"))).toBe(false);
  });
  ...
  it("puts every http01 certificate name in the DNS checklist", () => {
    ...
    const dns01 = generateWizardOutput({ ...VALID_GATEWAY, tls: "acme-dns01" });
