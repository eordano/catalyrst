...
export function wizardIssues(a: WizardAnswers): WizardIssue[] {
  ...
  if (publicShape && a.tls !== "acme-http01" && a.tls !== "acme-dns01") {
    issues.push({
      field: "tls",
      message:
        "public profiles need an ACME mode — the public vhosts consume a real certificate",
    });
  }
...
function squidEnvBody(a: WizardAnswers): string {
  const optional = [
    "",
    "# Optional archive-portal acceleration:",
    "# SQD_PORTAL_URL=",
    "# SQD_PORTAL_API_KEY=",
  ];
  return (
...
function checklist(a: WizardAnswers): string[] {
  ...
  if (!publicShape) {
    ...
    if (a.tls === "acme-dns01") {
      ...
      items.push(
        `DNS: A records (or CNAMEs) for ${d} and every certificate name, all pointing at this host — each one is challenged over HTTP-01 at issuance: ${subs
          .map((s) => `${s}.${d}`)
          .join(", ")}.`,
      );
    }
  ...
  if (a.profile === "public-gateway") {
    items.push(
      `Secrets: install squid.env (below) at ${SECRETS_DIR}/squid.env before first boot — the indexer units fail closed without it.`,
    );
  }
  items.push(
    "Session secrets, LiveKit API keys and the storage encryption key generate themselves on first boot — nothing to supply.",
  );
  if (parseWallets(a.adminAddresses).length > 0) {
    items.push(
      `Wallet auth: install sites.env (below) at ${SECRETS_DIR}/sites.env so /admin and /server require one of your wallets.`,
    );
  } else {
    items.push(
  ...
  items.push(
    `First boot: nixos-rebuild switch, then check from outside — /about on ${d} answers with the node's manifest as soon as the content server is up. Per-service health lives in systemctl status / journalctl on the node (the /server operations page exists only when the sites tier runs, which the exported module leaves off).`,
  );
  if (a.federationSeed) {
    items.push(
      "Federation: the shipped peers file carries blank root certificates, and the worlds server refuses to load it that way — the /worlds surface stays absent until you fill each peer's mtls_root_pem in it. Fill the roots, or untick the template to serve worlds non-federated.",
    );
  } else {
    items.push(
...
export function generateWizardOutput(a: WizardAnswers): WizardOutput {
  ...
  if (wallets.length > 0) {
    secrets.push({
      path: `${SECRETS_DIR}/sites.env`,
      body: `ADMIN_WALLETS=${wallets.join(",")}\n`,
    });
  }
  return {
