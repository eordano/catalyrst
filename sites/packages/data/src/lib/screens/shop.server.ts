...
type CatalogSection = MarketEnvelope<CatalogItem[]> & { state: ShopSectionState; freshness?: PublicFreshness };
...
export async function loadShopScreen(params: URLSearchParams, signal?: AbortSignal) {
  ...
  async function section(
    ...
    size: number,
    timeout: number,
    load: (signal: AbortSignal) => Promise<MarketEnvelope<CatalogItem[]> & { freshness?: PublicFreshness }>,
    ...
    try {
      ...
      state = "ready";
      return { data: result.data.filter(isCatalogItemBuyable).slice(0, size), total: result.total, state, freshness: result.freshness };
    } catch {
      signal?.throwIfAborted();
  ...
  const [items, top, trending] = await Promise.all([
    section("catalog", catalog, limit, SHOP_CATALOG_TIMEOUT_MS, () => {
      const query = {
        first: limit,
  ...
  const data: ShopScreen = {
    ...
    freshness: {
      updatedAt: Math.min(...[items, top, trending].flatMap(s => s.freshness ? [s.freshness.updatedAt] : [Date.now()])),
      refreshing: [items, top, trending].some(s => s.freshness?.refreshing),
      refreshFailed: [items, top, trending].some(s => s.freshness?.refreshFailed),
    },
