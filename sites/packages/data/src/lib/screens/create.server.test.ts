...
beforeEach(() => {
  ...
  vi.mocked(fetchProfile).mockResolvedValue({ name: "Alice", ethAddress: address, hasClaimedName: false, description: "" });
});
...
describe("creator home screen", () => {
  ...
  it("returns usable trending events without waiting for the active fallback", async () => {
    vi.mocked(fetchEvents).mockImplementation(async ({ list } = {}) => {
      if (list === "active") return new Promise(() => {});
      return { data: [{ id: "trending", name: "Popular event", live: true } as never], total: 1 };
    });
    let completed = false;
    const pending = loadCreateScreen(request()).then((result) => { completed = true; return result; });
    await vi.advanceTimersByTimeAsync(0);
    expect(completed).toBe(true);
    expect((await pending).data.happenings).toMatchObject([{ id: "trending" }]);
    await vi.advanceTimersByTimeAsync(10_000);
  });
  ...
  it("does not include a name returned for a different profile address", async () => {
    vi.mocked(fetchProfile).mockResolvedValue({ name: "Wrong viewer", ethAddress: "0x2222222222222222222222222222222222222222", hasClaimedName: false, description: "" });
    expect((await loadCreateScreen(request())).data.profileName).toBe("");
  });

  it("preserves the unavailable status when both event sources fail", async () => {
    vi.mocked(fetchEvents).mockRejectedValue(new Error("offline"));
    expect((await loadCreateScreen(request())).data.sections.events).toBe("unavailable");
  });
