...
const LAYOUTS: Record<string, EntitySpec[]> = {
  ...
  "memory-game": [
    ...
    { id: 514, name: "Arcade — Black", position: [3.5, 0, 13.5], rotY: 180, glb: GLB.arcadeBlack },
    { id: 515, name: "Arcade — Red", position: [6.5, 0, 13.5], rotY: 180, glb: GLB.arcadeRed },
    { id: 516, name: "Arcade — Green", position: [9.5, 0, 13.5], rotY: 180, glb: GLB.arcadeGreen },
    { id: 517, name: "Arcade — Blue", position: [12.5, 0, 13.5], rotY: 180, glb: GLB.arcadeBlue },
...
const META: Record<string, TemplateContentMeta> = {
  "tower-defense": {
    ...
    readmeNote:
      ...
      "the original Tower-defense game logic — the full original scene (SDK6) lives " +
      "at the link above.",
  },
  "nft-art-wall": {
    id: "nft-art-wall",
    ...
    readmeNote:
      ...
      "original NFT-wall code — the full original scene (SDK6) lives at the link above.",
  },
  "escape-room": {
    id: "escape-room",
    title: "Escape Room",
    ...
    readmeNote:
      ...
      "Escape-Room game — the full original scene (SDK6) lives at the link above.",
  },
  "memory-game": {
    id: "memory-game",
    title: "Memory Game",
    ...
    readmeNote:
      ...
      "starter inspired by — not a port of — the original Memory game (SDK6), which " +
  ...
  "castaway-2048": {
    ...
    readmeNote:
      ...
      "logic — the full original scene (SDK6) lives at the link above.",
  },
};

const STARTER_HEADER = (title: string, github: string) =>
  `// ${title} — SDK7 starter scaffolded by the Decentraland Creator Hub.\n` +
...
const STARTERS: Record<string, (name: string) => string> = {
  "tower-defense": () =>
    ...
    `  const board = makeStatusBoard('TOWER DEFENSE\\nWave 1 — hold the path!', Vector3.create(8, 2.8, 13.5))\n` +
    `  let breaches = 0\n\n` +
  ...
  "memory-game": () =>
    ...
    `        } else {\n` +
    `          setBoard('MEMORY GAME\\nWrong pad! Score: ' + (sequence.length - 1) + ' — starting over')\n` +
    `          sequence.length = 0\n` +
    `          guessIndex = 0\n` +
    `        }\n` +
    `      }\n` +
  ...
  "castaway-2048": () =>
    ...
    `const WIN = 2048\n\n` +
    `// 2048-style colour ramp (paler → deeper as the value doubles)\n` +
