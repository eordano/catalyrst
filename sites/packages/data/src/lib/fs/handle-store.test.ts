...
describe("handle-store — put/get round-trip", () => {
...
describe("handle-store — missing key", () => {
...
describe("handle-store — clearing an entry", () => {
...
describe("handle-store — local project library (meta)", () => {
  ...
  it("putMeta merges — a later save keeps the title and updates the composite", async () => {
    const store = createHandleStore(cloningBackend());
...
describe("handle-store — slugifyProjectTitle", () => {
...
describe("handle-store — SSR / no-IndexedDB safety", () => {
