...
async function pushServerCopy(
  ...
  try {
    const [{ pushServerDraft }, { slugifyProjectTitle }] = await Promise.all([
      import("../catalyst/creator-hub/scene-drafts-client"),
      import("./handle-store"),
    ]);
    ...
    return await pushServerDraft(slug, {
      ...
      ...(project?.base ? { base: project.base } : {}),
...
async function persistCompositeTextLocal(
  text: string,
  entities: number,
  opts: SaveSceneOptions = {},
): Promise<SaveSceneResult> {
