 ...
 * image). One narrowing on top: `tokenId` is nullable on the wire, but every
 * consumer here signs it into a buy meta-tx, so an order without a concrete
 * tokenId is not buyable and is dropped at the parse — the same refusal the
 * hand-written copy enforced.
 */
