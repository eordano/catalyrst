...
describe("ensOrderExpired — mixed seconds/ms expiries", () => {
...
describe("ensAvailability — three honest states", () => {
  it("no minted NFT → claimable", () => {
  ...
  it("open order with price and future expiry → listed, minted casing preserved", () => {
    const row = ens({
      name: "Automotive",
      order: {
        id: "o1",
  ...
  it("open order with zero price → taken, never buyable", () => {
    const row = ens({
      order: {
        id: "o1",
        status: "open",
