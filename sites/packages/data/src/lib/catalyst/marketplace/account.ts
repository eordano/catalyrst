...
const PUBLIC_CATALYST = "https://catalyst.example.com";

export function thumbnailFromUrn(urn: string | null | undefined): string | undefined {
  return urn
    ? `${PUBLIC_CATALYST}/lambdas/collections/contents/${urn}/thumbnail`
    : undefined;
}
...
export function toOnSaleRow(order: Order): OnSaleRow {
  ...
  const shortTok = tok.length > 10 ? `${tok.slice(0, 6)}…${tok.slice(-4)}` : tok;
  return {
    id: order.id,
