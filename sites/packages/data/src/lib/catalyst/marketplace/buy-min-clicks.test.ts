...
describe("GUARD A — checkout is one-step (single CONFIRM: review → fulfilling)", () => {
  ...
  it("has NO intermediate confirm state and review→CONFIRM targets `fulfilling` (structural)", () => {
    type ConfigView = {
      initial: string;
      states: Record<
        string,
...
describe("GUARD C — every Buy handler navigates directly to express checkout (no cart hop)", () => {
  ...
  for (const h of BUY_HANDLERS) {
    it(`${h.path}: ${h.fn} → /marketplace/checkout?express= with no cart step`, () => {
