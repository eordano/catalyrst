...
describe("catalogManaPrice — upstream free-mint + listing-floor semantics", () => {
  it("free primary mint (isOnSale, price 0, no listings) → '0', not null", () => {
    expect(catalogManaPrice(item({ isOnSale: true, price: "0" }))).toBe("0");
  });

  it("paid primary mint → the mint price", () => {
  ...
  it("not minting but listed (isOnSale false + listings) → the listing floor", () => {
    expect(
      catalogManaPrice(item({ isOnSale: false, minListingPrice: `1800000000000000000` })),
    ).toBe("1.8");
  });

  it("nothing to buy (no mint, no listings) → null (Make an offer)", () => {
    expect(catalogManaPrice(item({ isOnSale: false }))).toBeNull();
  });
});

describe("isCatalogItemBuyable — browse surfaces only show what can be bought", () => {
  ...
  it("no mint, no listings → not buyable", () => {
    expect(isCatalogItemBuyable(item({ isOnSale: false }))).toBe(false);
  });

  it("garbage price strings → not buyable", () => {
