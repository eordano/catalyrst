...
export function realZeroNote(takenAt: string): string {
  return `a real zero — sampled at ${hhmmss(takenAt)}, nobody in`;
}
...
export const LIVE_DATA_ZERO_NOTE =
  "a real zero — /live-data lists only rooms with users in them, and this world was not in that set at read time";
