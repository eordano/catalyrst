...
export const RUNTIME_NOTE =
  ...
  "prebuilt template runtime (bin/index.js) — the same one the editor's Play " +
...
export const EMPTY_SRC_ERROR = (count: number): string =>
  ...
  `model file (empty GltfContainer src) — this usually means the scene was ` +
...
export async function buildDraftDeployFiles(
  ...
  if (!byPath.has(main)) {
    ...
    if (!runtime) {
      throw new Error(
        "The prebuilt scene runtime is unavailable — cannot package this draft for publishing right now.",
      );
    }
