...
export const SOURCE_REGISTRY: SourceEntry[] = [
  ...
  },
  {
    ...
    klass: "live",
    note: "The worlds server's own instant figure. It lists only rooms that currently have users, and it routinely disagrees with the presence sample — both are shown, neither is reconciled.",
  },
  {
    id: "wcs-status",
  ...
  },
  {
    ...
    klass: "live",
    note: "Reception for one world. A 200 with an empty list means Places has no record of it — rendered as an empty state, not as zeros.",
  },
  {
    id: "world-about",
    ...
    usedBy: ["/creator-hub/activity/:world"],
    klass: "live",
    note: "404 here means either 'no scenes deployed' or 'never heard of it' — catalyst answers the same way for both, so the page does not claim to know which.",
  },
  {
  ...
  /* --------------------------------------------------------------- sampled */
  {
    ...
    usedBy: ["/creator-hub/activity", "/creator-hub/activity/:world"],
    klass: "sampled",
    note: "The most recent 5-minute snapshot. A world missing from it was not live when the sampler ran — that is 'no sample', not zero.",
  },
  {
  ...
  /* ----------------------------------------------------------- unavailable */
  {
    ...
    klass: "unavailable",
    note: "404. That API serves /healthz, /api/auth/*, /api/me, /api/admin/* and /api/worlds/{w}/metrics — this route is not among them. The client half exists in this repo with a generated zod model and a drift gate, and stays unused until the route does.",
  },
  {
    ...
    usedBy: [],
    klass: "unavailable",
    note: "The host serves the marketing SPA, so a JSON read gets HTML. The artifact behind it reports source: fixture (synthetic) — which would still not be rendered, because only a metabase export may become a snapshot. No retry control: a retry cannot succeed.",
  },
  {
    ...
    klass: "unavailable",
    note: "400 Invalid Auth Chain. It needs an ADR-44 signed fetch made by the scene runtime (realm + parcel metadata); this hub holds no such identity. It would also be a different number — that endpoint sums the KV store your scene writes at runtime, not the bytes you deployed.",
  },
  {
    id: "creators-me",
  ...
  },
  {
    ...
    usedBy: [],
    klass: "unbuilt",
    note: "catalyrst-notifications is email preferences — its substance is first-wear and email ports, with zero parcel, land or scene references.",
  ...
  },
  {
    ...
    // Was: "run the dclbots smoke test". That harness is unpublished and exists
    // on one developer's machine, so it was an escape hatch nobody could take.
    // get_scene_logs is on the explorer's own MCP surface, so this one is real
    // for anyone already running with --mcp.
    today:
  ...
  },
  {
    ...
    today: "Run it yourself from a terminal — the command is on the world page.",
  },
  {
    id: "live-scene-state",
  ...
  },
  {
    ...
    usedBy: [],
    klass: "excluded",
    note: "Returns [] and fails open to [] on any upstream error, non-JSON body or unreachable host — so an empty answer is indistinguishable from a broken one.",
  },
  {
