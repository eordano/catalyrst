 ...
 * (the ts-rs image of catalyrst-world-storage's DTOs). The hand copies these
 * replace guessed the wire wrong twice: usage bytes are plain numbers (never
 * strings, never absent — the old string|number|nullish -> 0 coercion turned
 * an unreadable quota into "0 bytes used"), and /env and /players answer
 * KeyListResponse with `data: string[]` — the old per-row `{key}` object
 * parse silently dropped every real row, so both reads always came back
 * empty against the live service.
 */
