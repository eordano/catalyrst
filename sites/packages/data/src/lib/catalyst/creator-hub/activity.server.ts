...
const NO_ADDRESS_NOTE =
  "No address yet. This page needs an address to pick which worlds to show. It is not a login — the data is public either way.";
...
export async function loadWorldActivity(
  ...
  const settledResults = await settleAll([
    ...
    },
    {
      ...
      load: () => loadWorldAbout(world, opts),
    },
    {
  ...
  const worldKnown = row !== null || showable(about) || inPresence;
