...
describe("postJSON — server error-message surfacing (checkout error root cause)", () => {
  it("surfaces the catalyrst envelope's message on 409 with status intact", async () => {
    mSignedFetch.mockResolvedValue(
      jsonResponse(409, {
        ok: false,
        message:
          "the order total changed after the purchase was signed (signed 3, current 4) — please review and sign again",
      }),
    );
  ...
  it("surfaces server messages for non-409 errors too (402, 500, …)", async () => {
    mSignedFetch.mockResolvedValue(
  ...
  it("uses only the message field — other body fields never leak", async () => {
    mSignedFetch.mockResolvedValue(
      jsonResponse(500, {
        ok: false,
...
describe("signedGetJSON — same surfacing on authenticated reads", () => {
