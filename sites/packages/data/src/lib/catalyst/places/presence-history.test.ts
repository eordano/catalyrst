...
describe("clampHistoryLimit", () => {
  it("clamps to [1, 5000] so the query string never lies about what was asked, and applies it to the outgoing request", async () => {
...
describe("bucketize \u{2014} a gap is not a zero", () => {
  it("emits null buckets across a deliberate 40-minute hole with exactly one gap band, and never extends the series past the last sample", () => {
    ...
    const short = bucketize(rowsEvery5Min("2026-07-20T00:00:00.000Z", [1, 2]), CADENCE);
    expect(short.points).toHaveLength(2);
    expect(short.lastSeen).toBe("2026-07-20T00:05:00.000Z");
    expect(short.gapBands).toEqual([]);
  });

  it("counts occupied snapshots without relabelling them visits, keeps a real zero as a zero, and reports peak as null with nothing to peak at", () => {
    const h = bucketize(
    ...
    const zeros = bucketize(rowsEvery5Min("2026-07-20T00:00:00.000Z", [0, 0]), CADENCE);
    expect(zeros.points.map((p) => p.value)).toEqual([0, 0]);
    expect(zeros.peak).toBe(0);
    expect(zeros.occupiedCount).toBe(0);

    const none = bucketize([], CADENCE);
    expect(none.peak).toBeNull();
    expect(none.points).toEqual([]);
    expect(none.firstSeen).toBeNull();
    expect(none.sampleCount).toBe(0);
  });

  it("drops rows with an unparseable timestamp rather than bucketing them at 1970, and takes the later row when two land in one bucket \u{2014} not their maximum", () => {
    const h = bucketize(
      [
        { taken_at: "", count: 9 },
    ...
    expect(later.points).toEqual([{ date: "2026-07-20T00:00:00.000Z", value: 2 }]);
  });
});

describe("loadWorldOccupancyHistory", () => {
  it("an empty history is no-sample with the reason (not a zero series) and a live history is sampled, dated by its newest row", async () => {
    const empty = await loadWorldOccupancyHistory("a.dcl.eth", 5000, {
      fetchImpl: (async () =>
        new Response(JSON.stringify({ world: "a.dcl.eth", history: [] }), {
          status: 200,
        })) as unknown as typeof fetch,
    });
    expect(empty.state).toBe("no-sample");
    if (empty.state !== "no-sample") throw new Error("unreachable");
    expect(empty.note).toContain("has not been in the poll set");
    ...
    const live = await loadWorldOccupancyHistory("a.dcl.eth", 5000, {
      fetchImpl: (async () =>
        new Response(JSON.stringify({ history }), { status: 200 })) as unknown as typeof fetch,
    });
    expect(live.state).toBe("sampled");
    if (live.state !== "sampled") throw new Error("unreachable");
    expect(live.takenAt).toBe("2026-08-01T09:50:32.602Z");
    expect(live.value.peak).toBe(2);
    expect(live.value.sampleCount).toBe(2);
  });
