  ...
  try {
    ...
    if (history.lastSeen === null) {
      return noSample(
        ...
        `No occupancy recorded for ${world}. The presence collector has no snapshots for it — that means it has not been in the poll set, not that it had no visitors.`,
      );
    }
  ...
  try {
    ...
    if (history.lastSeen === null) {
      return noSample(
        endpoint,
        new Date().toISOString(),
        `No occupancy recorded at ${pointer}. The presence collector has no snapshots for that parcel — it has not been in the poll set, which is not the same as nobody visiting it.`,
      );
    }
    return sampledAt(history, endpoint, history.lastSeen, cadence);
  } catch (err) {
