...
/**
 * The generated `EventUpsertResultSchema` is the wire truth: `local` is a
 * required `Record<string, unknown>`, so a moderation write whose response
 * cannot be read fails the parse instead of reporting that nothing changed.
 */
export const PatchResultSchema = EventUpsertResultSchema;
