...
export const ADMIN_CONTROLS = {
  ...
  "places.list.byOwner": ok(
    ...
    "Public data — no authorization required. `owner` is a filter, not a permission.",
  ),
  ...
  "communities.list.read": ok(
    ...
    "Public data — no authorization required. Do not label this admin data.",
  ),
  ...
  "governance.health.read": ok(
    ...
    "Public data — no authorization required.",
  ),
  ...
  "governance.proposal.submit": ok(
    ...
    "Signed with the connected wallet. 503 while GOVERNANCE_SUBMIT_URL is unset — that is correct.",
  ),
  ...
  "whatson.users.read": unavailable(
    ...
    "The What's-On moderator list cannot be read: the client call is misrouted.",
    {
      ...
      fix:
        ...
        "(3) it must sign the UN-prefixed /api/profiles/settings — nginx `location /events/api/` " +
