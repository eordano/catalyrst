 ...
 * asked": the front door used to collapse the two, so an unreachable events
 * service rendered the "Jump Into What's Happening" rail with nothing under it,
 * which reads as nothing happening. `unreadable` names the rails that were
 * dropped so a caller can say so.
 */
