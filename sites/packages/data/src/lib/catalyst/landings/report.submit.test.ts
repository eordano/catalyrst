...
describe("buildSubmitReport — real comms player-report write", () => {
  ...
  it("never sends a client-chosen reporter — the server binds it to the signer", async () => {
    happyPath();
  ...
  it("fails closed without an identity — no presign, no upload", async () => {
    await expect(
