...
/**
 * The copy fixture for the report wizard. Every key is required: with defaults
 * on all of them the schema accepted `{}` and handed the page a complete-looking
 * fixture -- a title, a success message and an empty reason list -- that no file
 * had supplied. `loadFixture` in landings.report-abuse.tsx already turns a
 * failed parse into `null`, and the wizard falls back to its own built-in copy,
 * so a partial file now reads as no file rather than as this one.
 */
