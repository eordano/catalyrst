 ...
 * ts-rs image of catalyrst-governance's `GET /proposals`. The hand copy this
 * replaces accepted `user`/`status`/`finish_at` as nullish and tolerated a
 * missing `limit`/`offset`; the wire always carries all of them.
 */
...
export function validateCoAuthors(coAuthors: string[]): FieldErrors {
  const errors: FieldErrors = {};
  if (coAuthors.length > GOVERNANCE_SCHEMA.coAuthorsMax) {
    errors.coAuthors = `You can add at most ${GOVERNANCE_SCHEMA.coAuthorsMax} co-authors.`;
    return errors;
  }
