...
export async function loadProposals(
  ...
  try {
    ...
    for (const r of rows) {
      ...
      if (r.user && /^0x[0-9a-fA-F]{40}$/.test(r.user)) {
...
function synthDetailFromLiveProposal(live: LiveProposal, now: number): ProposalDetail {
  ...
  if (type === "grant") {
    ...
    budget = {
      size: size > 0 ? `${size.toLocaleString("en-US")} ${token}` : "—",
 ...
 * `ProjectRow`). The hand copy this replaces made almost every field nullish;
 * on the real wire `proposal_id`, `author`, `configuration.category` and
 * `created_at` are always present, so the leniency only hid drift.
 */
...
function synthProposalFromProject(live: ProposalLiveProject): ProposalDetail {
  ...
  const statusMeta =
    ...
    { label: status ? status.charAt(0).toUpperCase() + status.slice(1) : "—", tone: "neutral" };

  const total =
    num(v?.total) || num(live.configuration?.size) || 0;
  const token = v?.token ?? live.funding?.one_time_payment?.token ?? "USDC";
  const sizeLabel =
    total > 0 ? `${Math.round(total).toLocaleString("en-US")} ${token}` : "—";
