...
export const BID_SCHEMA = {
  funding: { min: 100, max: 240000 },
  projectDuration: { min: 1, max: 12 },
  teamName: { minLength: 1, maxLength: 80 },
  deliverables: { minLength: 20, maxLength: 1500 },
  roadmap: { minLength: 20, maxLength: 1500 },
  milestones: { maxItems: 10 },
  members: { minItems: 1 },
  budgetBreakdown: { minItems: 1 },
...
const SAMPLE_DRAFT: BidSampleDraft = {
  ...
  budgetBreakdown: [{ concept: "Engineering (3 devs)", amount: "$90,000 · 4 months" }],
  coAuthors: [],
};
...
const SUCCESS = {
  ...
  note: "This Bid was created in a simulation — no on-chain transaction or Snapshot proposal was submitted.",
} as const;
...
function toParent(row: ProposalRow, type: "pitch" | "tender", kind: string): BidParent {
  ...
  return {
    ...
    status: row.status ?? "",
    requiredToPass: req,
    votingPowerLabel: req != null ? VP_FMT.format(req) : "—",
