 ...
 *
 * This used to call `/budget/all`, which is an upstream-DAO path that this
 * crate has never served; the 404 was absorbed by `return fixtureBudget()` and
 * the page served invented numbers for its whole life. Both halves of that bug
 * are fixed here: the path is the one the node actually routes, and a failure
 * is now reported instead of being replaced with plausible-looking money.
 */
