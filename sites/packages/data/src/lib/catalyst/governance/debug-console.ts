 ...
 *
 * The `?authorized=1` query parameter that used to switch this page between a
 * 17,640-byte and a 27,891-byte SSR response for the same anonymous caller was
 * never an access control. It is removed as a gate rather than hidden, and the
 * "tools" section it revealed renders a permanent unavailable state
 * (`control-availability.ts` -> `debug.tools`): no privileged tooling is wired
 * on this node, and `admin.debug-console.tsx:97-99` was telemetry, not an HTTP
 * call.
 */
...
async function probeHealth(opts: GetOptions): Promise<Probe<HealthInfo>> {
  ...
  const url = `${base}/governance/health`;
  try {
    ...
    return {
      value: null,
      reason: `/governance/health unreachable: ${
...
async function fetchBudgets(
  ...
  let raw: unknown;
  try {
    raw = await getJSON<unknown>("/governance/budgets", {
      ...opts,
    ...
    return {
      value: null,
      reason: status
        ? `/governance/budgets returned HTTP ${status}`
        : `/governance/budgets unreachable: ${
            err instanceof Error ? err.message : String(err)
          }`,
    };
  }
  ...
  if (!parsed.success) {
    return {
      value: null,
      reason: "/governance/budgets returned an unrecognised payload",
    };
  }
...
/**
 * Reads two public endpoints and reports exactly what they said.
 *
 * The previous version fell back to `src/fixtures/admin-debug-console.json`
 * whenever either probe failed, and only the `source` field distinguished the
 * two — so a node with a dead governance service rendered a healthy status and
 * a full budget table. Failures are now reported, not replaced.
 */
