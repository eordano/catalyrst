...
const STATIC_CONFIG: SubmitHiringData = {
  ...
  copy: {
    ...
    success: {
      ...
      note: "This proposal was created in a simulation — no on-chain transaction or Snapshot proposal was submitted.",
    },
  },
  committees: [
...
export function validateCoAuthors(coAuthors: string[]): FieldErrors {
  const errors: FieldErrors = {};
  if (coAuthors.length > SCHEMA.coAuthors.max) {
    errors.coAuthors = `You can add at most ${SCHEMA.coAuthors.max} co-authors.`;
    return errors;
  }
  for (const addr of coAuthors) {
    if (addr.trim() !== "" && !isEthAddress(addr)) {
