...
export const CurationFixtureSchema = z.object({
  committee: z.object({
    you: CommitteeMemberSchema,
    members: z.array(CommitteeMemberSchema),
  }),
  collections: z.array(CurationRowSchema),
});
...
export function toBdRow(
  ...
  return {
    ...
    assignee,
    assigneeName: member?.name ?? (assignee ? `${assignee.slice(0, 6)}…${assignee.slice(-4)}` : undefined),
    you,
    date: row.dateLabel,
