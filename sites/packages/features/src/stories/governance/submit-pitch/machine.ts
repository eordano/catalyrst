...
) as Record<PitchStepSlug, PitchStateId>;

export function stateToSlug(value: string): PitchStepSlug {
  return STATE_TO_SLUG[value as PitchStateId] ?? FIRST_STEP_SLUG;
}

export function slugToState(slug: string | null | undefined): PitchStateId {
  if (!slug) return "intro";
  return SLUG_TO_STATE[slug as PitchStepSlug] ?? "intro";
}
