...
) as Record<BidVoteStepSlug, BidVoteStateId>;

export function stateToSlug(value: string): BidVoteStepSlug {
  return STATE_TO_SLUG[value as BidVoteStateId] ?? FIRST_STEP_SLUG;
}

export function slugToState(slug: string | null | undefined): BidVoteStateId {
  if (!slug) return "review";
  return SLUG_TO_STATE[slug as BidVoteStepSlug] ?? "review";
}
    ...
    recordError: assign({
      error: ({ event }) => {
        ...
        return event.error instanceof Error
          ? event.error.message
          : String(event.error ?? "cast failed");
      },
    }),
