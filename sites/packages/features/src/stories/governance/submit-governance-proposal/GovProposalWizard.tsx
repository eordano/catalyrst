  ...
  const detailsSections = [
    {
      ...
      value: linkedDraftId,
      options: drafts.map((d) => ({ value: d.id, label: `${d.title} — ${d.author_short}` })),
      ...
      <div className="govprop-wizard" data-step={step} style={WRAP}>
        ...
        {value === "intro" && (
          <section style={CARD}>
            ...
            >
              {vpMet
                ? `You hold ${votingPower.toLocaleString()} VP — above the ${copy.vpThreshold.toLocaleString()} VP submission threshold.`
            ...
            <p style={SUB}>
              ...
              {live
                ...
                : "Live passed Drafts are unavailable right now — please try again shortly."}
            </p>
            {drafts.length === 0 ? (
        ...
        {value === "review" && (
          <section style={CARD}>
            ...
            <dl style={REVIEW}>
              ...
              <dd style={DD}>{selectedDraft ? `${selectedDraft.title} — ${selectedDraft.author_short}` : ctx.draft.linkedDraftId || "—"}</dd>
              <dt style={DT}>Title</dt>
              <dd style={DD}>{ctx.draft.title || "—"}</dd>
              {GOVERNANCE_SCHEMA.bodies.map((b) => {
                const full = (ctx.draft.bodies[b.name] ?? "").trim();
                const preview = full.length > 0 ? full.slice(0, 140) : "—";
                return (
                  <div key={b.name}>
        ...
        {value === "success" && (
          <section style={CARD}>
            ...
            <p style={CODE}>Proposal ID: {ctx.result?.id ?? "—"}</p>
            <p style={NOTE}>
