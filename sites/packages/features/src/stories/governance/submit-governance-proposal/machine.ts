...
) as Record<GovStepSlug, GovStateId>;

export function stateToSlug(value: string): GovStepSlug {
  return STATE_TO_SLUG[value as GovStateId] ?? FIRST_STEP_SLUG;
}

export function slugToState(slug: string | null | undefined): GovStateId {
  if (!slug) return "intro";
  return SLUG_TO_STATE[slug as GovStepSlug] ?? "intro";
}
