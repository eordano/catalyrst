...
) as Record<TenderStepSlug, TenderStateId>;

export function stateToSlug(value: string): TenderStepSlug {
  return STATE_TO_SLUG[value as TenderStateId] ?? FIRST_STEP_SLUG;
}

export function slugToState(slug: string | null | undefined): TenderStateId {
  if (!slug) return "parent";
  return SLUG_TO_STATE[slug as TenderStepSlug] ?? "parent";
}
