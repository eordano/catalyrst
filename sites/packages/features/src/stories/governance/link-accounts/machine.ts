...
) as Record<LinkStepSlug, LinkStateId>;

export function stateToSlug(value: string): LinkStepSlug {
  return STATE_TO_SLUG[value as LinkStateId] ?? FIRST_STEP_SLUG;
}

export function slugToState(slug: string | null | undefined): LinkStateId {
  if (!slug) return "choosing";
  return SLUG_TO_STATE[slug as LinkStepSlug] ?? "choosing";
}
          ...
          actions: assign({
            error: ({ event }) =>
              event.error instanceof Error
                ? event.error.message
                : String(event.error ?? "verification failed"),
          }),
        },
      },
    },
        ...
        onError: {
          ...
          actions: assign({
            error: ({ event }) =>
              event.error instanceof Error
                ? event.error.message
                : String(event.error ?? "unlink failed"),
          }),
        },
      },
    },
