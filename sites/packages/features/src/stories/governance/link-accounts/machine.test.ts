...
describe("linkAccountsMachine — URL ?step slug map", () => {
...
describe("linkAccountsMachine — deep-link hydration (snapshot, no event replay)", () => {
...
describe("linkAccountsMachine — model-based path coverage (@xstate/graph)", () => {
...
describe("linkAccountsMachine — telemetry events (happy path)", () => {
  ...
  it("CONFIRM before the last step is ignored (guard) — no premature verify", () => {
    const track = vi.fn();
    const verify = vi.fn(okVerify);
    const actor = createActor(linkAccountsMachine, {
      input: inputFor(verify, track),
...
describe("linkAccountsMachine — verify failure + retry", () => {
...
describe("linkAccountsMachine — push (single-step) + unlink", () => {
