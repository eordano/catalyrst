...
) as Record<UpdateStepSlug, UpdateStateId>;

export function stateToSlug(value: string): UpdateStepSlug {
  return STATE_TO_SLUG[value as UpdateStateId] ?? FIRST_STEP_SLUG;
}

export function slugToState(slug: string | null | undefined): UpdateStateId {
  if (!slug) return "general";
  return SLUG_TO_STATE[slug as UpdateStepSlug] ?? "general";
}
