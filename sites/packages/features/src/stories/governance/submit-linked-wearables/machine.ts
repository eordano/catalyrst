...
) as Record<LwStepSlug, LwStateId>;

export function stateToSlug(value: string): LwStepSlug {
  return STATE_TO_SLUG[value as LwStateId] ?? FIRST_STEP_SLUG;
}

export function slugToState(slug: string | null | undefined): LwStateId {
  if (!slug) return "identity";
  return SLUG_TO_STATE[slug as LwStepSlug] ?? "identity";
}
