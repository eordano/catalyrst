...
) as Record<DraftStepSlug, DraftStateId>;

export function stateToSlug(value: string): DraftStepSlug {
  return STATE_TO_SLUG[value as DraftStateId] ?? FIRST_STEP_SLUG;
}

export function slugToState(slug: string | null | undefined): DraftStateId {
  if (!slug) return "intro";
  return SLUG_TO_STATE[slug as DraftStepSlug] ?? "intro";
}
