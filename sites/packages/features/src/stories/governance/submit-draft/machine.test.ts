...
describe("draftMachine — URL ?step slug map", () => {
...
describe("draftMachine — deep-link hydration (snapshot, no event replay)", () => {
...
describe("draftMachine — model-based path coverage (@xstate/graph)", () => {
...
describe("draftMachine — telemetry events (happy path)", () => {
  it("full flow fires the complete funnel in order", async () => {
    ...
    actor.send({ type: "NEXT", coauthors: [{ addr: "0xabc…123" }] });
    actor.send({ type: "SUBMIT" });
...
describe("draftMachine — submit failure + retry", () => {
