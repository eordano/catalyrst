...
) as Record<CatalystStepSlug, CatalystStateId>;

export function stateToSlug(value: string): CatalystStepSlug {
  return STATE_TO_SLUG[value as CatalystStateId] ?? FIRST_STEP_SLUG;
}

export function slugToState(slug: string | null | undefined): CatalystStateId {
  if (!slug) return "details";
  return SLUG_TO_STATE[slug as CatalystStepSlug] ?? "details";
}
