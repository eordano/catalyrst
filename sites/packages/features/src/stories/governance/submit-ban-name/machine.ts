...
) as Record<BanNameStepSlug, BanNameStateId>;

export function stateToSlug(value: string): BanNameStepSlug {
  return STATE_TO_SLUG[value as BanNameStateId] ?? FIRST_STEP_SLUG;
}

export function slugToState(slug: string | null | undefined): BanNameStateId {
  if (!slug) return "details";
  return SLUG_TO_STATE[slug as BanNameStepSlug] ?? "details";
}
