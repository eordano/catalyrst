...
) as Record<PoiStepSlug, PoiStateId>;

export function stateToSlug(value: string): PoiStepSlug {
  return STATE_TO_SLUG[value as PoiStateId] ?? FIRST_STEP_SLUG;
}

export function slugToState(slug: string | null | undefined): PoiStateId {
  if (!slug) return "coordinates";
  return SLUG_TO_STATE[slug as PoiStepSlug] ?? "coordinates";
}
