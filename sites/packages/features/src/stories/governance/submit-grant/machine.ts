...
) as Record<GrantStepSlug, GrantStateId>;

export function stateToSlug(value: string): GrantStepSlug {
  return STATE_TO_SLUG[value as GrantStateId] ?? FIRST_STEP_SLUG;
}

export function slugToState(slug: string | null | undefined): GrantStateId {
  if (!slug) return "category";
  return SLUG_TO_STATE[slug as GrantStepSlug] ?? "category";
}
