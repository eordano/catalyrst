...
) as Record<CouncilVetoStepSlug, CouncilVetoStateId>;

export function stateToSlug(value: string): CouncilVetoStepSlug {
  return STATE_TO_SLUG[value as CouncilVetoStateId] ?? FIRST_STEP_SLUG;
}

export function slugToState(slug: string | null | undefined): CouncilVetoStateId {
  if (!slug) return "details";
  return SLUG_TO_STATE[slug as CouncilVetoStepSlug] ?? "details";
}
          ...
          actions: assign({
            error: ({ event }) =>
              event.error instanceof Error
                ? event.error.message
                : String(event.error ?? "createProposal failed"),
          }),
        },
      },
    },
