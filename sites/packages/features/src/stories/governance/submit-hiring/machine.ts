...
) as Record<HiringStepSlug, HiringStateId>;

export function stateToSlug(value: string): HiringStepSlug {
  return STATE_TO_SLUG[value as HiringStateId] ?? FIRST_STEP_SLUG;
}

export function slugToState(slug: string | null | undefined): HiringStateId {
  if (!slug) return "target";
  return SLUG_TO_STATE[slug as HiringStepSlug] ?? "target";
}
