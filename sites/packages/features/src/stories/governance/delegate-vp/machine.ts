...
) as Record<DelegateStepSlug, DelegateStateId>;

export function stateToSlug(value: string): DelegateStepSlug {
  return STATE_TO_SLUG[value as DelegateStateId] ?? FIRST_STEP_SLUG;
}

export function slugToState(slug: string | null | undefined): DelegateStateId {
  if (!slug) return "browsing";
  return SLUG_TO_STATE[slug as DelegateStepSlug] ?? "browsing";
}
          ...
          actions: assign({
            error: ({ event }) =>
              event.error instanceof Error
                ? event.error.message
                : String(event.error ?? "delegation failed"),
          }),
        },
      },
    },
