...
) as Record<SubmitPollStepSlug, SubmitPollStateId>;

export function stateToSlug(value: string): SubmitPollStepSlug {
  return STATE_TO_SLUG[value as SubmitPollStateId] ?? FIRST_STEP_SLUG;
}

export function slugToState(slug: string | null | undefined): SubmitPollStateId {
  if (!slug) return "intro";
  return SLUG_TO_STATE[slug as SubmitPollStepSlug] ?? "intro";
}
