...
describe("submitPollMachine \u{2014} validation helpers", () => {
  it("enforces the upstream poll limits, >= 2 non-empty options and 42-char co-authors", () => {
    ...
    expect(areDetailsValid({ ...VALID_DRAFT, title: "x".repeat(80) })).toBe(true);
    expect(areDetailsValid({ ...VALID_DRAFT, title: "x".repeat(81) })).toBe(false);
    expect(areDetailsValid({ ...VALID_DRAFT, description: "x".repeat(7000) })).toBe(true);
    expect(areDetailsValid({ ...VALID_DRAFT, description: "x".repeat(7001) })).toBe(false);
...
describe("submitPollMachine \u{2014} model-based path coverage + VP/connect gate", () => {
  it("event paths reach details, options, review and submitting; a blocked gate keeps intro and fires vp_blocked", () => {
