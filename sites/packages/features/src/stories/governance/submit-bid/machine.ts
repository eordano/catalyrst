...
) as Record<BidStepSlug, BidStateId>;

export function stateToSlug(value: string): BidStepSlug {
  return STATE_TO_SLUG[value as BidStateId] ?? FIRST_STEP_SLUG;
}

export function slugToState(slug: string | null | undefined): BidStateId {
  if (!slug) return "parents";
  return SLUG_TO_STATE[slug as BidStepSlug] ?? "parents";
}
