...
) as Record<UserBanStepSlug, UserBanStateId>;

export function stateToSlug(value: string): UserBanStepSlug {
  return STATE_TO_SLUG[value as UserBanStateId] ?? FIRST_STEP_SLUG;
}

export function slugToState(slug: string | null | undefined): UserBanStateId {
  if (!slug) return "authGate";
  return SLUG_TO_STATE[slug as UserBanStepSlug] ?? "authGate";
}
      ...
      invoke: {
        ...
        onError: {
          ...
          actions: [
            assign({
              error: ({ event }) =>
                event.error instanceof Error ? event.error.message : String(event.error ?? "commit failed"),
              errorReason: ({ event }) =>
