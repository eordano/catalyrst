...
) as Record<ManageStepSlug, ManageStateId>;

export function stateToSlug(value: string): ManageStepSlug {
  return STATE_TO_SLUG[value as ManageStateId] ?? FIRST_STEP_SLUG;
}

export function slugToState(slug: string | null | undefined): ManageStateId {
  if (!slug) return "pickPlace";
  return SLUG_TO_STATE[slug as ManageStepSlug] ?? "pickPlace";
}
