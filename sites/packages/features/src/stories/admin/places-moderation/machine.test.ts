...
describe("moderateMachine — deep-link hydration (snapshot, no event replay)", () => {
  ...
  it("boots straight into queue — there is no client-side gate to pass", () => {
    const track = vi.fn();
...
describe("moderateMachine — model-based path coverage (@xstate/graph)", () => {
  ...
  it("reaching decision passes through OPEN and DECIDE — never a client gate", () => {
    const paths = getShortestPaths(moderateMachine, {
      input: inputFor(okModerate, () => {}),
      events: TRAVERSAL_EVENTS,
    });
...
describe("moderateMachine — commit failure + retry", () => {
