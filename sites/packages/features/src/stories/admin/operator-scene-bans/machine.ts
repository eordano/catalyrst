...
) as Record<SceneBanStepSlug, SceneBanStateId>;

export function stateToSlug(value: string): SceneBanStepSlug {
  return STATE_TO_SLUG[value as SceneBanStateId] ?? FIRST_STEP_SLUG;
}

export function slugToState(slug: string | null | undefined): SceneBanStateId {
  if (!slug) return "pickPlace";
  return SLUG_TO_STATE[slug as SceneBanStepSlug] ?? "pickPlace";
}
