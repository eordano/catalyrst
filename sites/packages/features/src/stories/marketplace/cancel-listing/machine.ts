...
) as Record<CancelStepSlug, CancelStateId>;

export function stateToSlug(value: string): CancelStepSlug {
  return STATE_TO_SLUG[value as CancelStateId] ?? FIRST_STEP_SLUG;
}

export function slugToState(slug: string | null | undefined): CancelStateId {
  if (!slug) return "reviewing";
  return SLUG_TO_STATE[slug as CancelStepSlug] ?? "reviewing";
}
            ...
            assign({
              error: ({ event }) =>
                event.error instanceof Error
                  ? event.error.message
                  : String(event.error ?? "cancel failed"),
            }),
            "trackFailed",
          ],
        },
