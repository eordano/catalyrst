...
) as Record<AcceptStepSlug, AcceptStateId>;

export function stateToSlug(value: string): AcceptStepSlug {
  return STATE_TO_SLUG[value as AcceptStateId] ?? FIRST_STEP_SLUG;
}

export function slugToState(slug: string | null | undefined): AcceptStateId {
  if (!slug) return "reviewBid";
  return SLUG_TO_STATE[slug as AcceptStepSlug] ?? "reviewBid";
}
          ...
          actions: assign({
            error: ({ event }) =>
              event.error instanceof Error
                ? event.error.message
                : String(event.error ?? "approval failed"),
          }),
        },
      },
    },
        ...
        onError: {
          ...
          actions: assign({
            error: ({ event }) =>
              event.error instanceof Error
                ? event.error.message
                : String(event.error ?? "accept failed"),
          }),
        },
      },
    },
