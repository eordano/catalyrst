...
) as Record<MintStepSlug, MintStateId>;

export function stateToSlug(value: string): MintStepSlug {
  return STATE_TO_SLUG[value as MintStateId] ?? FIRST_STEP_SLUG;
}

export function slugToState(slug: string | null | undefined): MintStateId {
  if (!slug) return "review";
  return SLUG_TO_STATE[slug as MintStepSlug] ?? "review";
}
      ...
      invoke: {
        ...
        onError: {
          ...
          actions: assign({
            ...
            error: ({ event }) =>
              event.error instanceof Error ? event.error.message : String(event.error ?? "connect failed"),
          }),
        },
      },
    },
      ...
      invoke: {
        ...
        onError: {
          ...
          actions: assign({
            ...
            error: ({ event }) =>
              event.error instanceof Error ? event.error.message : String(event.error ?? "approval failed"),
          }),
        },
      },
    },
      ...
      invoke: {
        ...
        onError: {
          ...
          actions: assign({
            ...
            error: ({ event }) =>
              event.error instanceof Error ? event.error.message : String(event.error ?? "mint failed"),
          }),
        },
      },
    },
