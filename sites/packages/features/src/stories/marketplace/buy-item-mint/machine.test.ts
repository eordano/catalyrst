...
describe("buyMintMachine \u{2014} model-based path coverage (@xstate/graph)", () => {
  it("every event-reachable path ends in an expected state and START_MINT drives the simulated phases to confirming", async () => {
    ...
    for (const p of paths) {
      expect(EXPECTED_STATES.has(p.state.value as string)).toBe(true);
    }
