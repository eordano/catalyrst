...
) as Record<SellStepSlug, SellStateId>;

export function stateToSlug(value: string): SellStepSlug {
  return STATE_TO_SLUG[value as SellStateId] ?? FIRST_STEP_SLUG;
}

export function slugToState(slug: string | null | undefined): SellStateId {
  if (!slug) return "selectAsset";
  return SLUG_TO_STATE[slug as SellStepSlug] ?? "selectAsset";
}
          ...
          actions: assign({
            error: ({ event }) =>
              event.error instanceof Error
                ? event.error.message
                : String(event.error ?? "order create failed"),
          }),
        },
      },
    },
