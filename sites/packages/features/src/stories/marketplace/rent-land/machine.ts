...
) as Record<RentStepSlug, RentStateId>;

export function stateToSlug(value: string): RentStepSlug {
  return STATE_TO_SLUG[value as RentStateId] ?? FIRST_STEP_SLUG;
}

export function slugToState(slug: string | null | undefined): RentStateId {
  if (!slug) return "review";
  return SLUG_TO_STATE[slug as RentStepSlug] ?? "review";
}
    ...
    setError: assign({
      error: ({ event }) => {
        ...
        return event.error instanceof Error
          ? event.error.message
          : String(event.error ?? "rental failed");
      },
    }),
