...
Public catalog queries stay fresh for 60 seconds, then return results up to five
minutes old while a coalesced refresh runs in the background. The cache retains at
most 128 query keys. Responses preserve source age and refresh-failure status;
the page displays a freshness notice if a refresh fails. Visitor cancellation
