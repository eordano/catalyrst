...
) as Record<CheckoutStepSlug, CheckoutStateId>;

export function stateToSlug(value: string): CheckoutStepSlug {
  return STATE_TO_SLUG[value as CheckoutStateId] ?? FIRST_STEP_SLUG;
}

export function slugToState(slug: string | null | undefined): CheckoutStateId {
  if (!slug) return "review";
  return SLUG_TO_STATE[slug as CheckoutStepSlug] ?? "review";
}
    ...
    setError: assign({
      error: ({ event }) => {
        ...
        return e.error instanceof Error
          ? e.error.message
          : String(e.error ?? "checkout failed");
      },
    }),
