import { delay, hashHex } from "@core/lib/stories/index";
...
) as Record<ClaimStepSlug, ClaimStateId>;

export function stateToSlug(value: string): ClaimStepSlug {
  return STATE_TO_SLUG[value as ClaimStateId] ?? FIRST_STEP_SLUG;
}

export function slugToState(slug: string | null | undefined): ClaimStateId {
  if (!slug) return "entering";
  return SLUG_TO_STATE[slug as ClaimStepSlug] ?? "entering";
}
          ...
          actions: assign({
            error: ({ event }) =>
              event.error instanceof Error
                ? event.error.message
                : String(event.error ?? "mint failed"),
          }),
        },
      },
    },
