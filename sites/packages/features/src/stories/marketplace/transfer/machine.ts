...
) as Record<TransferStepSlug, TransferStateId>;

export function stateToSlug(value: string): TransferStepSlug {
  return STATE_TO_SLUG[value as TransferStateId] ?? FIRST_STEP_SLUG;
}

export function slugToState(slug: string | null | undefined): TransferStateId {
  if (!slug) return "selecting";
  return SLUG_TO_STATE[slug as TransferStepSlug] ?? "selecting";
}
          ...
          actions: assign({
            error: ({ event }) =>
              event.error instanceof Error
                ? event.error.message
                : String(event.error ?? "transfer failed"),
          }),
        },
      },
    },
