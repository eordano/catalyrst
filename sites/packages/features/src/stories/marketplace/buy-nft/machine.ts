...
) as Record<BuyStepSlug, BuyStateId>;

export function stateToSlug(value: string): BuyStepSlug {
  return STATE_TO_SLUG[value as BuyStateId] ?? FIRST_STEP_SLUG;
}

export function slugToState(slug: string | null | undefined): BuyStateId {
  if (!slug) return "review";
  return SLUG_TO_STATE[slug as BuyStepSlug] ?? "review";
}
