...
describe("jumpInMachine \u{2014} model-based path coverage (@xstate/graph)", () => {
  it("control never visits confirming; treatment reaches launched only through START then CONFIRM", () => {
    ...
    for (const p of control) {
      controlEnds.add(p.state.value as string);
      expect(p.steps.map((s) => s.event.type)).not.toContain("CONFIRM");
    }
    expect(controlEnds.has("confirming")).toBe(false);
    expect(controlEnds.has("launched")).toBe(true);
    ...
    for (const p of treatment) treatmentEnds.add(p.state.value as string);
    expect(treatmentEnds.has("confirming")).toBe(true);
    expect(treatmentEnds.has("launched")).toBe(true);
