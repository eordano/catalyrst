...
type ShopOpenTarget = "pill" | "rail_cta" | "rail_item";

type PlacesShopEntryProps = {
...
export default function PlacesShopEntry({
  arm,
  items,
  trackCtx,
  track = defaultTrack,
  navigate = defaultNavigate,
}: PlacesShopEntryProps) {
  const shownRef = useRef(false);
  useEffect(() => {
    if (arm === "base" || shownRef.current) return;
    shownRef.current = true;
    track("pl_shop_entry_shown", { variant: arm }, trackCtx);
  ...
  const open = (target: ShopOpenTarget, itemId: null | string, url: string) => {
    track("pl_shop_opened", { item_id: itemId, target, variant: arm }, trackCtx);
    navigate(url);
  };

  if (arm === "pill") {
    return (
      <div className="pl-shop-entry" data-variant="pill" style={PILL_ROW_STYLE}>
        <a
          href={PLACES_SHOP_ENTRY_TARGETS.shop}
          className="pl__pill"
          style={{ textDecoration: "none" }}
          onClick={(e) => {
            e.preventDefault();
            open("pill", null, PLACES_SHOP_ENTRY_TARGETS.shop);
          }}
        >
          Shop wearables &amp; emotes
        </a>
      </div>
    );
  }
  ...
  return (
    ...
    >
      <div style={RAIL_HEAD_STYLE}>
        ...
        <h2 style={RAIL_TITLE_STYLE}>On sale right now</h2>
        {items === null ? (
        ...
        <ul style={RAIL_LIST_STYLE}>
          {items.map((c) => (
            <li key={c.id} style={{ listStyle: "none" }}>
              <a
                href={shopItemPath(c.id)}
                style={ITEM_STYLE}
                onClick={(e) => {
                  e.preventDefault();
                  open("rail_item", c.id, shopItemPath(c.id));
                }}
              >
                {c.image ? (
      ...
      <a
        ...
        onClick={(e) => {
          e.preventDefault();
          open("rail_cta", null, PLACES_SHOP_ENTRY_TARGETS.shop);
        }}
      >
        Open the Shop
      </a>
