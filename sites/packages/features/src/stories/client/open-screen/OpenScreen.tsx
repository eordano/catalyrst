  ...
  return (
    <section style={SCREEN} aria-label="Things to do">
      <header style={HEAD}>
        ...
        <a href={OPEN_SCREEN_TARGETS.explore} style={SUBLINK}>
          Browse all places →
        </a>
      </header>
      {places.length > 0 ? (
...
function ThreeCards({
  ...
  function onCard(target: OpenCardTarget, place: OpenPlace | null) {
    ...
    <section style={{ ...SCREEN, ...CENTER }} aria-label="Main screen">
      ...
      <div style={CARD_ROW}>
        <ChooserCard
          ...
          subtitle={
            busiest
              ? `${placeLabel(busiest)} — ${busiest.user_count} online now`
              : "No live scene reading right now"
          }
          href={busiest ? placeJumpPath(busiest.id) : null}
          onClick={() => onCard("genesis", busiest)}
        />
        <ChooserCard
          title="Surprise me"
          subtitle={
