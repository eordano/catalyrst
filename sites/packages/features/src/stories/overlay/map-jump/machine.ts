...
) as Record<MapJumpStepSlug, MapJumpStateId>;

export function stateToSlug(value: string): MapJumpStepSlug {
  return STATE_TO_SLUG[value as MapJumpStateId] ?? FIRST_STEP_SLUG;
}

export function slugToState(slug: string | null | undefined): MapJumpStateId {
  if (!slug) return "browsing";
  return SLUG_TO_STATE[slug as MapJumpStepSlug] ?? "browsing";
}
          ...
          actions: assign({
            error: ({ event }) =>
              event.error instanceof Error
                ? event.error.message
                : String(event.error ?? "teleport failed"),
          }),
        },
      },
    },
