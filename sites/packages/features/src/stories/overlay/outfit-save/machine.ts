...
) as Record<OutfitStepSlug, OutfitStateId>;

export function stateToSlug(value: string): OutfitStepSlug {
  return STATE_TO_SLUG[value as OutfitStateId] ?? FIRST_STEP_SLUG;
}

export function slugToState(slug: string | null | undefined): OutfitStateId {
  if (!slug) return "browsing";
  return SLUG_TO_STATE[slug as OutfitStepSlug] ?? "browsing";
}
