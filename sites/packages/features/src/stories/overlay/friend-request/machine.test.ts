...
describe("friendMachine \u{2014} URL ?step slug map", () => {
  it("covers exactly the machine's states with unique slugs that round-trip and fall back to the first step", () => {
    ...
    for (const missing of [null, undefined, "", "nope"]) expect(slugToState(missing)).toBe("panel");
...
describe("transitionValid \u{2014} mirrors catalyrst-social-rpc service.rs", () => {
  it("allows request from none/cancel/reject, accept/cancel/reject only from pending, block from anywhere", () => {
    ...
    for (const to of ["accept", "cancel", "reject"] as const) {
      expect(transitionValid("request", to), to).toBe(true);
      expect(transitionValid(undefined, to), to).toBe(false);
      expect(transitionValid("accept", to), to).toBe(false);
    }

    for (const from of [undefined, "request", "cancel", "reject", "accept"] as const) {
      expect(transitionValid(from, "block"), String(from)).toBe(true);
    }
  });
});

describe("friendMachine \u{2014} deep-link hydration (no telemetry double-fire)", () => {
  it("pins the step and seeds action/address without firing telemetry or running the RPC", async () => {
    const track = vi.fn();
    const upsert = vi.fn(okUpsert);
    expect(
      resolveFriendSnapshot({ step: "panel", trackCtx: inputFor(upsert, track).trackCtx }),
    ).toBeUndefined();

    const submitting = createActor(friendMachine, {
      input: inputFor(upsert, track),
      snapshot: resolveFriendSnapshot({
        step: "submitting",
    ...
    const confirming = createActor(friendMachine, {
      input: inputFor(upsert, track),
      snapshot: resolveFriendSnapshot({
        step: "confirming",
        trackCtx: inputFor(upsert, track).trackCtx,
    ...
    expect(confirming.getSnapshot().context.action).toBe("accept");
    expect(confirming.getSnapshot().context.address).toBe(ADDR);
    expect(track).not.toHaveBeenCalled();
  });
  ...
  it("real transitions after hydration still fire telemetry", () => {
    ...
    expect(track.mock.calls.map((c) => c[0])).toContain(FRIEND_EVENTS.blockConfirmed);
  });
});

describe("friendMachine \u{2014} model-based path coverage (@xstate/graph)", () => {
  it("event paths reach confirming, blockPrompt and submitting, and a block reaches submitting through CONFIRM", () => {
...
describe("friendMachine \u{2014} telemetry events", () => {
  it("a cancelled confirm returns to the panel unsubmitted; request -> confirm -> submit -> done fires the funnel", async () => {
    const track = vi.fn();
    const upsert = vi.fn(okUpsert);
    const actor = createActor(friendMachine, {
      input: inputFor(upsert, track),
    ...
    expect(track.mock.calls.map((c) => c[0])).not.toContain(FRIEND_EVENTS.actionCompleted);
    expect(upsert).not.toHaveBeenCalled();
  ...
  it("a rejected (invalid) upsert -> failed -> RETRY recovers to done", async () => {
    ...
    expect(track.mock.calls.map((c) => c[0])).toContain(FRIEND_EVENTS.actionFailed);
