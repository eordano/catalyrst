...
) as Record<FriendStepSlug, FriendStateId>;

export function stateToSlug(value: string): FriendStepSlug {
  return STATE_TO_SLUG[value as FriendStateId] ?? FIRST_STEP_SLUG;
}

export function slugToState(slug: string | null | undefined): FriendStateId {
  if (!slug) return "panel";
  return SLUG_TO_STATE[slug as FriendStepSlug] ?? "panel";
}
            ...
            assign({
              error: ({ event }) =>
                event.error instanceof Error
                  ? event.error.message
                  : String(event.error ?? "upsert failed"),
            }),
            "trackFailed",
          ],
        },
