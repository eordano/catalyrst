...
) as Record<EmotesStepSlug, EmotesStateId>;

export function stateToSlug(value: string): EmotesStepSlug {
  return STATE_TO_SLUG[value as EmotesStateId] ?? FIRST_STEP_SLUG;
}

export function slugToState(slug: string | null | undefined): EmotesStateId {
  if (!slug) return "opening";
  return SLUG_TO_STATE[slug as EmotesStepSlug] ?? "opening";
}
