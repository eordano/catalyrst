...
) as Record<CommunityJoinStepSlug, CommunityJoinStateId>;

export function stateToSlug(value: string): CommunityJoinStepSlug {
  return STATE_TO_SLUG[value as CommunityJoinStateId] ?? FIRST_STEP_SLUG;
}

export function slugToState(slug: string | null | undefined): CommunityJoinStateId {
  if (!slug) return "browsing";
  return SLUG_TO_STATE[slug as CommunityJoinStepSlug] ?? "browsing";
}
          ...
          actions: assign({
            error: ({ event }) =>
              event.error instanceof Error
                ? event.error.message
                : String(event.error ?? "commit failed"),
          }),
        },
      },
    },
