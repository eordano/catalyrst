...
) as Record<BackpackStepSlug, BackpackStateId>;

export function stateToSlug(value: string): BackpackStepSlug {
  return STATE_TO_SLUG[value as BackpackStateId] ?? FIRST_STEP_SLUG;
}

export function slugToState(slug: string | null | undefined): BackpackStateId {
  if (!slug) return "opening";
  return SLUG_TO_STATE[slug as BackpackStepSlug] ?? "opening";
}
...
export const backpackMachine = setup({
  ...
  actors: {
    runSave: fromPromise<SaveResult, { wearables: string[]; save: SaveFn }>(
      ({ input, signal }) => input.save({ wearables: input.wearables, signal }),
    ),
  },
  actions: {
    trackOpened: ({ context }) => {
