...
) as Record<WorldSettingsStepSlug, WorldSettingsStateId>;

export function stateToSlug(value: string): WorldSettingsStepSlug {
  return STATE_TO_SLUG[value as WorldSettingsStateId] ?? FIRST_STEP_SLUG;
}

export function slugToState(slug: string | null | undefined): WorldSettingsStateId {
  if (!slug) return "details";
  return SLUG_TO_STATE[slug as WorldSettingsStepSlug] ?? "details";
}
