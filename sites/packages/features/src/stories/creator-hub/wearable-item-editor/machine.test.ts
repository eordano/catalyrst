...
describe("wearableEditorMachine — URL ?step slug map", () => {
...
describe("wearableEditorMachine — deep-link hydration (snapshot, no event replay)", () => {
...
describe("wearableEditorMachine — model-based path coverage (@xstate/graph)", () => {
...
describe("wearableEditorMachine — telemetry events (happy path)", () => {
...
describe("wearableEditorMachine — Revert (discard unsaved edits)", () => {
  ...
  it("saved offers ADD_ANOTHER (not REVERT — the save committed the baseline)", async () => {
    const track = vi.fn();
    const actor = createActor(wearableEditorMachine, {
      input: inputFor(okSave, track),
    }).start();
...
describe("wearableEditorMachine — save failure + retry", () => {
