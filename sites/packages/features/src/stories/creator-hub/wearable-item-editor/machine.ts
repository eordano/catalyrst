...
) as Record<WearableEditorStepSlug, WearableEditorStateId>;

export function stateToSlug(value: string): WearableEditorStepSlug {
  return STATE_TO_SLUG[value as WearableEditorStateId] ?? FIRST_STEP_SLUG;
}

export function slugToState(slug: string | null | undefined): WearableEditorStateId {
  if (!slug) return "selecting";
  return SLUG_TO_STATE[slug as WearableEditorStepSlug] ?? "selecting";
}
  ...
  actions: {
    ...
    recordError: assign({
      error: ({ event }) => {
        ...
        return e instanceof Error ? e.message : String(e ?? "save failed");
      },
    }),
  },
}).createMachine({
