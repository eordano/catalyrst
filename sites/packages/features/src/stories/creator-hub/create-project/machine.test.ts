...
describe("createProjectMachine \u{2014} model-based path coverage (@xstate/graph)", () => {
  it("event paths reach naming, templating and scaffolding, and scaffolding needs a valid name + a template", () => {
...
describe("CreateProjectView \u{2014} created screen create->edit seam", () => {
  ...
  it("offers an 'Open in editor' deep link + 'Go to My Scenes', notes a missing wallet without gating, and never fabricates file writes", () => {
  ...
  it("surfaces the name error inline (role=alert) and offers [Choose folder again] and [Retry] after a dismissed picker", () => {
    ...
    expect(error).toMatch(/no folder was chosen/i);
...
describe("createProjectMachine \u{2014} minimal-clicks contract", () => {
  ...
  it(`reaches scaffolding (scene-create trigger) in <= ${TARGET_EVENTS} events`, () => {
    const paths = getShortestPaths(createProjectMachine, {
      input: inputFor(okScaffold, () => {}),
      events: TRAVERSAL_EVENTS,
    });
