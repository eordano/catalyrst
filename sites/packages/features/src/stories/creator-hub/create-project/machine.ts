...
) as Record<CreateProjectStepSlug, CreateProjectStateId>;

export function stateToSlug(value: string): CreateProjectStepSlug {
  return STATE_TO_SLUG[value as CreateProjectStateId] ?? FIRST_STEP_SLUG;
}

export function slugToState(slug: string | null | undefined): CreateProjectStateId {
  if (!slug) return "naming";
  return SLUG_TO_STATE[slug as CreateProjectStepSlug] ?? "naming";
}
  ...
  actions: {
    ...
    setPathError: assign({
      pathError: ({ context, event }) => {
        ...
        return event.error ?? "That name is already taken — choose a different one.";
      },
    }),
              ...
              const code =
                ...
                (raw as { code?: unknown }).code
                  ...
                  : raw instanceof Error
                    ...
                    : undefined;
              const message =
                raw instanceof Error ? raw.message : String(raw ?? "scaffold failed");
              context.track(
