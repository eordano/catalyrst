...
) as Record<PermsStepSlug, PermsStateId>;

export function stateToSlug(value: string): PermsStepSlug {
  return STATE_TO_SLUG[value as PermsStateId] ?? FIRST_STEP_SLUG;
}

export function slugToState(slug: string | null | undefined): PermsStateId {
  if (!slug) return "access";
  return SLUG_TO_STATE[slug as PermsStepSlug] ?? "access";
}
          ...
          actions: assign({
            error: ({ event }) =>
              event.error instanceof Error
                ? event.error.message
                : String(event.error ?? "acl write failed"),
          }),
        },
      },
    },
