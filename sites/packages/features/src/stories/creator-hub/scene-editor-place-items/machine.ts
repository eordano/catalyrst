...
) as Record<EditorStepSlug, EditorStateId>;

export function stateToSlug(value: string): EditorStepSlug {
  return STATE_TO_SLUG[value as EditorStateId] ?? FIRST_STEP_SLUG;
}

export function slugToState(slug: string | null | undefined): EditorStateId {
  if (!slug) return "editing";
  return SLUG_TO_STATE[slug as EditorStepSlug] ?? "editing";
}
