...
function EditorWizardInner({ stateId, onRestart, trackCtx, seed, save, track, onExit, onPublish, viewportSrc, previewSrc, rawComposite, draftAssets, catalog, template, failedToLoadLocal }: InnerProps) {
  ...
  const realSave = useRef<SaveFn>(async (args) => {
    ...
    if (!res.written) {
      throw new Error("Save cancelled \u{2014} nothing was written to disk.");
    }
    ...
    <div className="editor-wizard" data-step={step}>
      ...
      {value === "editing" && (
        <>
          {localErrorBanner}
          {templateNotice}
          <Controls label={`Editing ${seed.scene.title}${seed.scene.live ? " \u{2014} your deployed copy; changes stay in this browser until saved" : ""}`}>
            <button
              type="button"
              className="editor-wizard__btn editor-wizard__btn--primary"
            ...
            <button
              ...
              title={
                enginePlaying
                  ? "Stop the scene first \u{2014} changes made while it runs are temporary and aren't saved."
                  : undefined
              }
            >
              {diskSave.phase === "saving" ? "Saving\u{2026}" : "Save to disk"}
            </button>
            {enginePlaying && (
              <span
                role="status"
      ...
      {value === "saving" && (
        <>
          <Controls label={previewSave ? "Applying\u{2026}" : "Saving\u{2026}"}>
            <span className="editor-wizard__spinner" aria-live="polite">
              {previewSave ? "Applying your changes\u{2026}" : "Saving your changes\u{2026}"}
            </span>
          </Controls>
        </>
      )}
