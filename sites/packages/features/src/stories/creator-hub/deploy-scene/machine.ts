...
) as Record<DeployStepSlug, DeployStateId>;

export function stateToSlug(value: string): DeployStepSlug {
  return STATE_TO_SLUG[value as DeployStateId] ?? FIRST_STEP_SLUG;
}

export function slugToState(slug: string | null | undefined): DeployStateId {
  if (!slug) return "destination";
  return SLUG_TO_STATE[slug as DeployStepSlug] ?? "destination";
}
