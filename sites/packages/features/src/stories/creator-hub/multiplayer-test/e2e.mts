...
function resolveChromiumBin(): string {
  ...
  const best = readdirSync("/nix/store")
    ...
    .at(-1);
  if (!best) throw new InfraError("no chromium in /nix/store — set CHROMIUM_BIN");
  return best.bin;
}
...
async function main(): Promise<void> {
  ...
  if (truthy("spawn-sidecar")) {
    ...
    if (held) throw new InfraError(`:${SIDECAR_PORT} already has a listener — stop it or pass --port`);
  }
  ...
  if (truthy("spawn-sidecar")) {
    ...
    if (await sidecarSpeaksCors(upstreamPort)) {
      log("mp-testd speaks CORS natively — drop the shim from this harness");
  ...
  ok("observed starting → running → analyzing → done over ws");
  ...
  console.log(`[mp-e2e] ALL PASS — shots in ${SHOTS}`);
}
