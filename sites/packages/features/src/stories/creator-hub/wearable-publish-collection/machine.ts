...
) as Record<PublishStepSlug, PublishStateId>;

export function stateToSlug(value: string): PublishStepSlug {
  return STATE_TO_SLUG[value as PublishStateId] ?? FIRST_STEP_SLUG;
}

export function slugToState(slug: string | null | undefined): PublishStateId {
  if (!slug) return "summary";
  return SLUG_TO_STATE[slug as PublishStepSlug] ?? "summary";
}
          ...
          actions: assign({
            error: ({ event }) =>
              event.error instanceof Error
                ? event.error.message
                : String(event.error ?? "publish failed"),
          }),
        },
      },
    },
