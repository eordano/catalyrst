import { slugify } from "@core/lib/stories/index";
...
) as Record<CreateStepSlug, CreateStateId>;

export function stateToSlug(value: string): CreateStepSlug {
  return STATE_TO_SLUG[value as CreateStateId] ?? FIRST_STEP_SLUG;
}

export function slugToState(slug: string | null | undefined): CreateStateId {
  if (!slug) return "naming";
  return SLUG_TO_STATE[slug as CreateStepSlug] ?? "naming";
}
          ...
          actions: assign({
            error: ({ event }) =>
              event.error instanceof Error
                ? event.error.message
                : String(event.error ?? "create failed"),
          }),
        },
      },
    },
