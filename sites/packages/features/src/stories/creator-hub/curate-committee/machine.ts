...
) as Record<CurationStepSlug, CurationStateId>;

export function stateToSlug(value: string): CurationStepSlug {
  return STATE_TO_SLUG[value as CurationStateId] ?? FIRST_STEP_SLUG;
}

export function slugToState(slug: string | null | undefined): CurationStateId {
  if (!slug) return "dashboard";
  return SLUG_TO_STATE[slug as CurationStepSlug] ?? "dashboard";
}
          ...
          actions: assign({
            error: ({ event }) =>
              event.error instanceof Error
                ? event.error.message
                : String(event.error ?? "assign failed"),
          }),
        },
      },
    },
        ...
        onError: {
          ...
          actions: assign({
            error: ({ event }) =>
              event.error instanceof Error
                ? event.error.message
                : String(event.error ?? "decision failed"),
          }),
        },
      },
    },
