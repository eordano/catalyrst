...
describe("curateCommittee — URL ?step slug map", () => {
...
describe("curateCommittee — deep-link hydration", () => {
...
describe("curateCommittee — model-based path coverage (@xstate/graph)", () => {
...
describe("curateCommittee — telemetry (review -> comment -> decide)", () => {
  it("approve WITH a comment fires comment_added before decided", async () => {
    ...
    actor.send({ type: "SUBMIT", comment: "All items pass — approving." });
    ...
    expect(call?.comment).toEqual({ raw: "All items pass — approving.", topic_id: 50121 });
...
describe("curateCommittee — assign + filter", () => {
...
describe("curateCommittee — decide failure keeps the comment draft", () => {
