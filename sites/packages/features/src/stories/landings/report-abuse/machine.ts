...
) as Record<ReportStepSlug, ReportStateId>;

export function stateToSlug(value: string): ReportStepSlug {
  return STATE_TO_SLUG[value as ReportStateId] ?? FIRST_STEP_SLUG;
}

export function slugToState(slug: string | null | undefined): ReportStateId {
  if (!slug) return "intro";
  return SLUG_TO_STATE[slug as ReportStepSlug] ?? "intro";
}
          ...
          actions: assign({
            error: ({ event }) =>
              event.error instanceof Error
                ? event.error.message
                : String(event.error ?? "report submit failed"),
          }),
        },
      },
    },
