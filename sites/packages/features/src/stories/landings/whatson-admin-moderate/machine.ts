...
) as Record<ModerateStepSlug, ModerateStateId>;

export function stateToSlug(value: string): ModerateStepSlug {
  return STATE_TO_SLUG[value as ModerateStateId] ?? FIRST_STEP_SLUG;
}

export function slugToState(slug: string | null | undefined): ModerateStateId {
  if (!slug) return "authGate";
  return SLUG_TO_STATE[slug as ModerateStepSlug] ?? "authGate";
}

/**
 * Layout / story data only. Never the default — see `failClosedModerateAction`.
 */
export const simulateModerateAction: ModerateFn = ({ eventId, action, rejectReasons, rejectNote, signal }) =>
  simulateModerate({ eventId, action, rejectReasons, rejectNote }, { signal });

/**
 * The default moderation actor: it fails closed and says why.
 *
 * `simulateModerateAction` used to be the default. It resolved successfully for
 * a decision that never left the browser — the wizard drove to its "event
 * approved" screen while catalyrst-events had heard nothing. That is the same
            ...
            assign({
              error: ({ event }) =>
                event.error instanceof Error
                  ? event.error.message
                  : String(event.error ?? "moderation failed"),
            }),
            "trackFailed",
          ],
        },
