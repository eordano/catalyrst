...
import type { PatchResult } from "@data/lib/catalyst/admin/whatson-admin";

const RESULT: PatchResult = { id: "evt-1", local: { approved: true, rejected: false } };
...
describe("moderateMachine — URL ?step slug map", () => {
...
describe("moderateMachine — deep-link hydration (snapshot, no event replay)", () => {
...
describe("moderateMachine — model-based path coverage (@xstate/graph)", () => {
...
describe("moderateMachine — telemetry events (happy path)", () => {
...
describe("moderateMachine — moderate failure + retry", () => {
...
describe("whatson-admin-moderate — the default actor fails closed", () => {
  ...
  it("a machine with no injected `moderate` lands in an error state, never approved", async () => {
    ...
    // `simulateModerateAction` used to be the default and resolved, driving the
    // wizard to "approved" for a decision that never left the browser.
    await expect(
