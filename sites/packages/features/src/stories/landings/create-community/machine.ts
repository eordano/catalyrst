...
) as Record<CommunityStepSlug, CommunityStateId>;

export function stateToSlug(value: string): CommunityStepSlug {
  return STATE_TO_SLUG[value as CommunityStateId] ?? FIRST_STEP_SLUG;
}

export function slugToState(slug: string | null | undefined): CommunityStateId {
  if (!slug) return "signinGate";
  return SLUG_TO_STATE[slug as CommunityStepSlug] ?? "signinGate";
}
