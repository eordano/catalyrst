...
) as Record<RsvpStepSlug, RsvpStateId>;

export function stateToSlug(value: string): RsvpStepSlug {
  return STATE_TO_SLUG[value as RsvpStateId] ?? FIRST_STEP_SLUG;
}

export function slugToState(slug: string | null | undefined): RsvpStateId {
  if (!slug) return "idle";
  return SLUG_TO_STATE[slug as RsvpStepSlug] ?? "idle";
}
          ...
          actions: assign({
            error: ({ event }) =>
              event.error instanceof Error
                ? event.error.message
                : String(event.error ?? "rsvp failed"),
          }),
        },
      },
    },
