import { delay } from "@core/lib/stories/index";
...
) as Record<CastStepSlug, CastStateId>;

export function stateToSlug(value: string): CastStepSlug {
  return STATE_TO_SLUG[value as CastStateId] ?? FIRST_STEP_SLUG;
}

export function slugToState(slug: string | null | undefined): CastStateId {
  if (!slug) return "tokenCheck";
  return SLUG_TO_STATE[slug as CastStepSlug] ?? "tokenCheck";
}
    ...
    setInvalidReason: assign({
      invalidReason: ({ event }) => {
        if ("error" in event) {
          ...
          return e instanceof Error ? e.message : String(e ?? "Invalid streaming key");
        }
