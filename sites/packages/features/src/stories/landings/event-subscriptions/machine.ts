...
) as Record<SubscriptionStepSlug, SubscriptionStateId>;

export function stateToSlug(value: string): SubscriptionStepSlug {
  return STATE_TO_SLUG[value as SubscriptionStateId] ?? FIRST_STEP_SLUG;
}

export function slugToState(slug: string | null | undefined): SubscriptionStateId {
  if (!slug) return "idle";
  return SLUG_TO_STATE[slug as SubscriptionStepSlug] ?? "idle";
}
          ...
          actions: assign({
            error: ({ event }) =>
              event.error instanceof Error
                ? event.error.message
                : String(event.error ?? "subscribe failed"),
          }),
        },
      },
    },
        ...
        onError: {
          ...
          actions: assign({
            error: ({ event }) =>
              event.error instanceof Error
                ? event.error.message
                : String(event.error ?? "unsubscribe failed"),
          }),
        },
      },
    },
