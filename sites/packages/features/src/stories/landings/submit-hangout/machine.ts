...
) as Record<HangoutStepSlug, HangoutStateId>;

export function stateToSlug(value: string): HangoutStepSlug {
  return STATE_TO_SLUG[value as HangoutStateId] ?? FIRST_STEP_SLUG;
}

export function slugToState(slug: string | null | undefined): HangoutStateId {
  if (!slug) return "signinGate";
  return SLUG_TO_STATE[slug as HangoutStepSlug] ?? "signinGate";
}
