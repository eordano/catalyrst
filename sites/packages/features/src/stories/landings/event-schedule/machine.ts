...
) as Record<ScheduleStepSlug, ScheduleStateId>;

export function stateToSlug(value: string): ScheduleStepSlug {
  return STATE_TO_SLUG[value as ScheduleStateId] ?? FIRST_STEP_SLUG;
}

export function slugToState(slug: string | null | undefined): ScheduleStateId {
  if (!slug) return "authGate";
  return SLUG_TO_STATE[slug as ScheduleStepSlug] ?? "authGate";
}
            ...
            assign({
              error: ({ event }) =>
                event.error instanceof Error
                  ? event.error.message
                  : String(event.error ?? "submit failed"),
            }),
            "trackSubmitFailed",
          ],
        },
