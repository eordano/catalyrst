...
function ManaPane({
    ...
    Promise.all([fetchPaymentsConfig(), quoteManaTopup(credits)])
      .then(([config, quote]) => {
        ...
        if (config.manaToken.toLowerCase() !== MANA_POLYGON.address) {
          setPhase({
            step: "unavailable",
            why: "MANA payments are misconfigured — please use another method.",
          });
          return;
        }
  ...
  async function confirmAndRedeem(txHash: string) {
    ...
    setPhase({
      step: "error",
      message:
        "The transfer is taking longer than expected. Your MANA is safe and your receipt is saved — come back to checkout any time and the Credits will finish arriving.",
    });
  }
