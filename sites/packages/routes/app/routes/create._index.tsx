...
import CreatorHubHome from "@ui/creatorhub/pages/CreatorHubHome";
...
export async function loader({ request }: Route.LoaderArgs) {
  ...
  try {
    scenes = await loadCreatorScenes({ creator: creator || undefined, limit: 6 });
  } catch {
    scenesError = true;
  }
  ...
  const payload = { sid, creator, scenes, scenesError, entry };
  return wrap(payload);
}
    ...
    case "download-hub":
      return (
        <EntryPanel
          ...
          subtitle="Download the desktop Creator Hub — the full toolset for building scenes, wearables and Worlds."
        >
          {downloadCta(true)}
        </EntryPanel>
      );
    ...
    case "capability-routed":
      return (
        <EntryPanel
          ...
          subtitle={
            fsAccess
              ? "Your browser supports local file access — open the Creator Hub right here, no install needed."
