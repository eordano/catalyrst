...
const reading: FdAskReadingVM = {
  ...
  confidence: "inferred",
  readAt: "2026-08-17",
  basis: "This program's own reading \u{2014} never a fact the ask carries.",
  crowdRange: "8\u{2013}50 recurring participants",
};
...
describe("the board card's reading chip", () => {
  it("links to the reading section of the ask's page, label and text unchanged", () => {
    ...
    expect(html).toContain("Game clubs \u{B7}");
...
describe("the ask page's reading section", () => {
  ...
  it("links open ground to the shelf itself \u{2014} never a #cell anchor the shelf does not emit", () => {
  ...
  it("links each doc grounded on this ask, the whole line the link", () => {
    ...
    expect(html).toContain("is grounded on this ask \u{2192}");
  });

  it("names the kind honestly \u{2014} a non-brief doc reads as a design doc", () => {
