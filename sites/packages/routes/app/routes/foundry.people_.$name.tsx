...
// GitHub profile page"). Looked up by the case-insensitively-unique display
// name; a name nobody claimed is a plain 404, never a scaffold for a person
// who does not exist.

export function meta({ loaderData }: Route.MetaArgs) {
  const name = loaderData?.person?.displayName;
  return [{ title: name ? `${name} \u{2014} The Foundry` : "People \u{2014} The Foundry" }];
}
