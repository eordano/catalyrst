  ...
  const events: Array<{ name: string; props: string; role: string }> = [
    {
      ...
      props: "{ from, reason }",
      role: "Dead-link leak counter — the recovery-rate denominator. Fired server-side on render.",
    },
    {
    ...
    >
      <div style={{ maxWidth: 760, margin: "0 auto" }}>
        <h1 style={{ fontSize: "2rem", margin: "0 0 8px 0" }}>
          Cast 404 — dead-link funnel
        </h1>
        <p style={{ color: "rgba(255,255,255,0.7)", margin: "0 0 24px 0" }}>
          Telemetry shape emitted by the Cast 2.0 “Page Not Found” fallback. This
          arrival was attributed as{" "}
          <code>from={from}</code> · <code>reason={reason}</code>.
        </p>
