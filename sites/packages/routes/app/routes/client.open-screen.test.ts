...
describe("GET /client/open-screen \u{2014} genesis arm", () => {
  it("no live scene \u{2192} redirects to /places BEFORE counting an exposure", async () => {
    activeMock.mockResolvedValue([]);
  ...
  it("live scene present \u{2192} no redirect, and the genesis exposure IS counted", async () => {
...
describe("GET /client/open-screen \u{2014} other arms never redirect", () => {
  it("base \u{2192} loads the browse grid and counts a base exposure", async () => {
