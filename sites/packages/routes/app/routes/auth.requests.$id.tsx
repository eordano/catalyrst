...
const UNVERIFIABLE_NOTICE: Record<UnverifiableReason, string> = {
  ...
  unverified_message:
    "Reading this message says nothing about what the signature is then used for. It could log you in to another site or app as you, or authorize an off-chain order, listing or spending permission over your assets. Only sign it if you trust the app that requested it.",
...
function messageDetail(params: unknown[]): string {
  ...
  switch (request.method) {
    case "personal_sign": {
      ...
      return {
        ...
        detail: isOpaqueSignatureMessage(message)
          ? truncateForDisplay(escapeUnreadableTypedDataText(message))
    ...
    case "eth_sendTransaction": {
      ...
      return {
        ...
        note: dropped.length > 0 ? droppedFieldsNote(dropped) : undefined,
      };
    }
  }
}
        ...
        {rejection ? (
          <>
            ...
            <span style={{ ...chip, whiteSpace: "pre-wrap", wordBreak: "break-word" }}>
              {truncateForDisplay(escapeUnreadableTypedDataText(rejection.message))}
            </span>
          </>
        ) : null}
      </Terminal>
...
export function ApprovalCard({
  ...
  }, [gatesOnReading, measureDetail, summary.detail]);
  const ackBlocked = acknowledgmentBlocked(gatesOnReading, readToEnd);
...
function shorten(address: string): string {
  ...
  return escapeUnreadableTypedDataText(cut);
}
