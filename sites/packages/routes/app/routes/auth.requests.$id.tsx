  ...
  switch (request.method) {
    ...
    case "eth_sendTransaction": {
      ...
      return {
        ...
        note: dropped.length > 0 ? `Not sent to the wallet: ${dropped.join(", ")}` : undefined,
      };
    }
  }
}
...
export function AuthRequestPage({ loaded }: { loaded: LoadedRequest }) {
  ...
  const onApprove = useCallback(async () => {
    ...
    if (unverifiable !== null && !effectsAcknowledged) return;
    setError(null);
    setPhase("signing");
...
export function ApprovalCard({
  ...
  const approveBlocked =
    isSigning || (mustValidate && !acknowledged) || (unverifiable !== null && !effectsAcknowledged);
  ...
  return (
    <div style={card}>
      ...
      <button style={button} onClick={onApprove} disabled={approveBlocked}>
