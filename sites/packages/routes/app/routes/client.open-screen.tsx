...
export async function loader({ request }: Route.LoaderArgs) {
  ...
  const { sid, assignment: resolved, wrap } = await storyLoader(
    request,
    STORY,
    FALLBACK,
  );

  // Draft: without OPEN_SCREEN_EXPERIMENT set, every session gets base (today's
  // behavior); ?arm= / ?variant=client_open_screen:<arm> still force a preview.
  const active =
    activeOpenScreenExperiment(
