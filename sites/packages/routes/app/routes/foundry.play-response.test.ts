...
describe("FdResponse readings and gaps", () => {
  const marketCell = {
    ...
    basis: "This program's own reading \u{2014} not a fact the deployment entity carries.",
  };
  ...
  it("an unread registry renders no gap list \u{2014} a gap nobody measured is not a gap", () => {
...
describe("FdResponse cell job slots", () => {
  ...
  it("a classified game renders the three job slots as chips \u{2014} covered ones linked, absences stated", () => {
  ...
  it("an unclassified game keeps the flat job list \u{2014} no cell slots apply", () => {
    const html = render({
