...
export default function MarketplaceBuyRoute({ loaderData }: Route.ComponentProps) {
  ...
  if (source === "unavailable") {
    return (
      <FlowFallback
        ...
        subtitle={`The marketplace didn't answer, so there is no price we can stand behind. This does not mean the item was sold — reload in a moment to try again.${reason ? ` (${reason})` : ""}`}
      />
    );
  }
  ...
  if (!listing || !display) {
    return (
      <FlowFallback
        ...
        subtitle="It may have just been sold or cancelled — listings move fast. The item itself may still be on sale from another seller."
      />
    );
  }
  ...
  const realConnect: SimFn = async () => {
    ...
    if (from && from.toLowerCase() === listing.seller.toLowerCase())
      throw new Error("This is your own listing — cancel it from My Assets instead of buying it.");
