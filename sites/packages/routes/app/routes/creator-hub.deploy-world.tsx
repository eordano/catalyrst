...
const CLAIM_TEST_MODE_NOTE =
  "NAME registration isn't connected on this realm yet — no real NAME can be " +
...
export async function loader({ request }: Route.LoaderArgs) {
  ...
  try {
    ...
    deployData = {
      ...
      owner: {
        network: "Mainnet",
        address: address ? shortAddress(address) : "0x…",
        username: "",
        verified: false,
        role: "Owner",
      },
...
export default function CreatorHubDeployWorld({ loaderData }: Route.ComponentProps) {
  ...
  if (landProbe.status === "denied") {
    landNotice = landProbe.owner
      ? `This scene sits on LAND ${landProbe.parcel}, owned by ${landProbe.owner} — ` +
      ...
      if (!dir && draftPack) {
        ...
        if (compositeKey) {
          ...
          try {
            ...
            console.warn(
              "[deploy] main.composite unreadable — skipping asset validation",
            );
          }
          ...
          if (!runtime) {
            throw new Error(
              `This project has no built ${main} and the prebuilt scene runtime is ` +
                `unavailable — build the project locally (npm install && npm run build) before publishing.`,
            );
          }
