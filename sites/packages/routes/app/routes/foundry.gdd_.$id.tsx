...
import {
  activeBuildForDoc,
  requestBuild,
} from "@data/lib/foundry/build-request.server";
import { readBuildStatusFile } from "@data/lib/foundry/build-status-file.server";
import {
...
export function meta({ loaderData }: Route.MetaArgs) {
  ...
  return [{ title: title ? `${title} \u{2014} The Foundry` : "Design doc \u{2014} The Foundry" }];
}
...
export async function loader({ params, request }: Route.LoaderArgs) {
  ...
  let build: FdGddBuildRequestVM | null = null;
  ...
  try {
    ...
    const [roles, viewerIsMaker, buildRow] = await Promise.all([
      activeRoles(sid),
      isDocChainMaker(db, params.id, sid),
      activeBuildForDoc(db, params.id),
    ]);
    // Presentation gate only: requestBuild re-checks maker-or-host in-tx.
    canRequestBuild = roles.includes("host") || viewerIsMaker;
    if (buildRow) {
      // The broker's on-disk step note enriches a live build's banner; the DB
      // row stays the source of truth and an absent file changes nothing.
      const live = ["queued", "building", "verifying"].includes(buildRow.status);
      const file = live ? await readBuildStatusFile(buildRow.sceneSlug) : null;
      build = {
        status: buildRow.status,
        sceneSlug: buildRow.sceneSlug,
        detail: buildRow.detail,
        createdAt: buildRow.createdAt,
        updatedAt: buildRow.updatedAt,
        fileNote: file ? (file.note ? `${file.step} \u{2014} ${file.note}` : file.step) : null,
      };
    }
    ...
    switch (intent) {
      ...
      case "request-build": {
        const res = await requestBuild({
          docId: params.id,
          sceneSlug: String(form.get("sceneSlug") ?? ""),
          sid,
          ip,
        });
        track(
          "fd_gdd_build_requested",
          { doc_id: params.id },
          { sid: sidBadge(sid) },
        );
        return data({
...
export default function FoundryGddDoc({ loaderData }: Route.ComponentProps) {
  ...
  const buildFetcher = useFetcher<typeof action>();
  ...
  return (
    <FdGddDocPage
      ...
      build={{
        request: d.build,
        canRequest: d.canRequestBuild,
        pending: buildFetcher.state !== "idle",
        error:
          buildFetcher.data && buildFetcher.data.ok === false
            ? buildFetcher.data.error
            : null,
        onRequest: (sceneSlug) =>
          buildFetcher.submit(
            { intent: "request-build", sceneSlug },
            { method: "post" },
          ),
      }}
