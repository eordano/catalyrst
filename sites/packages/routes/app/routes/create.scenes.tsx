    ...
    setDeleteNotice(
      hasFolder
        ? filesOutcome === "deleted"
          ...
          : `"${title}" was removed from My Scenes. Its files are still on your computer — use Import to bring it back.`
  ...
  async function onImport() {
    ...
    try {
      ...
      if (out.status === "no-composite") {
        ...
        setImportError(
          out.hasSceneJson
            ? "No main.composite found in that folder — it looks like a code-only SDK project (scene.json but no saved scene). Pick the project folder that contains your saved scene, the one holding main.composite."
            : "No main.composite found in that folder — pick the project folder that contains your saved scene, the one holding main.composite.",
        );
        return;
      }
      ...
      setImportError(
        "Couldn't read that folder — check that this site still has permission to access it, then try again.",
      );
    }
  }
  function onRetry() {
    ...
    <div className="create-scenes">
        ...
        <ChModalDeleteProject
          ...
          note={
            pendingDelete.hasFolder
              ? undefined
              : "This scene isn't connected to a folder on your computer — deleting it from My Scenes permanently removes its saved content."
          }
      ...
      {importPhase === "reading" || importPhase === "opening" ? (
        <div role="status" aria-live="polite" style={SCENES_IMPORT_PENDING_TOAST}>
          <span className="u-spinner" aria-hidden="true" />
          <span>
            {importPhase === "reading" ? "Reading scene folder…" : "Opening scene…"}
          </span>
        </div>
      ) : null}
      {importError ? (
