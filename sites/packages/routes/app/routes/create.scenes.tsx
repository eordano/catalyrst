...
function ScenesView({
  ...
  useEffect(() => {
    ...
    const load = () => {
      void (async () => {
        const [metas, serverDrafts] = await Promise.all([
          handleStore.list().catch(() => null),
          import("@data/lib/catalyst/creator-hub/scene-drafts-client")
            .then((m) => m.listServerDrafts())
            .catch(() => null),
        ]);
        if (cancelled) return;
        ...
        if (metas || serverDrafts) setLocalCards(cards);
      })();
    };
    load();
