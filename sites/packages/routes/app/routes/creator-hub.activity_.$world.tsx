...
export const meta = ({ params }: Route.MetaArgs) =>
  creatorHubMeta(`Activity · ${params.world}`);
...
const NO_HISTORY_NOTE =
  "The presence collector has no snapshots for this world — it has not been in the poll set. That is not the same as a reading of zero.";
...
function accessRows(d: WorldActivityData): Datum<FactRow[]> {
  ...
  const withList = (type: string, wallets: string[]) =>
    type === "allow-list"
      ? `${type} — ${wallets.length === 0 ? "nobody but the owner" : wallets.join(", ")}`
      : type;
  const rows: FactRow[] = [
...
function notBuiltPanels(world: string): NotBuiltSpec[] {
  ...
  if (notify) {
    specs.push({
      ...
      why: notify.note,
      today: "The ⟳ control at the top of this page.",
    });
  }
  if (sceneState) {
    specs.push({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const payload = {
    ...
    sceneKvStorage: unavailableBecause(
      KV_STORAGE_ENDPOINT,
      "It returns 400 Invalid Auth Chain. It needs an ADR-44 signed fetch made by the scene runtime (realm + parcel metadata); this hub holds no such identity. It would also be a different number — that endpoint sums the key-value store your scene writes at runtime, not the bytes you deployed.",
    ) as Datum<unknown>,
  ...
  return (
    <CreatorHubChrome active="activity">
      <ChWorldActivityPage
        ...
        access={d.access}
        permissionsCli={{
          command: `dcl-one-sdk world permissions grant ${d.world} deployment 0x…`,
          explain:
