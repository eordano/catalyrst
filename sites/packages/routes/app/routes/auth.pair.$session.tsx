...
export function meta(_args: Route.MetaArgs) {
  return [{ title: "Sign in with your phone — Decentraland" }];
}
  ...
  async function onApprove() {
    ...
    try {
      ...
      if (res.status === 410 || res.status === 404) {
        throw new Error("This code expired — scan a fresh one.");
      }
      if (!res.ok) {
  ...
  if (step === "done") {
    return (
      <div style={card}>
        ...
        <p style={subtle}>
          Head back to <strong>{loaded.host}</strong> — it picks up the sign-in
    ...
    <div style={card}>
      ...
      {walletReady ? (
        <button style={button} onClick={onApprove} disabled={step === "signing"}>
          {step === "signing" ? "Waiting for your wallet…" : "Approve in your wallet"}
        </button>
      ) : walletReady === false ? (
        <>
          <p style={subtle}>
            No wallet found in this browser. Open this page inside your wallet
            app instead — signing happens in the wallet's built-in browser:
          </p>
          ...
          <p style={{ ...subtle, fontSize: 13, marginTop: 16 }}>
            ...
            page's address. This is Decentraland's own pairing page — it works
      ...
      <p style={brandFooter}>
        Secured by <strong>LibreConnect</strong> — an open, self-hosted pairing
