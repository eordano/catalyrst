...
describe("recoverAuthRequest", () => {
  ...
  it("reports a MetaTransaction whose domain struct misses a domain field once", async () => {
    ...
    expect(d.postOutcome).toHaveBeenCalledWith(ID, {
      ...
      error: {
        ...
        message:
          'The "eth_signTypedData_v4" request parameters are malformed: the MetaTransaction domain type does not match the domain fields',
      },
    });
  });
