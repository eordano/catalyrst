...
const DOMAIN_MISMATCH = "the MetaTransaction domain type does not match the domain fields";
const MISSING_PARTS = "the MetaTransaction is missing its types, domain or message";
...
const DEVIATING_META_TRANSACTIONS: [string, () => unknown, string][] = [
  [
  ...
  ["it carries no types, domain or message at all", () => ({ primaryType: "MetaTransaction" }), MISSING_PARTS],
  ["it carries a domain that is not a record", () => ({ ...metaTransaction(), domain: MARKETPLACE }), MISSING_PARTS],
  [
    "it carries types as an array rather than a record",
    () => ({ ...metaTransaction(), types: DOMAIN_TYPE.map((field) => ({ ...field })) }),
    MISSING_PARTS,
  ],
  ["it declares no domain struct at all", withoutDomainStruct, "the MetaTransaction does not declare its domain struct"],
  [
...
describe("validateAuthRequest serves every MetaTransaction behind the acknowledgment", () => {
  it("accepts conforming, deviating and malformed MetaTransactions alike, as objects or JSON, behind unrecognized_typed_data", () => {
    ...
    const builds: [string, () => unknown][] = [
      ...
      ...DEVIATING_META_TRANSACTIONS.map(([label, build]): [string, () => unknown] => [label, build]),
    ];
...
describe("metaTransactionDomainProblem", () => {
  ...
  it("names what deviates from the domain the contract hashes", () => {
    for (const [label, build, problem] of DEVIATING_META_TRANSACTIONS) {
      expect(metaTransactionDomainProblem(build()), label).toBe(problem);
    }
  });
});
