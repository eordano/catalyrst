...
describe("validateAuthRequest signature payload bounds", () => {
  ...
  it("accepts a payload at the cap and refuses the one character past it", () => {
  ...
  it("measures typed data sent as an object too", () => {
    const oversized = { ...JSON.parse(typedDataOf({})), message: { memo: "a".repeat(MAX_SIGNATURE_PAYLOAD_CHARS) } };
    expect(rejection(method, [SIGNER, oversized], SIGNER).message).toContain(
      "the typed data is too large to review",
    );
  });

  it("refuses a personal_sign message past the same cap", () => {
    const atCap = "a".repeat(MAX_SIGNATURE_PAYLOAD_CHARS);
    expect(validateAuthRequest("personal_sign", [atCap, SIGNER], SIGNER).ok).toBe(true);
    const result = rejection("personal_sign", [`${atCap}a`, SIGNER], SIGNER);
    expect(result).toMatchObject({ code: RPC_INVALID_PARAMS, kind: "malformed_signature" });
  ...
  it("refuses typed data nested past the depth a review can render", () => {
    ...
    const within = typedDataOf({ memo: nest(40) });
    expect(validateAuthRequest(method, [SIGNER, within], SIGNER).ok).toBe(true);

    const beyond = typedDataOf({ memo: nest(80) });
    const result = rejection(method, [SIGNER, beyond], SIGNER);
    expect(result).toMatchObject({ code: RPC_INVALID_PARAMS, kind: "malformed_signature" });
    expect(result.message).toContain("the typed data is too deeply nested to review");
  });

  it("refuses a number literal the parse would rewrite before the wallet signs it", () => {
    const result = rejection(
      method,
      [SIGNER, '{"primaryType":"Permit","types":{},"domain":{},"message":{"tokenId":9007199254740993}}'],
      SIGNER,
    );
    expect(result).toMatchObject({ code: RPC_INVALID_PARAMS, kind: "malformed_signature" });
    expect(result.message).toContain("a number that cannot be represented exactly");
  });

  it.each([
    ["a fraction", "0.5"],
    ["an exponent", "1e3"],
    ["a negative value past the safe range", "-9007199254740993"],
  ])("refuses %s as a typed-data number", (_label, literal) => {
    expect(
      rejection(
        method,
        [SIGNER, `{"primaryType":"Permit","types":{},"domain":{},"message":{"amount":${literal}}}`],
        SIGNER,
      ).message,
    ).toContain("a number that cannot be represented exactly");
  });
  ...
  it.each([
    ...
    expect(
      validateAuthRequest(
        method,
        [SIGNER, `{"primaryType":"Permit","types":{},"domain":{},"message":${message}}`],
        SIGNER,
      ).ok,
    ).toBe(true);
  });
  ...
  it("reads a number inside a string as text, escapes included", () => {
    ...
    expect(
      validateAuthRequest(
        ...
        [
          SIGNER,
          `{"primaryType":"Permit","types":{},"domain":{},"message":{"id":"${uint256}","memo":"a \\" 9007199254740993"}}`,
        ],
        SIGNER,
      ).ok,
    ).toBe(true);
...
describe("buildTransactionParams", () => {
  ...
  it("shows the lowercased target in the preview the wallet is handed", () => {
  ...
  it.each([
    ...
    ["an object", { data: "0x" }],
    ["a boolean", true],
  ])("rejects %s in place of hex-encoded data", (_label, data) => {
    expect(() => buildTransactionParams([{ to: TO, data }])).toThrow(
      /"data" must be hex-encoded bytes/,
    );
  });
});
...
describe("unverifiableReason", () => {
  ...
  it("gates readable personal_sign text as a signature whose use cannot be checked", () => {
    ...
    expect(
      unverifiableReason("personal_sign", [`0x${toHex("Sign in to Decentraland\nNonce: 1234")}`, SIGNER]),
    ).toBe("unverified_message");
  });

  it("never exempts a request from the acknowledgment", () => {
    for (const [method, params] of [
      ["personal_sign", ["hello there, a perfectly readable sentence", SIGNER]],
      ["personal_sign", [`0x${"ab".repeat(32)}`, SIGNER]],
      ["eth_signTypedData_v4", [SIGNER, PERMIT]],
      ["eth_sendTransaction", [{ to: TO }]],
    ] as const) {
      expect(unverifiableReason(method, [...params])).not.toBeNull();
    }
  });
...
const MALFORMED_META_TRANSACTIONS: [string, () => unknown][] = [
  ...
  [
    "declares no domain struct at all",
    () => {
      const typedData = metaTransaction();
  ...
  [
    "carries a domain field EIP-712 does not define",
    () => {
      const typedData = metaTransaction();
...
describe("validateAuthRequest serves every MetaTransaction behind the acknowledgment", () => {
  ...
  it.each([
    ...
    ...MALFORMED_META_TRANSACTIONS,
  ] as [string, () => unknown][])("accepts a MetaTransaction where %s", (_label, build) => {
    const typedData = build();
    expect(validateAuthRequest(method, [SIGNER, typedData], SIGNER).ok).toBe(true);
    expect(unverifiableReason(method, [SIGNER, typedData])).toBe("unrecognized_typed_data");
  });

  it("accepts one sent as a JSON string the same way", () => {
    const typedData = metaTransaction();
    typedData.types.EIP712Domain = typedData.types.EIP712Domain.filter(
      (field) => field.name !== "salt",
    );
  ...
  it("leaves every other primary type to the same acknowledgment", () => {
    expect(validateAuthRequest(method, [SIGNER, PERMIT], SIGNER).ok).toBe(true);
...
describe("metaTransactionDomainProblem", () => {
  ...
  it("passes a domain struct listing the same fields in another order", () => {
    const typedData = metaTransaction();
    typedData.types.EIP712Domain = [...typedData.types.EIP712Domain].reverse();
    expect(metaTransactionDomainProblem(typedData)).toBeNull();
  });

  it("judges nothing whose primary type is not exactly MetaTransaction", () => {
    const typedData = metaTransaction();
    typedData.primaryType = "metatransaction";
    typedData.domain.extra = "unsigned";
    expect(metaTransactionDomainProblem(typedData)).toBeNull();
  ...
  it.each(DEVIATING_META_TRANSACTIONS)("refuses one where %s", (_label, build) => {
    expect(metaTransactionDomainProblem(build())).toBe(
      "the MetaTransaction domain type does not match the domain fields",
    );
  });

  it.each(MALFORMED_META_TRANSACTIONS.slice(0, 3))("refuses one that %s", (_label, build) => {
    expect(metaTransactionDomainProblem(build())).toBe(
      "the MetaTransaction is missing its types, domain or message",
    );
  });

  it("refuses one that declares no domain struct at all", () => {
    const typedData = metaTransaction();
    delete (typedData.types as Record<string, unknown>).EIP712Domain;
    expect(metaTransactionDomainProblem(typedData)).toBe(
      "the MetaTransaction does not declare its domain struct",
    );
  });

  it("refuses a domain carrying a field EIP-712 does not define", () => {
    const typedData = metaTransaction();
    typedData.domain.extra = "unsigned";
    expect(metaTransactionDomainProblem(typedData)).toBe(
      "the MetaTransaction domain has a field EIP-712 does not define",
    );
  });
});
